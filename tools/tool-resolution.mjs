// Generated from runtime/alpha3/operations/tool-resolution.mts; edit the TypeScript source.
...
export function resolveTar() {
    ...
    if (process.platform === 'win32' && fs.existsSync(system) && probe(system, ['--version']))
        return system;
    return 'tar';
}
...
export function resolvePwsh() {
    ...
    for (const candidate of candidates) {
        if (candidate.includes(path.sep) && !fs.existsSync(candidate))
            continue;
        if (probe(candidate, ['-NoLogo', '-NoProfile', '-NonInteractive', '-Command', 'exit 0']))
            return candidate;
    }
    return null;
}
 ...
 *  dependency and this repository has no package root of its own, so the
 *  resolver takes an explicit override, then the pinned build-tools install,
 *  then the upstream Harness checkout these suites used to borrow it from. */
export function resolveJsdom() {
    ...
    const candidates = override ? [override] : [
        ...
        path.resolve(base, '../../../..', 'deepseek-harness-alpha3/node_modules/jsdom')
    ];
    for (const candidate of candidates)
...
export function forceUtf8Output() {
