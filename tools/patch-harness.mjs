...
export function transform(bytes, patch) {
  if (digest(bytes) === patch.after) return {state: 'already-patched', bytes};
  if (digest(bytes) !== patch.before) throw Error('Unknown or partially modified fingerprint: ' + patch.package + '/' + patch.file);
  let position = 0;
  const chunks = [];
  ...
  for (const patch of patches) {
    ...
    try {manifest = resolver.resolve(patch.package + '/package.json');}
      ...
      for (const directory of resolver.resolve.paths(patch.package) ?? []) {
        ...
        if (fs.existsSync(candidate) && JSON.parse(fs.readFileSync(candidate)).name === patch.package) {manifest = candidate; break;}
      }
...
export function patchCli(args = process.argv.slice(2)) {
  ...
  for (let i = 0; i < args.length; i++) {
    const key = args[i];
    if (['--apply','--stopped'].includes(key)) options[key.slice(2)] = true;
    else if (['--harness','--patches','--backup','--rollback'].includes(key)) options[key.slice(2)] = args[++i];
  ...
  const data = JSON.parse(fs.readFileSync(patchFile));
