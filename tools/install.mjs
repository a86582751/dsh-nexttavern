...
for (let i = 0; i < args.length; i++) {
  ...
  if (['apply','stopped','documents','uninstall'].includes(key)) options[key] = true;
  else if (['package','home','harness','profile','backup','rollback'].includes(key)) options[key] = args[++i];
...
const receipt = JSON.parse(fs.readFileSync(path.join(packageRoot, 'provenance.json')));
...
const prior = fs.existsSync(stampFile) ? JSON.parse(fs.readFileSync(stampFile)) : null;
...
const profilePackage = JSON.parse(fs.readFileSync(profileFile));
...
if (options.uninstall) {
  ...
  const modulePath = name => {
    for (const base of [path.join(home, 'profiles', profile), harness]) {
      ...
      try {return resolver.resolve(name).replaceAll('\\', '/');} catch (error) {if (error.code !== 'MODULE_NOT_FOUND') throw error;}
    }
