...
function recordFor(home, profile, input) {
 ...
 */
export function readPreparedReferences(home, profile) {
    try {
 ...
 */
export function durableReferencesIntact(home, profile, references) {
    try {
        const root = fs.realpathSync(home);
...
export function planProtectedPackages(options) {
    ...
    const receipt = { schemaVersion: 1, state: 'references-prepared', profile, packages: next };
    const updates = [
    ...
    return { home, profile, changes, receipt, assertUnchanged, requiresRelink: true };
}
...
export function prepareProtectedPackages(options) {
    ...
    return { state: 'references-prepared', requiresRelink: true, receipt: plan.receipt, transaction };
}
