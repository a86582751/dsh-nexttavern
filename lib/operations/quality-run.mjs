// Generated from runtime/alpha3/src/operations/quality-run.mts; edit the TypeScript source.
...
export function loadPrevious(context, requested) {
    const file = requested ? path.resolve(context.root, requested) : path.join(context.stateDirectory, 'latest.json');
    if (!fs.existsSync(file)) {
        if (requested)
            throw Error('No check run at ' + file);
        return null;
    }
    const record = readJson(file);
    if (record.schemaVersion !== 1 || record.root !== context.root)
        throw Error('Check run belongs to a different package or schema');
    return record;
}
export function printPlan(context, plan) {
    const brief = (items, limit = 3) => items.slice(0, limit).join('; ') + (items.length > limit ? '; +' + (items.length - limit) + ' more' : '');
    console.log('check plan [' + context.audience + ']: ' + (plan.request.command || 'changed'));
    if (plan.files.length)
        console.log('files: ' + brief(plan.files, 6));
    ...
    for (const warning of plan.uncovered)
        console.log('UNCOVERED: ' + warning);
    for (const notice of plan.notice)
        console.log(notice);
}
export async function executePlan(context, plan, options = {}) {
    const previous = options.previous ?? loadPrevious(context);
    const run = { schemaVersion: 1, root: context.root, id: randomUUID(), request: plan.request,
    ...
    if (context.audience === 'maintenance' && process.platform === 'win32' && /^c:/i.test(tempRoot))
        throw Error('Maintenance temporary storage must stay on D:');
    ...
    try {
        for (const task of plan.tasks) {
            ...
            const result = { id: task.id, status: 'running', ms: 0 };
            run.results.push(result);
            const blocked = (task.depends ?? []).find(id => !run.results.some(result => result.id === id && ['passed', 'reused'].includes(result.status)));
            if (blocked) {
                result.status = 'blocked';
                result.reason = 'prerequisite ' + blocked;
                checkpoint();
                continue;
            }
            let unavailable;
            try {
                unavailable = (options.capability ?? capabilityProblem)(context, task);
            }
            catch (error) {
            ...
            try {
                ...
                if (result.status === 'passed') {
                    if (taskKey() !== key)
                        throw Error('Task inputs changed during execution; no success reused');
                    if (task.cache !== false)
                        run.successes[task.id] = { fingerprint: key, ms: result.ms };
                }
            }
            catch (error) {
        ...
        for (const result of run.results.filter(result => !['passed', 'reused'].includes(result.status)))
        ...
        if (path.dirname(temporary) !== tempRoot || fs.lstatSync(temporary).isSymbolicLink())
            throw Error('Temporary directory ownership changed');
