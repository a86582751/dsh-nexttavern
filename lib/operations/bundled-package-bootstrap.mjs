...
import { prepareProtectedPackages, protectedGeneration, recoverProtectedPackages, verifyProtectedPackage } from './protected-packages.mjs';
 ...
 */
export function readBundleIdentity(productRoot) {
...
export function inspectBundledPackages(productRoot, hostAnchor, resolveManifest) {
    ...
    const packages = identity.packages.map(spec => {
        ...
        return { ...spec, source, entry, ...(metadata.dsh?.client === undefined ? {} : { client: true }) };
    });
 ...
 */
function reclaimOrphanedLock(lockPath) {
    let owner;
    try {
...
export async function bootstrapBundledPackages(options) {
    ...
    const run = () => withFileLock(manifest, async () => {
        ...
        const verifiedSources = members.map(({ source, generation }) => ({ source, generation }));
    ...
    try {
        ...
        if (!isWriterLockBusy(error) || !reclaimOrphanedLock(manifest + '.lock'))
            throw error;
        return await run();
    }
}
