// Generated from runtime/alpha3/src/operations/quality-actions.mts; edit the TypeScript source.
...
export async function executeTask(context, plan, task, options) {
    const env = { ...process.env, TEMP: options.temporary, TMP: options.temporary,
    ...
    const mapped = (suffix) => {
        ...
        if (!artifact)
            throw Error('Not available in this package: ' + suffix);
    ...
    if (task.action === 'syntax') {
        ...
        for (const relative of (plan.syntaxFiles ?? plan.files).filter(file => !/\.(?:md|txt)$/.test(file))) {
            ...
            if (!fs.existsSync(file))
                throw Error('Cannot syntax-check missing/deleted file: ' + relative);
            ...
            if (/\.(?:[cm]?ts|tsx)$/.test(file)) {
                ...
                throw Error('No syntax checker for ' + relative + '; choose its feature checks');
        }
        return passed();
    }
    if (task.action === 'docs') {
        for (const relative of plan.files.filter(file => /\.(md|txt)$/.test(file))) {
            const file = path.join(context.root, relative);
            if (!fs.existsSync(file))
                continue; // deletion has no remaining links to validate
            const text = fs.readFileSync(file, 'utf8');
            for (const match of text.matchAll(/\]\(([^)\n]+)\)/g)) {
                const target = match[1].split('#')[0].replace(/^<|>$/g, '');
                if (!target || /^(?:https?:|mailto:|app:|codex:)/.test(target) || /["']/.test(target))
                    continue;
                if (!fs.existsSync(path.resolve(path.dirname(file), decodeURIComponent(target))))
                    throw Error('Broken local link: ' + relative + ' -> ' + target);
            }
            const lines = text.split('\n').filter(line => line.length > 160).length;
            if (lines)
                console.log('review signal: ' + relative + ' has ' + lines + ' long lines; content/data is reviewed by cohesion, not automatically reformatted');
        }
        return passed();
    }
    if (task.action === 'bundle') {
        ...
        if (!recipes.length)
            throw Error('No browser bundle mapping');
    ...
    if (task.action === 'candidate') {
        ...
        if (request.inputs)
            args.push('--inputs', path.resolve(context.root, request.inputs));
        ...
        if (request.foundation)
            args.push('--foundation', request.foundation);
    ...
    if (task.action === 'archive' || task.action === 'install') {
        ...
        if (fs.existsSync(path.join(bundle, 'package/provenance.json')))
            args.push('--public');
        if (task.action === 'install' && request.upgradeFrom) {
            if (!request.upgradeSha256)
                throw Error('--upgrade-from requires --upgrade-sha256');
    ...
    if (task.action === 'harness')
        return node(artifactFile(context, task.artifact), [path.resolve(context.root, request.bundle)]);
