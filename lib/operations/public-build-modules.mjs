// Generated from runtime/alpha3/src/operations/public-build-modules.mts; edit the TypeScript source.
/** Public multi-target delivery and legacy entry verification over the shared compiler. */
import fs from 'node:fs';
import path from 'node:path';
import { fileURLToPath } from 'node:url';
import { compileTypeScript } from './build-typescript.mjs';
export function publicBuildModulesCli(args = process.argv.slice(2), root = fileURLToPath(new URL('../../', import.meta.url))) {
    ...
    if (map.schemaVersion !== 3 || !map.compilerPlan)
        throw Error('Unsupported public source/build map');
    const safe = (file) => {
        if (!file || file.includes('\\') || file.includes(':') || file.startsWith('/')
            || file.split('/').some(part => !part || part === '.' || part === '..'))
            throw Error('Unsafe public build path: ' + file);
    ...
    const tree = (file, name) => {
        const parts = file.split('/'), index = parts.lastIndexOf(name);
        if (index < 0)
            throw Error('Expected ' + name + '/ in mapped path: ' + file);
        return parts.slice(0, index + 1).join('/');
    };
    const inspect = (directory, sourceTree) => {
        if (!fs.existsSync(safe(directory)))
            return;
        for (const item of fs.readdirSync(safe(directory), { withFileTypes: true })) {
            const file = directory + '/' + item.name;
            if (item.isSymbolicLink())
                throw Error('Unexpected link in generated tree: ' + file);
            if (item.isDirectory()) {
                inspect(file, sourceTree);
                continue;
            }
            if (/\.(?:tsx?|mts)$/.test(file)
                && (!sourceTree || (!sources.has(safe(file)) && !declarations.has(safe(file)))))
                throw Error('Unmapped TypeScript source: ' + file);
            if (/\.(?:js|mjs|cjs)$/.test(file) && (sourceTree || !allowedOutputs.has(file)))
                throw Error('Unmapped generated module: ' + file);
        }
    };
    for (const directory of new Set(map.modules.map(module => tree(module.source, 'src'))))
    ...
    for (const directory of new Set([...allowedOutputs].filter(file => file.split('/').includes('lib'))
        .map(file => tree(file, 'lib'))))
        inspect(directory, false);
    for (const bundle of map.bundles)
        if (!write && !fs.existsSync(safe(bundle)))
            throw Error('Missing registered browser bundle: ' + bundle);
    ...
    for (const module of map.modules) {
        ...
        if (!recipe || recipe.entry !== module.source)
            throw Error('Public compiler/delivery mapping differs: ' + module.artifact);
        for (const output of module.outputs)
            compare(output, recipe.text);
    }
    for (const entry of map.compatibilityEntrypoints ?? []) {
        const module = map.modules.find(module => module.artifact === entry.artifact);
        if (!module?.primaryOutput)
            throw Error('Compatibility entry has no canonical module: ' + entry.path);
    ...
    if (stale)
        throw Error(`${stale} generated files differ; run npm run build:modules`);
