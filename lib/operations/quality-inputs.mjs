// Generated from runtime/alpha3/src/operations/quality-inputs.mts; edit the TypeScript source.
...
export function walkFiles(root) {
    ...
    return fs.readdirSync(root, { withFileTypes: true }).flatMap(item => {
        if (item.isSymbolicLink() || ['node_modules', '.git', 'artifacts'].includes(item.name))
            return [];
        const file = path.join(root, item.name);
        return item.isDirectory() ? walkFiles(file) : [file];
    });
}
export function taskFiles(context, plan, task) {
    const files = new Set();
    const include = (relative) => files.add(path.resolve(context.root, relative));
    if (task.artifact) {
    ...
    if (task.action === 'bundle') {
        for (const recipe of context.mapping.builds.filter(build => build.kind !== 'typescript-module')) {
            include(recipe.entry);
            include(recipe.builder);
            for (const file of recipe.inputs)
                include(file);
    ...
    if (['syntax', 'docs'].includes(task.action)) {
        for (const file of task.action === 'syntax' ? plan.syntaxFiles ?? plan.files : plan.files) {
            if (task.action !== 'docs' || /\.(md|txt)$/.test(file))
                include(file);
        }
    }
    else if (['types', 'generated'].includes(task.action)) {
        if (context.audience === 'maintenance')
            include('release/source-manifest.json');
        else {
            include('tools/check-map.json');
            include('tools/build-map.json');
        }
        const roots = new Set();
        for (const build of context.mapping.builds) {
            ...
            if (task.action === 'generated' && output)
                include(output);
            // Include newly added, deleted and unregistered modules in ownership keys.
            for (const file of [build.entry, output].filter((file) => !!file)) {
                const match = /^(.*?(?:^|\/)(?:src|lib))\//.exec(file);
                if (match)
                    roots.add(path.join(context.root, match[1]));
            }
        }
        for (const root of roots)
            for (const owned of walkFiles(root))
                files.add(owned);
    }
    else {
        ...
        for (const artifact of context.mapping.artifacts) {
            ...
            if (otherTests.has(artifact.id))
                continue;
            if (owners(context, artifact.source).some(module => task.modules.includes(module.id)))
                include(artifact.source);
        }
    }
    ...
    while (pending.length) {
        ...
        if (visited.has(file) || !fs.existsSync(file))
            continue;
        visited.add(file);
        if (!/\.(?:[cm]?[jt]s|json)$/.test(file))
            continue;
        const text = fs.readFileSync(file, 'utf8');
        for (const match of text.matchAll(/['"](\.\.?\/[^'"\n]+)['"]/g)) {
            const dependency = path.resolve(path.dirname(file), match[1]);
            if (!dependency.startsWith(context.root + path.sep) || dependency.includes(path.sep + 'node_modules' + path.sep))
                continue;
    ...
    if (!['docs', 'syntax'].includes(task.action)) {
        if (context.mapping.typeScript?.checker)
            include(context.mapping.typeScript.checker);
        if (context.audience === 'public')
            include('tools/build-map.json');
        for (const artifact of context.mapping.artifacts) {
            if (/quality-.*\.(?:mts|mjs)$/.test(artifact.source)
                || /(?:package(?:-lock)?|tsconfig)\.json$/.test(artifact.source))
                include(artifact.source);
        }
        for (const directory of context.mapping.typeScript?.declarationPackages ?? []) {
            ...
            for (const key of Object.keys(lock.packages))
                if (key.startsWith('node_modules/'))
                    include(directory + '/' + key + '/package.json');
        }
    }
    if (plan.request.bundle && ['archive', 'install', 'installer', 'harness'].includes(task.action)) {
        for (const file of walkFiles(path.resolve(context.root, plan.request.bundle)))
            files.add(file);
    }
    if (plan.request.upgradeFrom && task.action === 'install')
        files.add(path.resolve(context.root, plan.request.upgradeFrom));
    return [...files].sort();
}
export function fingerprint(context, plan, task) {
