// Generated from runtime/alpha3/src/operations/test-temp.mts; edit the TypeScript source.
...
export function testTempRoot() {
    ...
    if (maintenance && process.platform === 'win32' && path.parse(target).root.toLowerCase() === 'c:\\')
        throw Error('Test temporary files must not use C:');
    fs.mkdirSync(target, { recursive: true });
    const root = fs.realpathSync(target);
    if (maintenance && process.platform === 'win32' && path.parse(root).root.toLowerCase() === 'c:\\')
