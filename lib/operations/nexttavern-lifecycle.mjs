// Generated from runtime/alpha3/src/operations/nexttavern-lifecycle.mts; edit the TypeScript source.
/** Product-level management. Only the official manager owns profile writes. */
export const NEXTTAVERN_BUNDLE = 'dsh-nexttavern';
export function createNextTavernLifecycle(managerOf, hotReloadAvailable) {
    let lastManager;
    let lastOperation;
    ...
    async function snapshot() {
        try {
            ...
            if (!current)
                return { available: false, selected: null, reason: 'This host does not provide bundle management / 此宿主不支持整包管理' };
            const bundle = (await current.listBundles()).find(row => row.name === NEXTTAVERN_BUNDLE);
            if (!bundle)
                return { available: false, selected: null, reason: 'NextTavern bundle is unavailable / 未找到 NextTavern 插件包' };
            // Alpha.7 applies a hot switch of this bundle for both directions, but a
            // rejected activation is not rolled back: the rows that already started
            // stay live and the official rows stay disposed
            // (plugin-native-lifecycle.test.mts). A half takeover of the host
            // services this product owns cannot be repaired by watching the result
            // afterwards, so this control refuses BEFORE any write and leaves the
            // restart to the caller.
            if (hotReloadAvailable())
                return { available: false, selected: bundle.enabled, lastOperation,
    ...
    async function switchEnabled(enabled) {
        ...
        if (!before.available || !current)
            return { ok: false, restart: false, reason: before.reason, product: before };
        ...
        if (hotReloadAvailable())
            return { ok: false, restart: false, product: await snapshot(),
