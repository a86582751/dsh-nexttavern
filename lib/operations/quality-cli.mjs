// Generated from runtime/alpha3/src/operations/quality-cli.mts; edit the TypeScript source.
...
export async function qualityCli(args = process.argv.slice(2)) {
...
npm run check -- build --out NEW_DIR [--inputs PINNED_INPUTS]
...
Checks do not format or repair sources. Records are local under artifacts/checks/.`);
    ...
    if (request.command === 'list') {
        for (const module of context.registry.modules)
            console.log(module.id + ': ' + module.aliases.join(' / '));
        for (const task of context.registry.tasks)
            console.log(task.id + (task.heavy ? ' [explicit/heavy]' : '')
    ...
    if (request.command === 'resume') {
        ...
        if (!previous)
            throw Error('No previous run; choose a scope first');
    ...
    if (request.plan)
        return plan.uncovered.length ? 2 : 0;
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
    try {
        process.exitCode = await qualityCli();
    }
    catch (error) {
