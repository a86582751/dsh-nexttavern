// Generated from runtime/alpha3/src/operations/quality-projection.mts; edit the TypeScript source.
...
export function projectChecks(plan, registry) {
    const paths = new Map();
    for (const artifact of plan.artifacts)
        if (artifact.public?.path)
            paths.set(artifact.id, artifact.public.path);
    for (const delivery of plan.deliveries) {
        const base = plan.publicRelease.layoutPaths[delivery.layout];
        if (base !== undefined && !paths.has(delivery.artifact))
            paths.set(delivery.artifact, path.posix.join(base, delivery.path));
    }
    for (const test of plan.publicRelease.publicTests)
        paths.set(test.artifact, test.path);
    ...
    for (const directory of plan.typeScript?.declarationPackages ?? []) {
        ...
        if (!target)
            throw Error('Public check missing declaration package: ' + directory);
        for (const artifact of plan.artifacts)
            if (artifact.sha256 && artifact.source.startsWith(directory + '/node_modules/')) {
                paths.set(artifact.id, path.posix.dirname(target) + artifact.source.slice(directory.length));
            }
    }
    const sourcePaths = new Map();
    for (const artifact of plan.artifacts)
        if (paths.has(artifact.id) && !sourcePaths.has(artifact.source)) {
    ...
    const mapping = {
        ...
        builds: plan.builds.filter(build => paths.has(build.artifact) && sourcePaths.has(build.entry)).map(build => ({
            ...
            inputs: build.inputs.map(input => sourcePaths.get(input)).filter((input) => !!input),
        })),
    ...
    mapping.artifacts.push({ id: 'quality-registry', source: 'tools/check-registry.json' }, { id: 'quality-public-map', source: 'tools/check-map.json' });
