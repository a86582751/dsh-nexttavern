...
export function capture(ctx, patches) {
    if (!ctx.profileContext)
        throw Error('NextTavern requires the alpha.7 profile context');
...
export function composition(ctx) {
    ...
    if (previous && isDeepStrictEqual(previous.patches, composed))
        return previous;
    let plan;
    try {
