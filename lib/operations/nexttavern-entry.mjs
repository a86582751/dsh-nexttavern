...
export default class NextTavernEntry extends EntryTree {
    ...
    constructor(ctx, config) {
        ...
        ctx.on('loader/patch-context', async (entry, next) => {
            ...
            if (entry.parent.tree === host && this.config.providers.some(provider => provider.id === entry.options.id && provider.original === entry.options.name)) {
                await this.stop();
            }
            await next();
        }, { global: true });
        ...
        ctx.on('internal/update', async function (candidate, _noSave, next) {
            ...
            const task = state.queue.then(async () => {
                ...
                try {
                    await next();
                    if (!owner.closing && state.owner === owner)
                        await owner.reconcile(before);
                }
                catch (error) {
                    state.plan = before;
