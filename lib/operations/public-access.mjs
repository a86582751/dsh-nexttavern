// Generated from runtime/alpha3/src/operations/public-access.mts; edit the TypeScript source.
...
for (let i = 0; i < args.length; i++) {
    ...
    if (['apply', 'stopped'].includes(key))
        options[key] = true;
    else if (['root', 'group', 'patches', 'backup', 'rollback'].includes(key))
        options[key] = args[++i];
    else
        throw Error('Unknown option: ' + args[i]);
}
if ((options.apply || options.rollback) && (!options.stopped || process.env.DSH_ALLOW_PUBLIC_ACCESS !== '1'))
    throw Error('Explicit --stopped and DSH_ALLOW_PUBLIC_ACCESS=1 required');
if (options.rollback) {
    console.log(JSON.stringify(rollbackTransaction(options.rollback), null, 2));
}
else {
    if (!options.root || !['harness', 'profile'].includes(options.group))
        throw Error('--root and --group harness|profile are required');
    const file = options.patches ?? fileURLToPath(new URL('../../tools/public-access.json', import.meta.url));
    const data = JSON.parse(fs.readFileSync(file, 'utf8'));
    if (data.schemaVersion !== 1 || data.kind !== 'nexttavern-public-access' || !Array.isArray(data.patches))
        throw Error('Invalid public-access manifest');
    const patches = data.patches.filter(patch => patch.group === options.group);
    if (patches.length !== (options.group === 'harness' ? 2 : 1))
        throw Error('Incomplete public-access patch group');
