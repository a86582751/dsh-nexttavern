...
import { durableReferencesIntact, readPreparedProfile } from './protected-packages.mjs';
/**
...
/**
 * Whether this profile already carries exactly the durable references this
 * installed bundle needs.
 *
 * Both halves decide, because either one alone would report a profile that is
 * not actually ready: the receipt must name this bundle's own versions and
 * generations, and the profile must still carry the references and generations
 * those records describe. No member's bytes are read here - that admission
 * stays in the locked transaction, which is also the only writer.
 */
export function bundleReferencesCurrent(productRoot, home, profile) {
...
export function createOwnedPackagePreparation(facts) {
    return createPackagePreparation({
        ...
        prepare: async () => {
            // A mismatch in either half of this read leaves the decision - and every
            // write - to the locked transaction below.
