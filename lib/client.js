...
function createTelemetryPanels({ React, sessionDrafts, jsonFetch, toast }) {
  ...
  function panelRange(preset, custom) {
    ...
    if (preset === "all")
      ...
      from = to - ({ "1h": 36e5, "24h": 864e5, "3d": 2592e5 }[preset] ?? 864e5);
    return { from, to };
  }
  ...
  function PricePanel({ sessionId, models, onSaved }) {
    ...
    React.useEffect(() => {
      const timer = setTimeout(() => loadCatalog(query, catalogKeys).catch((e) => setCatalog({ error: errorMessage(e) })), 250);
  ...
  function TelemetryPanel({ sessionId, mode }) {
    ...
    React.useEffect(() => {
      ...
      jsonFetch(`/api/roleplay/usage-requests?${q}`, { signal: controller.signal }).then((value) => {
        ...
        if (live)
          setRequests({ error: errorMessage(e), rows: [] });
      }).finally(() => {
        if (live)
    ...
    const select = (label, value, handler, options) => React.createElement("label", { className: "dsh-rp-filter" }, React.createElement("span", null, label), React.createElement("select", { "aria-label": label, value, onChange: (e) => {
...
function createManagementPanels({ React, sessionDrafts, jsonFetch, toast, confirmWithDialog: confirmWithDialog2, btn }) {
  function CharacterClusterPanel({ sessionId }) {
    ...
    const save = async (scope) => {
      ...
      setSaving(true);
      try {
        const d = await jsonFetch("/api/roleplay/character-cluster", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ sessionId, scope, settings: scope === "global" ? { defaultRoute: globalRoute } : draft, expectedRevision: scope === "global" ? data.global.revision : data.settings.revision }) });
        setData(d);
        if (scope === "session")
          setDraft(d.session);
        else
    ...
    const selector = (label, route, onChange, isDefault = false) => {
      ...
      return h("div", { className: "dsh-rp-item", key: label }, h("div", { className: "dsh-rp-item-head" }, label), h("div", { style: { display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(min(100%,240px),1fr))", gap: "12px", marginTop: "10px" } }, h("label", null, "\u6A21\u578B", h("select", { className: "dsh-rp-input", style: { width: "100%", minWidth: 0 }, "aria-label": label + "\u6A21\u578B", value, onChange: (e) => {
      ...
      } }, h("option", { value: "" }, isDefault ? "\u8DDF\u968F\u4E3B\u4EE3\u7406\u6A21\u578B" : "\u4F7F\u7528\u96C6\u7FA4\u9ED8\u8BA4\u6A21\u578B"), h("option", { value: "main" }, "\u4E0E\u4E3B\u4EE3\u7406\u76F8\u540C\uFF08\u72EC\u7ACB\u89D2\u8272\u4EE3\u7406\uFF09"), catalog.map((m) => h("option", { key: m.provider + ":" + m.model, value: `${m.provider}\0${m.model}` }, `${m.label ?? m.model} \xB7 ${m.provider}`)))), h("label", null, "\u601D\u8003\u5F3A\u5EA6", h("select", { className: "dsh-rp-input", style: { width: "100%", minWidth: 0 }, "aria-label": label + "\u601D\u8003\u5F3A\u5EA6", value: route?.reasoningEffort ?? "", disabled: !model, onChange: (e) => onChange({ ...route ?? effective, reasoningEffort: e.target.value || void 0 }) }, options.map((o) => h("option", { key: o.id || "default", value: o.id }, o.name))))));
    };
    return h("div", { className: "dsh-rp-panel", "data-roleplay-character-cluster": true }, h("div", { className: "dsh-rp-row" }, h("h4", { style: { flex: 1, margin: 0 } }, "\u89D2\u8272 agent \u96C6\u7FA4"), btn("\u5237\u65B0\u4EBA\u7269\u4E0E\u8BBE\u7F6E", () => setRevision((r) => r + 1), { disabled: saving })), h("p", { className: "dsh-rp-muted" }, "\u8BA9\u4E3B\u8981\u4EBA\u7269\u5148\u72EC\u7ACB\u63A8\u6F14\u81EA\u5DF1\u7684\u884C\u52A8\u4E0E\u53F0\u8BCD\uFF0C\u518D\u7531\u4E3B\u7B14\u534F\u8C03\u6210\u6545\u4E8B\u3002\u5F00\u5173\u4E0E\u4EBA\u7269\u5355\u72EC\u8BBE\u7F6E\u4EC5\u5F53\u524D\u5BF9\u8BDD\u751F\u6548\uFF1B\u9ED8\u8BA4\u6A21\u578B\u5BF9\u6240\u6709\u5BF9\u8BDD\u751F\u6548\uFF0C\u5267\u60C5\u8D44\u6599\u59CB\u7EC8\u72EC\u7ACB\u3002"), error ? h("p", { className: "dsh-rp-error", role: "alert" }, error) : !draft ? h("p", { role: "status" }, "\u6B63\u5728\u52A0\u8F7D\u89D2\u8272\u96C6\u7FA4\u2026") : h("fieldset", { disabled: saving, style: { border: 0, padding: 0, margin: 0, minWidth: 0 } }, h("label", { className: "dsh-rp-row" }, h("input", { type: "checkbox", checked: draft.enabled, onChange: (e) => setDraft({ ...draft, enabled: e.target.checked }) }), "\u5F00\u542F\u89D2\u8272 agent \u96C6\u7FA4\u6A21\u5F0F"), h("p", { className: "dsh-rp-muted" }, "\u8BFB\u5361\u56DE\u5408\u4E0D\u8FD0\u884C\u3002\u6BCF\u4F4D\u51FA\u573A\u4E3B\u8981\u4EBA\u7269\u5404\u81EA\u8FD0\u884C\u4E00\u6B21\uFF0C\u53EF\u6309\u9700\u67E5\u9605\u5386\u53F2\uFF1B\u5931\u8D25\u6216\u8D85\u65F6\u4F1A\u4F7F\u7528\u4E3B\u4EE3\u7406\u6A21\u578B\u518D\u8BD5\u4E00\u6B21\uFF0C\u4ECD\u5931\u8D25\u7531\u4E3B\u7B14\u81EA\u7531\u53D1\u6325\u3002\u5F00\u542F\u4F1A\u589E\u52A0\u8BF7\u6C42\u7528\u91CF\u548C\u6B63\u6587\u7B49\u5F85\u65F6\u95F4\u3002"), h("section", { className: "dsh-rp-cluster-default", "aria-label": "\u89D2\u8272\u96C6\u7FA4\u9ED8\u8BA4\u6A21\u578B\uFF08\u5168\u5C40\uFF09" }, h("span", { className: "dsh-rp-cluster-scope" }, "\u5168\u5C40\u9ED8\u8BA4 \xB7 \u6240\u6709\u5BF9\u8BDD"), selector("\u89D2\u8272\u96C6\u7FA4\u9ED8\u8BA4\u6A21\u578B\uFF08\u5168\u5C40\uFF09", globalRoute, setGlobalRoute, true), h("p", { className: "dsh-rp-muted" }, "\u6240\u6709\u672A\u5355\u72EC\u6307\u5B9A\u6A21\u578B\u7684\u4EBA\u7269\u90FD\u4F7F\u7528\u6B64\u9ED8\u8BA4\u503C\uFF1B\u4FDD\u5B58\u4E0D\u4F1A\u5F00\u542F\u5176\u4ED6\u5BF9\u8BDD\u7684\u96C6\u7FA4\u3002"), Number(data.global.revision) === 0 && data.session.defaultRoute ? h("p", { className: "dsh-rp-muted" }, "\u5F53\u524D\u5BF9\u8BDD\u6682\u65F6\u6CBF\u7528\u65E7\u7248\u9ED8\u8BA4\u6A21\u578B\uFF1B\u4FDD\u5B58\u4E00\u6B21\u5168\u5C40\u9009\u62E9\u540E\uFF0C\u6240\u6709\u5BF9\u8BDD\u7EDF\u4E00\u7EE7\u627F\u5168\u5C40\u9ED8\u8BA4\u3002") : null, h("div", { className: "dsh-rp-row" }, btn(saving ? "\u4FDD\u5B58\u4E2D\u2026" : "\u4FDD\u5B58\u5168\u5C40\u9ED8\u8BA4\u6A21\u578B", () => save("global"), { disabled: saving }))), h("section", { className: "dsh-rp-cluster-characters", "aria-label": "\u4EBA\u7269\u5355\u72EC\u6A21\u578B" }, h("h4", null, "\u4EBA\u7269\u5355\u72EC\u6A21\u578B"), h("p", { className: "dsh-rp-muted" }, "\u4EC5\u5F53\u524D\u5BF9\u8BDD\u751F\u6548\u3002\u9009\u62E9\u201C\u4F7F\u7528\u96C6\u7FA4\u9ED8\u8BA4\u6A21\u578B\u201D\u5373\u53EF\u8DDF\u968F\u4E0A\u65B9\u5DF2\u4FDD\u5B58\u7684\u5168\u5C40\u9ED8\u8BA4\u8BBE\u7F6E\u3002"), data.characters.map((character) => selector(character.name, draft.characters?.[character.id], (route) => setDraft({ ...draft, characters: { ...draft.characters, [character.id]: route } }))), !data.characters.length ? h("p", { className: "dsh-rp-muted" }, "\u5F53\u524D\u8FD8\u6CA1\u6709\u72EC\u7ACB\u4EBA\u7269\u8BBE\u5B9A\u3002\u8BFB\u5361\u540E\u4F1A\u5728\u8FD9\u91CC\u663E\u793A\uFF1B\u6545\u4E8B\u4E2D\u65B0\u767B\u8BB0\u7684\u91CD\u8981\u4EBA\u7269\u4E5F\u4F1A\u52A0\u5165\u3002") : null), h("div", { className: "dsh-rp-row" }, btn(saving ? "\u4FDD\u5B58\u4E2D\u2026" : "\u4FDD\u5B58\u5F53\u524D\u5BF9\u8BDD\u8BBE\u7F6E", () => save("session"), { disabled: saving }))));
  }
  function ModelPanel({ sessionId }) {
    ...
    React.useEffect(() => {
      let live = true;
      jsonFetch(`/api/roleplay/models?sessionId=${encodeURIComponent(sessionId)}`).then((value) => live && setData(value)).catch((e) => live && setData({ error: String(errorMessage2(e)) }));
      return () => {
        live = false;
      };
    }, [sessionId]);
    ...
    if (!data)
      return React.createElement("div", { className: "dsh-rp-muted" }, "\u52A0\u8F7D\u6A21\u578B\u2026");
    if (data.error)
      return React.createElement("div", { className: "dsh-rp-error" }, data.error);
    ...
    return React.createElement("fieldset", { className: "dsh-rp-panel", disabled: saving, style: { border: 0, margin: 0, minWidth: 0 } }, React.createElement("h4", null, "\u6A21\u578B\u8DEF\u7531"), React.createElement("label", { className: "dsh-rp-row" }, "\u4FDD\u5B58\u8303\u56F4", React.createElement("select", { className: "dsh-rp-input", value: scope, onChange: (e) => setScope(e.target.value) }, React.createElement("option", { value: "session" }, "\u5F53\u524D\u4F1A\u8BDD"), React.createElement("option", { value: "global" }, "\u5168\u5C40"))), React.createElement("label", { className: "dsh-rp-row" }, React.createElement("input", { type: "checkbox", checked: allMain, onChange: (e) => rememberModel(e.target.checked, routes) }), "\u5168\u90E8\u4F7F\u7528\u4E3B\u6A21\u578B\uFF08\u4FDD\u7559\u7528\u9014\u914D\u7F6E\uFF09"), React.createElement("div", { className: "dsh-rp-muted" }, `\u5F53\u524D\u4E3B\u6A21\u578B\uFF1A${data.main?.model ?? data.effective?.main?.model ?? "\u672A\u8BBE\u7F6E"}`), MODEL_PURPOSE_IDS.map((id) => {
  ...
  function ResourcesPanel({ sessionId }) {
    ...
    const load = React.useCallback(async () => {
      ...
      try {
        ...
        if (ticket === listSequence.current)
          setData((previous) => ({ ...previous, error: String(errorMessage2(e)) }));
      } finally {
        if (ticket === listSequence.current)
          setLoading(false);
      }
    ...
    const importResource = async (resource) => {
      ...
      if (!await confirmWithDialog2(document, "\u8F7D\u5165\u4F1A\u66FF\u6362\u5F53\u524D\u4EBA\u8BBE\uFF0C\u786E\u5B9A\u7EE7\u7EED\uFF1F", { title: "\u8F7D\u5165\u5F53\u524D\u4EBA\u8BBE", confirmLabel: "\u786E\u8BA4\u8F7D\u5165" }))
        return;
      try {
    ...
    return React.createElement("div", { className: "dsh-rp-panel" }, React.createElement("div", { className: "dsh-rp-row" }, React.createElement("h4", null, "\u8D44\u6E90\u5E93"), React.createElement("button", { className: "dsh-rp-btn", disabled: loading, onClick: load }, loading ? "\u5237\u65B0\u4E2D\u2026" : "\u5237\u65B0\u8D44\u6E90\u5E93")), React.createElement("div", { className: "dsh-rp-row" }, React.createElement("input", { className: "dsh-rp-input", placeholder: "\u641C\u7D22\u8D44\u6E90", value: query, onChange: (e) => setQuery(e.target.value) }), React.createElement("select", { className: "dsh-rp-input", "aria-label": "\u8D44\u6E90\u7C7B\u578B", value: type, onChange: (e) => setType(e.target.value) }, React.createElement("option", { value: "all" }, "\u5168\u90E8\u7C7B\u578B"), types.map((value) => React.createElement("option", { key: value, value }, resourceTypeLabel(value)))), React.createElement("select", { className: "dsh-rp-input", "aria-label": "\u8D44\u6E90\u6392\u5E8F", value: order, onChange: (e) => setOrder(e.target.value) }, [["time-desc", "\u4FEE\u6539\u65F6\u95F4\uFF1A\u6700\u65B0\u5728\u524D"], ["time-asc", "\u4FEE\u6539\u65F6\u95F4\uFF1A\u6700\u65E9\u5728\u524D"], ["name-asc", "\u540D\u79F0\uFF1A\u5347\u5E8F"], ["name-desc", "\u540D\u79F0\uFF1A\u964D\u5E8F"]].map(([value, label]) => React.createElement("option", { key: value, value }, label)))), loading && !data ? React.createElement("p", { className: "dsh-rp-muted", role: "status" }, "\u6B63\u5728\u52A0\u8F7D\u8D44\u6E90\u2026") : null, pendingItems.length ? React.createElement("div", { className: "dsh-rp-item" }, React.createElement("div", { className: "dsh-rp-item-head" }, "\u5F85\u5904\u7406\u8D44\u6E90"), pendingItems.map((item, index) => React.createElement("div", { className: "dsh-rp-row", key: index }, React.createElement("span", null, item.name ?? "\u672A\u547D\u540D\u8D44\u6E90"), React.createElement("span", { className: "dsh-rp-error" }, item.reason))), React.createElement("button", { className: "dsh-rp-btn", onClick: load, disabled: loading }, "\u91CD\u65B0\u68C0\u67E5")) : null, preview ? React.createElement("pre", { className: "dsh-rp-item" }, String(preview.text ?? preview.resource?.text ?? "")) : null, data?.error ? React.createElement("div", { className: "dsh-rp-error", role: "alert" }, data.error) : null, !loading && data && !data.error && !resources.length ? React.createElement("p", { className: "dsh-rp-muted" }, "\u6CA1\u6709\u627E\u5230\u8D44\u6E90\uFF0C\u53EF\u8C03\u6574\u7B5B\u9009\u6216\u70B9\u51FB\u5237\u65B0\u3002") : null, resources.map((resource) => {
      const timestamp = resource.modifiedAt ?? resource.updatedAt ?? resource.createdAt;
      return React.createElement("div", { className: "dsh-rp-item", key: resource.id }, React.createElement("div", { className: "dsh-rp-item-head" }, resource.name, React.createElement("span", { className: "dsh-rp-muted" }, `${resourceTypeLabel(resource.type)} \xB7 ${resource.bytes} bytes`)), React.createElement("div", { className: "dsh-rp-muted" }, "\u4FEE\u6539\u65F6\u95F4\uFF1A", timestamp && Number.isFinite(Date.parse(timestamp)) ? new Date(timestamp).toLocaleString() : "\u672A\u77E5"), React.createElement("div", { className: "dsh-rp-row" }, React.createElement("button", { className: "dsh-rp-btn", onClick: () => view(resource) }, "\u67E5\u770B"), React.createElement("button", { className: "dsh-rp-btn", onClick: () => navigator.clipboard?.writeText(resource.path).then(() => toast("\u8D44\u6E90\u8DEF\u5F84\u5DF2\u590D\u5236")).catch(() => toast("\u590D\u5236\u5931\u8D25")) }, "\u590D\u5236\u8DEF\u5F84"), React.createElement("a", { className: "dsh-rp-btn", href: `/api/roleplay/download?sessionId=${encodeURIComponent(sessionId)}&resourceId=${encodeURIComponent(resource.id)}` }, "\u4E0B\u8F7D"), isCardReadableResource(resource) ? React.createElement("button", { className: "dsh-rp-btn", onClick: () => importResource(resource) }, "\u8F7D\u5165\u5F53\u524D") : null));
    }));
  }
...
function createAuthorPanels({ React, sessionDrafts, fetchState, invalidateState, saveState, runMaintenance, jsonFetch, toast }) {
  ...
  const PanelShell = ({ title, children, visible, sessionId, error, loading, refresh, extra }) => React.createElement("div", { className: "dsh-rp-panel" }, React.createElement("div", { className: "dsh-rp-row" }, React.createElement("h4", { style: { margin: 0, flex: 1 } }, title), extra, btn("\u5237\u65B0", refresh)), loading ? React.createElement("div", { className: "dsh-rp-muted", role: "status" }, "\u52A0\u8F7D\u8BBE\u5B9A\u4E2D\u2026") : null, error ? React.createElement("div", { className: "dsh-rp-error", role: "alert" }, `\u8BBE\u5B9A\u52A0\u8F7D\u5931\u8D25\uFF1A${error}`, React.createElement("button", { type: "button", className: "dsh-rp-btn", onClick: refresh }, "\u91CD\u65B0\u52A0\u8F7D")) : null, children);
  function MemorySettingsControls({ sessionId, visible, onSaved, retrievalControls }) {
    ...
    const input = ([field, label, min]) => {
      ...
      return React.createElement("label", { key: field, className: "dsh-rp-field", style: { flex: "1 1 200px", minWidth: 0 } }, label, React.createElement("input", { className: "dsh-rp-input", type: "number", min, step: 1, disabled: saving, value: values[field] ?? "", placeholder: `\u7EE7\u627F\uFF1A${inherited}`, onChange: (e) => change(field, e.target.value) }), React.createElement("small", { className: "dsh-rp-muted" }, field === "autoNotesEveryTurns" ? `\u6BCF ${values[field] ?? inherited} \u4E2A\u6B63\u53F2\u5267\u60C5\u8F6E\u6B21\uFF1B\u91CD\u65B0\u751F\u6210\u4E0D\u8BA1\u6570` : `${values[field] ?? inherited} tokens${values[field] == null ? " \xB7 \u7EE7\u627F" : ""}`));
    };
    return React.createElement("section", { "data-roleplay-memory-settings": scope }, React.createElement("h4", null, "\u7A97\u53E3\u4E0E\u540E\u53F0\u7B14\u8BB0"), React.createElement("div", { className: "dsh-rp-row", role: "group", "aria-label": "\u8BB0\u5FC6\u8BBE\u7F6E\u8303\u56F4" }, ["session", "global"].map((s) => React.createElement("button", { key: s, type: "button", className: "dsh-rp-btn" + (scope === s ? " dsh-rp-primary" : ""), "aria-pressed": scope === s, disabled: saving, onClick: () => setScope(s) }, s === "session" ? "\u672C\u4F1A\u8BDD" : "\u5168\u5C40\u9ED8\u8BA4"))), React.createElement("p", { className: "dsh-rp-muted" }, scope === "session" ? "\u7559\u7A7A\u7684\u9879\u76EE\u6CBF\u7528\u5168\u5C40\u8BBE\u7F6E\uFF1B\u4EC5\u5F71\u54CD\u5F53\u524D\u4F1A\u8BDD\u3002" : "\u4F5C\u4E3A\u6240\u6709\u672A\u5355\u72EC\u8986\u76D6\u4F1A\u8BDD\u7684\u9ED8\u8BA4\u8BBE\u7F6E\u3002\u7559\u7A7A\u6062\u590D\u8F6F\u4EF6\u9ED8\u8BA4\u503C\u3002"), error ? React.createElement("div", { role: "alert", className: "dsh-rp-error" }, error, React.createElement("button", { className: "dsh-rp-btn", onClick: () => setRevision((v) => v + 1) }, "\u91CD\u65B0\u52A0\u8F7D")) : null, policy ? React.createElement(React.Fragment, null, React.createElement("div", { className: "dsh-rp-row", style: { alignItems: "flex-start", flexWrap: "wrap" } }, fields.map(input)), React.createElement("p", { className: "dsh-rp-muted" }, "\u7A97\u53E3\u5373\u5C06\u6DD8\u6C70\u65E7\u6B63\u6587\u65F6\uFF0C\u4F1A\u5148\u786E\u8BA4\u68C0\u67E5\u70B9\u5DF2\u4FDD\u5B58\uFF1B\u540E\u53F0\u6574\u7406\u9891\u7387\u4E0D\u53D6\u6D88\u8FD9\u9053\u4FDD\u5B58\u5173\u53E3\u3002"), retrievalControls, React.createElement("details", null, React.createElement("summary", { style: { cursor: "pointer", padding: "12px 0" } }, "\u9AD8\u7EA7\u517C\u5BB9\u9009\u9879"), React.createElement("p", { className: "dsh-rp-muted" }, "\u65E7\u538B\u7F29\u6A21\u5F0F\u7684\u517C\u5BB9\u4FDD\u62A4\u53C2\u6570\uFF0C\u4EC5\u7528\u4E8E\u6A21\u578B\u4E0A\u4E0B\u6587\u538B\u529B\u515C\u5E95\uFF1B\u4E0D\u63A7\u5236\u65E5\u5E38\u786C\u5207\u7A97\u53E3\u548C\u540E\u53F0\u7B14\u8BB0\u9891\u7387\u3002"), React.createElement("div", { className: "dsh-rp-row", style: { flexWrap: "wrap" } }, legacy.map(input))), React.createElement("div", { className: "dsh-rp-row" }, React.createElement("button", { className: "dsh-rp-btn dsh-rp-primary", disabled: saving || !sessionDrafts.has(draftKey), onClick: () => save() }, saving ? "\u4FDD\u5B58\u4E2D\u2026" : "\u4FDD\u5B58\u8BB0\u5FC6\u8BBE\u7F6E"), React.createElement("button", { className: "dsh-rp-btn", disabled: saving, onClick: () => save(true) }, scope === "session" ? "\u6062\u590D\u5168\u5C40\u8BBE\u7F6E" : "\u6062\u590D\u9ED8\u8BA4\u503C"))) : !error ? React.createElement("p", { className: "dsh-rp-muted" }, "\u6B63\u5728\u8BFB\u53D6\u8BB0\u5FC6\u8BBE\u7F6E\u2026") : null);
  }
  function MemoryPanel(props) {
    ...
    const rememberMemory = (patch) => {
      ...
      if (!prior?.dirty)
        updatePanelDraft(sessionDrafts, sessionId, "memory", { baseVersions: data?.recordVersions });
      setDirty(true);
    };
    React.useEffect(() => {
    ...
    }, placeholder: "\u5C1A\u65E0\u5BFC\u6F14\u7B14\u8BB0" }), React.createElement("div", { className: "dsh-rp-row" }, btn(saving ? "\u4FDD\u5B58\u4E2D\u2026" : "\u4FDD\u5B58\u603B\u7ED3", save)), React.createElement("h4", null, "\u9501\u5B9A\u4E8B\u5B9E\uFF08\u538B\u7F29\u65F6\u6C38\u4E0D\u88AB\u6539\u5199\uFF09"), facts.length ? facts.map((f, i) => React.createElement("div", { key: i, className: "dsh-rp-item" }, typeof f === "string" ? f : f.text)) : React.createElement("div", { className: "dsh-rp-muted" }, "\uFF08\u65E0\uFF1B/memory lock <\u4E8B\u5B9E> \u6216\u89D2\u8272\u5361\u9501\u5B9A\uFF09"), React.createElement(MemorySettingsControls, { sessionId, visible, onSaved: refresh, retrievalControls: props.retrievalControls }), React.createElement("h4", null, "\u6700\u8FD1\u589E\u91CF\uFF08" + deltas.length + " \u6761\uFF09"), deltas.slice(-12).reverse().map((d, i) => React.createElement("div", { key: i, className: "dsh-rp-item" }, React.createElement("div", { className: "dsh-rp-muted" }, "seq " + d.evidenceSeq + " \xB7 " + (d.status ?? "")), d.summary)));
  }
  function WorldbookPanel(props) {
    ...
    const field = (label, value, onChange, type = "text") => React.createElement("label", { className: "dsh-rp-row" }, React.createElement("span", { className: "dsh-rp-muted", style: { width: "64px" } }, label), React.createElement("input", { className: "dsh-rp-input", type, value: value ?? "", onChange: (e) => onChange(e.target.value) }));
    return React.createElement(PanelShell, {
      title: "\u4E16\u754C\u4E66",
      visible,
      sessionId,
      error,
      refresh,
      extra: btn("\uFF0B \u65B0\u589E", () => startEdit("new"), { disabled: saving })
    }, editing ? React.createElement("fieldset", { disabled: saving, style: { border: 0, padding: 0, margin: 0 } }, React.createElement("div", { className: "dsh-rp-item" }, field("id", form.id, (v) => rememberForm({ ...form, id: v })), field("\u540D\u79F0", form.name, (v) => rememberForm({ ...form, name: v })), field("\u7C7B\u578B", form.kind, (v) => rememberForm({ ...form, kind: v })), field("\u5173\u952E\u8BCD", form.keywords, (v) => rememberForm({ ...form, keywords: v })), React.createElement("textarea", { className: "dsh-rp-textarea", value: form.content ?? "", onChange: (e) => rememberForm({ ...form, content: e.target.value }), placeholder: "\u6761\u76EE\u5185\u5BB9\uFF08\u89E6\u53D1\u65F6\u4EE5\u9690\u85CF\u4E0A\u4E0B\u6587\u6CE8\u5165\uFF09" }), React.createElement("div", { className: "dsh-rp-row" }, field("\u4F18\u5148\u7EA7", form.priority, (v) => rememberForm({ ...form, priority: v }), "number"), field("token\u9884\u7B97", form.token_budget, (v) => rememberForm({ ...form, token_budget: v }), "number"), React.createElement("label", null, React.createElement("input", { type: "checkbox", checked: form.always_on === true, onChange: (e) => rememberForm({ ...form, always_on: e.target.checked }) }), "\u65E7\u5361\u5E38\u9A7B\u517C\u5BB9\uFF08\u5EFA\u8BAE\u6574\u7406\u5230\u6838\u5FC3\u8BBE\u5B9A\uFF09"), React.createElement("span", { className: "dsh-rp-muted" }, "\u65B0\u80CC\u666F\u8D44\u6599\u653E\u6838\u5FC3\u8BBE\u5B9A\uFF1B\u666E\u901A\u4E16\u754C\u4E66\u6309\u67E5\u8BE2\u8BFB\u53D6\u3002"), React.createElement("label", null, React.createElement("input", { type: "checkbox", checked: form.locked === true, onChange: (e) => rememberForm({ ...form, locked: e.target.checked }) }), "\u9501\u5B9A")), React.createElement("div", { className: "dsh-rp-row" }, btn(saving ? "\u4FDD\u5B58\u4E2D\u2026" : "\u4FDD\u5B58", save), btn("\u53D6\u6D88", () => {
      sessionDrafts.delete(sessionId + ":worldbook");
      setEditing(null);
    })))) : entries.length ? entries.map((e) => React.createElement("div", { key: e.id, className: "dsh-rp-item" }, React.createElement("div", { className: "dsh-rp-item-head" }, React.createElement("span", { className: "dsh-rp-item-title" }, e.name + (e.locked ? " \u{1F512}" : "")), React.createElement("span", { className: "dsh-rp-muted" }, e.kind + " \xB7 \u4F18\u5148\u7EA7" + e.priority + (e.alwaysOn ? " \xB7 \u5E38\u9A7B" : ""))), React.createElement("div", { className: "dsh-rp-muted", style: { margin: "2px 0" } }, String(e.content ?? "").slice(0, 120)), React.createElement("div", { className: "dsh-rp-row" }, btn("\u7F16\u8F91", () => startEdit(e)), btn("\u5220\u9664", () => remove(e.id))))) : React.createElement("div", { className: "dsh-rp-muted" }, "\uFF08\u6682\u65E0\u6761\u76EE\uFF1B\u70B9\u51FB\u300C\uFF0B \u65B0\u589E\u300D\u6216\u8BA9 AI \u7528 rp_worldbook_add \u5199\u5165\uFF09"));
  }
  function CardsPanel(props) {
    ...
    const field = (label, value, onChange) => React.createElement("label", { className: "dsh-rp-row" }, React.createElement("span", { className: "dsh-rp-muted", style: { width: "64px" } }, label), React.createElement("input", { className: "dsh-rp-input", value: value ?? "", onChange: (e) => onChange(e.target.value) }));
    return React.createElement(PanelShell, {
      title: "\u4EBA\u7269\u8BBE\u5B9A",
      visible,
      sessionId,
      error,
      refresh,
      extra: btn("\uFF0B \u65B0\u589E\u4EBA\u7269", () => startEdit("new"), { disabled: saving })
    }, editing ? React.createElement("fieldset", { disabled: saving, style: { border: 0, padding: 0, margin: 0 } }, React.createElement("div", { className: "dsh-rp-item" }, field("card_id", form.card_id, (v) => rememberForm({ ...form, card_id: v })), field("\u540D\u79F0", form.name, (v) => rememberForm({ ...form, name: v })), React.createElement("label", { className: "dsh-rp-row" }, React.createElement("span", { className: "dsh-rp-muted", style: { width: "64px" } }, "\u7C7B\u578B"), React.createElement("select", { className: "dsh-rp-input", value: form.kind, onChange: (e) => rememberForm({ ...form, kind: e.target.value }) }, React.createElement("option", { value: "user" }, "user\uFF08\u73A9\u5BB6\u89D2\u8272\uFF09"), React.createElement("option", { value: "npc" }, "npc"), React.createElement("option", { value: "other" }, "other"))), React.createElement("textarea", { className: "dsh-rp-textarea", value: form.content ?? "", onChange: (e) => rememberForm({ ...form, content: e.target.value }), placeholder: "\u4EBA\u8BBE\u6B63\u6587\uFF08\u6027\u683C/\u53E3\u543B/\u52A8\u673A/\u79D8\u5BC6/\u5916\u8C8C/\u80FD\u529B/\u4EF7\u503C\u5E95\u7EBF\uFF09" }), React.createElement("label", null, React.createElement("input", { type: "checkbox", checked: form.locked === true, onChange: (e) => rememberForm({ ...form, locked: e.target.checked }) }), " \u9501\u5B9A\u4E8B\u5B9E\uFF08\u538B\u7F29\u65F6\u6C38\u4E0D\u88AB\u6539\u5199\uFF09"), React.createElement("div", { className: "dsh-rp-row" }, btn(saving ? "\u4FDD\u5B58\u4E2D\u2026" : "\u4FDD\u5B58", save), btn("\u53D6\u6D88", () => {
      sessionDrafts.delete(sessionId + ":cards");
      setEditing(null);
    })))) : cards.length ? cards.map((c) => React.createElement("div", { key: c.id, className: "dsh-rp-item" }, React.createElement("div", { className: "dsh-rp-item-head" }, React.createElement("span", { className: "dsh-rp-item-title" }, c.name + (c.locked ? " \u{1F512}" : "")), React.createElement("span", { className: "dsh-rp-muted" }, c.kind + " \xB7 v" + c.version)), React.createElement("div", { className: "dsh-rp-muted", style: { margin: "2px 0" } }, String(c.content ?? "").slice(0, 120)), React.createElement("div", { className: "dsh-rp-row" }, btn("\u7F16\u8F91", () => startEdit(c))))) : React.createElement("div", { className: "dsh-rp-muted" }, "\uFF08\u6682\u65E0\u4EBA\u7269\u8BBE\u5B9A\uFF1B\u70B9\u51FB\u300C\uFF0B \u65B0\u589E\u4EBA\u7269\u300D\u6216\u8BA9 AI \u7528 rp_card_set \u5199\u5165\uFF09"));
  }
  function RuleSlicePanel({ scope, visible, field, title, guidance, rows }) {
    ...
    const remember = (next) => {
      ...
      if (!prior?.dirty)
        updatePanelDraft(sessionDrafts, sessionId, "settings", { baseVersions: data?.recordVersions });
    };
    React.useEffect(() => {
      setValue(null);
      setSaving(false);
    ...
    return React.createElement(PanelShell, { title, visible, sessionId, error, loading, refresh }, React.createElement("p", null, guidance), React.createElement("textarea", { className: "dsh-rp-textarea", style: { minHeight: `${rows * 20}px` }, value: value ?? "", disabled: loading || !data, onChange: (e) => {
      setValue(e.target.value);
      remember(e.target.value);
    } }), legacyEntries.length ? React.createElement("details", null, React.createElement("summary", null, `\u6838\u5FC3\u8BBE\u5B9A\xB7\u65E7\u5361\u5E38\u9A7B\u6807\u8BB0\u517C\u5BB9\u6295\u5F71\uFF08${legacyEntries.length}\uFF09`), React.createElement("p", { className: "dsh-rp-muted" }, "\u539F\u4EF6\u672A\u6539\u5199\u3001\u53EF\u5728\u4E16\u754C\u4E66\u7BA1\u7406\u65E7\u8BB0\u5F55\u3002\u4EE5\u4E0B\u4E3A\u53EA\u8BFB\u5168\u6587\uFF1B\u4E0D\u4F1A\u81EA\u52A8\u8FC1\u79FB\u3001\u5220\u9664\u6216\u4FEE\u6539\u6570\u636E\u3002"), legacyEntries.map((entry) => React.createElement("div", { key: entry.id, className: "dsh-rp-item" }, React.createElement("div", { className: "dsh-rp-item-title" }, entry.name || entry.id || "\u672A\u547D\u540D\u6761\u76EE"), React.createElement("pre", { style: { whiteSpace: "pre-wrap", overflowWrap: "anywhere", margin: "8px 0 0" } }, String(entry.content ?? ""))))) : null, React.createElement("div", { className: "dsh-rp-row" }, btn(`\u4FDD\u5B58${title}`, save, { disabled: saving || loading || !data })));
  }
  ...
  function SettingsPanel(props) {
    ...
    React.useEffect(() => {
      ...
      if (!draft?.dirty)
        return;
      for (const [key, setter] of [["statusText", setStatusText], ["core", setCore], ["plot", setPlot], ["narrative", setNarrative], ["reply", setReply], ["styleKw", setStyleKw], ["opening", setOpening], ["beautyCss", setBeautyCss], ["beautyJs", setBeautyJs], ["beautyRules", setBeautyRules]])
        if (draft[key] !== void 0)
          setter(draft[key]);
    }, [sessionId]);
    React.useEffect(() => {
    ...
    return React.createElement(PanelShell, { title: "\u81EA\u5B9A\u4E49\u89C4\u5219", visible, sessionId, error, refresh }, React.createElement("h4", null, "\u72B6\u6001\u680F\u8BBE\u5B9A\uFF08\u6B63\u6587\u540E\u751F\u6210\u72B6\u6001\u680F\u7684\u683C\u5F0F\uFF09"), ta(statusText, (value) => {
      setStatusText(value);
      rememberSettings({ statusText: value });
    }, 8), React.createElement("div", { className: "dsh-rp-row" }, btn(saving ? "\u2026" : "\u4FDD\u5B58\u72B6\u6001\u680F", () => save({ kind: "status", text: statusText, expectedRevision: data?.recordVersions?.status ?? null }, "\u72B6\u6001\u680F\u5DF2\u4FDD\u5B58"))), React.createElement("h4", null, "\u53D9\u4E8B\u89C4\u5219"), ta(narrative, (value) => {
      setNarrative(value);
      rememberSettings({ narrative: value });
    }, 5), React.createElement("h4", null, "\u56DE\u590D\u89C4\u5219"), ta(reply, (value) => {
      setReply(value);
      rememberSettings({ reply: value });
    }, 5), React.createElement("div", { className: "dsh-rp-row" }, btn(saving ? "\u2026" : "\u4FDD\u5B58\u89C4\u5219", () => save({ ...buildCustomRulesSaveBody(narrative, reply), expectedRevision: data?.recordVersions?.rules ?? null }, "\u89C4\u5219\u5DF2\u4FDD\u5B58"), { disabled: saving })), React.createElement("h4", null, "\u521D\u59CB\u5267\u60C5\uFF08\u4EC5\u6545\u4E8B\u5F00\u59CB\u65F6\u6CE8\u5165\u4E00\u6B21\uFF09"), ta(opening, (value) => {
      setOpening(value);
      rememberSettings({ opening: value });
    }, 8), React.createElement("div", { className: "dsh-rp-row" }, btn(saving ? "\u2026" : "\u4FDD\u5B58\u5F00\u573A", () => save({ kind: "opening", text: opening, expectedRevision: data?.recordVersions?.opening ?? null }, "\u5F00\u573A\u5DF2\u4FDD\u5B58"))), React.createElement("h4", null, "\u6392\u7248\u7F8E\u5316 \xB7 \u6B63\u5219\u89C4\u5219\uFF08\u9605\u8BFB\u89C6\u56FE\u6B63\u6587\u7279\u6548\uFF1BJSON \u6570\u7EC4 [{match, replace}]\uFF09"), React.createElement("p", null, "\u5148\u5339\u914D\u539F\u6587\uFF1B\u672A\u547D\u4E2D\u65F6\u5339\u914D Markdown \u6E32\u67D3\u540E\u7684 HTML\u3002\u4E2D\u6587\u53CC\u5F15\u53F7\u89C4\u5219\u4E5F\u517C\u5BB9\u6B63\u6587\u4E2D\u7684\u6210\u5BF9\u76F4\u5F15\u53F7\uFF0C\u4E0D\u6539\u5199\u5267\u60C5\u8BB0\u5F55\u3002"), ta(beautyRules, (value) => {
    ...
    }, 6), React.createElement("div", { className: "dsh-rp-row" }, btn(saving ? "\u2026" : "\u4FDD\u5B58\u6392\u7248\u7F8E\u5316", () => {
      let rr;
      try {
  ...
  return { btn, PanelShell, MemoryPanel, WorldbookPanel, CardsPanel, CoreRulesPanel, PlotGuidancePanel, StyleSpecializationPanel, SettingsPanel };
}

// memory-retrieval-panel.js
var bytes = (n) => n == null ? "\u2014" : `${(n / 1024 ** 2).toFixed(1)} MiB`;
var protocols = { "openai": "OpenAI \u5B98\u65B9 / \u517C\u5BB9\u63A5\u53E3", "dashscope-text": "\u963F\u91CC\u4E91 DashScope \u6587\u672C", "dashscope-multimodal": "\u963F\u91CC\u4E91 DashScope \u591A\u6A21\u6001" };
...
function createMemoryRetrievalPanel({ React, jsonFetch, toast }) {
  const h = React.createElement;
  const field = (label, node) => h("label", { className: "dsh-rp-retrieval-field" }, label, node);
  const note = (value) => h("p", { className: "dsh-rp-muted" }, value);
  const badge = (value) => h("span", { className: "dsh-rp-provider-badge", key: value }, value);
  function RetrievalPanel({ sessionId, view }) {
    ...
    const load = React.useCallback(async () => {
      ...
      try {
        ...
        if (next.activeProviderId)
          setModelLists((old) => ({ ...old, [next.activeProviderId]: next.modelList }));
        if (!dirtyRef.current)
          setDraft((previous) => ({ ...next.providers.find((p) => p.id === previous.id) ?? next.providers.find((p) => p.id === next.activeProviderId) ?? next.providers[0] }));
    ...
    React.useEffect(() => {
      ...
      void jsonFetch(`/api/roleplay/memory-retrieval?sessionId=${encodeURIComponent(progressSession)}`).then((reply) => {
        ...
        setProgressReply({ id: progressSession, data: reply });
    ...
    const action = async (name, extra = {}) => {
      ...
      setError(null);
      try {
        const next = await jsonFetch("/api/roleplay/memory-retrieval", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ sessionId, expectedRevision: data.revision, action: name, ...extra }) });
        if (request !== requestRef.current || sessionRef.current !== sessionId)
          return;
        if (next.ok === false)
        ...
        if (name === "refresh-models")
          setModelLists((old) => ({ ...old, [String(extra.providerId)]: next.modelList }));
        if (name === "save-provider" && editRef.current === edit) {
          ...
          setDraft({ ...next.providers.find((p) => p.id === draft.id) ?? next.providers.find((p) => p.id === next.activeProviderId) ?? next.providers[0] });
        }
      } catch (e) {
        if (request === requestRef.current && sessionRef.current === sessionId) {
          setError(String(e));
    ...
    const button = (label, onClick, disabled = false, primary = false) => h("button", { type: "button", className: "dsh-rp-btn" + (primary ? " dsh-rp-primary" : ""), onClick, disabled: !data || busy !== null || disabled }, label);
    ...
    const requestIndex = (enabled) => {
      if (enabled && data?.progress.stale === true) {
        ...
        if (providerId) {
          setStaleConfirmation({ sessionId, providerId, revision: data.revision, fingerprint });
          return;
        }
      }
      void action("index", { enabled });
    };
    const confirmStaleRebuild = () => {
      ...
      setStaleConfirmation(null);
      void action("rebuild-index", { confirmProviderId: target.providerId });
    };
    ...
    const staleConfirmCard = staleConfirmation ? h("dialog", { ref: staleDialogRef, className: "dsh-rp-stale-confirm", "role": "alertdialog", "aria-modal": "true", "aria-labelledby": "dsh-rp-stale-confirm-title", onCancel: closeStaleDialog, onKeyDown: trapStaleDialogFocus }, h("h6", { id: "dsh-rp-stale-confirm-title" }, "\u8BED\u4E49\u7D22\u5F15\u914D\u7F6E\u5DF2\u53D8\u5316"), note(`\u76EE\u6807\u6A21\u578B\uFF1A${activeProvider?.name ?? staleConfirmation.providerId}`), note("\u8303\u56F4\uFF1A\u4EC5\u5F53\u524D\u5BF9\u8BDD\u53CA\u5176\u5168\u90E8\u4E16\u754C\u7EBF\u3002\u65E7\u5411\u91CF\u4F1A\u6E05\u7A7A\u5E76\u6309\u5F53\u524D\u6A21\u578B\u91CD\u5EFA\uFF1B\u5267\u60C5\u6B63\u6587\u3001\u6765\u6E90\u548C\u8D26\u672C\u4FDD\u7559\u4E0D\u53D8\u3002"), note("\u5728\u7EBF\u6A21\u578B\u53EF\u80FD\u4EA7\u751F API \u6D88\u8017\uFF1B\u786E\u8BA4\u540E\u4F1A\u5F00\u542F\u5F53\u524D\u5BF9\u8BDD\u81EA\u52A8\u66F4\u65B0\u3002"), note(`\u76EE\u6807\u7248\u672C\u6307\u7EB9\uFF1A${staleConfirmation.fingerprint.length > 20 ? `${staleConfirmation.fingerprint.slice(0, 20)}\u2026` : staleConfirmation.fingerprint || "\u672A\u63D0\u4F9B"}`), h("div", { className: "dsh-rp-row" }, h("button", { type: "button", className: "dsh-rp-btn", onClick: clearStaleConfirmation, disabled: busy !== null, "data-dsh-rp-stale-cancel": true }, "\u53D6\u6D88"), h("button", { type: "button", className: "dsh-rp-btn dsh-rp-primary", onClick: confirmStaleRebuild, disabled: busy !== null }, "\u786E\u8BA4\u91CD\u5EFA\u5E76\u5F00\u542F"))) : null;
    const modeCard = h("div", { className: "dsh-rp-retrieval-card" }, h("h5", null, "\u8BB0\u5FC6\u68C0\u7D22\u65B9\u5F0F"), note("\u53EA\u68C0\u7D22\u5F53\u524D\u4E16\u754C\u7EBF\u7684\u73A9\u5BB6\u8F93\u5165\u548C\u6709\u6548\u5267\u60C5\u6B63\u6587\uFF0C\u5305\u542B\u5F52\u6863\u65E7\u6B63\u6587\u3002\u5411\u91CF\u6A21\u578B\u9700\u5728\u300CEmbedding \u6A21\u578B\u8BBE\u7F6E\u300D\u4E2D\u624B\u52A8\u914D\u7F6E\u542F\u7528\u3002"), field("\u5C06\u6240\u9009\u65B9\u5F0F\u5E94\u7528\u5230 / Apply to", h("select", { value: modeScope, onChange: (e) => setModeScope(e.target.value) }, h("option", { value: "session" }, "\u5F53\u524D\u4F1A\u8BDD / Session"), h("option", { value: "global" }, "\u5168\u5C40\u9ED8\u8BA4 / Global default"))), h("div", { className: "dsh-rp-row" }, ["keyword", "semantic", "hybrid"].map((mode, i) => h("button", { key: mode, type: "button", className: "dsh-rp-btn" + (data?.mode === mode ? " dsh-rp-primary" : ""), "aria-pressed": data?.mode === mode, disabled: !data || busy !== null, onClick: () => void action("mode", { mode, scope: modeScope }) }, ["A \u5173\u952E\u8BCD / Keyword", "B \u8BED\u4E49 / Semantic", "C \u6DF7\u5408 / Hybrid"][i]))), note(`\u9009\u62E9\u8303\u56F4\uFF0C\u518D\u70B9\u51FB\u68C0\u7D22\u65B9\u5F0F\u4FDD\u5B58\u3002\u5F53\u524D\u6709\u6548\u8BBE\u7F6E${data?.scope === "global" ? "\u6CBF\u7528\u5168\u5C40\u9ED8\u8BA4" : "\u6765\u81EA\u672C\u4F1A\u8BDD"}\u3002\u5207\u6362\u68C0\u7D22\u65B9\u5F0F\u4E0D\u4F1A\u5220\u9664\u5DF2\u6709\u7D22\u5F15\u3002`), h("label", { className: "dsh-rp-row" }, h("input", { type: "checkbox", checked: data?.indexEnabled === true, disabled: !data || busy !== null, onChange: (e) => requestIndex(e.target.checked) }), "\u81EA\u52A8\u66F4\u65B0\u8BED\u4E49\u7D22\u5F15 / Auto-update semantic index"), h("label", { className: "dsh-rp-row" }, h("input", { type: "checkbox", checked: data?.autoIndexNewConversations === true, disabled: !data || busy !== null || !data.autoIndexNewConversations && !activeProvider?.ready, onChange: (e) => void action("auto-index-new-conversations", { enabled: e.target.checked }) }), "\u65B0\u5BF9\u8BDD\u81EA\u52A8\u5F00\u542F\u8BED\u4E49\u7D22\u5F15 / Auto-enable semantic indexing for new conversations"), h("label", { className: "dsh-rp-row" }, h("input", { type: "checkbox", checked: data?.autoRebuildLocalOnRetrieval === true, disabled: !data || busy !== null, onChange: (e) => void action("auto-rebuild-local-on-retrieval", { enabled: e.target.checked }) }), "\u5BF9\u8BDD\u6D3B\u8DC3\u65F6\uFF0C\u6A21\u578B\u914D\u7F6E\u53D8\u5316\u65F6\u81EA\u52A8\u91CD\u5EFA\uFF08\u4EC5\u672C\u5730\u6A21\u578B\uFF09"), note("\u81EA\u52A8\u66F4\u65B0\u4EC5\u4F5C\u7528\u4E8E\u5F53\u524D\u53EF\u89C1\u5BF9\u8BDD\u53CA\u5176\u5168\u90E8\u4E16\u754C\u7EBF\uFF0C\u4E3A\u5386\u53F2\u548C\u65B0\u589E\u7684\u73A9\u5BB6\u8F93\u5165\u3001\u6709\u6548\u6B63\u6587\u5EFA\u7ACB\u7D22\u5F15\uFF0C\u4E0D\u7B49\u5F85\u5BFC\u6F14\u7B14\u8BB0\u3002\u5173\u95ED\u540E\u4ECD\u53EF\u67E5\u8BE2\u5DF2\u6709\u5411\u91CF\uFF0C\u4F46\u65B0\u5267\u60C5\u4E0D\u4F1A\u52A0\u5165\uFF1B\u5173\u952E\u8BCD\u68C0\u7D22\u7167\u5E38\u5DE5\u4F5C\u3002"), note("\u65B0\u5BF9\u8BDD\u9009\u9879\u662F\u5168\u5C40\u9ED8\u8BA4\uFF0C\u4EC5\u5BF9\u4E4B\u540E\u65B0\u5EFA\u7684\u73A9\u5BB6\u5BF9\u8BDD\u751F\u6548\uFF0C\u8981\u6C42\u5F53\u524D\u5DF2\u542F\u7528\u6A21\u578B\u53EF\u7528\uFF1B\u65E7\u5BF9\u8BDD\u548C\u540E\u53F0\u5B50\u4EE3\u7406\u4E0D\u4F1A\u6279\u91CF\u5F00\u542F\u3002\u5173\u95ED\u6B64\u9ED8\u8BA4\u4E0D\u6539\u53D8\u5DF2\u6709\u5BF9\u8BDD\u7684\u7D22\u5F15\u5F00\u5173\uFF0C\u4E5F\u4E0D\u6539\u53D8\u68C0\u7D22\u6A21\u5F0F\u3002"), note("\u4EC5\u5F53\u524D\u5BF9\u8BDD\u5728\u8BED\u4E49\uFF0F\u6DF7\u5408\u68C0\u7D22\u53D1\u73B0\u6A21\u578B\u6307\u7EB9\u4E0D\u5339\u914D\u65F6\u540E\u53F0\u91CD\u5EFA\uFF1B\u672C\u8F6E\u5148\u7528\u5173\u952E\u8BCD\u3002\u4E0D\u6279\u91CF\u5904\u7406\u5176\u4ED6\u5BF9\u8BDD\uFF0C\u4E0D\u81EA\u52A8\u91CD\u5EFA\u5728\u7EBF\u6A21\u578B\uFF1B\u5173\u95ED\u5F53\u524D\u5BF9\u8BDD\u81EA\u52A8\u7D22\u5F15\u65F6\u4E0D\u4F1A\u81EA\u52A8\u6062\u590D\u3002"), note("\u4F7F\u7528\u8BED\u4E49\uFF0F\u6DF7\u5408\u68C0\u7D22\u65F6\uFF0C\u67E5\u8BE2\u8BCD\u672C\u8EAB\u4ECD\u53EF\u80FD\u8C03\u7528\u5411\u91CF\u6A21\u578B\u3002\u5173\u95ED\u81EA\u52A8\u66F4\u65B0\u4E0D\u7B49\u4E8E\u505C\u6B62\u6240\u6709 API \u6D88\u8017\u3002"), staleConfirmCard);
    ...
    const saveChunkSize = () => {
      if (chunkValid && !busyRef.current)
        void action("index-chunk-size", { chunkChars: chunkNumber });
    };
    const ids = [...new Set([draft.model, ...models?.ids ?? []].filter((id) => !!id))];
    const modelSelect = local ? h("select", { "aria-label": "\u6A21\u578B ID", value: draft.model ?? "", onChange: (e) => update({ model: e.target.value, dimensions: null }) }, h("option", { value: "" }, "\u9009\u62E9\u5DF2\u5B89\u88C5\u6A21\u578B"), data?.catalog.filter((m) => m.status === "installed").map((m) => h("option", { key: m.id, value: m.id }, m.name))) : h("div", { className: "dsh-rp-model-picker" }, h("select", { "aria-label": "\u6A21\u578B ID \u4E0B\u62C9\u9009\u62E9", value: draft.model ?? "", onChange: (e) => update({ model: e.target.value }) }, h("option", { value: "" }, "\u9009\u62E9\u6A21\u578B\uFF0C\u6216\u5728\u4E0B\u65B9\u624B\u52A8\u8F93\u5165"), ids.map((id) => h("option", { key: id, value: id }, id))), h("button", { type: "button", className: "dsh-rp-btn", title: "\u5237\u65B0\u6A21\u578B\u5217\u8868", "aria-label": "\u5237\u65B0\u6A21\u578B\u5217\u8868", disabled: !draft.id || dirty || busy !== null, onClick: () => void action("refresh-models", { providerId: draft.id }) }, "\u21BB"));
    const providerCard = h("div", { className: "dsh-rp-retrieval-card" }, h("h5", null, "Embedding \u6A21\u578B\u4E0E\u4F9B\u5E94\u5546"), note("\u672C\u5730\u6A21\u578B\u63A8\u8350\u987A\u5E8F\u4E0E\u5B9E\u6D4B\u9650\u5236\u89C1\u4E0B\u65B9\u300C\u672C\u5730\u6A21\u578B\u4E0B\u8F7D\u300D\u8BF4\u660E\u3002"), h("div", { className: "dsh-rp-provider-list" }, data?.providers.map((p) => h("button", { type: "button", key: p.id, className: "dsh-rp-provider-choice" + (draft.id === p.id ? " dsh-rp-provider-selected" : ""), "aria-pressed": draft.id === p.id, disabled: busy !== null, onClick: () => {
      editRef.current++;
      dirtyRef.current = false;
      setDirty(false);
      setDraft({ ...p, apiKey: "" });
    } }, h("strong", null, p.name), h("div", null, badge(p.kind === "local" ? "\u672C\u5730" : "\u5728\u7EBF"), p.kind === "online" ? badge(protocols[p.protocol]) : null, p.kind === "online" && geminiEmbeddingModels.has(p.model ?? "") ? badge("Gemini \u68C0\u7D22\u683C\u5F0F") : null, draft.id === p.id ? badge("\u6B63\u5728\u7F16\u8F91") : null, p.id === data.activeProviderId ? badge("\u5267\u60C5\u8BB0\u5FC6\u4F7F\u7528\u4E2D") : null, p.id === data.adaptationProviderId ? badge("\u5C0F\u8BF4\u7814\u7A76\u4F7F\u7528\u4E2D") : null), h("small", null, p.kind === "online" ? `${p.model ?? "\u672A\u9009\u62E9\u6A21\u578B"} \xB7 ${hostname(p.baseUrl)} \xB7 ${p.ready ? "\u6D4B\u8BD5\u901A\u8FC7" : "\u5F85\u6D4B\u8BD5"} \xB7 ${p.keySet ? "\u5DF2\u4FDD\u5B58 Key" : "\u5C1A\u672A\u914D\u7F6E Key"}` : `${p.model ?? "\u672A\u9009\u62E9\u6A21\u578B"} \xB7 ${p.ready ? "\u6D4B\u8BD5\u901A\u8FC7" : "\u5F85\u6D4B\u8BD5"}`), p.error ? h("small", { className: "dsh-rp-error" }, p.error) : null))), button("\u65B0\u589E\u63A5\u5165 / New provider", () => {
      editRef.current++;
      dirtyRef.current = true;
      setDirty(true);
      setDraft({ kind: "online", protocol: "openai", dimensions: null, apiKey: "" });
    }), note(draft.id ? `\u6B63\u5728\u7F16\u8F91\uFF1A${draft.name ?? "\u672A\u547D\u540D"}${dirty ? "\uFF08\u6709\u672A\u4FDD\u5B58\u4FEE\u6539\uFF09" : ""}` : "\u63A5\u5165\u76EE\u5F55\u7531\u4E24\u79CD\u7528\u9014\u5171\u4EAB\uFF1B\u6B64\u5904\u9009\u62E9\u5267\u60C5\u8BB0\u5FC6\u6A21\u578B\uFF0C\u5C0F\u8BF4\u7814\u7A76\u6A21\u578B\u5728\u957F\u6587\u672C\u8F6C\u89D2\u8272\u5361\u4E2D\u5355\u72EC\u9009\u62E9\u3002"), h("div", { className: "dsh-rp-retrieval-grid" }, field("\u540D\u79F0", h("input", { value: draft.name ?? "", onChange: (e) => update({ name: e.target.value }), placeholder: "My embedding provider" })), field("\u7C7B\u578B", h("select", { value: draft.kind ?? "online", onChange: (e) => update({ kind: e.target.value, model: "", dimensions: null, apiKey: "", baseUrl: "", protocol: "openai" }) }, h("option", { value: "online" }, "\u5728\u7EBF / Online"), h("option", { value: "local" }, "\u672C\u5730 / Local"))), !local ? field("\u534F\u8BAE", h("select", { value: draft.protocol ?? "openai", onChange: (e) => update({ protocol: e.target.value }) }, Object.entries(protocols).map(([id, label]) => h("option", { key: id, value: id }, label)))) : null, !local ? field("Base URL", h("input", { value: draft.baseUrl ?? "", onChange: (e) => update({ baseUrl: e.target.value }), placeholder: "https://\u2026" })) : null, field("\u6A21\u578B ID", modelSelect), local ? field("\u672C\u5730\u7F16\u7801\u9884\u7B97\uFF08tokens\uFF09", h("input", { type: "number", min: 64, max: data?.catalog.find((m) => m.id === draft.model)?.maxInputTokens ?? 512, step: 1, value: draft.localMaxTokens ?? 512, onChange: (e) => update({ localMaxTokens: Number(e.target.value) }) })) : null, !local ? field("\u624B\u52A8\u8F93\u5165\u6A21\u578B ID", h("input", { value: draft.model ?? "", onChange: (e) => update({ model: e.target.value }), placeholder: "\u652F\u6301\u81EA\u5B9A\u4E49\u522B\u540D" })) : null, !local ? field("\u7EF4\u5EA6\uFF08\u7559\u7A7A\u4F7F\u7528\u6A21\u578B\u9ED8\u8BA4\u503C\uFF09", h("input", { type: "number", min: 1, max: 8192, value: draft.dimensions ?? "", onChange: (e) => update({ dimensions: e.target.value ? Number(e.target.value) : null }) })) : null, !local ? field("\u6A21\u578B\u4FEE\u8BA2\u7248\u672C\uFF08\u53EF\u9009\uFF09", h("input", { value: draft.embeddingRevision ?? "", onChange: (e) => update({ embeddingRevision: e.target.value }), placeholder: "\u4F8B\u5982 2026-09-14" })) : null, !local ? field("API Key\uFF08\u7559\u7A7A\u4FDD\u7559\u5DF2\u4FDD\u5B58 Key\uFF0C\u4E0D\u56DE\u663E\uFF09", h("input", { type: "password", autoComplete: "off", value: draft.apiKey ?? "", onChange: (e) => update({ apiKey: e.target.value }) })) : null), !local ? note("\u670D\u52A1\u672A\u63D0\u4F9B\u56FA\u5B9A\u7248\u672C\u65F6\u53EF\u81EA\u884C\u6807\u8BB0\uFF1B\u6A21\u578B\u5347\u7EA7\u540E\u4FEE\u6539\u6B64\u503C\u5E76\u91CD\u5EFA\u7D22\u5F15\u3002") : null, local ? note(`\u9ED8\u8BA4 512 tokens\uFF0C\u9002\u5408 4 \u6838 8G \u7684\u4FDD\u5B88\u9884\u7B97\uFF1B\u6240\u9009\u6A21\u578B\u8F93\u5165\u4E0A\u9650 ${data?.catalog.find((m) => m.id === draft.model)?.maxInputTokens ?? "\u2014"} tokens\u3002\u5305\u542B Query\uFF0FDocument \u524D\u7F00\u53CA\u7279\u6B8A token\uFF0C\u7531\u6A21\u578B tokenizer \u5B9E\u9645\u68C0\u67E5\u3002\u5F3A\u786C\u4EF6\u53EF\u8C03\u9AD8\uFF1B\u5185\u5B58\u4FDD\u62A4\u4ECD\u6709\u6548\uFF0C\u8F83\u5927\u9884\u7B97\u53EF\u80FD\u5931\u8D25\uFF0C\u8D85\u957F\u67E5\u8BE2\u4F1A\u56DE\u9000\u5173\u952E\u8BCD\u3002\u4FDD\u5B58\u5E76\u6D4B\u8BD5\u540E\uFF0C\u65E7\u7D22\u5F15\u9700\u6309\u65B0\u6307\u7EB9\u91CD\u5EFA\u3002`) : null, !local && draft.protocol === "openai" && !geminiEmbeddingModels.has(draft.model ?? "") ? note("OpenAI \u5B98\u65B9 Base URL\uFF1Ahttps://api.openai.com/v1\uFF1B\u63A8\u8350\u6A21\u578B\uFF1Atext-embedding-3-small \u6216 text-embedding-3-large\u3002\u4E5F\u652F\u6301\u4E2D\u8F6C\u7AD9\uFF0C\u8BF7\u6309\u4F9B\u5E94\u5546\u63D0\u4F9B\u7684\u5730\u5740\u586B\u5199\uFF0C\u7A0B\u5E8F\u4F1A\u8FFD\u52A0 /embeddings\uFF1B\u201COpenAI \u5B98\u65B9 / \u517C\u5BB9\u63A5\u53E3\u201D\u8868\u793A\u534F\u8BAE\uFF0C\u4E0D\u4EE3\u8868\u5F53\u524D\u914D\u7F6E\u4E00\u5B9A\u8FDE\u63A5 OpenAI \u5B98\u65B9\u3002") : null, !local && draft.protocol === "openai" && geminiEmbeddingModels.has(draft.model ?? "") ? h("div", { className: "dsh-rp-retrieval-card" }, h("strong", null, "Gemini 2 \u68C0\u7D22\u914D\u7F6E"), note("\u901A\u8FC7 OpenAI \u517C\u5BB9\u63A5\u53E3\u63A5\u5165\uFF1B\u7A0B\u5E8F\u4F1A\u81EA\u52A8\u5339\u914D\u67E5\u8BE2\u4E0E\u6B63\u6587\u89D2\u8272\uFF0C\u65E0\u9700\u624B\u5199 task_type\u3002\u5411\u91CF\u7EDF\u4E00\u8FDB\u884C L2 \u5F52\u4E00\u5316\uFF1B\u7EF4\u5EA6\u9ED8\u8BA4 3072\uFF0C\u4E5F\u53EF\u586B\u5199 768\u30011536 \u6216 3072\u3002\u65E7\u539F\u6587\u7D22\u5F15\u9700\u91CD\u5EFA\u540E\u624D\u4F1A\u4F7F\u7528\u65B0\u7684\u68C0\u7D22\u683C\u5F0F\u3002")) : null, saved?.actualDimensions ? note(`\u5B9E\u9645\u5411\u91CF\u7EF4\u5EA6\uFF1A${saved.actualDimensions}`) : null, !local && models?.error ? h("p", { className: "dsh-rp-error" }, `${models.error}\uFF1B\u4ECD\u53EF\u624B\u52A8\u8F93\u5165\u6A21\u578B ID\u3002`) : null, h("div", { className: "dsh-rp-row" }, button("\u4FDD\u5B58\u4F9B\u5E94\u5546", () => void action("save-provider", { provider: draft }), !dirty, true), button("\u6D4B\u8BD5", () => void action("test-provider", { providerId: draft.id }), dirty || !draft.id), button("\u7528\u4E8E\u5267\u60C5\u8BB0\u5FC6", () => void action("activate-provider", { providerId: draft.id }), dirty || !saved?.ready), button("\u5220\u9664\u63A5\u5165", () => void action("delete-provider", { providerId: draft.id }), !draft.id)));
    ...
    const indexActionButton = (label, actionName) => h("button", { type: "button", className: "dsh-rp-btn", disabled: !data || busy !== null || actionName === "rebuild-index" && !activeProvider?.ready, onClick: () => setIndexConfirmation(actionName) }, label);
    const confirmation = indexConfirmation && data ? h("div", { className: "dsh-rp-index-confirm", "role": "alertdialog", "aria-label": "\u786E\u8BA4\u5411\u91CF\u6570\u636E\u5E93\u64CD\u4F5C" }, h("strong", null, indexConfirmation === "rebuild-index" ? "\u786E\u8BA4\u91CD\u5EFA\u5411\u91CF\u6570\u636E\u5E93\uFF1F" : "\u786E\u8BA4\u6E05\u7A7A\u5411\u91CF\u6570\u636E\u5E93\uFF1F"), note(`\u76EE\u6807\u4F9B\u5E94\u5546\uFF1A${activeProvider?.name ?? "\u5F53\u524D\u672A\u542F\u7528\u4F9B\u5E94\u5546"}\uFF1B\u8303\u56F4\uFF1A\u5F53\u524D\u53EF\u89C1\u5BF9\u8BDD\u7684\u5168\u90E8\u6A21\u578B\u5411\u91CF\u7D22\u5F15\uFF08\u5305\u542B\u65E7\u7248\u9057\u7559\u7D22\u5F15\uFF0C\u8986\u76D6\u5176\u6240\u6709\u4E16\u754C\u7EBF\uFF09\u3002`), note(indexConfirmation === "rebuild-index" ? "\u5C06\u6E05\u7A7A\u5F53\u524D\u5BF9\u8BDD\u5168\u90E8\u5411\u91CF\u540E\uFF0C\u4EC5\u4E3A\u5F53\u524D\u542F\u7528\u6A21\u578B\u91CD\u65B0\u7D22\u5F15\uFF1B\u5207\u6362\u5176\u4ED6\u6A21\u578B\u540E\u9700\u91CD\u65B0\u7D22\u5F15\u3002" : "\u5C06\u6E05\u7A7A\u5F53\u524D\u5BF9\u8BDD\u5168\u90E8\u5411\u91CF\u5E76\u6682\u505C\u81EA\u52A8\u66F4\u65B0\uFF1B\u6B63\u6587\u4E0E\u5173\u952E\u8BCD\u8BB0\u5F55\u4FDD\u7559\u3002\u65E7\u5728\u9014\u6A21\u578B\u8BF7\u6C42\u65E0\u6CD5\u64A4\u56DE\uFF0C\u5176\u7ED3\u679C\u4F1A\u4E22\u5F03\uFF0C\u4F46\u53EF\u80FD\u5DF2\u7ECF\u4EA7\u751F API \u6D88\u8017\uFF1B\u67E5\u8BE2\u8BCD\u4ECD\u53EF\u80FD\u4EA7\u751F API \u6D88\u8017\u3002"), h("div", { className: "dsh-rp-row" }, h("button", { type: "button", className: "dsh-rp-btn dsh-rp-primary", disabled: busy !== null, onClick: () => {
      void action(indexConfirmation, { confirmProviderId: data.activeProviderId ?? null });
      setIndexConfirmation(null);
    } }, "\u786E\u8BA4"), h("button", { type: "button", className: "dsh-rp-btn", disabled: busy !== null, onClick: () => setIndexConfirmation(null) }, "\u53D6\u6D88"))) : null;
    const progressCard = h("div", { className: "dsh-rp-retrieval-card" }, h("h5", null, "\u7D22\u5F15\u4E0E\u8FDB\u5EA6"), note("\u81EA\u52A8\u7D22\u5F15\u5F00\u5173\u4F4D\u4E8E\u300C\u8BB0\u5FC6\u300DTAB \u7684\u8BB0\u5FC6\u68C0\u7D22\u65B9\u5F0F\u9762\u677F\u3002"), field("\u7D22\u5F15\u5757\u5927\u5C0F\uFF08\u5B57\u7B26\uFF09", h("input", { type: "number", min: 128, max: 2048, step: 1, value: chunkValue, disabled: busy !== null, onChange: (e) => {
      chunkDirtyRef.current = true;
      setChunkDirty(true);
      setChunkDraft(e.target.value);
    } })), h("div", { className: "dsh-rp-row" }, h("button", { type: "button", className: "dsh-rp-btn", disabled: busy !== null || !chunkDirty || !chunkValid, onClick: saveChunkSize }, "\u4FDD\u5B58\u7D22\u5F15\u5757\u5927\u5C0F")), note(`\u9ED8\u8BA4 480\uFF1B\u5EFA\u8BAE 192\uFF08\u7EC6\u7EBF\u7D22\uFF09\u3001480\uFF08\u5747\u8861\uFF09\u3001960\uFF08\u8F83\u957F\u6BB5\u843D\uFF09\u3002\u5F53\u524D\u5B57\u7B26\u76EE\u6807 ${data?.chunkChars ?? 480}\uFF0C\u8303\u56F4 128\u20132048\uFF1B\u5C0F\u8BF4\u7814\u7A76\u4E0E\u5267\u60C5\u5171\u7528\u3002\u5B57\u7B26\u6570\u4E0D\u7B49\u4E8E tokens\uFF1A\u672C\u5730\u4F1A\u6309\u8BE5\u6A21\u578B tokenizer \u4E0E\u7F16\u7801\u9884\u7B97\u7EE7\u7EED\u5206\u6BB5\uFF0C\u4FDD\u8BC1\u542B\u524D\u7F00\u7684\u5B8C\u6574\u8F93\u5165\u4E0D\u88AB\u622A\u65AD\u3002\u8F83\u5927\u5757\u589E\u52A0\u5F00\u9500\u5E76\u964D\u4F4E\u5B9A\u4F4D\u7C92\u5EA6\uFF1B\u66F4\u6539\u9884\u7B97\u6216\u5206\u6BB5\u914D\u65B9\u9700\u91CD\u5EFA\u7D22\u5F15\u3002`), note("\u5757\u5927\u5C0F\u540C\u65F6\u5F71\u54CD\u5267\u60C5\u4E0E\u5C0F\u8BF4\u5411\u91CF\u6307\u7EB9\uFF1B\u4FEE\u6539\u53EA\u6807\u8BB0\u65E7\u7D22\u5F15\u9700\u8981\u91CD\u5EFA\uFF0C\u4E0D\u4F1A\u7ACB\u5373\u6279\u91CF\u91CD\u5EFA\u3002\u6B64\u8BBE\u7F6E\u4FDD\u5B58\u4E8E\u5168\u5C40\u914D\u7F6E\uFF0C\u4E0D\u968F\u4E0B\u65B9\u67E5\u770B\u8FDB\u5EA6\u7684\u4F1A\u8BDD\u9009\u62E9\u6539\u53D8\u3002"), h("div", { className: "dsh-rp-row" }, button("\u8865\u9F50\u5386\u53F2 / \u91CD\u8BD5\u5931\u8D25", () => void action("reindex")), indexActionButton("\u91CD\u5EFA\u5411\u91CF\u6570\u636E\u5E93", "rebuild-index"), indexActionButton("\u6E05\u7A7A\u5411\u91CF\u6570\u636E\u5E93", "clear-index")), confirmation, note("\u8865\u9F50\u4F1A\u5F00\u542F\u5F53\u524D\u5BF9\u8BDD\u7684\u81EA\u52A8\u7D22\u5F15\uFF0C\u4FDD\u7559\u6210\u529F\u5411\u91CF\u5E76\u8865\u9F50\u7F3A\u5931\u3001\u91CD\u8BD5\u5931\u8D25\u9879\u3002\u8865\u9F50\u3001\u91CD\u5EFA\u548C\u6E05\u7A7A\u53EA\u4F5C\u7528\u4E8E\u5F53\u524D\u53EF\u89C1\u5BF9\u8BDD\u53CA\u5176\u5168\u90E8\u4E16\u754C\u7EBF\uFF1B\u67E5\u770B\u8FDB\u5EA6\u4E0B\u62C9\u4E0D\u6539\u53D8\u64CD\u4F5C\u76EE\u6807\u3002"), field("\u67E5\u770B\u8FDB\u5EA6\u7684\u4F1A\u8BDD", h("select", { value: progressSession, onChange: (e) => {
      setProgressReply(null);
      setProgressError(null);
      setProgressSession(e.target.value);
    } }, h("option", { value: sessionId }, "\u5F53\u524D\u4F1A\u8BDD"), sessions.filter((s) => s.id !== sessionId).map((s) => h("option", { key: s.id, value: s.id }, s.label)))), note("\u6765\u6E90\u8986\u76D6\u548C\u8FDB\u5EA6\u6570\u5B57\u5C5E\u4E8E\u6240\u9009\u67E5\u770B\u5BF9\u8BDD\uFF1B\u67E5\u770B\u8FDB\u5EA6\u4E0B\u62C9\u53EA\u6539\u53D8\u663E\u793A\uFF0C\u4E0D\u6539\u53D8\u4E0A\u65B9\u5F53\u524D\u5BF9\u8BDD\u7684\u7D22\u5F15\u5F00\u5173\u6216\u5411\u91CF\u7BA1\u7406\u76EE\u6807\u3002"), progress2 ? h("div", { className: "dsh-rp-progress" }, h("div", null, `\u6B63\u6587\u6765\u6E90 ${progress2.sources} \xB7 \u5DF2\u8986\u76D6 ${progress2.covered}`), h("div", { className: "dsh-rp-progress-bar", "aria-label": `\u6B63\u6587\u8986\u76D6 ${covered}%` }, h("span", { style: { width: `${covered}%` } })), progress2.stale ? h("div", { className: "dsh-rp-error" }, `\u914D\u7F6E\u4E0D\u5339\u914D\uFF0C\u9700\u91CD\u5EFA\u7D22\u5F15\uFF1B\u8BED\u4E49\u7D22\u5F15\u5DF2\u505C\u7528\uFF0C\u65E7\u5411\u91CF ${progress2.staleVectors ?? 0} \u6761\u4E0D\u4F1A\u53C2\u4E0E\u67E5\u8BE2\u3002`) : null, progress2.fingerprint ? note(`\u7D22\u5F15\u6307\u7EB9\uFF1A${progress2.fingerprint.length > 20 ? `${progress2.fingerprint.slice(0, 20)}\u2026` : progress2.fingerprint}`) : null, progress2.embedding ? note(`\u5D4C\u5165\u5143\u6570\u636E\uFF1A${progress2.embedding.embedding_model} \xB7 \u4FEE\u8BA2 ${progress2.embedding.embedding_revision} \xB7 \u7EF4\u5EA6 ${progress2.embedding.dimensions ?? "\u9ED8\u8BA4"} \xB7 task=${progress2.embedding.task} \xB7 pooling=${progress2.embedding.pooling}`) : null, note(`\u6240\u9009\u5BF9\u8BDD\u5411\u91CF\uFF1A${progress2.vectors} \xB7 \u5F85\u5904\u7406 ${progress2.pending} \xB7 \u8FD0\u884C\u4E2D ${progress2.running} \xB7 \u5931\u8D25 ${progress2.failed} \xB7 \u7ED3\u679C\u672A\u77E5 ${progress2.unknown}`), note(`\u5171\u4EAB\u6570\u636E\u5E93\u78C1\u76D8\u5360\u7528\uFF1A${bytes(progress2.bytes)}`), progress2.lastError ? h("div", { className: "dsh-rp-error" }, progress2.lastError) : null) : note(progressError ?? "\u6B63\u5728\u8BFB\u53D6\u6240\u9009\u4F1A\u8BDD\u8FDB\u5EA6\u2026"));
    const catalogCard = h("div", { className: "dsh-rp-retrieval-card" }, h("h5", null, "\u672C\u5730\u6A21\u578B\u4E0B\u8F7D"), note("\u672C\u7EC4\u670D\u52A1\u5668\u8BED\u4E49 MRR \u5B9E\u6D4B\u63A8\u8350\uFF1AJina Nano > Nomic v2 MoE > Qwen3 0.6B > BGE small > Jina Small INT8\u3002\u6392\u540D\u4EC5\u57FA\u4E8E 26 \u6761\u6B63\u6587\uFF0F29 \u9053\u6709\u7B54\u6848\u9898\u7684\u5C0F\u6837\u672C\u53EC\u56DE\u7ED3\u679C\uFF1BBGE small \u9002\u5408\u4E2D\u6587\u3001\u901F\u5EA6\u4E0E\u5185\u5B58\u4F18\u5148\uFF1B\u4E0E Small INT8 \u7684 MRR \u5DEE\u8DDD\u5F88\u5C0F\uFF0C\u5728\u7EBF API \u4F18\u5148\u8282\u7701\u672C\u5730\u5185\u5B58\u3002\u672A\u6D4B E5\u3001BGE-M3 \u4E0D\u5217\u5165\u6392\u540D\u3002"), note("\u8FD0\u884C\u73AF\u5883\u4F1A\u6309\u9700\u5B89\u88C5\u6216\u66F4\u65B0\uFF1B\u5DF2\u6821\u9A8C\u7684\u6A21\u578B\u6587\u4EF6\u4F1A\u590D\u7528\uFF0C\u4E0D\u4F1A\u91CD\u590D\u4E0B\u8F7D\u3002"), data?.catalog.map((model) => {
      const pct = model.totalBytes ? Math.min(100, Math.round((model.downloadedBytes ?? 0) / model.totalBytes * 100)) : 0;
      return h("div", { className: "dsh-rp-item", key: model.id }, h("strong", null, model.name), note(`${model.languages} \xB7 ${model.status ?? "not-installed"}`), model.license ? note(`\u8BB8\u53EF\uFF1A${model.license}`) : null, model.description ? note(model.description) : null, note(`${bytes(model.downloadedBytes)} / ${bytes(model.totalBytes)}`), h("div", { className: "dsh-rp-progress-bar", "aria-label": `${model.name} ${pct}%` }, h("span", { style: { width: `${pct}%` } })), model.error ? h("div", { className: "dsh-rp-error" }, model.error) : null, h("div", { className: "dsh-rp-row" }, button("\u4E0B\u8F7D\uFF0F\u5B89\u88C5", () => void action("download-model", { modelId: model.id }), model.status === "installed"), button("\u6682\u505C", () => void action("pause-download", { modelId: model.id }), !["downloading", "installing"].includes(model.status ?? "")), button("\u5220\u9664\u6A21\u578B", () => void action("delete-model", { modelId: model.id }), model.status === "not-installed")));
    }));
    return h("section", { className: "dsh-rp-retrieval", "aria-label": view === "memory" ? "\u8BB0\u5FC6\u8BED\u4E49\u68C0\u7D22" : "Embedding \u6A21\u578B\u8BBE\u7F6E", "data-dsh-rp-adaptation-model-settings": view === "embedding" ? true : void 0, tabIndex: view === "embedding" ? -1 : void 0 }, ...view === "memory" ? [modeCard] : [providerCard, progressCard, catalogCard], error ? h("div", { className: "dsh-rp-error", role: "alert" }, error) : null);
  }
  function MemoryPanel({ sessionId, LegacyPanel }) {
    return h(LegacyPanel, { scope: { sessionId }, visible: true, retrievalControls: h(RetrievalPanel, { sessionId, view: "memory" }) });
  }
  function EmbeddingPanel({ sessionId }) {
    return h("div", { className: "dsh-rp-panel" }, h(RetrievalPanel, { sessionId, view: "embedding" }));
  }
  function RebuildPrompt({ sessionId }) {
    ...
    const submit = async (action) => {
      ...
      requestRef.current++;
      try {
        const next = await jsonFetch("/api/roleplay/retrieval-confirmations", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ sessionId, id: target.id, action }) });
        if (next.ok === false)
    ...
    return h("dialog", { ref: dialogRef, className: "dsh-rp-stale-confirm", "role": "alertdialog", "aria-modal": "true", "aria-labelledby": "dsh-rp-rebuild-title", onCancel: close, onKeyDown: trap }, h("h6", { id: "dsh-rp-rebuild-title" }, configure ? "\u7C97\u9897\u7C92\u5EA6\u9700\u8981\u5C0F\u8BF4\u68C0\u7D22\u6A21\u578B" : "\u9700\u8981\u786E\u8BA4\u91CD\u5EFA\u8BED\u4E49\u7D22\u5F15"), note(configure ? "\u7C97\u9897\u7C92\u5EA6\u7814\u7A76\u9700\u8981\u53EF\u7528\u7684\u5C0F\u8BF4\u4E13\u7528 Embedding \u6A21\u578B\uFF1B\u5F53\u524D\u751F\u6210\u8F6E\u6B21\u5DF2\u505C\u6B62\u3002\u8BF7\u5148\u914D\u7F6E\u5E76\u6D4B\u8BD5\u5C0F\u8BF4\u6A21\u578B\u3002" : `\u7528\u9014\uFF1A${purpose}`), configure ? null : note(`\u6A21\u578B\uFF1A${pending.model}`), configure ? null : note(`\u76EE\u6807\uFF1A${pending.target}`), note(`\u539F\u56E0\uFF1A${pending.reason}`), configure ? note("\u786E\u8BA4\u540E\u53EA\u5173\u95ED\u63D0\u793A\uFF0C\u4E0D\u4F1A\u542F\u52A8\u7D22\u5F15\u91CD\u5EFA\u3002") : note("\u5728\u7EBF\u6A21\u578B\u91CD\u5EFA\u53EF\u80FD\u4EA7\u751F API \u8D39\u7528\uFF1B\u786E\u8BA4\u540E\u5C06\u5728\u540E\u53F0\u542F\u52A8\u3002"), h("div", { className: "dsh-rp-row" }, h("button", { type: "button", className: "dsh-rp-btn", "data-dsh-rp-rebuild-cancel": true, disabled: busy, onClick: () => void submit("cancel") }, configure ? "\u5173\u95ED" : "\u53D6\u6D88"), h("button", { type: "button", className: "dsh-rp-btn dsh-rp-primary", "data-dsh-rp-rebuild-confirm": true, disabled: busy, onClick: () => {
      if (configure) {
        void (async () => {
          if (await submit("cancel"))
            window.dispatchEvent(new CustomEvent("dsh-roleplay-needs-config", { detail: { sessionId, action: "configure-adaptation-model" } }));
        })();
      } else
        void submit("confirm");
    } }, configure ? "\u524D\u5F80\u914D\u7F6E" : "\u786E\u8BA4\u5E76\u91CD\u5EFA")));
  }
  return { MemoryPanel, EmbeddingPanel, RebuildPrompt };
}
...
var noteDraft = (note) => ({ segment: note.segment, facts: note.facts, implications: note.implications, questions: note.questions, evidence: note.evidence, seq: note.seq });
  ...
  return function CardAdaptationPanel({ sessionId, focusModelSettings = false, focusModelSettingsNonce = 0 }) {
    ...
    const mutate = React.useCallback(async (action, extra = {}, target) => {
      ...
      try {
        ...
        if (error2?.code === "ADAPTATION_COARSE_MODEL_REQUIRED" || error2?.message?.includes("ADAPTATION_COARSE_MODEL_REQUIRED")) {
          const detail = { sessionId: targetSessionId, code: "ADAPTATION_COARSE_MODEL_REQUIRED", reason: error2.details?.reason, adaptationRevision: error2.details?.adaptationRevision, providerId: error2.details?.providerId, action: error2.details?.action ?? "configure-adaptation-model" };
    ...
    const saveDraft = () => {
      if (!draft || busyRef.current)
        return;
      void mutate("save-note", { note: { segment: draft.segment, facts: draft.facts, implications: draft.implications, questions: draft.questions, evidence: draft.evidence } }).then((next) => {
        if (next)
          cancelEdit();
      });
    };
    ...
    const confirmAction = (action, message, title, confirmLabel, after, extra = {}) => {
      ...
      const target = { sessionId, sourceId: sourceId || void 0, expectedRevision: data.revision, expectedModelRevision: data.modelRevision, expectedLibraryRevision: data.library?.revision ?? 0, expectedResearchRevision: data.research?.revision, indexFingerprint: data.index?.fingerprint, action };
    ...
    const saveResearchBudget = () => {
      ...
      void mutate("research-budget", { maxQueryBatches: queryLimit, maxReadPackets: readLimit }, { sessionId, sourceId: selectedRef.current || data.selectedId || void 0, expectedRevision: data.revision, expectedModelRevision: data.modelRevision, expectedLibraryRevision: data.library?.revision ?? 0, expectedResearchRevision: data.research.revision }).then((next) => {
        if (next) {
          researchDirtyRef.current = false;
          setResearchDirty(false);
        }
    ...
    const toggleResearchPause = () => {
      ...
      void mutate(action, {}, { sessionId, sourceId: selectedRef.current || data.selectedId || void 0, expectedRevision: data.revision, expectedModelRevision: data.modelRevision, expectedLibraryRevision: data.library?.revision ?? 0, expectedResearchRevision: data.research.revision });
    };
    ...
    const semanticCard = h("div", { className: "dsh-rp-adaptation-card" }, h("h5", null, "\u5C0F\u8BF4\u7814\u7A76\u6A21\u578B\uFF08\u5168\u5C40\uFF09"), muted("\u4E0E Embedding \u8BBE\u7F6E\u5171\u7528\u540C\u4E00\u7EC4\u4F9B\u5E94\u5546\u548C\u6A21\u578B\uFF0C\u4E0D\u5728\u8FD9\u91CC\u91CD\u590D\u914D\u7F6E\u4F9B\u5E94\u5546\u3002\u6B64\u9009\u62E9\u4EC5\u5F71\u54CD\u5C0F\u8BF4\u539F\u6587\u7D22\u5F15\u548C\u68C0\u7D22\uFF0C\u4E0D\u6539\u53D8\u5267\u60C5\u8BB0\u5FC6\u6A21\u578B\uFF1B\u4FDD\u7559\u5DF2\u6709\u7D22\u5F15\uFF0C\u914D\u7F6E\u4E0D\u5339\u914D\u65F6\u53EF\u518D\u4FEE\u590D\u7D22\u5F15\u3002"), data ? field("\u6A21\u578B", h("select", { ref: (element) => {
      modelSettingsRef.current = element;
    }, "data-dsh-rp-novel-model-settings": true, className: "dsh-rp-input", value: modelSelection, disabled: busy !== null || confirming || !!draft, onChange: (e) => {
    ...
    const libraryCard = h("div", { className: "dsh-rp-adaptation-card" }, h("h5", null, "\u539F\u8457\u8D44\u6599\u5E93"), muted("\u540C\u4E00\u672C\u539F\u8457\u53EA\u9700\u5EFA\u5E93\u4E00\u6B21\u3002\u591A\u4E2A\u72EC\u7ACB\u5BF9\u8BDD\u81EA\u52A8\u590D\u7528\u539F\u8457\u7814\u7A76\u7B14\u8BB0\u4E0E\u9605\u8BFB\u65AD\u70B9\uFF1B\u5F53\u524D\u5361\u7684\u7B14\u8BB0\u4FEE\u6539\u72EC\u7ACB\u4FDD\u5B58\uFF0C\u6539\u7F16\u8BBE\u60F3\u4E0D\u81EA\u52A8\u6210\u4E3A\u5267\u60C5\u3002"), libraryAssets.length ? field("\u5DF2\u767B\u8BB0\u539F\u8457", h("select", { className: "dsh-rp-input", value: "", disabled: busy !== null || confirming || !!draft, onChange: (e) => {
      if (e.target.value)
    ...
    const sourceCard = h("div", { className: "dsh-rp-adaptation-card" }, h("h5", null, "\u5C0F\u8BF4\u539F\u6587"), muted("\u9009\u62E9\u8981\u6574\u7406\u7684\u5C0F\u8BF4\u539F\u6587\u3002\u591A\u4E2A\u5BF9\u8BDD\u81EA\u52A8\u590D\u7528\u539F\u8457\u7814\u7A76\u7B14\u8BB0\u4E0E\u9605\u8BFB\u65AD\u70B9\uFF1B\u5F53\u524D\u5361\u7684\u7B14\u8BB0\u4FEE\u6539\u72EC\u7ACB\u4FDD\u5B58\uFF0C\u6539\u7F16\u8BBE\u60F3\u4E0D\u81EA\u52A8\u6210\u4E3A\u5267\u60C5\u3002"), hasSources ? field("\u5C0F\u8BF4", h("select", { className: "dsh-rp-input", value: source?.sourceId ?? "", disabled: busy !== null || confirming || !!draft, onChange: (e) => selectSource(e.target.value) }, data.sources.map((item) => h("option", { key: item.sourceId, value: item.sourceId }, item.name)))) : null, source ? h("div", { className: "dsh-rp-adaptation-meta" }, h("span", null, `${source.name} \xB7 ${bytes2(source.bytes)} \xB7 ${source.characters.toLocaleString()} \u5B57 \xB7 ${source.segments} \u6BB5`), h("span", null, `\u5DF2\u8BFB\uFF1A${progress(source.read, source.segments)}`), h("span", null, `\u5DF2\u8BB0\u7B14\u8BB0\uFF1A${progress(source.reviewed, source.segments)}`), h("span", null, `\u72B6\u6001\uFF1A${statusLabel(source.status)}`)) : null, source?.requiresReregister ? h("div", { className: "dsh-rp-error", role: "alert" }, "\u65E7\u5206\u6BB5\u53EF\u80FD\u53D7\u8F93\u51FA\u4E0A\u9650\u622A\u65AD\uFF0C\u65E7\u5DF2\u8BFB\u6807\u8BB0\u4E0D\u80FD\u8BC1\u660E\u5168\u6587\u9001\u8FBE\u3002\u8BF7\u91CD\u65B0\u767B\u8BB0\u539F\u6587\u4EF6\u540E\u5B8C\u6574\u91CD\u8BFB\uFF1B\u65E7\u7814\u7A76\u7B14\u8BB0\u4F1A\u4FDD\u7559\u3002") : null, data?.shared ? muted("\u81EA\u52A8\u590D\u7528\u539F\u8457\u7814\u7A76\u7B14\u8BB0\u4E0E\u9605\u8BFB\u65AD\u70B9\uFF1B\u5F53\u524D\u5361\u7684\u7B14\u8BB0\u4FEE\u6539\u72EC\u7ACB\u4FDD\u5B58\uFF0C\u6539\u7F16\u8BBE\u60F3\u4E0D\u81EA\u52A8\u6210\u4E3A\u5267\u60C5\u3002") : null, !hasSources && h("p", { role: "status" }, data ? "\u5C1A\u65E0\u5C0F\u8BF4\u539F\u6587\u3002\u8BF7\u5148\u5728\u8D44\u6E90\u5E93\u4E0A\u4F20 TXT\uFF0C\u518D\u5411 Agent \u63D0\u51FA\u6539\u7F16\u8981\u6C42\u3002" : "\u6B63\u5728\u8BFB\u53D6\u539F\u6587\u2026"));
    const research = data?.research;
    const coveredBins = research?.coverage.bins.filter((bin) => bin.covered).length ?? 0;
    const researchCard = research && source ? h("div", { className: "dsh-rp-adaptation-card" }, h("h5", null, "\u9605\u8BFB\u4E0E\u6539\u7F16\u6A21\u5F0F"), muted("\u5148\u9009\u62E9\u539F\u8457\u9605\u8BFB\u7B56\u7565\uFF0C\u518D\u5F00\u59CB\u9605\u8BFB\u7B14\u8BB0\u3002\u539F\u521B\u4E92\u52A8\u5199\u5361\u4E0E\u8FD9\u91CC\u72EC\u7ACB\uFF1B\u5207\u6362\u6A21\u5F0F\u4E0D\u4F1A\u5220\u9664\u7D22\u5F15\u6216\u539F\u6587\u3002"), h("div", { className: "dsh-rp-adaptation-grid" }, field("\u9605\u8BFB\u6A21\u5F0F", h("select", { className: "dsh-rp-input", value: researchMode ?? "", disabled: busy !== null || confirming, onChange: (e) => {
      const value = e.target.value;
      setResearchMode(value || null);
      markResearchDirty();
    } }, h("option", { value: "" }, "\u8BF7\u9009\u62E9"), h("option", { value: "close-reading" }, "\u7CBE\u8BFB"), h("option", { value: "coarse" }, "\u7C97\u9897\u7C92\u5EA6"))), field("\u4E3B\u89D2\u540D\uFF08\u7C97\u9897\u7C92\u5EA6\u5FC5\u586B\uFF09", h("input", { className: "dsh-rp-input", value: researchProtagonist, disabled: busy !== null || confirming, onChange: (e) => {
      setResearchProtagonist(e.target.value);
      markResearchDirty();
    } })), field("\u5F00\u7BC7\u4F4D\u7F6E", h("input", { className: "dsh-rp-input", value: researchOpeningPoint, disabled: busy !== null || confirming, placeholder: "\u4F8B\u5982\uFF1A\u7B2C 1 \u7AE0", onChange: (e) => {
      setResearchOpeningPoint(e.target.value);
      markResearchDirty();
    } }))), researchMode === "close-reading" ? muted("\u7CBE\u8BFB\uFF1A\u5B8C\u6574\u901A\u8BFB\u539F\u8457\uFF0C\u8017\u65F6\u8F83\u9AD8\uFF0C\u9002\u5408\u9700\u8981\u8FDE\u7EED\u7EC6\u8282\u548C\u5B8C\u6574\u80CC\u666F\u7684\u6539\u7F16\u3002") : researchMode === "coarse" ? muted("\u7C97\u9897\u7C92\u5EA6\uFF1A\u5B9A\u5411\u8BC1\u636E + \u5168\u5C40\u5206\u5C42\u62BD\u67E5\uFF1B\u8986\u76D6\u6709\u9650\uFF0C\u4E0D\u80FD\u58F0\u79F0\u8BFB\u5B8C\u6574\u672C\u539F\u8457\u3002") : muted("\u8BF7\u9009\u62E9\u7CBE\u8BFB\u6216\u7C97\u9897\u7C92\u5EA6\u6A21\u5F0F\u3002"), h("div", { className: "dsh-rp-adaptation-actions" }, actionButton("\u786E\u8BA4\u9605\u8BFB\u6A21\u5F0F", () => chooseResearchMode(researchMode), !researchMode || researchMode === "coarse" && !researchProtagonist.trim() || !researchDirty, true)), h("div", { className: "dsh-rp-adaptation-grid" }, field("\u6700\u5927\u68C0\u7D22\u6279\u6B21\uFF081\u2013200\uFF09", h("input", { className: "dsh-rp-input", type: "number", min: 1, max: 200, value: maxQueryBatches, disabled: busy !== null || confirming, onChange: (e) => {
      setMaxQueryBatches(e.target.value);
      markResearchDirty();
    } })), field("\u6700\u5927\u9605\u8BFB\u5305\uFF081\u2013500\uFF09", h("input", { className: "dsh-rp-input", type: "number", min: 1, max: 500, value: maxReadPackets, disabled: busy !== null || confirming, onChange: (e) => {
...
function createAuthorRuntime(root, fill) {
  ...
  const add = (type, handler, options) => {
    if (typeof type !== "string" || !handler)
      return;
    const registration = { type, handler, options };
...
function createReaderView({ React, resolveActiveSessionId, isRoleplaySession, useTavernActivity, fetchState, sessionsService, toast, UserActions, AssistantActions, actionIconButton, ICONS, pendingPlayerBubble, ActivityBanner, ACTIVITY_CSS: ACTIVITY_CSS2 }) {
  ...
  function ReaderView(props) {
    ...
    const sessionSnapshot = React.useSyncExternalStore(React.useCallback((notify) => sessionFace?.subscribe?.(notify) ?? (() => {
    }), [sessionFace]), React.useCallback(() => sessionFace?.getSnapshot?.() ?? null, [sessionFace]), () => null);
    ...
    React.useEffect(() => {
      ...
      void (async () => {
        for (const raw of beautyTasks) {
          ...
          let value;
          try {
            value = { html: await renderReaderNarrativeAsync(raw, rules, runBeautyRegex) };
          } catch (error) {
            value = { html: renderReaderNarrative(raw, []), error: String(error && typeof error === "object" && "message" in error ? error.message : error) };
          }
          if (cancelled || beautyScopeRef.current !== beautyScope)
            break;
          cache.set(raw, value);
...
function createStatusSurface({ React, createRoot, resolveActiveSessionId, isRoleplaySession, useTavernActivity, BackgroundNotesBanner, fetchState, stateVersion, subscribeState, invalidateState, applyImmersive, readImmersive, runMaintenance, toast }) {
  function DecisionCard({ decision, decisionMinimized, onMinimize, onDismiss, fillInput, renderText }) {
    ...
    return React.createElement("div", { className: "dsh-rp-decision-backdrop" }, React.createElement("div", { className: cardClass, ref: cardRef, style: { transform: `translate(${offset.x}px, ${offset.y}px)`, "--rp-decision-visible-height": Math.max(84, viewportHeight - 16) + "px" } }, React.createElement("div", {
    ...
    }, decisionMinimized ? "\u2303" : "\u2014"), React.createElement("button", { type: "button", className: "dsh-rp-decision-close", title: "\u7A0D\u540E\u5904\u7406", "aria-label": "\u7A0D\u540E\u5904\u7406", onClick: onDismiss }, "\xD7"))), decisionMinimized ? null : React.createElement("div", { className: "dsh-rp-decision-scroll" }, header !== null && question !== "" && question !== header ? React.createElement("div", { className: "dsh-rp-decision-question" }, question) : null, React.createElement("div", { className: "dsh-rp-decision-options" }, decision.options.map((o, i) => {
  ...
  function StatusOverlay({ sessionId }) {
    ...
    if (!currentState) {
      ...
      const loadingMain = statusMode === "side" && !minimized ? React.createElement("div", { className: "dsh-rp-status-dock" }, React.createElement("div", { className: "dsh-rp-status-dock-head" }, React.createElement("span", { className: "dsh-rp-status-title" }, loadingTitle)), renderLoadingBody()) : minimized ? React.createElement("div", { className: "dsh-rp-status-badge", role: "status", "aria-live": "polite" }, React.createElement("span", { className: "dsh-rp-status-badge-icon" }, "\u2726"), React.createElement("span", { className: "dsh-rp-status-badge-label" }, loadingTitle), React.createElement("span", { className: "dsh-rp-status-badge-dot" })) : React.createElement("div", { className: "dsh-rp-status-window", style: { width: size.w, height: size.h, ...pos ? { left: pos.left, top: pos.top, right: "auto", bottom: "auto" } : {} } }, React.createElement("div", { className: "dsh-rp-status-head", onPointerDown: onHeaderPointerDown }, React.createElement("span", { className: "dsh-rp-status-title" }, loadingTitle), React.createElement("button", { type: "button", className: "dsh-rp-status-min", title: "\u6700\u5C0F\u5316", onClick: () => setMinimized(true) }, "\u2014")), renderLoadingBody(), React.createElement("div", { className: "dsh-rp-status-resize", onPointerDown: onResizePointerDown, title: "\u62D6\u52A8\u8C03\u6574\u5927\u5C0F" }));
...
function createMessageActionComponents({ React, isRoleplaySession, fetchState, subscribeState, invalidateState, applyImmersive, readImmersive, toast, confirmWithDialog: confirmWithDialog2, runCommand, replaceMessage, forkAndPrompt, forkWithoutUserTurn, openNativeBranch, retryBranchMutation }) {
  ...
  function MessageEditor({ title, initialText, allowSend = false, busy = false, onClose, onSave, onSend }) {
    ...
    }), React.createElement("div", { className: "dsh-rp-edit-buttons" }, React.createElement("span", { className: "dsh-rp-edit-hint" }, allowSend ? "\u4FDD\u5B58\uFF1A\u539F\u5730\u4FEE\u6539\uFF1B\u53D1\u9001\uFF1A\u65B0\u5EFA\u73A9\u5BB6\u5206\u652F\u5E76\u751F\u6210\u5267\u60C5" : "\u4FDD\u5B58\u53EA\u4FEE\u6539\u5F53\u524D Agent \u56DE\u590D\uFF0C\u4E0D\u8C03\u7528\u6A21\u578B"), React.createElement("button", { type: "button", className: "dsh-rp-edit-action", disabled: busy, onClick: onClose }, "\u53D6\u6D88"), React.createElement("button", { type: "button", className: "dsh-rp-edit-action", disabled: !ready, onClick: () => onSave(text.trim()) }, "\u4FDD\u5B58"), allowSend ? React.createElement("button", { type: "button", className: "dsh-rp-edit-action", "data-primary": "true", disabled: !ready, onClick: () => onSend(text.trim()) }, "\u53D1\u9001") : null)));
  }
  ...
  function UserActions(props) {
    ...
    return React.createElement(React.Fragment, null, React.createElement("span", { className: "dsh-rp-actions" }, group && members.length > 0 ? [
      ...
      if (!await confirmWithDialog2(document, "\u4ECE\u8FD9\u91CC\u622A\u65AD\u5F53\u524D\u5267\u60C5\uFF1F\u8FD9\u6761\u73A9\u5BB6\u6D88\u606F\u53CA\u5168\u90E8\u540E\u7EED\u5185\u5BB9\u4E0D\u4F1A\u8FDB\u5165\u65B0\u5206\u652F\uFF1B\u539F\u59CB\u5BA1\u8BA1\u65E5\u5FD7\u4ECD\u53EF\u6062\u590D\u3002", { title: "\u622A\u65AD\u5F53\u524D\u5267\u60C5", confirmLabel: "\u786E\u8BA4\u622A\u65AD" }))
        return;
    ...
    return React.createElement(React.Fragment, null, React.createElement("span", { className: "dsh-rp-actions" }, group && members.length > 0 ? [
      ...
      perform(async () => {
        if (recoveryAvailable || !group || members.length <= 1)
          return;
        if (!await confirmWithDialog2(document, `\u5220\u9664\u5F53\u524D\u7B2C ${currentIndex + 1}/${members.length} \u4E2A\u56DE\u590D\u5206\u652F\uFF1F\u539F\u59CB\u5BA1\u8BA1\u65E5\u5FD7\u4ECD\u4F1A\u4FDD\u7559\u3002`, { title: "\u5220\u9664\u56DE\u590D\u5206\u652F", confirmLabel: "\u786E\u8BA4\u5220\u9664" }))
          return;
...
function createRoleplayActions({ sessionsService, workspacesService, remoteSession, resolveActiveSessionId, isRoleplaySession, wakeSessionForState, invalidateState, acceptConversations, loadConversations, toast, fetch: fetch2 = globalThis.fetch, storage, now = Date.now, wait = (ms) => new Promise((resolve) => setTimeout(resolve, ms)) }) {
  const localStorage2 = {
  ...
  const requireSessionBinding = async (sessionId) => {
    ...
    if (!binding)
      throw new Error(`\u65B0\u5206\u652F ${sessionId} \u5C1A\u672A\u8FDB\u5165\u4F1A\u8BDD\u5217\u8868`);
    return binding;
  };
  ...
  return { saveState, branchRequest, runMaintenance, registerBranchOperation, retryBranchMutation, requireSessionBinding, repairActiveConversationDraft, openSessionPreservingView, createFirstTurnBranch, waitForBranchOperation, replaceMessage, forkAndPrompt, forkWithoutUserTurn, openNativeBranch };
}
...
function apply(ctx) {
  ...
  const loadConversations = async () => {
    ...
    conversationLoad = (async () => {
      ...
      conversationCatalog = { ...conversationCatalog, status: "error", error: String(errorMessage4(error)) };
      notifyConversations();
      throw error;
    }).finally(() => {
  ...
  function TavernWorkspacePresentation({ browserProps, renderDefault }) {
    ...
    if (projections.current?.catalog !== catalog)
      projections.current = { catalog, lists: /* @__PURE__ */ new WeakMap(), workspaces: /* @__PURE__ */ new WeakMap() };
    if (!catalog.value)
      return React.createElement("div", { role: "status", style: { padding: 12 } }, catalog.error ?? "\u6B63\u5728\u8BFB\u53D6\u9152\u9986\u4F1A\u8BDD\u2026", catalog.error && React.createElement("button", { onClick: () => void loadConversations().catch(() => {
      }) }, "\u91CD\u8BD5"));
    ...
    return renderDefault({
      ...
      open: (id) => {
        void loadConversations().then((latest) => browserProps.open(resolveConversationExecution(id, latest))).catch((error) => toast(errorMessage4(error)));
      },
  ...
  const readServiceSnapshot = (value) => {
    const service = value;
    if (!service)
      return null;
    ...
    for (const key of ["current", "active", "session", "snapshot"]) {
      if (service[key] !== void 0 && service[key] !== null)
        return service[key];
    }
    return null;
  };
  ...
  const UserInfoSettings = createUserInfoSettings({ React, toast });
  {
    let RoleplayManager = function({ sessionId, onClose }) {
      ...
      const primaryTabs = [["core-rules", "\u6838\u5FC3\u8BBE\u5B9A"], ["cards", "\u4EBA\u7269\u8BBE\u5B9A"], ["worldbook", "\u4E16\u754C\u4E66"], ["plot-guidance", "\u5267\u60C5\u6307\u5F15"], ["style-specialization", "\u6587\u98CE\u7279\u5316"], ["settings", "\u81EA\u5B9A\u4E49\u89C4\u5219"], ["memory", "\u8BB0\u5FC6"], ["character-cluster", "\u89D2\u8272\u96C6\u7FA4"], ["card-adaptation", "\u957F\u6587\u672C\u8F6C\u89D2\u8272\u5361"]];
      const secondaryTabs = [["presets", "\u9884\u8BBE"], ["resources", "\u8D44\u6E90\u5E93"], ["models", "\u6A21\u578B"], ["embedding", "Embedding \u6A21\u578B\u8BBE\u7F6E"], ["usage", "\u4F7F\u7528\u7EDF\u8BA1"], ["export", "\u5BFC\u51FA"], ["userinfo", "\u7528\u6237\u4FE1\u606F"], ["logs", "\u65E5\u5FD7"]];
    ...
    const sidebarIcon = (kind) => React.createElement("svg", { viewBox: "0 0 16 16", fill: "none", stroke: "currentColor", strokeWidth: 1.3, strokeLinecap: "round", strokeLinejoin: "round", "aria-hidden": true }, React.createElement("path", { d: kind === "tavern" ? "M2 7 8 2l6 5M3.5 6v7.5h9V6M6.5 13.5V9h3v4.5" : kind === "decision" ? "M2 2.5h12v9H8l-3.5 3v-3H2zM4.5 5.5h7M4.5 8h5" : "M2 2.5h12v11H2zM10 2.5v11M4 5h3M4 8h3" }));
    ...
    const decisionButton = (props) => {
      const sessionId = props?.useSessions?.((state) => state.current);
      return sidebarTooltip(props, "\u663E\u793A\u6216\u9690\u85CF\u51B3\u7B56\u5361", React.createElement("button", { type: "button", className: "dsh-rp-status-toggle", disabled: !isRoleplaySession(sessionId), "aria-label": "\u663E\u793A\u6216\u9690\u85CF\u51B3\u7B56\u5361", onClick: () => window.dispatchEvent(new Event("dsh-roleplay-decision-toggle")) }, sidebarIcon("decision")));
    };
    if (slots !== void 0) {
    ...
    const onSidebarState = () => {
      ...
      if (sid && sid !== lastSessionId) {
        ...
        if (typeof overlayRender === "function")
          overlayRender();
      }
    };
    onSidebarState();
