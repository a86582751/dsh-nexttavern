...
export function createConversationCatalog({read,write}) {
  const loaded=read()
  if(loaded&&(loaded.schemaVersion!==1||!loaded.worldlines||!loaded.conversations))throw new Error('酒馆会话目录版本损坏，未覆盖原记录')
  ...
  return {
    ...
    fail:(sessionId,operationId)=>mutate(next=>{
      ...
      if(book?.activeSessionId===sessionId){
        ...
        book.updatedAt=Date.now();changed=true
      }
      return changed
    }),
    activate:sessionId=>mutate(next=>{
      ...
      next.conversations[conversationId]={schemaVersion:1,conversationId,activeSessionId:sessionId,updatedAt:Date.now()}
    }),
    migrate:operations=>mutate(next=>{
      let recovered=false
      for(const op of operations){
        if(!['failed','aborted'].includes(op?.state)&&!op?.abortedAt)continue
        const childId=op.reservedChildSessionId??op.childSessionId,member=next.worldlines[childId]
        if(!member||member.operationId!==op.operationId)continue
        if(member.status!=='failed'){member.status='failed';recovered=true}
        const book=next.conversations[member.conversationId]
        if(book?.activeSessionId===childId){book.activeSessionId=member.conversationId;book.updatedAt=Date.now();recovered=true}
      }
      const pending=new Map(operations.filter(op=>(op?.groupId||op?.registration?.truncated===true)&&op.childSessionId&&op.anchor?.sourceSessionId&&kinds.has(op.kind)&&!['failed','aborted'].includes(op.state)&&!op.abortedAt&&(op.consumed===true||op.state==='registered'||Number(op.registeredAt)>0))
        .map(op=>[op.childSessionId,op]))
      ...
      while(pending.size&&progress){progress=false
        for(const [child,op] of pending){
          ...
          changed=reserve(next,{sourceSessionId:op.anchor.sourceSessionId,childSessionId:child,operationId:op.operationId,kind:op.kind,sourceHash:hash(op.anchor)},true)||changed
