// Generated from runtime/alpha3/src/ui/telemetry-panels.ts; edit the TypeScript source.
import { createPricePanel } from './pricing-panel.js';
...
export function createTelemetryPanels({ React, sessionDrafts, jsonFetch, toast }) {
    ...
    function UsageChart({ data }) {
        ...
        }, [0, .5, 1].map(r => React.createElement('g', {
            key: r
        ...
        }, React.createElement('title', null, `${statTime(p.at)} · ${label} ${statNumber(p[key])}`))))), React.createElement('text', {
    ...
    function TelemetryPanel({ sessionId, mode }) {
        ...
        const aggregateUsageRow = (r, providerOnly = false) => {
            ...
            return React.createElement('tr', {
                ...
                : React.createElement(React.Fragment, null, React.createElement('strong', null, r.model ?? '未知模型'), React.createElement('div', {
                    className: 'dsh-rp-muted'
                ...
                className: 'dsh-rp-muted'
            }, `命中 ${tokenValue(r, 'cacheReadTokens')} tokens${rateNote(r) ? ' · ' + rateNote(r) : ''}`)), React.createElement('td', null, cost.value === null ? 'N/A' : `${data?.currency} ${statNumber(cost.value)}`, React.createElement('small', {
                className: 'dsh-rp-muted'
            }, cost.caption)), React.createElement('td', null, `${Math.round(r.successes / Math.max(1, r.calls) * 100)}%`), React.createElement('td', null, r.timedCalls ? `${(r.durationMs / r.timedCalls / 1000).toFixed(2)}s` : '—'), React.createElement('td', null, embeddingView ? '不适用' : r.tokensPerSecond == null ? '—' : `${Number(r.tokensPerSecond).toFixed(1)} tok/s`, embeddingView ? null : r.speedCalls ? React.createElement('small', {
                className: 'dsh-rp-muted'
        ...
        const requestUsageRow = (r) => {
            ...
            return React.createElement('tr', {
                ...
                className: 'dsh-rp-muted'
            }, pricingNote(r.pricingStatus)) : null), React.createElement('td', null, `${r.durationMs == null ? '—' : (r.durationMs / 1000).toFixed(2) + 's'} / ${embedding ? '不适用' : r.firstTokenMs == null ? '—' : (r.firstTokenMs / 1000).toFixed(2) + 's'}`), React.createElement('td', null, React.createElement('span', {
        ...
        return React.createElement('div', {
        ...
        }, `正在回填历史记录 · ${data.coverage.scannedSessions ?? 0}/${data.coverage.totalSessions ?? 0} 个会话。当前数字尚不完整，完成后自动更新。`) : null, data?.fx && mode === 'usage' ? React.createElement('div', {
            className: 'dsh-rp-row dsh-rp-muted'
        }, React.createElement('span', null, `汇率来源：${data.fx.source ?? '未知'} · 更新时间：${data.fx.rateAt ? statTime(data.fx.rateAt) : data.fx.fetchedAt ? statTime(data.fx.fetchedAt) : '未知'}${data.fx.stale ? ' · 缓存已过期，继续显示上次成功结果' : ''}${data.fx.syncing ? ' · 正在更新…' : ''} · `), React.createElement('a', {
        ...
        }, metric('请求次数', statNumber(total.calls), `成功 ${total.successes} · 失败/取消/截断 ${total.failures}`), metric('输入 · 未缓存', tokenValue(total, 'inputTokens'), 'tokens', tokenNote(total, 'inputTokens')), metric('输出', tokenValue(total, 'outputTokens'), 'tokens', tokenNote(total, 'outputTokens')), metric('缓存命中率', rateValue(total.cacheHitRate), `命中 ${tokenValue(total, 'cacheReadTokens')} tokens`, rateNote(total)), metric('估算成本', costView(total).value === null ? 'N/A' : `${data?.currency} ${statNumber(costView(total).value)}`, costView(total).caption === '全部请求均有单价和用量' ? '按当前定价配置估算' : costView(total).caption)), React.createElement('nav', {
            ...
            : null, mode === 'logs' ? React.createElement(React.Fragment, null, React.createElement('div', {
            className: 'dsh-rp-table-wrap'
        }, React.createElement('table', {
            className: 'dsh-rp-table'
            ...
            className: `dsh-rp-status-badge dsh-rp-state-${r.status}`
        }, telemetryStatus[r.status] ?? r.status)), React.createElement('td', null, `${r.durationMs == null ? 'N/A' : (r.durationMs / 1000).toFixed(2) + 's'} / ${r.firstTokenMs == null ? 'N/A' : (r.firstTokenMs / 1000).toFixed(2) + 's'}`), React.createElement('td', null, r.usage ? `${statNumber(r.usage.inputTokens)} / ${statNumber(r.usage.outputTokens)} / ${statNumber(r.usage.cacheReadTokens)}` : 'N/A', r.usageNote ? React.createElement('small', {
            className: 'dsh-rp-muted'
