// Generated from runtime/alpha3/src/ui/author-panels.ts; edit the TypeScript source.
...
export function createAuthorPanels({ React, sessionDrafts, fetchState, invalidateState, saveState, runMaintenance, jsonFetch, toast }) {
    ...
    function MemorySettingsControls({ sessionId, visible, onSaved, retrievalControls }) {
        ...
        const save = async (reset = false) => {
            if (!policy)
                return;
            const settings = Object.fromEntries([...fields, ...legacy]
    ...
    function RuleSlicePanel({ scope, visible, field, title, guidance, rows }) {
        ...
        }), legacyEntries.length ? React.createElement('details', null, React.createElement('summary', null, `核心设定·旧卡常驻标记兼容投影（${legacyEntries.length}）`), React.createElement('p', {
            className: 'dsh-rp-muted'
    ...
    function SettingsPanel(props) {
        ...
        }, '开场已保存'))), React.createElement('h4', null, '排版美化 · 正则规则（阅读视图正文特效；JSON 数组 [{match, replace}]）'), React.createElement('p', null, '先匹配原文；未命中时匹配 Markdown 渲染后的 HTML。中文双引号规则也兼容正文中的成对直引号，不改写剧情记录。'), ta(beautyRules, value => {
