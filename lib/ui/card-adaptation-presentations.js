// Generated from runtime/alpha3/src/ui/card-adaptation-presentations.ts; edit the TypeScript source.
...
export function createCardAdaptationPresentations(React) {
    ...
    function renderSourceCards({ data, source, hasSources, busy, confirming, draft, modelSelection, modelDirty, modelSettingsRef, changeModelSelection, selectModel, selectSource, toggleAutoIndex, toggleAutoRebuild, libraryMutation, indexAction, pauseIndex, }) {
        ...
        const semanticCard = h('div', { className: 'dsh-rp-adaptation-card' }, h('h5', null, '小说研究模型（全局）'), muted('与 Embedding 设置共用同一组供应商和模型，不在这里重复配置供应商。此选择仅影响小说原文索引和检索，不改变剧情记忆模型；保留已有索引，配置不匹配时可再修复索引。'), data ? field('模型', h('select', {
            ref: (element) => { modelSettingsRef.current = element; },
        ...
        }, h('option', { value: '' }, '未选择（不继承剧情记忆模型）'), semanticModels.map(item => h('option', { key: item.id, value: item.id, disabled: !item.ready }, `${item.name} · ${item.model}${item.ready ? '' : '（未就绪）'}`)))) : muted('正在读取语义模型…'), data
            && h('div', { className: 'dsh-rp-adaptation-actions' }, actionButton('应用小说研究模型', selectModel, (!!modelSelection && !semanticModels.some(item => item.id === modelSelection && item.ready)) || !modelDirty, true)), data && modelDirty && muted('有未应用的小说研究模型选择。'), data
            && h('div', { className: 'dsh-rp-adaptation-field' }, h('label', null, h('input', {
                type: 'checkbox',
                checked: data.autoIndexNewSources,
                disabled: busy !== null || confirming,
                onChange: toggleAutoIndex
            }), ' 导入或关联小说时自动准备语义索引'), muted('仅对新导入或新关联的原著生效。模型可用时自动建库，已有兼容索引保留并补缺、重试失败或未知任务；在线模型可能产生费用。不恢复当前对话已暂停或清空的资料。')), data
            && h('div', { className: 'dsh-rp-adaptation-field' }, h('label', null, h('input', {
                type: 'checkbox',
    ...
    function renderResearchCards({ data, source, hasSources, busy, confirming, draft, researchMode, researchProtagonist, researchOpeningPoint, researchDirty, maxQueryBatches, maxReadPackets, noteView, queryDraft, editingSegment, refreshing, setResearchMode, setResearchProtagonist, setResearchOpeningPoint, setMaxQueryBatches, setMaxReadPackets, markResearchDirty, chooseResearchMode, saveResearchBudget, toggleResearchPause, changeNoteView, setQueryDraft, search, refreshNotes, clearNotes, updateDraft, saveDraft, cancelEdit, startEdit, deleteNote, loadMoreNotes, }) {
        ...
        const researchCard = research && source ? h('div', { className: 'dsh-rp-adaptation-card' }, h('h5', null, '阅读与改编模式'), muted('先选择原著阅读策略，再开始阅读笔记。原创互动写卡与这里独立；切换模式不会删除索引或原文。'), h('div', { className: 'dsh-rp-adaptation-grid' }, field('阅读模式', h('select', {
            ...
            disabled: busy !== null || confirming,
            onChange: (e) => { setResearchProtagonist(e.target.value); markResearchDirty(); }
        })), field('开篇位置', h('input', {
            className: 'dsh-rp-input',
            value: researchOpeningPoint,
            disabled: busy !== null || confirming,
            placeholder: '例如：第 1 章',
            onChange: (e) => { setResearchOpeningPoint(e.target.value); markResearchDirty(); }
        }))), researchMode === 'close-reading'
            ? muted('精读：完整通读原著，耗时较高，适合需要连续细节和完整背景的改编。')
            : researchMode === 'coarse'
                ? muted('粗颗粒度：定向证据 + 全局分层抽查；覆盖有限，不能声称读完整本原著。')
                : muted('请选择精读或粗颗粒度模式。'), h('div', { className: 'dsh-rp-adaptation-actions' }, actionButton('确认阅读模式', () => chooseResearchMode(researchMode), !researchMode || (researchMode === 'coarse' && !researchProtagonist.trim()) || !researchDirty, true)), h('div', { className: 'dsh-rp-adaptation-grid' }, field('最大检索批次（1–200）', h('input', {
            className: 'dsh-rp-input',
            type: 'number',
            min: 1,
            max: 200,
            ...
            disabled: busy !== null || confirming,
            onChange: (e) => { setMaxQueryBatches(e.target.value); markResearchDirty(); }
            ...
            disabled: busy !== null || confirming,
            onChange: (e) => { setMaxReadPackets(e.target.value); markResearchDirty(); }
