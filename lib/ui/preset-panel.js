// Generated from runtime/alpha3/src/ui/preset-panel.ts; edit the TypeScript source.
import { projectConversationList, resolveConversationExecution } from './conversation-projection.js';
export function presetConversationChoices(snapshot, catalog, isRoleplaySession) {
...
const modes = [['system', '沿用系统美学'], ['blend', '融合卡片文风'], ['card', '完全采用卡片文风']];
const describe = (mode) => mode === 'system' ? '不注入卡片的叙事规则、回复规则、文风示例。' : mode === 'blend' ? '同时注入预设和卡片文风；冲突时预设优先。' : '只采用卡片文风，不注入预设正文。';
export function createPresetPanel({ React, jsonFetch, toast, confirmWithDialog, conversations }) {
    ...
    return function PresetPanel({ sessionId }) {
        ...
        React.useEffect(() => { let live = true; conversations().then(value => { if (live)
            setTargets(value); }).catch(e => { if (live)
            setError(String(e.message)); }); return () => { live = false; }; }, [sessionId]);
