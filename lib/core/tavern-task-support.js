// Generated from runtime/alpha3/src/core/tavern-task-support.ts; edit the TypeScript source.
...
export function taskValidationFailure(job) {
    ...
    if (failure?.schemaVersion === 1 && failure.category === 'validation')
        return copy(failure);
    // Legacy jobs retained this local message but had no structured failure.
    if (job.error === '状态结果没有保留作者模板')
        return new TaskValidationError(job.error, [{ path: 'html', rule: 'author-template', expected: 'author-template', actual: 'mismatch' }], 'STATUS_TEMPLATE_MISMATCH').failure;
    return null;
}
...
export function taskFailureDetails(raw) {
    if (raw instanceof TaskValidationError)
        return copy(raw.failure);
    let f = raw ?? {};
    const seen = new Set();
