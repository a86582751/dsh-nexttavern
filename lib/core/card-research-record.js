// Generated from runtime/alpha3/src/core/card-research-record.ts; edit the TypeScript source.
...
export const categories = ['world', 'rule', 'faction', 'relation', 'event'];
const allCategories = [...categories, 'character'];
export const topics = ['identity', 'motive', 'relations', 'affiliation', 'limits', 'turning-point', 'opening'];
export const modes = ['close-reading', 'coarse'];
export const queryModes = ['keyword', 'semantic', 'hybrid'];
...
export const object = (v) => v !== null && typeof v === 'object' && !Array.isArray(v) ? v : {};
export function text(v, name, max = 1600, optional = false) {
...
export function unique(values, key, name) {
    const seen = new Set();
...
export function resolveInputCitations(source, receipts, raw, label = 'citations') {
    if (!Array.isArray(raw) || raw.length < 1 || raw.length > 6)
        throw Error(`${label} 需要 1–6 条原文引用`);
...
export function resolveInputFact(source, receipts, input) {
    const result = {
    ...
    if (input.chain !== undefined) {
        ...
        result.chain = {
            ...chain,
            actions: chain.actions.map((item, index) => {
    ...
    if (input.edges !== undefined) {
        ...
        result.edges = input.edges.map((item, index) => {
...
export function factFromStored(source, raw, scope) {
...
export function privateFact(source, raw, id) {
    return factFromStored(source, {
        ...object(raw), id, certainty: 'unknown', citations: []
    }, 'private');
}
export function sharedFact(source, raw) {
    const value = object(raw), fact = {
        id: '0'.repeat(64),
...
export const sameFact = (a, b) => JSON.stringify(a) === JSON.stringify(b);
function receiptFromStored(source, raw, label = '阅读回执') {
    ...
    return {
        segment, start, end, sha256: sha256
    };
}
...
function frontierFromStored(source, raw) {
    ...
    if (normalized !== normalizedQuery(query) || !frontierSources.includes(sourceKind)
        || !frontierStates.includes(state))
...
export function validatePlan(source, raw) {
    ...
    if (![1, 2].includes(Number(value.schemaVersion)) || value.kind !== 'adaptation-research-plan' || typeof value.owner !== 'string'
        ...
        || !(value.mode === null || modes.includes(value.mode))
    ...
    plan.queries = value.queries.map(item => {
        const query = object(item), rawQuery = text(query.query, '研究查询', 1000), fingerprint = text(query.fingerprint, '检索指纹', 300), mode = query.mode === undefined
            ? (fingerprint === 'keyword'
                ? 'keyword'
                : 'semantic')
            : text(query.mode, '检索模式', 20), seq = integer(query.seq, '查询位置', -1, Number.MAX_SAFE_INTEGER), rangesRaw = query.ranges;
        ...
        return {
            query: rawQuery, fingerprint, mode: mode, seq, ranges, candidates
        };
    });
    ...
    plan.web = value.web.map(item => {
        ...
        return {
            url, callId, resultSeq, excerptHash: excerptHash
        };
    });
...
export const frontierId = (source, normalized) => hash(`${source.textSha256}\u0000${normalized}\u0000frontier-v1`);
export function queueFrontier(plan, source, query, sourceKind, priority, parentFactIds = []) {
...
export function sampleSegments(source) {
...
export function seedFrontier(plan, source) {
...
export function currentPackets(plan, raw) {
    if (raw === undefined)
        return [];
...
export function checkpoint(owner, source, plan, packets) {
    return {
        ...delivery(owner, source, plan, packets), revision: plan.revision
    };
}
export function acknowledgePackets(plan, packets) {
    for (const packet of packets) {
...
export function addPackets(plan, source, input) {
    ...
    if (query !== undefined && completeQuery) {
        ...
        const nextCandidates = candidates.map((candidate, index) => {
            ...
            return candidateFromStored(source, {
