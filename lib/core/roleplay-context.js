...
import { projectStoryEvent, messageViewGeneration } from './roleplay-message-view.js';
...
export function surfaceEvents(session) {
    ...
    for (const seq of nodes) {
        ...
        if (e)
            out.push(projectStoryEvent(session, e));
    }
    return out;
}
...
export function readRoleplayActivity(session, preparation, jobs = [], now = Date.now()) {
    ...
    for (let index = events.length - 1; startIndex >= 0 && index > startIndex; index--) {
        ...
        if (type === 'user/message') {
            ...
            if (!phase && source?.kind === 'roleplay-tasks' && source.form === 'phase')
                phase = event;
...
export function retireRoleplayContexts(session, turn, prepared = []) {
    ...
    for (const form of ['director-notes', 'state']) {
        ...
        const valid = (source) => source?.kind === 'roleplay-context'
        ...
        for (const event of old) {
            ...
            session.append('user/message', { id: randomUUID(), role: 'user', source: { kind: 'roleplay-context', form: 'retired', schemaVersion: 1,
                    retiredForm: form, retiredAtTurn: turn, branchId: session.id, sourceSeq: event.seq }, content: [{ type: 'text', text: '[旧动态上下文已回收；以当前锚点为准。]' }] }, { surfaceOp: { op: 'replace', startSeq: event.seq, endSeq: event.seq }, sourceEventSeqs: [event.seq] });
            retired++;
        }
    }
    return retired;
...
export function surfaceEntries(session) {
    ...
    for (const event of surface) {
        ...
        if (!ended.has(turn) || completed.has(turn) || committedStory.has(event.seq))
    ...
    for (const e of surface) {
        if (e.type === 'user/message') {
            ...
            if (src?.kind !== 'user' && !(src?.kind === 'roleplay' && src?.form === 'branch-user'))
                continue;
...
export function canonicalAssistantForTurn(session, turn) {
    ...
    for (let index = events.length - 1; index >= 0; index -= 1) {
        ...
        if (original?.type !== 'assistant/message' || byTurn.has(numericTurn))
            continue;
    ...
    if (generation !== null)
        canonicalAssistantCache.set(session, {
...
export function retainRoleplayWindowContinuity(session, messages, budgetTokens) {
    ...
    const selected = list.filter((message) => {
        ...
        if ((source?.kind === 'roleplay-tasks' || source?.kind === 'roleplay-context'))
            return false;
...
export function roleplayWindowCutStartIndex(surface, storyStartIndex) {
    ...
    while (index > 0) {
        ...
        const source = event?.type === 'user/message' ? event.data?.source : null;
        if (source?.kind !== 'roleplay-context')
            break;
        index -= 1;
    }
    return index;
