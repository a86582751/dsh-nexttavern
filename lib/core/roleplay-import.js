// Generated from runtime/alpha3/src/core/roleplay-import.ts; edit the TypeScript source.
import { IMPORT_NORMALIZER, IMPORT_LIMITS, resolveImportSource, assertImportRecordIntegrity, assertAssignmentBudget, assertReviewProof, normalizeSourceSpans, spanText, assertReferenceBudget, sourceDescriptor, importCoverage, validateAssignmentIdentities, importSummary, } from './roleplay-import-record.js';
    ...
    registerCardExport(ctx, {
        ...
        collect: async (session) => {
            ...
            const original = (pointer?.importId
                ? T.branch.get(importRecordKey(pointer.sourceRecordSessionId
                    ?? session.id, pointer.importId))
                : null);
    ...
    ctx.effect(() => ctx.tools.register(simpleTool('rp_card_import_finalize', '校验全文已审阅、原文字符 100% 已分类后以回滚日志激活；遗漏、主归属重叠、跨度哈希变化或写后校验失败都会拒绝提交。', {
        ...
        return withImportLock(session.id, args.import_id, async () => {
            ...
            const joinImported = (parts) => parts
                ...
                .sort((left, right) => left.startLine - right.startLine || left.endLine - right.endLine || Number(left.partIndex) - Number(right.partIndex)
            ...
            const mergeUniqueStrings = (...groups) => [
                ...new Set(groups.flat().filter((value) => value !== undefined
            ...
            try {
                ...
                const nextRules = {
                    ...
                    beauty: nextBeauty,
                    sources: {
                        ...Object.fromEntries(RULE_TEXT_FIELDS.map(field => [
                            field,
