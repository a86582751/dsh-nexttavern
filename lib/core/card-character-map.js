// Generated from runtime/alpha3/src/core/card-character-map.ts; edit the TypeScript source.
...
export function characterOccurrenceMap(source, terms, options = {}) {
    const bounded = (v, min, max, name) => { if (!Number.isSafeInteger(v) || v < min || v > max)
        throw Error(`${name} 必须在 ${min}–${max} 内`); return v; };
    if (!Array.isArray(terms) || !terms.length || terms.length > 16 || terms.some(t => typeof t !== 'string' || !t.trim() || t.length > 100))
        throw Error('人物定位需要 1–16 个不超过 100 字符的姓名/候选别名');
    ...
    const covered = (range, ranges) => {
        let total = 0;
        for (const r of ranges) {
            if (r.start >= range.end)
                break;
            if (r.end > range.start)
                total += Math.max(0, Math.min(r.end, range.end) - Math.max(r.start, range.start));
        }
        return total;
    };
    const segmentAt = (position) => {
        let lo = 0, hi = source.segments.length - 1;
        while (lo <= hi) {
            const mid = (lo + hi) >>> 1, seg = source.segments[mid];
    ...
    for (const hit of found) {
        const w = window(hit), last = groups.at(-1);
        if (last && w.start <= last.end) {
            ...
            groups.push({ ...w, occurrences: 1, terms: new Set([hit.term]) });
    }
    const describe = (r) => ({ start: r.start, end: r.end, chapterStart: segmentAt(r.start)?.chapter ?? '', chapterEnd: segmentAt(Math.max(r.start, r.end - 1))?.chapter ?? '',
    ...
    if (found.length) {
        chosen.set(found[0].start, found[0]);
        chosen.set(found.at(-1).start, found.at(-1));
        for (let bin = 0; bin < samples; bin++) {
            ...
            let best;
            for (const hit of found)
                if (hit.start >= left && hit.start < right && (!best || Math.abs(hit.start - mid) < Math.abs(best.start - mid)))
                    best = hit;
            if (best)
                chosen.set(best.start, best);
        }
    }
    const sampleFor = (hit) => {
    ...
    return { terms: names, matching: 'literal-case-sensitive', occurrences: found.length, intervalCount: groups.length, truncated, scannedCompletely: !truncated,
