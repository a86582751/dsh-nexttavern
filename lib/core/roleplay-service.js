// Generated from runtime/alpha3/src/core/roleplay-service.ts; edit the TypeScript source.
...
export function createRoleplayService(deps) {
    ...
    const svc = {
        ...
        branchLineage(session) {
            ...
            let depth = 0;
            while (cur && depth < 64) {
                chain.push({ sessionId: cur.id, parentSession: cur.header?.parentSession ?? null, seedLength: cur.inheritedEventCount ?? null });
                if (!cur.header?.parentSession)
                    break;
        ...
        async recordVersion(branchId, anchorSeq, turn, seq) {
            ...
            if (!T.branch.get(key))
                await T.branch.put(key, { anchors: {} });
            return T.branch.update(key, (current) => {
                ...
                if ((group.entries ?? []).some((entry) => Number(entry?.seq) === nextEntry.seq))
                    return rec;
        ...
        async setStatusSpec(branchId, text, atSeq, repairId) {
