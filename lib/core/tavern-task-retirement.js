...
export const taskContextRecord = (value) => value !== null
...
export function createTaskRetirement({ internalTaskSeqs, inlineTaskEnvelope }) {
    function completedImportSpan(nodes, lookup) {
        const callsOf = (e) => e?.type === 'assistant/message' ? (e.data?.message?.content
    ...
    function retireCompletedTaskContexts(session, currentTurn, activeImport) {
    ...
    function retireSettledInlineContexts(session, currentTurn, jobs) {
        ...
        for (const selected of groups) {
            ...
            const text = `先前维护尝试已结束或被新尝试替代，完整参数、输出与错误记录保留在历史 seq ${selected[0]}–${selected.at(-1)}。成功结果采用最新状态及笔记；失败、取消或过期不是剧情事实。任务概况：${JSON.stringify(details.slice(0, 8).map(({ id, kind, state, failure }) => ({
                id,
                kind,
                state,
                ...(failure ? {
