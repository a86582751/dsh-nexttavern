// Generated from runtime/alpha3/src/core/tavern-conversations.ts; edit the TypeScript source.
import { createHash } from 'node:crypto';
const kinds = new Set(['regenerate', 'player-edit', 'player-edit-send', 'edit', 'delete-user']);
const copy = (value) => structuredClone(value);
const id = (value) => {
    if (typeof value !== 'string' || !/^[a-zA-Z0-9][a-zA-Z0-9_.:-]{0,199}$/.test(value) || ['__proto__', 'constructor', 'prototype'].includes(value))
        throw new Error('会话或操作标识无效');
    return value;
};
...
export function createConversationCatalog({ read, write }) {
    const loaded = read();
    if (loaded && (typeof loaded !== 'object' || !('schemaVersion' in loaded) || loaded.schemaVersion !== 1 || !('worldlines' in loaded) || !loaded.worldlines || !('conversations' in loaded) || !loaded.conversations))
        throw new Error('酒馆会话目录版本损坏，未覆盖原记录');
    let state = loaded ? copy(loaded) : { schemaVersion: 1, revision: 0, worldlines: {}, conversations: {} };
    for (const book of Object.values(state.conversations))
        if (book?.selectionRevision !== undefined && (!Number.isSafeInteger(book.selectionRevision) || book.selectionRevision < 0))
            throw Error('世界线选择版本损坏，未覆盖原记录');
    let chain = Promise.resolve();
    const rootOf = (sessionId, snapshot = state) => snapshot.worldlines[id(sessionId)]?.conversationId ?? sessionId;
    const mutate = (work) => {
    ...
    function reserve(next, { sourceSessionId, childSessionId, operationId, kind, sourceHash }, migration = false) {
        for (const value of [sourceSessionId, childSessionId, operationId])
            id(value);
        if (!kinds.has(kind) || !/^[a-f0-9]{64}$/.test(sourceHash ?? ''))
            throw new Error('世界线来源证据无效');
        const conversationId = rootOf(sourceSessionId, next);
        if (childSessionId === sourceSessionId || childSessionId === conversationId)
            throw new Error('世界线不能指向自身');
        const previous = next.worldlines[childSessionId];
        if (previous) {
            if (previous.conversationId !== conversationId || previous.operationId !== operationId || previous.sourceSessionId !== sourceSessionId)
                throw new Error('世界线已绑定其他会话归属');
            return false;
        }
    ...
    return {
        ...
        reserve: (reservation) => mutate(next => reserve(next, reservation)),
        markReady: (sessionId) => mutate(next => {
            const member = next.worldlines[id(sessionId)];
            if (!member)
                throw new Error('世界线尚未登记');
            if (member.status === 'failed')
                throw new Error('世界线已失效，请重新发起');
            if (member.status === 'ready')
                return false;
            member.status = 'ready';
        }),
        fail: (sessionId, operationId) => mutate(next => {
            const member = next.worldlines[id(sessionId)];
            if (!member)
                return false;
            if (operationId && member.operationId !== operationId)
                throw new Error('世界线失败操作归属不匹配');
            ...
            if (book?.activeSessionId === sessionId) {
                ...
                book.selectionRevision = (book.selectionRevision ?? 0) + 1;
                changed = true;
            }
            return changed;
        }),
        activate: (sessionId) => mutate(next => {
            id(sessionId);
            const conversationId = rootOf(sessionId, next), member = next.worldlines[sessionId];
            if (member && member.status !== 'ready')
                throw new Error('世界线尚未就绪或已失效');
            const previous = next.conversations[conversationId];
            if (previous?.activeSessionId === sessionId && (!member || state.worldlines[sessionId]?.status === 'ready'))
                return false;
        ...
        migrate: (operations) => mutate(next => {
            let recovered = false;
            for (const op of operations) {
                if (!op || !['failed', 'aborted'].includes(op.state ?? '') && !op.abortedAt)
                    continue;
                const childId = op.reservedChildSessionId ?? op.childSessionId, member = childId === undefined ? undefined : next.worldlines[childId];
                if (!member || member.operationId !== op.operationId)
                    continue;
                if (member.status !== 'failed') {
                    member.status = 'failed';
                    recovered = true;
                ...
                const book = next.conversations[member.conversationId];
                if (book && book.activeSessionId === childId) {
                    book.activeSessionId = member.conversationId;
                    book.updatedAt = Date.now();
                    book.selectionRevision = (book.selectionRevision ?? 0) + 1;
                    recovered = true;
                }
            }
            const eligible = (op) => Boolean(op &&
            ...
            while (pending.size && progress) {
                ...
                for (const [child, op] of pending) {
                    if (pending.has(op.anchor.sourceSessionId) && !next.worldlines[op.anchor.sourceSessionId])
                        continue;
