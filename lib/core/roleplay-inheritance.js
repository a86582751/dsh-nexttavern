// Generated from runtime/alpha3/src/core/roleplay-inheritance.ts; edit the TypeScript source.
...
export function createRoleplayInheritance(deps) {
                ...
                if (parent) {
                    ...
                    if (!inheritanceReady) {
                        ...
                        for (const [k, v] of T.cards.entries()) {
                            if (k.startsWith(pPrefix) && v)
                                await T.cards.put(cP + k.slice(pPrefix.length), cloneBranchRecord(v));
                        }
                        for (const [k, v] of T.worldbook.entries()) {
                            if (k.startsWith(pPrefix) && v)
                                await T.worldbook.put(cP + k.slice(pPrefix.length), cloneBranchRecord(v));
                        }
                        for (const table of [T.rules, T.opening]) {
                            for (const [k, v] of table.entries()) {
                                if (k.startsWith(pPrefix) && v)
                                    await table.put(cP + k.slice(pPrefix.length), cloneBranchRecord(v));
                            }
                        }
                        const parentSettings = T.branch.get(keyOf(parent, 'settings'));
                        for (const [k, v] of [...T.branch.entries()])
                            if (k.startsWith(`${parent}__cluster-lore-`) && Number(v.seq) < Number(seedLength) && clusterLoreVisible(session, v, Number(seedLength)))
                        ...
                        if (parentSettings)
                            await T.branch.put(keyOf(session.id, 'settings'), {
                        ...
                        if (parentStatusSpec)
                            await T.status.put(keyOf(session.id, 'spec'), cloneBranchRecord(parentStatusSpec));
    ...
    async function carryTruncationBoundary(session) {
        ...
        if (!parentId || String(meta?.truncatedFrom ?? '') !== parentId || meta?.boundaryState)
            return null;
        ...
        if (entries.some((entry) => Number(entry.seq) >= seedLength))
            return null;
        let boundaryTurn = -1;
        let boundarySeq = -1;
        for (const entry of entries) {
            ...
            if (entry.kind !== 'assistant' || !Number.isSafeInteger(seq) || seq >= seedLength || !Number.isSafeInteger(turn))
                continue;
            if (canonicalAssistantForTurn(session, turn)?.seq !== seq)
                continue;
            if (seq > boundarySeq) {
                boundaryTurn = turn;
                boundarySeq = seq;
        ...
        if (boundarySeq < 0)
            return null;
        const turnKey = (sessionId) => keyOf(sessionId, `turn-${boundaryTurn}-${boundarySeq}`);
        const result = {
            schemaVersion: 1,
            turn: boundaryTurn,
            seq: boundarySeq,
