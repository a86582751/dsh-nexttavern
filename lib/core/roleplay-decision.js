// Generated from runtime/alpha3/src/core/roleplay-decision.ts; edit the TypeScript source.
...
export function taskCancellation(initial) {
    const controller = new AbortController(), listeners = new Map();
    const follow = (signal) => {
        if (!signal || listeners.has(signal))
            return;
    ...
    return { signal: controller.signal, follow, retrySignal: () => listeners.size ? AbortSignal.any([...listeners.keys()]) : undefined,
        dispose: () => { for (const [signal, abort] of listeners)
            signal.removeEventListener('abort', abort); listeners.clear(); } };
}
export function createRoleplayDecision(deps) {
    ...
    function publishTurnDecision(session, event, snapshot, narrative, signal) {
        ...
        const work = Promise.resolve().then(() => generateTurnDecision(session, event, snapshot, narrative, admitted, cancellation.signal))
            .finally(() => { cancellation.dispose(); admitted(null); if (decisionJobs.get(key) === work)
            decisionJobs.delete(key); });
    ...
    async function generateTurnDecision(session, event, snapshot, narrative, onAdmission, signal) {
        ...
        const canPublish = () => {
            ...
            if (current && Number.isFinite(currentTurn) && currentTurn > requestedTurn)
                return false;
            if (current && currentTurn === requestedTurn) {
                if (current.answered === true || current.superseded === true)
                    return false;
        ...
        let decisionOptions = [];
        const panelBelongsToTurn = Number(panelNow?.turnId) === Number(snapshot.turnId) ||
            ...
            if (T.decision.get(key) !== undefined && typeof T.decision.update === 'function') {
                await T.decision.update(key, (currentRaw) => {
                    ...
                    if (Number(current?.turnId) === Number(snapshot.turnId) &&
                        (current?.answered === true || current?.superseded === true))
                        return currentRaw;
                    if (current && Number.isFinite(Number(current.turnId))
                        && Number(current.turnId) > Number(snapshot.turnId))
                        return currentRaw;
                    return record;
                });
            }
