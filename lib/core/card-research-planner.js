// Generated from runtime/alpha3/src/core/card-research-planner.ts; edit the TypeScript source.
import { hash, categories, topics, modes, queryModes, hex64, object, text, integer, unique, normalizedQuery, resolveInputCitations, resolveInputFact, factFromStored, privateFact, sharedFact, sameFact, validatePlan, frontierId, queueFrontier, sampleSegments, seedFrontier, queueFactEdges, delivery, currentPackets, checkpoint, acknowledgePackets, addPackets, } from './card-research-record.js';
...
export function createResearchPlanner(table, loadSource, assessLongitudinal) {
    ...
    function assess(owner, id) {
        ...
        })), target = plan?.target.protagonist ?? '', missing = [];
        if (!target)
            missing.push('指定重点主角');
    ...
    function view(owner, id) {
        ...
        if (!plan)
            return undefined;
        const all = entries(owner, id), pending = plan.frontier.filter(item => item.state === 'pending').sort((a, b) => b.priority - a.priority
    ...
    async function offer(owner, id, seq) {
        return lock(planKey(owner, id), async () => {
            ...
            const source = loadSource(owner, id), plan = {
                schemaVersion: 2,
        ...
        return lock(planKey(owner, id), async () => {
            ...
            const completed = new Set(plan.queries.filter(item => item.fingerprint === checkedFingerprint && item.mode === mode
                && (visiblePacketIds === undefined
                    || item.ranges.every(range => plan.packets.some(packet => packet.segment === range.segment && packet.start <= range.start && packet.end >= range.end
                        && (packet.acknowledged || packet.generation === plan.generation && visiblePacketIds.has(packet.id)))))).map(item => normalizedQuery(item.query)));
    ...
    async function recordPacket(owner, id, generation, rows, query, fingerprint = 'keyword', seq = -1, frontierId, candidates = [], mode = fingerprint === 'keyword'
        ? 'keyword'
        : 'semantic', completeQuery = true, valid = () => true) {
        return (await recordBatch(owner, id, generation, [{
    ...
    async function read(owner, id, segment, offset = 0, maxChars = 2400, valid = () => true) {
        ...
        const generation = await reserve(owner, id, 'readPackets'), start = part.start + offset, end = Math.min(part.end, start + maxChars), row = {
    ...
    function citations(owner, id, raw) {
        ...
        for (const cite of result)
            if (!plan.receipts.some(receipt => receipt.segment === cite.segment && receipt.start <= cite.start && receipt.end >= cite.end
    ...
    async function append(owner, id, revision, raws, sourcePacketIds = []) {
        ...
        return lock(baseKey(source), async () => lock(planKey(owner, id), async () => {
            ...
            const nextPlan = structuredClone(plan), nextBase = {
    ...
    async function save(owner, id, revision, raw, sourcePacketIds = []) {
        ...
        return lock(planKey(owner, id), async () => {
            ...
            const requested = typeof input.id === 'string'
                && hex64(input.id)
                ? input.id
                : undefined, existing = requested
                ? plan.entries.find(f => f.id === requested)
                : undefined, fact = privateFact(source, input, existing?.id
                ?? hash(randomUUID()));
