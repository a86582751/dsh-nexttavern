	...
	(async () => {
		...
		try {
			...
			for await (const chunk of openaiStream) {
				...
				// Some providers deliver final usage after finish_reason with choices: [].
				const terminalUsage = chunk.usage ?? (choice as any)?.usage;
				if (hasFinishReason && Number.isFinite(terminalUsage?.total_tokens)) sawTerminalUsage = true;
				if (!choice) {
					if (sawTerminalUsage) break;
					continue;
				}
...
function addCacheControlToLastConversationMessage(
	...
	for (let i = messages.length - 1; i >= 0; i--) {
		const message = messages[i]!; // Loop bounds prove the element exists.
