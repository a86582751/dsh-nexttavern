// Generated from runtime/alpha3/compat/pi-ai/src/api/bedrock-converse-stream.ts; edit the TypeScript source.
...
function convertMessages(context, model, cacheRetention, env) {
    ...
    for (let i = 0; i < transformedMessages.length; i++) {
        const m = transformedMessages[i]; // The bounded loop visits the dense transformed array.
        switch (m.role) {
            case "user": {
                const content = [];
    ...
    if (cacheRetention !== "none" && supportsPromptCaching(model, env) && result.length > 0) {
        const lastMessage = result[result.length - 1]; // The enclosing condition requires a nonempty result.
