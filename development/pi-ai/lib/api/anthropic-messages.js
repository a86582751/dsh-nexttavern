// Generated from runtime/alpha3/compat/pi-ai/src/api/anthropic-messages.ts; edit the TypeScript source.
...
async function* iterateSseMessages(body, signal) {
    ...
    try {
        ...
        try {
            ...
            // Reader cleanup must not replace the terminal protocol result.
        }
        finally {
            reader.releaseLock();
        }
...
function convertMessages(transformedMessages, isOAuthToken, cacheControl, allowEmptySignature = false, deferredToolNames = new Set(), normalizeToolName = (name) => name, managedProvider) {
    ...
    for (let i = 0; i < transformedMessages.length; i++) {
        const msg = transformedMessages[i]; // Loop bounds prove the element exists.
        if (msg.role === "user") {
    ...
    if (cacheControl && params.length > 0) {
        const lastMessage = params[params.length - 1]; // Non-empty params checked above.
...
function insertThinkingLevelMessages(converted, activeEffort) {
    ...
    for (let index = 0; index < converted.messages.length; index++) {
        ...
        messages.push(converted.messages[index]); // Loop bounds prove the element exists.
    }
