...
type EditPrice = z.infer<typeof editPriceSchema>
...
export type RetainedPrice = z.infer<typeof retainedPriceSchema>

function editablePrice(message: Message): EditPrice | undefined {
...
export function planMessageEdit<Node extends Pick<MeterSurfaceNode, 'seq' | 'heuristicTokens' | 'edit'>>(
  nodes: readonly Node[],
  event: SessionEvent<'roleplay/message-edit'>,
): SurfaceTokenPlan<Node> {
...
export function planRetainedSurface(
  nodes: readonly RetainedPrice[], event: SessionEvent,
): SurfaceTokenPlan<RetainedPrice> | undefined {
  if (isMessageEdit(event)) return planMessageEdit(nodes, event)
