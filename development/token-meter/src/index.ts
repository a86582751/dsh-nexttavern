...
export type * from './usage-projection.js'
export type * from './breakdown-projection.js'
...
export class TokenMeter extends Service {
  ...
  private _foldEvent(state: ReplayState, event: SessionEvent): void {
    ...
    // A separate edit decision adjusts current prices only. Immutable anchor
    // nodes still describe the request that actually incurred the usage.
    const plan = isMessageEdit(event)
      ? planMessageEdit(state.surface, event)
      : isSurfaceEvent(event) ? planSurfaceTokens(state.surface, event) : undefined
