...
import type { ContextPressureProjection, TokenUsageProjection } from './projection.js'
 ...
 * one. Required edits need the prior price at their exact event position, so
 * this owned fold keeps compact prices for the retained surface. Replacements
 * remove those exact prices; shadow claims still validate adjacency/ranges.
 * State is O(retained surface), with no story text. A usage sample is stamped
...
export const contextPressureProjectionDefinition = {
  ...
  init: (): ContextPressureState => ({ surfaceTokens: 0, nodes: [] }),
  apply: (state, event) => {
    const plan = planRetainedSurface(state.nodes, event)
    // Preserve upstream adjacent-range validation, including on corrupt
    // replay. Exact retained-node prices also cover replacements without claims.
    ...
    if (plan !== undefined) {
      ...
      next = { ...next, nodes, surfaceTokens: next.surfaceTokens + plan.deltaTokens }
    }
