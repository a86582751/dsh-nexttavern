import type {BeforeForkPublish} from './fork-reservation.js'
...
export type {BeforeForkPublish, ForkReservation} from './fork-reservation.js'
...
export class SessionController extends TypertRemoteService {
  ...
  constructor(ctx: Context, config: Config, internals: SessionControllerInternals = {}) {
    ...
    const fileUploads: Context['fileUploads'] = Reflect.get(ctx.fileUploads, Symbol.for('cordis.original')) ?? ctx.fileUploads
  ...
  /** Reserve lineage before native publication; deliberately not a Remote endpoint. */
  forkPrepared(request: SessionForkRequest, beforePublish: BeforeForkPublish): Promise<SessionForkValue> {
    if (typeof beforePublish !== 'function') throw new TypeError('forkPrepared requires a reservation callback')
    return this.commands.fork(request, beforePublish)
  }
