...
function activity(name) {
    ...
    if (name === 'subagent' || name.startsWith('subagent_'))
        return 'subagents';
    if (['todo_write', 'create_goal', 'update_goal', 'get_goal'].includes(name))
        return 'plan';
...
function liveReasoningDetail(nodes) {
    for (let nodeIndex = nodes.length - 1; nodeIndex >= 0; nodeIndex--) {
        ...
        if (node?.kind !== 'assistant-step' || node.data.status !== 'running')
            continue;
 ...
 */
export function processActivity(nodes) {
    ...
    if (running === undefined)
        runningDetail = liveReasoningDetail(nodes);
    return {
