// Generated from runtime/alpha3/compat/ui-chat/src/client/chat/TurnNavigator.tsx; edit the TypeScript source.
...
import { defaultRangeExtractor, elementScroll, observeElementOffset, useVirtualizer } from '@tanstack/react-virtual';
...
const TurnMark = memo(function TurnMark({ item, index, active, busy, previewId, registerElement, onNavigate, onPreview, onFocusChange, t, }) {
    ...
    return (_jsx("button", { ref: registerElement, "data-index": index, type: "button", className: classes.join(' '), "aria-label": t(item.anchor.kind === 'loaded' ? 'chat.turnNavigation.jump' : 'chat.turnNavigation.jumpLoad', { turn: item.turn }), "aria-current": active ? 'true' : undefined, "aria-busy": busy ? 'true' : undefined, "aria-describedby": previewId, onPointerMove: () => { onPreview(item.turn); }, onClick: () => { onNavigate(item); }, onFocus: () => { onFocusChange(item.turn); }, onBlur: () => { onFocusChange(null); } }));
});
function TurnNavigatorRail({ items, activeTurn, busyTurn, onNavigate, t }, ref) {
    const [previewTurn, setPreviewTurn] = useState(null);
    const [focusedTurn, setFocusedTurn] = useState(null);
    ...
    const onFocusChange = useCallback((turn) => {
        setFocusedTurn(turn);
        setPreviewTurn(turn);
    }, []);
    const virtualizer = useVirtualizer({
        ...
        getItemKey: useCallback((index) => items[index]?.turn ?? index, [items]),
        estimateSize: () => TURN_SPACING_PX,
        measureElement: () => TURN_SPACING_PX,
        initialRect: { width: 0, height: 0 },
        initialOffset: 0,
        scrollToFn: (offset, options, instance) => {
            if (initialization.current.placed)
    ...
    const scrollToIndex = useCallback((index, reveal, behavior = preferredScrollBehavior()) => {
        const item = virtualizer.measurementsCache[index];
        const height = virtualizer.scrollRect?.height ?? 0;
        if (item === undefined || height <= 0)
            return;
        ...
        if (delta !== 0)
            virtualizer.scrollBy(delta, { behavior });
    }, [virtualizer]);
    ...
    useEffect(() => {
        ...
        if (activeIndex === undefined || pointerInsideRef.current)
            return;
        const previous = initialization.current.follow;
        if (previous?.index === activeIndex && previous.count === items.length && previous.height === viewHeight)
            return;
    ...
    return (_jsx("div", { className: css.slot, children: _jsxs("nav", { className: css.frame, "aria-label": t('chat.turnNavigation.label'), onPointerEnter: () => { pointerInsideRef.current = true; }, onPointerLeave: () => {
                pointerInsideRef.current = false;
                setPreviewTurn(null);
            }, children: [_jsx("div", { ref: scrollerRef, className: fadeClasses.join(' '), children: _jsx("div", { ref: virtualizer.containerRef, className: css.marks, children: virtualItems.map(({ index, key }) => {
                            const item = items[index];
                            if (item === undefined)
                                return null;
                            return (_jsx(TurnMark, { item: item, index: index, active: item.turn === activeTurn, busy: item.turn === busyTurn, previewId: item.turn === previewTurn ? previewId : undefined, registerElement: virtualizer.measureElement, onNavigate: onNavigate, onPreview: setPreviewTurn, onFocusChange: onFocusChange, t: t }, key));
                        }) }) }), preview !== undefined && previewPosition !== undefined && (_jsxs("div", { id: previewId, role: "tooltip", className: css.preview, style: {
                        '--turn-preview-center': `${String(previewPosition.start + previewPosition.size / 2 - scrollTop)}px`,
                    }, children: [_jsx("div", { className: css.previewPrompt, children: preview.prompt || t('chat.turnNavigation.turn', { turn: preview.turn }) }), preview.response !== '' && _jsx("div", { className: css.previewResponse, children: preview.response })] }))] }) }));
}
/**
