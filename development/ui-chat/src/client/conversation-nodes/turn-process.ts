...
import {roleplayPhase, roleplayStory, type RoleplayPhase} from './roleplay-story.ts'
...
interface TurnProcessState {
  readonly roleplayPhase?: RoleplayPhase
...
function latestAnswer(turn: TurnLocation, state: TurnProcessState): Readonly<FinalAssistantChatData> | null {
  if (state.roleplayPhase !== undefined) return roleplayStory(turn, state.roleplayPhase)
...
function updateProcessState(state: TurnProcessState, event: ConversationEvent): TurnProcessState {
  ...
  if (phase?.turn === state.turn && current.roleplayPhase === undefined) current = {...current, roleplayPhase: phase}
