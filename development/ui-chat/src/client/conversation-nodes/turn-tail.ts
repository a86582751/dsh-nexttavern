...
import { phaseFromMatches, roleplayPhase, roleplayStory } from './roleplay-story.ts'
...
function turnLocation(context: ConversationNodeContext<TurnTailState>): TurnLocation | undefined {
  ...
  const location = context.start?.location ?? context.matches.find(match =>
    (match.location.kind === 'turn' || match.location.kind === 'step')
...
export const turnTailDefinition: ConversationNodeDefinition<TurnTailState> = {
  ...
  match: (event) => {
    ...
    if (phase !== undefined) return {id: String(phase.turn), role: 'update'}
