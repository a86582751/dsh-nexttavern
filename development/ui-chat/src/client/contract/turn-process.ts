...
export interface TurnProcessSpec {
  ...
  /** Verified after-story marker; later process rows belong to the same disclosure. */
  readonly maintenanceStartSeq: number | null
...
export function turnProcessMember(node: ChatNode, spec: TurnProcessSpec): boolean {
  if (TURN_PROCESS_INDEPENDENT_KINDS.has(node.kind)
    && !(node.kind === 'system-prompt' && spec.maintenanceStartSeq !== null)) return false
