...
type TurnTailNodeViewProps = ChatNodeViewProps<'turn-tail'>
  & PropsRenderSlots<'conversation.chat.turnTail' | 'conversation.chat.assistant-actions' | 'conversation.chat.roleplay-failure-actions'>
...
export const TurnTailNodeView = memo(function TurnTailNodeView({
  ...
  const hasLaterChatNode = useChat(snapshot =>
    (lastContent(snapshot, data.turn, true)?.anchorSeq ?? -1) > data.seq)
  ...
  const owner: TurnTailOwnerProps = { turn, seq: closing?.presentationSeq ?? data.seq, openFile }
  ...
  if (closing === null) {
    ...
    return tail === null && failed === null ? null
      : <div className={css.root} data-turn-tail={data.turn}>{tail}{failed}</div>
  }
