...
// The shared Context retains the pinned public service contract.
...
/** Older hosts do not expose the exact inherited-prefix fact; their units ignore it. */
function inheritedEventCountOf(session: Session): number {
  return (session as Session & {readonly inheritedEventCount?: number}).inheritedEventCount ?? 0
}

/** Read only after the watermark guard: either host API may materialize a full log. */
function snapshotEventsOf(session: Session): readonly SessionEvent[] {
  const history = session as Session & {
    snapshotEvents?: () => readonly SessionEvent[]
    readonly events?: readonly SessionEvent[]
  }
  if (typeof history.snapshotEvents === 'function') return history.snapshotEvents()
  const events = history.events
  if (events !== undefined) return events
  throw new Error('session projection requires a public Session history reader')
}
...
> {
   ...
   * @param inheritedEventCount - exact inherited prefix, distinct from a resumed constructor seed.
   * @returns the initial state.
   */
  init(header: SessionHeader, inheritedEventCount: number): NoInfer<S>
  /**
...
interface ErasedDefinition {
  ...
  init(header: SessionHeader, inheritedEventCount: number): unknown
...
export class SessionProjectionRegistry extends Service {
    ...
    ctx.on('session/created', (session: Session) => {
      ...
      for (const registration of this.registrations.values()) {
        ...
        registration.cells.set(session, {
          state: registration.def.init(session.header, inheritedEventCountOf(session)),
          observedSeq: -1,
   ...
   * @param inheritedEventCount - inherited prefix; omitted only by the older host API.
  ...
  hydrate(
    ...
    const restored = this.restore(checkpoint, events, baseSeq, session.header, inheritedEventCountOf(session))
    for (const registration of this.registrations.values()) {
  ...
  private cellFor(registration: Registration, session: Session): UnitCell {
    ...
    if (cell === undefined) {
      cell = this.buildCell(registration.def, session.header, snapshotEventsOf(session), inheritedEventCountOf(session))
      registration.cells.set(session, cell)
    } else if (cell.observedSeq < session.seq - 1) {
      // Guard before reading Session history; its immutable snapshot is
      // otherwise copied even when advanceCell immediately returns.
      this.advanceCell(registration.def, cell, snapshotEventsOf(session), session.seq - 1)
    }
    return cell
  }
  ...
  private drive(session: Session, event: SessionEvent): void {
    for (const registration of this.registrations.values()) {
      ...
      if (cell === undefined) {
        ...
        cell = this.buildCell(
          registration.def, session.header, snapshotEventsOf(session).slice(0, event.seq), inheritedEventCountOf(session),
        )
        registration.cells.set(session, cell)
      } else if (cell.observedSeq < event.seq - 1) {
        this.advanceCell(registration.def, cell, snapshotEventsOf(session), event.seq - 1)
      }
