// Generated from runtime/alpha3/compat/session-projection/src/index.ts; edit the TypeScript source.
/**
...
function inheritedEventCountOf(session) {
    return session.inheritedEventCount ?? 0;
}
/** Read only after the watermark guard: either host API may materialize a full log. */
function snapshotEventsOf(session) {
    const history = session;
    if (typeof history.snapshotEvents === 'function')
        return history.snapshotEvents();
...
export class SessionProjectionRegistry extends Service {
     ...
     */
    restore(checkpoint, events, baseSeq, header, inheritedEventCount = 0) {
    ...
    buildCell(def, header, events, inheritedEventCount) {
    ...
    drive(session, event) {
        for (const registration of this.registrations.values()) {
            ...
            if (cell === undefined) {
                ...
                cell = this.buildCell(registration.def, session.header, snapshotEventsOf(session).slice(0, event.seq), inheritedEventCountOf(session));
                registration.cells.set(session, cell);
            }
