...
import {KNOWN_SESSION_EVENT_TYPES, Session, SessionId, SessionLogOffset,
  type SessionEvent, type SessionHeader} from '@deepseek-ai/dsh-session'
import {createSessionFormatCatalog, type SessionFormatArtifact} from '@deepseek-ai/dsh-session-format'
...
import {releasedV3SessionFormatCodec,
  sessionFormatV2ToV3} from '@deepseek-ai/dsh-session-format-v2-to-v3'
import {assertReleasedV4Header, releasedV4SessionFormatCodec, restoreReleasedV4Artifact,
...
export const knownSessionEventTypes: ReadonlySet<string> = new Set([...KNOWN_SESSION_EVENT_TYPES, MESSAGE_EDIT_EVENT])
...
function restore(artifact: SessionFormatArtifact): SessionFormatArtifact {
  ...
  Session.fromRestore(SessionId(restored.header.id), restored.events as unknown as readonly SessionEvent[],
    restored.header as unknown as SessionHeader,
    SessionLogOffset(restored.inheritedEventCount), 'detached', currentSessionMessageProjections)
  return restored
}
...
export const sessionFormatCatalog = createSessionFormatCatalog({
  ...
  restoreCurrentHeader(header) {
    ...
    Session.fromRestore(SessionId(header.id), [], header as unknown as SessionHeader, SessionLogOffset(0), 'detached')
    return header
  },
})
