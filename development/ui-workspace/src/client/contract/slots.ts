...
/** Props supplied by the WorkspaceBrowser owner to extension entries in both sidebar modes. */
export interface WorkspaceSlotProps {
  wide: boolean
}

/** Owner share for the optional whole-browser presentation wrapper. */
export interface WorkspacePresentationProps {
  browserProps: WorkspaceBrowserProps
  renderDefault: (overrides: Partial<WorkspaceBrowserProps>) => ReactNode
}
...
declare module '@deepseek-ai/dsh-client-ui-slots' {
  interface SlotMap {
    ...
    /** Extension seat before the WorkspaceBrowser title; renders in wide sidebar and rail. */
    'sidebar.workspaces.before': { kind: 'list'; scope: 'root'; owner: WorkspaceSlotProps }
    /** Extension action in the WorkspaceBrowser title row before native controls; renders in wide sidebar and rail. */
    'sidebar.workspaces.header.action': { kind: 'list'; scope: 'root'; owner: WorkspaceSlotProps }
    /** Optional whole-browser presentation wrapper retaining the native browser as renderDefault. */
    'sidebar.workspaces.presentation': { kind: 'single'; scope: 'root'; owner: WorkspacePresentationProps }
    /**
...
export type WorkspaceBrowserInjected = {
  hooks: DirectoryPickingInjected['hooks'] & {
    ...
    /** True while the optional whole-browser presentation slot is occupied. */
    presentation: HostObservable<boolean>
...
export type WorkspaceBrowserProps =
  ...
  & PropsRenderSlots<
    'sidebar.workspaces.directoryFlow' | 'sidebar.workspaces.before' | 'sidebar.workspaces.header.action'
    | 'sidebar.workspaces.presentation' | 'sidebar.workspaces.session.menu.item' | 'sidebar.workspaces.session.row.action'
