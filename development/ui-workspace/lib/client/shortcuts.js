// Generated from runtime/alpha3/compat/ui-workspace/src/client/shortcuts.ts; edit the TypeScript source.
 ...
 */
export function createWorkspaceShortcutControls() {
    ...
    return {
        ...
        directoryBusy: (busy) => { state.set({ ...state.getSnapshot(), directoryBusy: busy }); },
 ...
 */
export function installWorkspaceShortcuts(ctx, navigation, controls, archiveSession) {
    ...
    register('session.new', () => t('session.new'), ['new session', 'new chat'], 'KeyN', ['primary'], ['primary', 'alt'], () => ({ status: 'handled', run: () => { navigation.startSession(); } }));
    register('session.search', () => t('search.sessions.aria'), ['search sessions'], 'KeyK', ['primary'], ['primary', 'alt'], () => ({ status: 'handled', run: controls.search }));
    ...
    register('session.fork', () => t('menu.fork'), ['fork session'], 'KeyF', ['primary', 'alt'], ['primary', 'shift'], () => {
        ...
        if (target.blank)
            return { status: 'blocked', reason: t('shortcut.noCompletedTurn') };
        return { status: 'handled', run: () => {
                void navigation.forkSession(target.id).catch((error) => {
                    ...
                    const unavailable = error instanceof Error && error.name === 'SessionForkError'
                        && error.rpcError.code === 'session/fork-unavailable';
