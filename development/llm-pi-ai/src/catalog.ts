...
import { builtinProviders, getBuiltinModels, getBuiltinProviders } from 'dsh-nexttavern-pi-ai/providers/all'
import type { BuiltinProvider } from 'dsh-nexttavern-pi-ai/providers/all'
import type {
...
const CHAT_TEMPLATE_VAR_GATE: Record<PiAiChatTemplateVar, boolean> = {
  'thinking.enabled': true,
  'thinking.effort': true,
  // Keep the existing Harness configuration surface while auditing pi-ai 0.84.4.
  'thinking.budget': false,
}

/** The request-state placeholders a profile may name. */
export const CHAT_TEMPLATE_VARS = (Object.keys(CHAT_TEMPLATE_VAR_GATE) as PiAiChatTemplateVar[])
  .filter(name => CHAT_TEMPLATE_VAR_GATE[name])
