...
export interface CompactionSession extends StorySession {
  header?: unknown
...
export interface MeasuredNode { seq: number; tokens?: unknown; heuristicTokens?: unknown }
export interface TokenMeasurement { nodes?: readonly MeasuredNode[]; totalTokens?: unknown }
export interface CompactionSettings {
  targetContextTokens?: unknown
  hysteresisTokens?: unknown
  archiveTokens?: unknown
  maxSummaryTokens?: unknown
  summaryTimeoutMs?: unknown
  autoTimeoutMs?: unknown
}
export interface CompactionMemory extends NotesRecord {
  summary?: string
  lockedFacts?: readonly (string | { text?: unknown })[]
  archives?: readonly unknown[]
}
export interface CompactionEngine {
  memoryHead?: (sessionId: string) => unknown
...
export interface CheckpointMessage extends Record<string, unknown> {
  id: string
  role: string
...
export interface CompactionOptions<S extends CompactionSession> {
  ...
  getEngine: () => CompactionEngine | null | undefined
  memoryForSession: (session: S) => CompactionMemory | null | undefined
  branchSettings: (session: S) => CompactionSettings | null | undefined
  assertBranchActive: (session: S) => void
  summarizeDetailed: (options: SummaryOptions<S>) => Promise<SummaryResult>
  getTokenMeter: () => {
    measure: (session: S) => TokenMeasurement
    estimateMessage?: (message: CheckpointMessage) => unknown
  }
  flush?: (session: S) => unknown | Promise<unknown>
...
export interface ArchiveRange {
  ...
  routeTokenCount: number
  storyTokenCount: number
  totalTokens: number
}
interface SurfaceNode { seq: number; position: number; event: CompactionEvent | undefined; measurement: MeasuredNode | undefined; story: boolean; context: boolean }
interface Lifecycle extends Record<string, unknown> { compactionId: string; sourceCommandId?: unknown; turn: number | null }
...
export function durableCompactionArchives(session: CompactionSession) {
  const events = eventsOf(session)
...
export function inspectCompactionState(session: CompactionSession) {
  let openTurn: number | null = null
  let openTurnKnown = false
  let unmatchedStart: CompactionEvent | null = null
...
export function createMemoryCompactor<S extends CompactionSession>(options: CompactionOptions<S>) {
  ...
  const activeCompactions = new Map<string, Promise<CompactionResult | null>>()
  function storySurface(session: S, measurement: TokenMeasurement): SurfaceNode[] {
  ...
  function branchTokens(surface: readonly SurfaceNode[]) {
    return surface
  ...
  function pressureTokens(session: S, measurement: TokenMeasurement) {
  ...
  function selectArchiveRange(session: S, { force = false } = {}): ArchiveRange | null {
    let measurement
    try {
    ...
    const lastSurfacePositionByTurn = new Map<number, number>()
  ...
  function assertSelectedSurfaceStable(session: S, range: ArchiveRange) {
  ...
  function reusableSummary(session: S, range: ArchiveRange, mem: CompactionMemory, sourceText: string, locked: readonly unknown[]): CompactionSummary | null {
    ...
    for (const candidate of candidates) {
      if (candidate?.validated !== true || candidate.manual === true || typeof candidate.text !== 'string' || !Array.isArray(candidate.sourceKeys)) continue
  ...
  function compact(session: S, ...args: [agent?: unknown, signal?: AbortSignal, sourceCommandId?: unknown, options?: { force?: boolean; manual?: boolean }]): Promise<CompactionResult | null> {
  ...
  async function performCompaction(session: S, _agent?: unknown, signal?: AbortSignal, sourceCommandId?: unknown, { force = false, manual = false } = {}): Promise<CompactionResult | null> {
    assertBranchActive(session)
    if (busy.has(session.id)) throw new Error('记忆整理正在进行中，请稍后再试')
    signal?.throwIfAborted?.()
    busy.add(session.id)
    let startEvent: { seq: number } | null = null
    ...
    try {
      ...
      const summaryResult: CompactionSummary = reusableSummary(session, range, mem, sourceText, locked) ?? await summarizeDetailed({
      ...
      if (manual) await options.flush?.(session)
      return {
        compactionId,
        ...(sourceCommandId === undefined ? {} : { sourceCommandId }),
  ...
  const busyView: ReadonlySet<string> = busy
  return { compact, pressureTokens, busy: busyView,
    pending: (sessionId: string): Promise<CompactionResult | null> | undefined => activeCompactions.get(sessionId) }
}
