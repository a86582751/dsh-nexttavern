...
export function localTokenBudget(modelId:string,requested:unknown=512):number {
  const model=LOCAL_MODELS.find(m=>m.id===modelId)
  if(!model)throw Error('本地模型不在受信目录')
  if(!Number.isSafeInteger(requested)||Number(requested)<64||Number(requested)>model.maxInputTokens)throw Error(`本地编码预算须为 64–${model.maxInputTokens} tokens（包含前缀与特殊 token）`)
  return Number(requested)
}
export const localInputPrefix=(model:LocalModel,purpose:string)=>purpose==='query'?model.queryPrefix:(model.documentPrefix??(model.id==='e5-small'?'passage: ':''))
export interface LocalTextRange {offset:number;end:number}
/** Original UTF-16 offsets, with scalar boundaries and verified complete encoder inputs. */
export async function localTextRanges(text:string,chars:number,budget:number,count:(text:string)=>Promise<number>):Promise<LocalTextRange[]> {
  if(!Number.isSafeInteger(chars)||chars<128||chars>2048)throw Error('索引块大小无效')
  const boundary=(at:number)=>at>0&&at<text.length&&/[\uD800-\uDBFF]/.test(text[at-1]!)&&/[\uDC00-\uDFFF]/.test(text[at]!)?at-1:at
  const ranges:LocalTextRange[]=[],overlap=Math.floor(chars/6)
  for(let offset=0;offset<text.length;){
    let end=boundary(Math.min(text.length,offset+chars))
    if(await count(text.slice(offset,end))>budget){
      const ends:number[]=[];for(let at=offset;at<end;){at+=text.codePointAt(at)!>0xffff?2:1;ends.push(at)}
      let low=0,high=ends.length-1,best=-1
      while(low<=high){const middle=Math.floor((low+high)/2);if(await count(text.slice(offset,ends[middle]))<=budget){best=middle;low=middle+1}else high=middle-1}
      if(best<0)throw Error('本地编码预算无法容纳单个字符及任务前缀／特殊 token；请提高预算或更换模型')
      end=ends[best]!
    }
    // Never trust token counts inferred from character counts or a previous substring.
    if(end<=offset||await count(text.slice(offset,end))>budget)throw Error('本地 tokenizer 分段校验失败，未截断或标记覆盖')
