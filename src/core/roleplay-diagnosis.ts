...
import type { DiagnosisDependencies, DiagnosisAttempts, DiagnosisCall } from './roleplay-diagnosis-types.js'

export function registerRoleplayDiagnosis({ctx, T, simpleTool, sessionOf, modelPolicy, tavernTasks, telemetry, preparationRecordKey, characterCluster, characterRoster, memorySettingsPolicy,enhancements}: DiagnosisDependencies) {
  ctx.effect(()=>ctx.tools.register(simpleTool(
    'rp_diagnose',
    '按需查看当前酒馆会话的只读诊断概览：模型路由、四层记忆/Embedding配置与可用进度、预设、长文本研究摘要、任务及近期调用。冷索引状态可能不可见；不启动建库或测试模型。用于玩法/故障问答，不用于普通剧情，不重试任务或修改状态。',
    {type:'object',properties:{},additionalProperties:false},
    async(_args,exec)=>{
      const session=await sessionOf(exec)
      const policy=await modelPolicy.read(session,exec.agent)
      const route=(r: {provider?: unknown; model?: unknown; reasoningEffort?: unknown} | null | undefined)=>r?{provider:r.provider??null,model:r.model??null,reasoningEffort:r.reasoningEffort??null}:null
      ...
      const errorCode=(e: unknown)=>{
        const value=typeof e==='string'?e:(e as {code?: unknown; message?: unknown})?.code??(e as {message?: unknown})?.message??''
      ...
      const byId=new Map<string, DiagnosisCall>()
      for(const call of telemetry.calls())if(call.ownerSessionId===session.id&&call.startedAt<turnStart)byId.set(call.id,call)
      const historyCalls=[...byId.values()]
      const calls=historyCalls.sort((a,b)=>b.startedAt-a.startedAt).slice(0,12)
      const stamp=(t: unknown)=>Number.isFinite(t)?new Date(t as number).toISOString():null
      ...
      const local=(t: unknown)=>Number.isFinite(t)?formatter.format(t as number):null
      const visibleJobs=jobs.sort((a,b)=>Number(b.createdAt)-Number(a.createdAt)).slice(0,12)
      const attemptsByJob=new Map<string, DiagnosisAttempts>(visibleJobs.map(job=>[job.id,{count:0,failed:0,started:Infinity,completed:null,models:new Set<string>()}]))
      for(const call of historyCalls){
        ...
        for(const id of ids){
          ...
          attempts.count++
          if(call.status==='failed')attempts.failed++
