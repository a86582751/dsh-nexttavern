...
import type { PanelRouteBody, SettingArguments, SettingToolDependencies } from './roleplay-panel-routes-types.js';
...
type Repair = {
    ...
    status: 'queued' | 'running' | 'completed' | 'failed' | 'needs-input';
    request: SettingArguments;
    createdAt: number;
    jobId?: string;
    evidenceReads?: number[];
    reportThroughTurn?: number;
    error?: string;
    result?: unknown;
};
...
export function registerSettingTool(deps: SettingToolDependencies) {
    ...
    const repairs = (sid: string) => [...T.branch.entries()].flatMap(([k,
         v]) => k.startsWith(`${sid}__setting-repair-`)
        && v.schemaVersion === 1
        && v.branchId === sid ? [v as unknown as Repair] : []);
    ...
    const active = (s: HostSession, signal?: AbortSignal, stamp?: string) => {
    ...
    async function apply(s: HostSession, a: SettingArguments, signal?: AbortSignal, repairId?: string, stamp = deps.selectionStamp(s) ?? undefined) {
        ...
        s.id,
            ...
            async () => {
                ...
                const saved = await savePanelSetting(
                deps,
                    s,
                    {
                        ...body, ...(repairId ? {
    ...
    function launch(s: HostSession, agent: HostAgent, record: Repair, retry = false) {
        ...
        void (async () => {
            try {
                    ...
                    validate: async (value) => {
                        ...
                        if (['patch', 'append'].includes(String(v.outcome))) {
                            ...
                            s,
                                {
                                    ...a,
                                    action: v.outcome as 'patch' | 'append',
                                    changes: v.changes as SettingArguments['changes'],
                ...
                active(s, undefined, record.selectionStamp);
                const receipt = ['patch', 'append'].includes(String(result.outcome)) ? await apply(
                s,
                    {
                        ...a,
                        action: result.outcome as 'patch' | 'append',
                        changes: result.changes as SettingArguments['changes'],
                        text: result.text as string | undefined,
                        reason: String(result.reason)
                    },
                    undefined,
                    record.id,
                    record.selectionStamp
                ) : {
    ...
    ctx.effect(
    () => ctx.tools.register(simpleTool<SettingArguments, {
        agent?: HostAgent;
        signal?: AbortSignal;
    }, Promise<unknown>>(
    'rp_setting',
        ...
        async (a, exec) => {
            ...
            if (a.action === 'list') {
                const rows = settingTargets.flatMap<{
                    target: string;
                    id: unknown;
                    name: unknown;
                }>(t => t === 'character'
                    || t === 'worldbook' ? [...T[t === 'character' ? 'cards' : 'worldbook'].entries()].filter(([k]) => k.startsWith(`${s.id}__`)).map(([,
                     v]) => ({
            ...
            if (a.action === 'repair') {
                ...
                return withTavernLock(
                T.branch,
                    `setting-repair-admission:${s.id}`,
                    async () => {
    ...
    ctx.effect(
    () => ctx.tools.register(simpleTool<Record<string, unknown>, {
        agent?: HostAgent;
    }, Promise<unknown>>(
    'rp_setting_evidence',
