...
export const safeHeading = (value: unknown): string =>
...
export const sourceLines = (text: string): string[] => text.match(/[^\n]*\n|[^\n]+$/g) ?? ['']
...
export const stableJson = (value: unknown): string => JSON.stringify(value,
    ...
    && !Array.isArray(nested) ? Object.fromEntries(Object.entries(nested as Record<string,
     unknown>).sort(([a],
     [b]) => a.localeCompare(b,
     'en'))) : nested)
...
export function renderOrganizedExport(snapshot: ExportSnapshot, title: unknown, sections: ExportSection[]): string {
  ...
  const sources = new Map(snapshot.units.map(unit => [unit.id,
       ...
       lineCache = new Map<string,
       string[]>(),
       output = [`# ${safeHeading(title)
      || '角色扮演设定'}\n`]
  let references = 0
  for (const section of sections) {
    if (!safeHeading(section.heading)
        ...
        && !Array.isArray(section.source_parts))) throw new Error('章节缺少标题或来源')
    const requested = [...(Array.isArray(section.source_ids) ? section.source_ids : []).map(source_id => ({ source_id })),
         ...(Array.isArray(section.source_parts) ? section.source_parts : [])] as { source_id?: unknown;
         start_line?: unknown;
         end_line?: unknown }[]
    ...
    for (const ref of requested) {
      ...
      if (!Number.isSafeInteger(start)
          ...
          || (end as number) > lines.length) throw new Error('导出来源行号跨度无效')
      const spans = used.get(id)
          ?? [];
           if (spans.some(span => (start as number) <= span.end
          && (end as number) >= span.start)) throw new Error('导出章节来源重复或不存在');
           spans.push({ start: start as number,
           end: end as number });
           used.set(id,
           spans)
  ...
  for (const [id,
       spans] of used) { let next = 1;
       for (const span of spans.sort((a,
       b) => a.start - b.start)) { if (span.start !== next) throw new Error('导出来源有漏行，必须覆盖全部原文');
       next = span.end + 1 } if (next !== sourceLines(sources.get(id)!.text).length + 1) throw new Error('导出来源有漏行，必须覆盖全部原文') }
