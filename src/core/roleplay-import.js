// Generated from runtime/alpha3/core/roleplay-import.ts; edit the TypeScript source.
...
export function importActiveKey(sessionId) { return keyOf(sessionId, 'import-active'); }
...
export function registerRoleplayImports(deps) {
        ...
        collect: async (session) => {
            ...
            for (const [tableName, table, id] of [['rules', T.rules, 'spec'], ['status', T.status, 'spec'], ['opening', T.opening, 'scene']]) {
                ...
                for (const field of (tableName === 'rules' ? RULE_TEXT_FIELDS : ['text'])) {
                    if (record[field])
                        material.push({ label: `${tableName}: ${field}`, text: String(record[field]), source });
                    delete metadata[field];
                }
                if (metadata.beauty) {
                    for (const field of ['css', 'js'])
                        if (metadata.beauty[field])
                            material.push({ label: `正文美化 ${field}`, text: '```' + field + '\n' + metadata.beauty[field] + '\n```', source });
            ...
            for (const ref of T.rules.get(keyOf(session.id, 'spec'))?.sources?.archiveOnly ?? []) {
                ...
                if (text.trim())
                    material.push({ label: `附加资料（原文保留）: ${ref.name ?? ref.importId}`, text, source: ref });
            }
            for (const original of structuredOriginals.values()) {
                ...
                for (const field of ['name', 'description', 'personality', 'scenario', 'first_mes', 'mes_example', 'system_prompt', 'post_history_instructions', 'character_book'])
                    delete extras[field];
                if (decoded.data.character_book) {
                    const { entries, ...bookSettings } = decoded.data.character_book;
    ...
    const assertImportRecordIntegrity = (record) => {
        ...
        if (structured) {
            ...
            if (envelope?.schemaVersion !== 1 || typeof envelope.base64 !== 'string' || envelope.base64.length > 27_000_000)
    ...
    const parseRegexRules = (sourceText) => {
        ...
        const list = Array.isArray(parsed) ? parsed : Array.isArray(wrapped?.regexRules) ? wrapped.regexRules : [parsed];
    ...
    ctx.effect(() => ctx.tools.register(simpleTool('rp_card_import_begin', '开始无损读卡导入。直接读取 ST/TauriTavern PNG/JSON，或 Markdown/TXT；保留原件、字段来源与哈希。PNG/JSON 不经过 anydoc。返回 import_id、行数和审阅入口。', {
        type: 'object',
        properties: {
        ...
        if (!requestedPath)
            return { ok: false, error: 'source_file 不能为空' };
        ...
        if (source.sourceBytes > IMPORT_LIMITS.maxBytes)
            return { ok: false, error: `角色卡超过 ${IMPORT_LIMITS.maxBytes.toLocaleString()} 字节，拒绝静默截断；请先拆成多个来源文件` };
        ...
        if (!decoded && !Buffer.from(rawSource, 'utf8').equals(rawBytes))
            return { ok: false, error: '角色卡不是有效 UTF-8；拒绝以替换字符损坏原文' };
        if (rawSource.length > IMPORT_LIMITS.maxChars)
            return { ok: false, error: `角色卡超过 ${IMPORT_LIMITS.maxChars.toLocaleString()} 字符，拒绝静默截断；请先拆成多个来源文件` };
        const normalizedSource = projected?.text ?? deescapeMarkdown(rawSource).replace(/\r\n?/g, '\n');
        if (normalizedSource.length > IMPORT_LIMITS.maxChars)
            return { ok: false, error: '角色卡投影超过字符上限；拒绝静默截断' };
        if (!normalizedSource.trim())
            return { ok: false, error: '角色卡规范化后没有任何有效内容，拒绝创建空导入' };
        ...
        if (lineCount < 1 || lineCount > IMPORT_LIMITS.maxLines)
            return { ok: false, error: `角色卡行数超过上限 ${IMPORT_LIMITS.maxLines.toLocaleString()}` };
    ...
    ctx.effect(() => ctx.tools.register(simpleTool('rp_card_import_chunk', '按行分页读取已归档角色卡原文。首次审阅必须严格按 nextCursor 从 1 连续读到 null；完成后可按任意合法 cursor 复查。', {
        type: 'object',
        properties: {
        ...
        return withImportLock(session.id, args.import_id, async () => {
            ...
            if (!record)
                return { ok: false, error: 'import_id 不存在' };
            try {
                assertImportRecordIntegrity(record);
            }
            catch (error) {
    ...
    ctx.effect(() => ctx.tools.register(simpleTool('rp_card_import_stage', '提交角色卡分类元数据与原文行跨度。必须先读完全文；同时通过 resource_title 给资源库起一个体现整本书主题、方便辨认的名称，不使用 UUID 或乱码原文件名；这不是修改角色名字。禁止传 content/正则正文，后端只从 sourceSpans 原样物化。可分批追加，纠错时 replace_all=true。\n' + CARD_CLASSIFICATION_GUIDE, {
        type: 'object',
        properties: {
            import_id: { type: 'string' },
        ...
        return withImportLock(session.id, args.import_id, async () => {
            ...
            if (!record || record.status !== 'staging')
                return { ok: false, error: 'import_id 不存在或已结束 staging' };
            try {
                assertImportRecordIntegrity(record);
            }
            catch (error) {
            ...
            try {
                ...
                for (const raw of args.assignments ?? []) {
                    ...
                    if (!sourceSpans.length)
                        return { ok: false, error: `${target} 缺少 sourceSpans` };
                    const secondary = raw.secondary === true;
                    const reuseReason = String(raw.reuse_reason ?? '').trim();
                    if (secondary && target === 'archive-only')
                        return { ok: false, error: 'archive-only 不能标记为 secondary' };
                    if (secondary && !reuseReason)
                        return { ok: false, error: `${target} 的 secondary 复用必须填写 reuse_reason` };
                    let id;
    ...
    ctx.effect(() => ctx.tools.register(simpleTool('rp_card_import_finalize', '校验全文已审阅、原文字符 100% 已分类后以回滚日志激活；遗漏、主归属重叠、跨度哈希变化或写后校验失败都会拒绝提交。', {
        type: 'object',
        properties: {
            import_id: { type: 'string' },
        ...
        return withImportLock(session.id, args.import_id, async () => {
            ...
            if (record.status !== 'staging')
                return { ok: false, error: `import 当前状态为 ${String(record.status)}，不能激活` };
            if (![3, 4].includes(record.schemaVersion))
                return { ok: false, error: '旧版 staging 记录缺少全文审阅证明；请重新执行 rp_card_import_begin' };
            try {
                assertAssignmentBudget(record.assignments ?? []);
                assertReferenceBudget(record, record.assignments ?? []);
            ...
            const tavernWorldbookById = new Map();
            ...
            const joinImported = (parts) => parts
                ...
                if (typeof part === 'string')
                    return [{ startLine: partIndex, endLine: partIndex, text: part }];
