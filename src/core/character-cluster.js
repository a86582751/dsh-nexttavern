// Generated from runtime/alpha3/core/character-cluster.ts; edit the TypeScript source.
...
function assertSettings(value) {
    ...
    if (typeof data.revision !== 'number' || !Number.isSafeInteger(data.revision) || data.revision < 0 ||
        typeof data.enabled !== 'boolean' || !record(data.characters))
...
export function createCharacterCluster({ table, subagents, rootOf = s => s.id, main, isCurrent = () => true, timeoutMs = 45000 }) {
    ...
    const readGlobal = () => {
        ...
        if (value === undefined || value === null)
            return { schemaVersion: 1, revision: 0, enabled: false, defaultRoute: null, characters: {} };
    ...
    async function saveGlobal(value, expectedRevision) {
        ...
        return withTavernLock(table, globalKey, async () => {
            ...
            const next = { schemaVersion: 1, revision: previous.revision + 1, enabled: false, defaultRoute: route, characters: {}, updatedAt: Date.now() };
    ...
    async function run({ session, agent, turn, characters, context, signal }) {
        ...
        const result = await Promise.all(characters.map(async (character) => {
            ...
            if (old?.status === 'completed')
                return { characterId: character.id, name: character.name, status: 'completed', suggestion: old.result, taskId: id };
            const work = (async () => {
                ...
                for (let attempt = 0; attempt < routes.length; attempt++) {
                    ...
                    let child;
                    try {
                        job = { ...job, status: 'running', childSessionId: null, actualRoute: attemptRoute, startedAt: job.startedAt ?? Date.now(), attemptStartedAt: Date.now(), updatedAt: Date.now() };
                        await table.put(key, job);
                        ...
                        if (!current() || signal?.aborted)
                            break;
                        job = { ...job, childSessionId: child.id, childSessionIds: [...(job.childSessionIds ?? []), child.id] };
                        await table.put(key, job);
                        ...
                        if (!text)
                            throw Object.assign(new Error('角色返回空结果'), { code: 'EMPTY_TASK_RESULT' });
                        if (!current() || signal?.aborted)
                            break;
                        ...
                        if (record(table.get(key))?.generation !== generation)
                            break;
                        ...
                        if (attempt + 1 < routes.length && !signal?.aborted && current())
                            job.fallback = { from: attemptRoute, to: primary, reason: failure.category, failure, at: Date.now() };
                        await table.put(key, job);
                    }
                    finally {
                        clearTimeout(timer);
                ...
                if (record(table.get(key))?.generation === generation)
                    await table.put(key, { ...job, status: signal?.aborted ? 'cancelled' : current() ? 'failed' : 'stale', updatedAt: Date.now() });
        ...
        if (signal?.aborted)
            throw signal.reason;
        if (!isCurrent(session, { source: { turn } }))
