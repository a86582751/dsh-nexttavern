...
import type { SearchHit, SearchCandidate, ResearchPacketRow } from './card-research-packets.js';
import {
    expandSourceHit,
    packetizeResearchHits,
    splitDenseResearchPackets,
    safeDeferredEvidence,
    splitDeferredEvidence,
} from './card-research-packets.js';
...
interface Input {
    ...
    entries?: unknown[];
    protagonist?: string;
    opening_point?: string;
    candidate_limit?: number;
    max_hits?: number;
    context_chars?: number;
    hits_per_query?: number;
    round_budget?: number;
...
interface Dependencies extends DraftDependencies {
    ...
    retrieval: Pick<ReturnType<typeof createMemoryRetrieval>, 'adaptation' | 'adaptationModels' | 'adaptationCurrent' | 'rebuildConfirmation' | 'mutate'>;
    ...
    askReadingMode?(exec: TaskToolExecution, target: {
        protagonist: string;
        openingPoint: string;
    }): Promise<{
        mode: ReadingMode;
...
function boundedRows<T>(rows: T[], limit = 20000): T[] {
    const kept: T[] = [];
...
export function registerAdaptationTools({
    ctx,
    table,
    sessionOf,
    retrieval,
    active,
    scopeOf = async (s) => s.id,
    selectionStamp = s => s.id,
    askReadingMode,
}: Dependencies) {
    ...
    async function chooseReadingMode(s: Session, owner: string, id: string, previousIds: readonly string[], exec: TaskToolExecution, args: Input) {
        ...
        if (answer.mode === 'coarse') {
            ...
            if (!provider?.ready) {
                ...
                (exec as TaskToolExecution & {
                    concludeTurn?: () => unknown;
                }).concludeTurn?.();
                return {
                    ok: false as const,
                    code: 'ADAPTATION_COARSE_MODEL_REQUIRED' as const,
    ...
    const complete = (p: Progress, count: number) => count > 0
        && p.sources === count
        && p.covered === count
        && p.stale !== true
        && ['pending', 'running', 'failed', 'unknown']
            .every(key => p[key as keyof Progress] === 0);
    ...
    function watchIndex(s: Session, owner: string, id: string) {
        const key = `${owner}:${id}`;
        ...
        const timer = setTimeout(() => {
            ...
            void (async () => {
                ...
                const status = await index(s, id, 'status', vectors(source), '', owner) as {
                    progress: Progress;
                };
    ...
    async function autoPrepare<T extends {
        sourceId: string;
    }>(s: Session, owner: string, result: T, previousIds: readonly string[]) {
    ...
    async function index(
        s: Session,
        id: string,
        action: Parameters<typeof retrieval.adaptation>[2],
        rows: VectorSource[] = [],
        query = '',
        owner = s.id,
        fingerprint?: string,
    ...
    async function enqueue(s: Session, owner: string, id: string, retry = false, expectedModelRevision?: number, confirmed = false) {
        ...
        if (!confirmed && models.providers.some(p => p.id === models.activeProviderId && p.kind === 'online')) {
            const status = await index(s, id, 'status', [], '', owner, undefined, expectedModelRevision) as {
                progress: Progress;
            };
            if (status.progress.stale)
                return {
                    queued: false as const, confirmation: await offerRepair(s, owner, id, '在线模型配置已变化，补齐原文前需要确认重建')
                };
        }
        ...
        const status = await index(s, id, 'status', rows, '', owner) as {
            progress: unknown;
        };
        watchIndex(s, owner, id);
        return {
            queued: true as const, progress: status.progress
        };
    }
    async function offerRepair(s: Session, owner: string, id: string, reason: string) {
        ...
        const status = await index(s, id, 'status', [], '', owner) as {
            progress: Progress;
        }, fingerprint = status.progress.fingerprint;
        if (!fingerprint)
    ...
    async function awaitIndexReady(s: Session, owner: string, id: string, signal?: AbortSignal) {
        ...
        while (Date.now() < deadline) {
            ...
            try {
                progress = (await awaitAbortable(index(s, id, 'status', vectors(source), '', owner), signal) as {
                    progress: Progress;
                }).progress;
            }
            catch (error) {
                if ((error as {
                    ...
                    throw error;
                const offer = await offerRepair(s, owner, id, (error as Error).message);
    ...
    async function view(s: Session, selectedId?: string, cursor = 0, query = '', noteMode = 'close-reading') {
        ...
        try {
            ...
            progress = {
                ...status.progress, model: (status.progress.embedding as {
                    embedding_model?: string;
                } | undefined)?.embedding_model
            };
        }
        catch (error) {
    ...
    async function mutate(s: Session, body: Record<string, unknown>) {
        ...
        if (action === 'auto-index-new-sources' || action === 'auto-rebuild-local-on-active') {
            await store.configureIndex(
                body.enabled as boolean,
                Number(body.expectedAutoIndexSettingsRevision),
                action === 'auto-index-new-sources'
                    ? 'autoIndexNewSources'
        ...
        if (action === 'delete-original') {
            await store.deleteOriginal(
                String(body.assetId),
                Number(body.expectedLibraryRevision),
                source => retrieval.adaptation(s, source.id, 'clear', [], '', owner, undefined, {
    ...
    type SourceReader = (
        args: Input,
        session: Awaited<ReturnType<typeof sessionOf>>,
        owner: string,
        exec: TaskToolExecution,
    ) => Promise<unknown>;
    ...
    function register(
        ...
        ctx.effect(() => {
            const tool = simpleTool<Input, TaskToolExecution, Promise<unknown>>(name, description, {
    ...
    register('rp_source_index', '管理独立小说索引：index 一次提交全部原文，与顺序 read/note 并行；笔记齐全后用 wait 由程序等待完整索引，可取消且不需要模型循环 status。就绪后用 semantic/hybrid 核查疑问并回读证据，再 finish 写卡。成功向量复用；stale 的 rebuild 需当前指纹，在线重建须玩家确认，不能因重新生成重建正常索引。', {
        ...
        if (action === 'status') {
            const {
                source,
            } = store.load(owner, id), status = await index(s, id, 'status', vectors(source), '', owner) as {
                progress: unknown;
            };
            return {
                sourceId: id, result: {
    ...
    register('rp_source_search', '按关键词/语义/混合查小说并返回定位片段；语义未就绪或失败明确回退关键词。命中不计为完整阅读，关键事实用 rp_source_read 复读。', {
        ...
        try {
            const model = await retrieval.adaptationModels(s), before = await index(s, source.id, 'status', vectors(source), '', owner) as {
                progress: Progress;
            };
            const hits = await index(s, source.id, 'query', vectors(source), a.query, owner) as {
                id: string;
                offset: number;
                score: number;
            }[];
            ...
            const hybrid = mode === 'hybrid' ? [
                ...selectedHits,
                ...lexical
                    .filter(row => !selectedHits.some(hit =>
                        hit.segment === row.segment
                        && hit.start === row.start
                        && hit.end === row.end))
    ...
    register('rp_source_research', '粗颗粒度研究：status 显示程序去重的关系探索前沿、定向抽样和缺项；一次 query 是一轮，首轮一个种子、以后默认三个前沿关键词，每词最多交付十个约千字原文包，程序合并重叠并跳过已交付范围。它先等待已选小说模型的完整索引；read 只作局部回读；append 原子追加 1–8 条带实际原文引用的事实，save 保留单条/unknown 编辑兼容。query/read 返回 researchDelivery；成功的 verified append/save 可用 source_packet_ids 确认已整理的完整包，返回 researchCheckpoint，供程序安全回收已整理的原文上下文。只保存原著信息，当前卡改编选择留在问卷/草稿。web_search/web_fetch 仅提供低信任线索，不能替代原著引用。', {
        ...
        if (a.action === 'character-map') {
            ...
            const mapping = characterOccurrenceMap(source, terms, {
                ...
                evidence: research.entries(owner, id)
                    .filter(fact =>
                        fact.certainty === 'verified-original'
        ...
        if (a.action === 'save') {
            ...
            const researchCheckpoint = (saved as {
                researchCheckpoint?: unknown;
            } | undefined)?.researchCheckpoint;
            return {
        ...
        if (
            ...
            && (
                ...
                || a.queries.length > 3
                || a.queries.some(q =>
                    typeof q !== 'string' || !q.trim() || q.length > 1000)
            )
        )
        ...
        const researchMode = requestedMode as 'keyword' | 'semantic' | 'hybrid';
        const models = await retrieval.adaptationModels(s);
            ...
            try {
                ...
                for (const packet of delivery.packetIds)
                    if (typeof packet === 'string') {
                        ...
                        if (typeof result.text === 'string' || result.packets?.some((p: {
                            packetId?: string;
                            text?: string;
                        }) => p.packetId === packet && typeof p.text === 'string'))
        ...
        for (const packet of contextIds)
            if (deliveredIds.has(packet))
                visibleIds.add(packet);
        const previous = currentPlan.receipts.filter(receipt => currentPlan.packets.some(packet =>
            packet.segment === receipt.segment
        ...
        const frontier = await research.prepareQueries(
            owner,
            id,
            queryFingerprint,
            a.queries ?? [],
        ...
        const effectiveContextChars = canDeferEvidence
            ...
            : rawHitCount
                ? Math.max(256, Math.min(
                    contextChars,
                    Math.floor(deliveryTextBudget / rawHitCount),
                ))
                : contextChars;
        ...
        const packetId = (packet: ResearchPacketRow) => {
            const packetTextHash = createHash('sha256').update(packet.text).digest('hex');
            return createHash('sha256')
                .update(
                    `${owner}\u0000${source.id}\u0000${source.textSha256}\u0000${packet.segment}\u0000${packet.start}\u0000${packet.end}\u0000${packetTextHash}`,
                )
                .digest('hex');
        };
        const makeResult = () => {
            ...
            return {
                ...
                researchDelivery: {
                    schemaVersion: 1 as const, owner, sourceId: id, textSha256: source.textSha256, generation, packetIds: packets.map(packet => packet.packetId)
                },
                deliveryEnvelope: {
    ...
    async function beforeWrite(exec: TaskToolExecution) {
        ...
        if (models.activeProviderId) {
            const progress = (await index(s, id, 'status', vectors(source), '', owner) as {
                progress: Progress;
            }).progress;
            unchanged();
            ...
            if (
                !complete(progress, source.segments.length)
