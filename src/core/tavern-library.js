// Generated from runtime/alpha3/core/tavern-library.ts; edit the TypeScript source.
...
async function lockArchive(key, fn) { const prior = archiveLocks.get(key) ?? Promise.resolve(); const next = prior.catch(() => { }).then(fn); archiveLocks.set(key, next); try {
    return await next;
}
finally {
...
export function createTavernLibrary({ workspace, table }) {
    if (typeof workspace !== 'string' || !isAbsolute(workspace) || !table || typeof table.get !== 'function' || typeof table.entries !== 'function' || typeof table.put !== 'function')
        fail('资源库初始化参数无效');
    ...
    const assertDirectory = (path) => {
        if (lstatSync(path).isSymbolicLink() || !statSync(path).isDirectory() || realpathSync(path) !== path || !within(root, path))
    ...
    const pending = () => {
        ...
        for (const record of records()) {
            try {
                ...
                result.push({ schemaVersion: 1, id: `resource-${record.id}`, resourceId: record.id, state: 'pending', name: record.name, type: record.type, source: record.source, reason: String(error.message) });
            }
        }
        result.sort((left, right) => left.id.localeCompare(right.id, 'en'));
        return result;
    ...
    const verify = (record) => {
        ...
        if (node.isSymbolicLink() || !node.isFile() || node.size !== record.bytes || node.size < 1 || node.size > TAVERN_LIBRARY_LIMITS.bytes || realpathSync(path) !== path || !within(objectDir, realpathSync(path)))
            fail('资源文件缺失或不安全');
    ...
    const writeObject = (path, bytes, hash) => {
        ...
        try {
            ...
            if (existing.isSymbolicLink() || !existing.isFile() || existing.size !== bytes.length || realpathSync(path) !== path || !within(objectDir, realpathSync(path)) || digest(readFileSync(path)) !== hash)
                fail('同名资源对象冲突或已损坏');
            return;
        }
        catch (error) {
    ...
    function openDownload(id) {
        ...
        try {
            ...
            if (!opened.isFile() || opened.size !== record.bytes || opened.dev !== node.dev || opened.ino !== node.ino || digest(readFileSync(fd)) !== record.fullSha256)
                fail('资源文件在下载前改变');
