...
import type { StatusDependencies, StatusSession, StatusField, StatusPanel, StatusRecord, StatusSource, StatusEvent, StatusJob, StatusTable, FixedStatusContext, StatusInputBasis, PriorStatusSource, StoryEvidence, StatusRunOptions } from './roleplay-status-types.js'
import type { TaskAgent } from './tavern-task-types.js'
import type { ImportRecord, ImportPointer } from './roleplay-import-types.js'

export function createRoleplayStatus(deps: StatusDependencies) {
  ...
  const textAlias = (object: unknown, keys: readonly string[]) => {
    const source = (object && typeof object === 'object' ? object : {}) as Record<string, unknown>
  ...
  const normalizeStatusField = (field: unknown) => {
  ...
  const normalizeStatusOption = (option: unknown) => {
  ...
  const escapeStatusHtml = (value: unknown) => String(value ?? '')
    .replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
    .replace(/\"/g, '&quot;')
  const fallbackStatusHtml = (title: string, fields: readonly StatusField[], rawText: string) => {
  ...
  const normalizeStatusPanel = (panel: unknown): StatusPanel => {
    const source = (panel && typeof panel === 'object' ? panel : {}) as Record<string, unknown>
  ...
  const authoredStatusTemplate = (spec: StatusRecord | null | undefined) => {
  ...
  const statusPanelForSpec = (value: unknown, spec: StatusRecord | null | undefined, {strict=false}={}) => {
    const raw = value as Record<string, unknown> | null
    const reject=(path: string,rule: string,expected: unknown,actual: unknown)=>{
    ...
    if (template) {
      ...
      const attributes = (source: string) => [...source.matchAll(/\b(style|class|id)\s*=\s*(?:"([^"]*)"|'([^']*)')/gi)]
        .map(([,name,double,single])=>({name:name!.toLowerCase(),value:double??single!}))
      const actual=attributes(body)
      const canonical=(value: string)=>value.replace(/⟦([^⟦⟧]+)⟧/g,'{{$1}}')
      const matches=(expected: {name: string; value: string},candidate: {name: string; value: string})=>{
    ...
    return { ...normalized, ...(template ? { templateHtml: spec?.templateHtml || spec?.text || template } : {}) }
  }
  ...
  const statusJobs = new Map<string, StatusJob>()
  const statusLocks = new Map<string, Promise<unknown>>()
  const statusRetryTimers = new Map<string, ReturnType<typeof setTimeout>>()
  const statusRetryCounts = new Map<string, number>()
  const statusRunStartSeq = new Map<string, number>()
  const statusRecoveredSessions = new Set<string>()
  ...
  const withStatusLock = async <T,>(branchId: string, fn: () => Promise<T>): Promise<T> => {
  ...
  const statusSource = (session: StatusSession, event: StatusEvent | null | undefined): StatusSource | null => {
    ...
    const boundary=opening.prose.has(event.seq)?opening.boundaries.findLast(item=>item.turn===Number(event.data!.turn)&&item.storyPhaseSeq<event.seq):undefined
    ...
    const user = boundary?{seq:boundary.storyPhaseSeq,text:''}:surfaceEntries(session).filter(entry => entry.kind === 'user' && entry.seq > start.seq && entry.seq < event.seq).at(-1)
    if (!user) return null
    return {
  ...
  const statusSourceLive = (session: StatusSession, event: StatusEvent | null | undefined, source: StatusSource | undefined) =>
  ...
  const latestStatusEvent = (session: StatusSession) => {
  ...
  const selectedStatusGeneration = (session: StatusSession) => {
  ...
  const statusAuthorRecord = (record: Record<string, unknown> | null) => record && Object.fromEntries(Object.entries(record).filter(([key]) =>
  ...
  const statusAuthorContext = (fixed: FixedStatusContext) => ({
  ...
  const statusStoryContext = (session: StatusSession, event: StatusEvent, specHash: string, fixedContextHash: string, inputBasis: StatusInputBasis | null | undefined) => {
    const compaction = ctx.get('compaction') as { storyEvidence?(session: StatusSession): StoryEvidence[] } | undefined
    const history = (compaction?.storyEvidence?.(session) ?? surfaceEntries(session))
      .map((entry: StoryEvidence) => ({ seq: entry.seq, role: entry.role ?? entry.kind, text: entry.text }))
    ...
    let prior: StatusPanel | null = null, priorSource: PriorStatusSource | null = null, priorIndex = -1
    ...
    return { selectedHistoryHash, previousStatus: prior, previousStatusSource: priorSource,
      selectedStorySinceStatus: selected.slice(priorIndex + 1) }
  }

  function runStatusObligation(session: StatusSession, event: StatusEvent | null | undefined, via = 'turn-end', { force = false, agent, signal }: StatusRunOptions = {}): Promise<StatusRecord | null | undefined> & { admission?: Promise<unknown> } {
    const source = statusSource(session, event)
    if (!event || !source || statusDisposed) return Promise.resolve(null)
    ...
    const job = Promise.resolve().then(async () => {
      ...
      let spec: StatusRecord | undefined
    ...
    }).finally(() => { cancellation.dispose(); admitted(null); if (statusJobs.get(taskKey) === job) statusJobs.delete(taskKey) }) as StatusJob
  ...
  function recoverStatusObligations(session: StatusSession, via: string, agent?: TaskAgent) {
    ...
    if (session.header?.parentSession && Number(latest?.seq) < Number(session.header.seedLength)) return
  ...
  const recoverStatusSpecFromImport = async (session: StatusSession) => {
    ...
    if(edited?.editedFrom)return edited
    const active = T.branch.get(importActiveKey(session.id)) as ImportPointer | undefined
    const sourceSessionId = active?.sourceRecordSessionId ?? session.id
    const record = (active?.importId ? T.branch.get(importRecordKey(sourceSessionId, active.importId)) : null) as ImportRecord | null
  ...
  const normalizeStatusRecord = (record: StatusRecord | undefined) => {
    if (!record || typeof record !== 'object') return null
    return { ...record, panel: normalizeStatusPanel(record.panel ?? record) }
  }
  const normalizeDecisionRecord = (value: unknown): (Record<string, unknown> & {options: ReturnType<typeof normalizeStatusOption>[]}) | null => {
  ...
  return { statusFixedContext, runStatusObligation, textAlias, normalizeDecisionRecord, normalizeStatusOption, selectedStatusRecord, statusSource, normalizeStatusRecord, queueStatusObligation, statusRunStartSeq, recoverStatusObligations, statusRecoveredSessions, selectedStatusGeneration, latestStatusEvent }
}
