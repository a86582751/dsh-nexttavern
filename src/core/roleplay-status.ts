...
textAlias,
    ...
    authoredStatusTemplate,
    statusPanelForSpec,
    normalizeStatusRecord,
    normalizeDecisionRecord,
} from './roleplay-status-presentation.js';
...
export function createRoleplayStatus(deps: StatusDependencies) {
    ...
    const statusSourceLive = (session: StatusSession,
         event: StatusEvent | null | undefined,
    ...
    const statusAuthorRecord = (record: Record<string, unknown> | null) => record
    ...
    const statusStoryContext = (session: StatusSession,
         event: StatusEvent,
         specHash: string,
         fixedContextHash: string,
         inputBasis: StatusInputBasis | null | undefined) => {
        const compaction = ctx.get('compaction') as {
            storyEvidence?(session: StatusSession): StoryEvidence[];
        } | undefined;
        const history = (compaction?.storyEvidence?.(session) ?? surfaceEntries(session))
            .map((entry: StoryEvidence) => ({
    ...
    function runStatusObligation(session: StatusSession,
         event: StatusEvent | null | undefined,
         via = 'turn-end',
         { force = false,
         agent,
         signal }: StatusRunOptions = {}): Promise<StatusRecord | null | undefined> & {
        ...
        const job = Promise.resolve().then(async () => {
            ...
            try {
                ...
                if (record?.state !== 'completed' || force || record.source?.sourceHash !== source.sourceHash
                    ...
                    const selection = force ? await modelPolicy.resolve(session,
                         'status',
                         agent) : record?.selection
                        ?? await modelPolicy.resolve(session,
                         'status',
                         agent);
                    record = {
                        schemaVersion: 1,
                    ...
                    let panel = force
                        || !sameModelRoute(toolInput?.provenance?.actualRoute,
                         selection.actualRoute) ? null : statusPanelForSpec(toolInput?.panel,
                         spec);
                    ...
                    if (!panel) {
                        ...
                        const queriedWorldbook = await retrieveWorldbook(
                        T,
                            session.id,
                            `${selectedUserAction}\n${textOf(event.data!.message?.content)}`,
                            null,
                            Number(cfg.maxWorldTokens) || 6000
                        );
                        const context = {
                        ...
                        ctx,
                            resolveRoute(session, agent),
                            {
                                ...
                                context,
                                    `状态栏设定：\n${fenceCardContent(
                                    template && spec?.text?.trim() === template.trim() ? '设定与作者模板相同，完整内容见下方模板。' : spec?.text ?? '根据本轮正文展示已确认的位置、时间与人物状态。',
                                        'status'
                                    )}\n${template ? `\n作者原始状态栏模板（完整保留 HTML 结构、class/id、内联样式和 style/script 块；只更新动态状态值）：\n${fenceCardContent(template,
