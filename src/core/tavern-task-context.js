// Generated from runtime/alpha3/core/tavern-task-context.ts; edit the TypeScript source.
...
export function maintenancePrompt(context, before, after, domain = 'status') {
    const promptContext = { schemaVersion: 1, before, context, after, domain };
...
function checkedPrompt(job) {
    ...
    if (!value || value.schemaVersion !== 1 || typeof value.before !== 'string' || typeof value.after !== 'string' || typeof value.domain !== 'string')
        return null;
    try {
...
export function maintenanceTaskInputs(jobs, resident = []) {
    ...
    for (const prompt of prompts)
        if (prompt)
            mapPromptStrings(prompt.context, text => { if (text.length >= 256)
                counts.set(text, (counts.get(text) ?? 0) + 1); return text; });
    const references = new Map(), sharedContext = [];
    for (const [text, count] of counts) {
        const present = resident.find(part => part.text.includes(text));
        if (present)
            references.set(text, { currentContextRef: present.name });
        else if (count > 1) {
            const id = `source-${sharedContext.length + 1}`;
            references.set(text, { sharedContextRef: id });
            sharedContext.push({ id, text: fenceCardContent(text, 'source', { stable: true }) });
        }
    }
    const inputs = jobs.map((job, index) => {
        const prompt = prompts[index];
        return prompt ? { ...job.input, user: renderMaintenancePrompt(prompt, mapPromptStrings(prompt.context, text => references.get(text) ?? text)) } : job.input;
    });
...
export function taskProjectionEvents(session) {
    ...
    if (immutable)
        eventProjectionCache.set(session, { length: raw.length, first: raw[0], last: raw.at(-1), events });
    return events;
}
...
export async function awaitTaskAdmissions(jobs) {
    ...
    return results.map(r => { if (r.status === 'rejected')
        throw r.reason; return r.value; });
}
...
function inlineTaskEnvelope(text) {
    ...
    if (!Array.isArray(shared) || shared.some(item => { const row = object(item); return !row || typeof row.id !== 'string' || typeof row.text !== 'string' || Object.keys(row).some(key => key !== 'id' && key !== 'text'); }))
...
export function inlineTaskInstruction(jobs, { sessionId, maxChars = 128000, resident = [] } = {}) {
    ...
    for (const job of jobs) {
        if (job.execution !== 'inline' || !['queued', 'running'].includes(job.status) || sessionId && job.sessionId !== sessionId)
            continue;
...
export function taskStorySeqs(session) {
    ...
    for (const event of events) {
        ...
        if (source?.kind === 'plugin' && source.plugin === 'roleplay-tasks' && source.stage === 'after-story' && typeof source.storySeq === 'number' && Number.isSafeInteger(source.storySeq))
    ...
    for (let i = begin; i < nodes.length; i++) {
        ...
        if (e?.type === 'assistant/message') {
            ...
            if (calls.get(id) === 'rp_card_import_finalize') {
                ...
                let proof;
                try {
                    proof = object(JSON.parse((result.content ?? []).filter(b => b.type === 'text').map(b => b.text).join('\n')));
                }
                catch {
                    return null;
                }
                ...
                if (m.isError || result.isError || proof?.ok !== true || !(typeof proof.activatedAt === 'number' && proof.activatedAt > 0) || !Number.isFinite(proof.activatedAt)
                    || !/^[a-f0-9]{64}$/.test(String(proof.normalizedSha256 ?? '')) || proof.coverage !== 1 || !proof.importId || calls.size !== results.size)
                    return null;
            ...
            || e.data.source.form !== 'phase' || ['story', 'after-story'].includes(e.data.source.stage ?? ''))
            return null;
    }
    return null;
}
...
export function retireCompletedTaskContexts(session, currentTurn, activeImport) {
    ...
    const events = session.events ?? session.log ?? [], groups = [], bySeq = new Map(), lookup = new Map();
    let group = null;
    for (const e of events) {
        ...
        if (e.type === 'turn/start') {
            group = { turn: e.data?.turn, export: false, story: false, completed: false };
            groups.push(group);
        }
        if (group)
        ...
        if (group && e.type === 'tool/call' && /^(?:rp_card_export_(?:begin|chunk|finalize)|rp_novel_export)$/.test(e.data?.name ?? ''))
            group.export = true;
    ...
    const visible = [...(session.surface?.nodes ?? [])], nodesByGroup = new Map(), positions = new Map();
    ...
    for (const candidate of groups) {
        ...
        if (currentImport && (!imported || imported.importId !== activeImport.importId || imported.normalizedSha256 !== activeImport.normalizedSha256))
            continue;
        ...
        session.append('user/message', { id: randomUUID(), role: 'user', source: receipt,
            content: [{ type: 'text', text: imported ? `读卡已完整激活。设定由当前角色扮演栏目提供，原始读卡消息与工具结果保存在历史 seq ${start}–${end}。${currentImport ? '作者原始开场（原样展示，不提前续写）：\n' + activeImport.opening : '以下继续剧情。'}` : `导出操作已结束。原始消息与工具结果保存在历史 seq ${start}–${end}，导出文件可在资源库查看。以下继续剧情。` }] }, { surfaceOp: { op: 'replace', start, end }, sourceEventSeqs: selected });
...
export function retireSettledInlineContexts(session, currentTurn, jobs) {
    if (!session.append)
        return 0;
    const events = session.events ?? session.log ?? [], lookup = new Map(events.map(e => [e.seq, e])), owners = new Map(), ended = new Set();
    let turn;
    for (const e of events) {
        if (e.type === 'turn/start')
            turn = e.data?.turn;
    ...
    const internal = internalTaskSeqs(session), visible = [...(session.surface?.nodes ?? [])], groups = [];
    let group = [];
    ...
    for (const seq of visible) {
        ...
        if (owner === undefined || owner >= currentTurn || !ended.has(owner) || group.length && owners.get(group[0]) !== owner) {
            flush();
            if (owner === undefined || owner >= currentTurn || !ended.has(owner))
                continue;
        }
        if (e.type === 'user/message') {
            if (source?.kind === 'plugin' && source.plugin === 'roleplay-tasks' && source.form === 'phase' && ['after-story', 'prepare', 'management'].includes(source.stage ?? '')) {
                flush();
                group.push(seq);
                continue;
            }
    ...
    for (const selected of groups) {
        const attempts = new Map(), calls = new Set(), results = new Set();
        let valid = true;
        for (const seq of selected) {
            ...
            if (e.type === 'user/message') {
                ...
                if (source?.storySeq !== undefined) {
                    ...
                    if (!story || !Number.isSafeInteger(source.storySeq) || source.turn !== owners.get(seq) || story.type !== 'assistant/message' || owners.get(story.seq) !== owners.get(seq) || internal.has(story.seq) || story.seq >= seq) {
                        valid = false;
                        break;
                    }
                }
                ...
                for (const raw of tasks) {
                    const task = object(raw), job = typeof task?.id === 'string' ? jobById.get(task.id) : undefined;
                    if (!task || !job || job.sessionId !== session.id || job.execution !== 'inline' || !['status', 'decision', 'memory'].includes(job.kind) || job.kind !== task.kind || typeof task.generation !== 'string' || !task.generation || !job.generation || !job.sourceHash || job.sourceHash !== task.sourceHash) {
                        valid = false;
                        break;
                    }
                    ...
                    const failure = job.status === 'failed' ? (taskValidationFailure(job) ? '任务结果校验失败' : typeof category === 'string' && Object.hasOwn(FAILURE_LABELS, category) ? FAILURE_LABELS[category] : taskFailureDetails(job.error).label) : job.status === 'stale' ? '来源已变化，旧结果不可使用' : undefined;
                ...
                for (const b of e.data?.message?.content ?? [])
                    if (b.type === 'tool-call') {
                        if (typeof b.id !== 'string' || calls.has(b.id) || !['rp_task_read', 'rp_task_submit', 'run_code'].includes(b.name ?? '')) {
                            valid = false;
                            break;
                        }
                        calls.add(b.id);
        ...
        if (visible.some(seq => { const e = lookup.get(seq); return !selectedSet.has(seq) && e?.type === 'tool/result' && calls.has(e.data?.message?.source?.callId ?? ''); }))
            continue;
        ...
        session.append('user/message', { id: randomUUID(), role: 'user', source: { kind: 'plugin', plugin: 'roleplay-tasks', form: 'maintenance-receipt', schemaVersion: 1, sessionId: session.id, sourceTurn, attempts: details, sourceSha256: taskHash(selected.map(seq => lookup.get(seq))) }, content: [{ type: 'text', text }] }, { surfaceOp: { op: 'replace', start: selected[0], end: selected.at(-1) }, sourceEventSeqs: selected });
        retired += selected.length;
    }
    return retired;
}
...
export function retireUsedStoryReads(session, currentTurn) {
    if (!session.append)
        return 0;
    const events = session.events ?? session.log ?? [], lookup = new Map(events.map(e => [e.seq, e])), nodes = [...(session.surface?.nodes ?? [])], owners = new Map(), completed = new Set(), bodies = new Map();
    let turn;
    for (const e of events) {
        if (e.type === 'turn/start')
            turn = e.data?.turn;
    ...
    for (let i = 0; i < nodes.length; i++) {
        ...
        if (e.type !== 'assistant/message' || owner === undefined || owner >= currentTurn || !completed.has(owner) || body === undefined || body <= e.seq || internal.has(body))
            continue;
        ...
        for (; j < nodes.length && pending.size; j++) {
            ...
            if (result.type !== 'tool/result' || owners.get(result.seq) !== owner || result.seq >= body || !id || !pending.has(id))
                break;
            pending.delete(id);
        ...
        session.append('user/message', { id: randomUUID(), role: 'user', source: { kind: 'plugin', plugin: 'roleplay-tasks', form: 'read-evidence-receipt', schemaVersion: 1, sessionId: session.id, sourceTurn: owner, bodySeq: body, tools, sourceSha256: taskHash(selected.map(seq => lookup.get(seq))) },
            content: [{ type: 'text', text: `先前 ${tools.join(' / ')} 读取已完成本轮使用；原始调用与结果保留于历史 seq ${start}–${end}。需要旧事实或世界知识时重新按当前世界线查阅，读取记录本身不是已发生剧情；失败结果不作事实依据。` }] }, { surfaceOp: { op: 'replace', start, end }, sourceEventSeqs: selected });
        retired += selected.length;
        i = j - 1;
    }
    return retired;
...
export function internalTaskSeqs(session) {
    ...
    for (const e of events) {
        ...
        if (e?.type === 'tool/call' && ['rp_card_import_begin', 'rp_commit_card'].includes(e.data?.name ?? ''))
            authoring = false;
        if (e?.type === 'user/message' && e.data?.source?.plugin === 'roleplay-tasks' && e.data.source.stage === 'after-story')
            authoring = false;
        if (authoring && e?.type === 'tool/call' && e.data?.name === 'ask_user_question')
            managementTurns.add(e.data?.turn ?? turn);
        if (e?.type === 'tool/call' && (/^(?:rp_card_export_(?:begin|chunk|finalize)|rp_novel_export|rp_diagnose|rp_preset|rp_card_draft_check)$/.test(e.data?.name ?? '') || isSettingManagementCall(e.data)))
            managementTurns.add(e.data?.turn ?? turn);
        if (e?.type === 'user/message' && e.data?.source?.plugin === 'roleplay-tasks' && ['card-export', 'novel-export'].includes(e.data?.source?.jobKind ?? ''))
            managementTurns.add(turn);
        if (e?.type === 'turn/start' || e?.type === 'turn/end')
            internal = false;
        if (e?.type === 'user/message' && e.data?.source?.kind === 'plugin' && e.data?.source?.plugin === 'roleplay-tasks' && e.data?.source?.form === 'phase')
    ...
    for (const e of events)
        if (!resumed.has(e.seq) && managementTurns.has(e.data?.turn ?? turns.get(e.seq)) && (e.type === 'assistant/message' || (e.type === 'user/message' && e.data?.source?.kind === 'user')))
            hidden.add(e.seq);
...
export function tavernTaskToolBoundary(table, agent) {
    ...
    return job ? new Set((Array.isArray(job.allowedTools) ? job.allowedTools : []).filter((name) => typeof name === 'string' && !['subagent', 'spawn', 'fork', 'send_message', 'list_agents', 'interrupt_agent'].includes(name))) : null;
}
