...
import type { LoopSession, LoopPreparation, LoopDependencies } from './roleplay-loop-types.js'
import type { DecisionJob } from './roleplay-decision-types.js'
...
export function createFirstResponseWatchdog(timeoutMs=90000) {
  ...
  const clear=(id:string)=>{const item=pending.get(id);if(!item)return;clearTimeout(item.timer);item.signal.removeEventListener('abort',item.abort);pending.delete(id)}
  return {
    begin(agent:{session:{id:string};cancel(cause:{kind:'hook';reason:string},options:{keepInbox:true}):void},turn:number,step:number,signal:AbortSignal){
      const id=agent.session.id;clear(id);if(signal.aborted)return
      const abort=()=>clear(id)
      const item:Pending={turn,step,signal,abort,timer:setTimeout(()=>{
        if(pending.get(id)!==item||signal.aborted)return
        clear(id)
    ...
    observe(session:{id:string},event:{type:string;data?:{turn?:unknown;step?:unknown;chunk?:{type:string}}}){
      const item=pending.get(session.id);if(!item||Number(event.data?.turn)!==item.turn)return
      if(event.type==='turn/end'||(Number(event.data?.step)===item.step&&(event.type==='step/end'||event.type==='assistant/message'||event.type==='assistant/chunk'&&['text-delta','reasoning-delta','tool-call-delta','finish'].includes(event.data?.chunk?.type??''))))clear(session.id)
    },
    dispose(){for(const id of pending.keys())clear(id)},
  }
}
...
export function registerRoleplayLoop({ctx, T, tavernTasks, clusterJob, isRoleplaySession, characterCluster, activeCardWorkflow, taskAgents, ensureState, clusterPhase, withDecisionMutationLock, normalizeDecisionRecord, resumeStatusMaintenance, resumeMemoryWork, resumeCardWorkflows, resumeNovelExports, withImportLock, buildPhaseA, characterRoster, storyWindowSettings, runStatusObligation, publishTurnDecision, runPhaseBC,adaptationScope,importPromptCheckpoint,authorContext}: LoopDependencies) {
  ...
  const residentContext=(session:HostSession)=>{
    const cached=authorPrompts.get(session),turn=currentTurn(session)
    if(turn===undefined||!cached||cached.turn!==turn)return []
  ...
  ctx.on('agent/pre-step', async (payload, next) => {
    ...
    if(hasUser && payload.step===1) {
      ...
      }else retireDeliveredDraft(session,payload.turn)
  ...
  ctx.on('agent/turn-stopping',async({agent,turn,signal})=>{
    ...
    const mark=(stage:string)=>{const now=performance.now(),sample={stage,wallAt:Date.now(),elapsedMs:Math.round(now-timingStart),deltaMs:Math.round(now-timingPrevious)};timings.push(sample);ctx.logger?.info?.('roleplay: maintenance-timing '+JSON.stringify({sessionId:session.id,turn,...sample}));timingPrevious=now}
    mark('enter')
    try {
    ...
    if(preparation?.status!=='preparing') {
      ...
      if(canonical) {
        ...
        try {
          ...
          admitted=await (statusWork.admission??statusWork) as Partial<StoredTask> | null | undefined
    ...
    const prior=T.branch.get(attemptKey),attempt=prior?.fingerprint===fingerprint?(prior.attempt as number)+1:1
    ...
    mark(pending.length||preparation?.status==='preparing'?'maintenance-steered':'finished')
      ...
      try {await T.branch.put(key,{schemaVersion:1,sessionId:session.id,turn,updatedAt:Date.now(),samples:[...samples,...timings].slice(-64)})}
      catch {ctx.logger?.warn?.('roleplay: maintenance timing persistence failed')}
    }
  })
