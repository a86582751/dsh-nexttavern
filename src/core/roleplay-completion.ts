...
export function createRoleplayCompletion(deps: CompletionDependencies) {
      ...
      try {
        ...
        if (!backgroundMemory) await T.memory.update(memoryKey, (current) => {
          ...
          const priorConfirmations = Array.isArray(current?.pendingConfirmations)
            ...
            : []
          const hasOrigin = (item: Record<string, unknown>) => item && item.sessionId === branchId
            && Number(item.atSeq) === Number(event.seq)
            && Number(item.turnId) === Number(snapshot.turnId)
          const merge = (prior: Record<string, unknown>[], incoming: Record<string, unknown>[], limit: number) => {
  ...
  function completeManagementFork(session: ContextSession, st: CompletionState, event: ContextEvent) {
    const turn=Number(event.data?.turn)
    if(st.phaseBStarted.has(turn))return
    ...
    Promise.resolve().then(()=>reconcileNativeFork(session,event)).then(()=>{
      ...
      st.snapshots.delete(turn);st.pendingScenes.delete(turn)
      if(st.snapshot===snapshot)st.snapshot=null
      if(st.pendingTurn===turn){st.pendingTurn=null;st.pendingScene=null}
      st.regenerateAnchor=null
    }).catch(error=>ctx.logger?.warn?.(`roleplay: management fork completion failed: ${String(error)}`))
      .finally(()=>st.phaseBStarted.delete(turn))
  }

  function queueCompletedTurn(session: ContextSession, event: ContextEvent) {
    ...
    if (!canonical) {
      ...
      if(receipt){completeManagementFork(session,st,receipt);return}
      st.snapshots.delete(turn)
      st.pendingScenes.delete(turn)
