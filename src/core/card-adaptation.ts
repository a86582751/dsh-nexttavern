...
export interface AdaptationTable {get(key: string): unknown; put(key: string, value: unknown): unknown | PromiseLike<unknown>;delete?(key:string):unknown|PromiseLike<unknown>}
...
export function createAdaptationStore(table: AdaptationTable) {
  async function select(owner:string,id:string){load(owner,id);const current=table.get(selectedKey(owner)) as {schemaVersion?:number;sourceId?:string}|undefined;if(current?.schemaVersion===1&&current.sourceId===id)return;await table.put(selectedKey(owner),{schemaVersion:1,owner,sourceId:id})}
  ...
  async function configureIndex(enabled:boolean,expectedRevision:number,field:'autoIndexNewSources'|'autoRebuildLocalOnActive'='autoIndexNewSources'){return locked(indexSettingsKey,async()=>{
  ...
  function libraryRecord():OriginalCatalog {
    ...
    if(value.schemaVersion!==1||!Number.isSafeInteger(value.revision)||!Array.isArray(value.assets)||value.assets.some(a=>!/^[a-f0-9]{64}$/.test(a.assetId)||!Array.isArray(a.owners)||a.owners.some(o=>typeof o!=='string')))throw Error('原著资料库目录损坏')
  ...
  function original(assetId:string) {
    ...
    if(!metadata||!asset||asset.schemaVersion!==1||asset.kind!=='original-asset'||asset.assetId!==assetId||asset.source.id!==metadata.sourceId||asset.source.textSha256!==hash(asset.source.text)||asset.source.rawSha256!==metadata.rawSha256||asset.source.textSha256!==metadata.textSha256)throw Error('共享原著不存在或完整性校验失败')
    return {metadata,source:asset.source}
  }
  function library(owner:string){const data=libraryRecord();return {revision:data.revision,assets:data.assets.map(({owners,...a})=>({...a,references:owners.length,attached:owners.includes(owner)&&catalog(owner).includes(a.sourceId)}))}}
  function notebook(source:AdaptationSource):OriginalNotebook|null {
    ...
    if(value.schemaVersion!==1||value.assetId!==source.assetId||value.textSha256!==source.textSha256||!Number.isSafeInteger(value.revision)||!Array.isArray(value.notes)||new Set(value.notes.map(n=>n.segment)).size!==value.notes.length||value.notes.some(n=>!source.segments[n.segment]||typeof n.sessionId!=='string'||!source.text.slice(source.segments[n.segment]!.start,source.segments[n.segment]!.end).includes(n.evidence)))throw Error('共享研究笔记证据校验失败')
    return value
  }
  async function shareNotebook(owner:string,source:AdaptationSource,state:AdaptationState){
    ...
    await locked(notebookKey(source.assetId),async()=>{
      const before=notebook(source),next:OriginalNotebook=before?structuredClone(before):{schemaVersion:1,assetId:source.assetId!,textSha256:source.textSha256,revision:0,status:'reading',notes:[]}
      ...
      if(next.notes.length!==(before?.notes.length??0)||(finished&&next.status!=='finished')){if(finished)next.status='finished';next.revision++;await table.put(notebookKey(source.assetId!),next)}
    })
  }
  function shared(owner:string,id:string){const binding=table.get(bindingKey(owner,id)) as OriginalBinding|undefined;if(!binding)return undefined
    ...
    if(binding.schemaVersion!==1||binding.kind!=='original-binding'||binding.owner!==owner||binding.sourceId!==id||!asset?.owners.includes(owner))throw Error('当前对话的原著引用已解除或无效')
    return {assetId:asset.assetId,references:asset.owners.length,revision:data.revision,indexEnabled:asset.owners.some(o=>(table.get(stateKey(o,asset.sourceId)) as AdaptationState|undefined)?.indexPolicy==='running')}
  }
  async function bind(owner:string,metadata:OriginalMetadata,data:OriginalCatalog){
    ...
    if(!table.get(stateKey(owner,id)))await table.put(stateKey(owner,id),{schemaVersion:1,kind:'adaptation-state',owner,sourceId:id,revision:0,status:'reading',reads:[],notes:[],checkpointSeq:-1} satisfies AdaptationState)
  ...
  async function publishValue(owner:string,source:AdaptationSource,data:OriginalCatalog){
    ...
    if(!metadata){
      ...
      metadata={assetId,sourceId:source.id,name:source.name,bytes:source.bytes,characters:source.text.length,rawSha256:source.rawSha256,textSha256:source.textSha256,owners:[]};data.assets.push(metadata)
    }
  ...
  async function detach(owner:string,id:string,expectedRevision:number,expectedLibraryRevision:number,reconcile?:(remaining:{assetId:string;indexEnabled:boolean})=>Promise<unknown>){return locked(libraryKey,async()=>{
    ...
    if(!binding||data.revision!==expectedLibraryRevision||!state||state.owner!==owner||state.sourceId!==id||state.revision!==expectedRevision+(retry?1:0)||(retry&&state.indexPolicy!=='paused'))throw Error('原著引用已变化，请刷新后重试')
    ...
    const remaining={assetId:binding.assetId,sourceId:id,references:metadata.owners.length,indexEnabled:metadata.owners.some(o=>(table.get(stateKey(o,id)) as AdaptationState|undefined)?.indexPolicy==='running')}
  ...
  function load(owner: string, id: string) {
    ...
    const source = reference?{...original(reference.assetId).source,owner,assetId:reference.assetId}:table.get(sourceKey(owner,id)) as AdaptationSource | undefined
    ...
    if (!source || !value || source.schemaVersion !== 1 || value.schemaVersion !== 1 || source.owner !== owner || value.owner !== owner || source.id !== id || value.sourceId !== id || source.textSha256 !== hash(source.text)) throw Error('写卡资料不可用或来源完整性检查失败；不能读取其他会话资料')
    if(!Array.isArray(source.segments)||!source.segments.length||source.segments.some((p,i)=>p.id!==i||p.start!==(i?source.segments[i-1]!.end:0)||!Number.isSafeInteger(p.end)||p.end<=p.start||p.end-p.start>24000)||source.segments.at(-1)!.end!==source.text.length)throw Error('写卡原文分段记录损坏')
    if(!['reading','finished'].includes(value.status)||!Number.isSafeInteger(value.revision)||!Number.isSafeInteger(value.checkpointSeq)||!Array.isArray(value.reads)||!Array.isArray(value.notes)||new Set(value.reads).size!==value.reads.length||new Set(value.notes.map(n=>n.segment)).size!==value.notes.length||value.reads.some(i=>!Number.isSafeInteger(i)||!source.segments[i])||value.notes.some(n=>!value.reads.includes(n.segment)||typeof n.evidence!=='string'||!source.text.slice(source.segments[n.segment]!.start,source.segments[n.segment]!.end).includes(n.evidence)))throw Error('写卡进度或笔记记录损坏')
    if(value.indexPolicy!==undefined&&!['idle','running','paused','completed'].includes(value.indexPolicy))throw Error('小说索引策略记录损坏')
    if(value.semanticChecks!==undefined&&(!Array.isArray(value.semanticChecks)||value.semanticChecks.length>12||value.semanticChecks.some(c=>c.schemaVersion!==1||!/^[a-f0-9]{64}$/.test(c.queryHash)||!/^[a-f0-9]{64}$/.test(c.fingerprint)||!Number.isSafeInteger(c.seq))))throw Error('小说语义核查记录损坏')
  ...
  async function begin(owner: string, cwd: string, path: string, encoding?: string) {
    ...
    return locked(libraryKey,async()=>{
      ...
      if (!ids.includes(id) && ids.length >= 12) throw Error('当前会话最多保留 12 份改编资料')
      const source: AdaptationSource = ids.includes(id)?load(owner,id).source:{schemaVersion:1,kind:'adaptation-source',owner,id,name:path.split(/[\\/]/).at(-1)!,rawSha256,textSha256:hash(decoded.text),encoding:decoded.encoding,bytes:file.sourceBytes,text:decoded.text,segments:splitAdaptation(decoded.text)}
  ...
  async function note(owner: string, id: string, input: Omit<AdaptationNote,'seq'>, seq: number, expectedRevision?:number) {
    return locked(stateKey(owner,id),async()=>{
      ...
      persistedState.notes = persistedState.notes.filter(n => n.segment !== input.segment).concat({...input,seq});persistedState.revision++;persistedState.checkpointSeq=seq
  ...
  async function manage(owner:string,id:string,action:'delete-note'|'clear-notes'|'index-policy',expectedRevision:number,segment?:number,indexPolicy?:AdaptationState['indexPolicy']){
    return locked(stateKey(owner,id),async()=>{
      const {state,persistedState}=load(owner,id)
      if(expectedRevision!==state.revision)throw Error('研究资料已更新，请刷新后重试')
      if(action==='delete-note'){if(!state.notes.some(n=>n.segment===segment))throw Error('阅读笔记不存在');persistedState.notes=persistedState.notes.filter(n=>n.segment!==segment);persistedState.hiddenNotes=[...new Set([...(persistedState.hiddenNotes??[]),segment!])];persistedState.status='reading'}
      else if(action==='clear-notes'){persistedState.notes=[];persistedState.status='reading';persistedState.inheritNotebook=false;persistedState.reads=state.reads}
...
export function keywordAdaptation(source: AdaptationSource, query: string, limit = 8) {
  ...
  return source.segments.map(segment=>{
    ...
    for(const term of terms){let i=lower.indexOf(term),count=0;while(i>=0&&count<100){if(offset<0)offset=i;count++;i=lower.indexOf(term,i+Math.max(1,term.length))}score+=count}
