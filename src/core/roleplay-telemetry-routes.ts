...
import type { CatalogEntry } from './tavern-pricing.js'
...
export function registerTelemetryRoutes({ctx, resolveRoleplaySession, exchangeRates, priceCatalog, telemetry}: TelemetryRoutesDependencies) {
  ...
  ctx.effect(()=>ctx.connection.fetch.register({path:'/api/roleplay/price-catalog',methods:['GET','POST'],fetch:async request=>{
    try{const url=new URL(request.url),body=request.method==='POST'?await request.json() as TelemetryRouteBody:null,session=await resolveRoleplaySession(body?.sessionId??url.searchParams.get('sessionId'))
      ...
      const requestedKeys=new Set<string>(requested),selected=new Map<string, CatalogEntry>(),matching: CatalogEntry[]=[]
      for(const entry of priceCatalog.state().entries){
        if(requestedKeys.has(entry.key))selected.set(entry.key,entry)
        if(matching.length<100&&`${entry.key} ${entry.name}`.toLowerCase().includes(search))matching.push(entry)
      }
      for(const entry of matching)selected.set(entry.key,entry)
  ...
  for(const path of ['/api/roleplay/usage','/api/roleplay/usage-requests','/api/roleplay/logs','/api/roleplay/prices'])ctx.effect(()=>ctx.connection.fetch.register({path,methods:path.endsWith('/prices')?['GET','POST']:['GET'],fetch:async request=>{
    try {
      ...
      if(path.endsWith('/usage')){
        if(telemetry.prices().autoSync)priceCatalog.refresh()
        let stats: ReturnType<typeof aggregateUsage> | ReturnType<typeof convertUsageCurrency>=aggregateUsage(telemetry.calls().filter(r=>r.ownerSessionId!==null||(r.kind==='embedding'&&!!r.source.workspaceId)),filter,telemetry.prices(),priceCatalog.state().entries)
