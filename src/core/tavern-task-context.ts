...
export function maintenancePrompt(context: unknown, before: string, after: string, domain = 'status') {
    const promptContext: MaintenancePromptContext = {
...
function checkedPrompt(job: {
    ...
    try {
        return (job.input as {
            user?: unknown;
        } | undefined)?.user === renderMaintenancePrompt(value) ? value : null;
    }
    catch {
        return null;
    }
...
export function maintenanceTaskInputs(jobs: readonly {
    input?: unknown;
    promptContext?: unknown;
}[], resident: readonly {
    name: string;
    ...
    const references = new Map<string, unknown>(), sharedContext: {
        id: string;
        text: string;
    }[] = [];
...
const eventProjectionCache = new WeakMap<object, {
    length: number;
    first: TaskEvent | undefined;
    last: TaskEvent | undefined;
...
const internalProjectionCache = new WeakMap<object, {
    events: readonly TaskEvent[];
    surfaceKey: string;
    hidden: ReadonlySet<number>;
}>();
const storyProjectionCache = new WeakMap<object, {
    events: readonly TaskEvent[];
    story: ReadonlySet<number>;
}>();
...
export function inlineTaskInstruction(jobs: readonly InlineContextJob[], { sessionId, maxChars = 128000, resident = [] }: {
    sessionId?: string;
    maxChars?: number;
    resident?: readonly {
        name: string;
...
export function inlineTaskMessages(session: TaskContextSession, stage: string, jobs: readonly InlineContextJob[], { force = false, maxChars, resident }: {
    force?: boolean;
    maxChars?: number;
    resident?: readonly {
        name: string;
...
export function internalTaskSeqs(session: TaskContextSession) {
    ...
    const hidden = new Set<number>(), managementTurns = new Set<number | null | undefined>(), turns = new Map<number, number | null | undefined>();
    let internal = false, turn: number | null | undefined = null, authoring = false;
...
export function tavernTaskToolBoundary(table: {
    get(key: string): unknown;
}, agent: {
    session?: TaskContextSession;
    options?: {
    ...
    return job ? new Set((Array.isArray(job.allowedTools) ? job.allowedTools : []).filter((name: unknown): name is string => typeof name === 'string'
