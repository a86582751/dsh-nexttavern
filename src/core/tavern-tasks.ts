...
import { taskHash, isInlinePending, decodeTaskRecord, untilAborted } from './tavern-task-primitives.js'
...
function batchEntries(raw: unknown, field: 'value' | 'failure'): [string, unknown][] {
  if(raw==null)return []
  if(!Array.isArray(raw))throw new Error('批量任务结果列表格式无效')
  return raw.map(item=>{
    if(item==null)throw new Error('批量任务结果项无效')
    const value=Object(item) as Record<string,unknown>
    return [`${value.taskId}:${value.generation}`,value[field]]
  })
}
function taskResultText(result: TaskResult,format?: string) {
...
export function createTavernTasks<S extends TaskSession, A extends TaskAgent>({table,policy,subagents,isCurrent=()=>true}: TaskOptions<S,A>) {
  ...
  function ownedRecord(session: S,id: string) {
    const job=copy(storedTask(table.get(keyOf(id))))
    if (!job || job.id!==id || job.sessionId!==session.id) throw new Error('任务不属于当前所属会话')
    return job
  }
  function owned(session: S,id: string): TaskJob {
    const job=executableTask(ownedRecord(session,id))
    if(!job)throw new Error('任务记录缺少执行数据，保留原记录以便恢复')
    return job
  }
  const priority=(kind: string)=>kind==='status'?0:kind==='decision'?1:2
  const groupKey=(session: S,job: TaskJob,spec: TaskSpec<S,A>)=>taskHash({sessionId:session.id,branchId:job.branchId,route:job.actualRoute,
    lane:['status','decision'].includes(job.kind)?'story-ui':job.kind,
    tools:[...(spec.tools??[])].sort(),background:false})
  const holdKey=(session: S)=>session.id
  function schedule(batch: Batch<S,A>) {
    if(!batch.running&&!batch.items.size) {
      if(batches.get(batch.key)===batch)batches.delete(batch.key)
      return
    }
    if(batch.scheduled||batch.running||(holds.get(holdKey(batch.session))??0)>0)return
    batch.scheduled=true
    queueMicrotask(()=>{batch.scheduled=false;if((holds.get(holdKey(batch.session))??0)>0||batch.running)return
      const ready=[...batch.items.values()].filter(item=>item.ready)
      if(ready.length)void runBatch(batch,ready)
    })
  }
  async function inlineFallback(session: S,item: Admission<S,A>,raw: unknown) {
  ...
  function resultValue(item: Admission<S,A>,result: TaskResult): unknown {
  ...
  function batchPrompt(active: Admission<S,A>[]) {
    if(active.length===1) {
      const {job}=active[0]!
    ...
    for(const input of prepared.inputs)for(const value of Object.values(input as Record<string,unknown>??{}))if(typeof value==='string'&&value.length>=1024)counts.set(value,(counts.get(value)??0)+1)
    ...
    const tasks=active.map((item,index)=>{
      const input=Object.fromEntries(Object.entries(prepared.inputs[index] as Record<string,unknown>??{}).map(([key,value])=>[key,typeof value==='string'&&refs.has(value)?{sharedContextRef:refs.get(value)}:value]))
  ...
  async function runBatch(batch: Batch<S,A>,items: Admission<S,A>[]) {
    batch.running=true
    for(const item of items)batch.items.delete(item.job.id)
    const active: Admission<S,A>[]=[]
    for(const item of items) {
      try {
        const live=owned(batch.session,item.job.id)
        if(live.generation!==item.job.generation)throw new Error('任务已取消或结果已失效')
        if(live.status==='completed'){item.resolve(copy(live.result));continue}
        if(terminal.has(live.status)||live.execution!=='spawn')throw new Error('任务已取消或结果已失效')
        active.push(item)
    ...
    const shared: SharedBatch<S,A>={session:batch.session,items:active,expiry:null}
    ...
    try {
      ...
      if(result.stopReason!=='completed') {
        // The child history is ready before awaiting its terminal result.
        const end=sessionEvents(child.localAgent?.session).findLast(event=>event.type==='turn/end')
  ...
  function enqueue(session: S,job: TaskJob,spec: TaskSpec<S,A>) {
    ...
    const entry: Admission<S,A>={job,spec,generation:job.generation,ready:false,promise:new Promise<unknown>((res,rej)=>{resolve=res;reject=rej}),resolve,reject,readyForDispatch:()=>{}}
  ...
  function abortControl(session: S,id: string,control=controllers.get(id)) {
  ...
  async function timeoutItem(session: S,item: Admission<S,A>) {
    const live=owned(session,item.job.id)
    ...
    if(control?.generation===item.job.generation&&(control.batch?.items.length??0)>1) {
      ...
      shared.expiry??=(async()=>{
        const transferred: Admission<S,A>[]=[]
  ...
  async function cancelJob(session: S,id: string) {
    const result=await lock(id,async()=>{const job=ownedRecord(session,id);if(job.status==='completed')return job
  ...
  async function submit({session,id,generation,value}: {session:S;id:string;generation:string;value:unknown}): Promise<unknown> {
    return lock(id,async()=>{
      ...
      try {
        ...
        const rejected=error instanceof TaskValidationError?error:new TaskValidationError(String(taskObject(error)?.message),
  ...
  async function request(spec: TaskSpec<S,A>): Promise<unknown> {
    const {session,agent,kind,source,input,signal}=spec
    if(spec.background===true&&!((kind==='memory'&&input?.taskStage==='background-notes')||(kind==='setting-repair'&&input?.taskStage==='setting-repair')))throw new Error('仅跨轮后台记忆或有界设定修补允许独立同模型子代理')
    ...
    let job=await lock(id,async()=>{
      ...
      if(raw!==undefined&&(!found||found.id!==id||found.sessionId!==session.id))throw new Error('任务记录损坏或归属不符，保留原记录以便恢复')
    ...
    const run=(async()=>{
      ...
      try {
        ...
        ac.signal.throwIfAborted()
        if (result.stopReason!=='completed') {
          // Preserve the native terminal failure details for retries.
  ...
  return {list,request,submit,holdBatch,
    ...
    async invalidate(session: S) {
      for(const job of list(session).filter(j=>!terminal.has(j.status)))if(!await isCurrent(session,job)) {
        ...
        if(admission&&admission.generation===job.generation)admission.reject(new Error('任务来源已变化'))
      }
    },
    async fail(session: S,id: string,error: unknown) {
      return lock(id,async()=>{const job=ownedRecord(session,id);if(terminal.has(job.status))return job
    ...
    read(session: S,id: string,offset=0,maxChars=64000) {
      const job=ownedRecord(session,id), text=JSON.stringify(job.input)
      if(text===undefined)throw new Error('任务记录缺少来源输入')
    ...
    pending(session: S) {return list(session).filter(j=>j.execution==='inline'&&!terminal.has(j.status))},
  }
}
