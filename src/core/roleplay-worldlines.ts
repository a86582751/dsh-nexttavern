...
export function createRoleplayWorldlines(deps: WorldlineDependencies) {
  ...
  function hydrateForkGroup(value: unknown): ForkGroup | null {
    ...
    for (const [index, member] of ordered.entries()) {
      ...
      member.playerTextRevision = Math.max(
        1, Number(member.playerTextRevision) || Number(group.playerVariants[member.playerVariantId]?.revision) || 1,
      )
  ...
  function recoveryRegistrationResult(group: ForkGroup, ordinal: number, playerOrdinal: number | undefined): RegistrationResult {
