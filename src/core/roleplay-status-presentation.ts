...
import type { StatusField, StatusPanel, StatusRecord } from './roleplay-status-types.js';
export const textAlias = (object: unknown, keys: readonly string[]) => {
...
export const normalizeStatusField = (field: unknown) => {
...
export const normalizeStatusOption = (option: unknown) => {
...
export const escapeStatusHtml = (value: unknown) => String(value ?? '')
    .replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
    .replace(
/\"/g,
    '&quot;'
);
export const fallbackStatusHtml = (title: string, fields: readonly StatusField[], rawText: string) => {
...
export const normalizeStatusPanel = (panel: unknown): StatusPanel => {
...
export const authoredStatusTemplate = (spec: StatusRecord | null | undefined) => {
...
export const statusPanelForSpec = (value: unknown, spec: StatusRecord | null | undefined, { strict = false } = {}) => {
    ...
    if (template) {
        ...
        for (let index = 0; index < expected.length; index++)
            if (!actual.some(candidate => matches(expected[index]!, candidate)))
                return reject(
                `html.attributes[${index}].${expected[index]!.name}`,
                    'author-static-attribute',
                    'author-value-with-dynamic-slots',
                    'missing-or-changed'
                );
...
export const normalizeStatusRecord = (record: StatusRecord | undefined) => {
...
export const normalizeDecisionRecord = (value: unknown): (Record<string, unknown> & {
    options: ReturnType<typeof normalizeStatusOption>[];
}) | null => {
