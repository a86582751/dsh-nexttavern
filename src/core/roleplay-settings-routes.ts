...
export function registerSettingsRoutes({ctx, T, resolveRoleplaySession, modelPolicy, ensureBranch, taskAgents, characterCluster, characterRoster, memorySettingFields, memorySettingsPolicy, svc, narrativePresets}: SettingsRoutesDependencies) {
  ctx.effect(() => ctx.connection.fetch.register({requestBody: 'buffered',path: '/api/roleplay/presets', methods: ['GET', 'POST'], fetch: async request => {
    try {
      ...
      if (!session) return jsonResponse(404, {ok: false, error: '角色扮演对话不存在'})
  ...
  ctx.effect(()=>ctx.connection.fetch.register({requestBody: 'buffered',path:'/api/roleplay/character-cluster',methods:['GET','POST'],fetch:async request=>{
    try{
      ...
      await ctx.get('tavernConversations')?.ready
      if(body){
        if(body.scope!==undefined&&!['global','session'].includes(body.scope))throw new Error('必须指定全局或会话范围')
        ...
        for(const selected of [settings.defaultRoute,...Object.values(settings.characters)].filter((route): route is NonNullable<typeof route>=>route!=null)){
        ...
        if(body.scope==='global')await characterCluster.saveGlobal(settings,body.expectedRevision!)
        else await characterCluster.save(session,settings,body.expectedRevision!)
      }
