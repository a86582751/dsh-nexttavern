// Generated from runtime/alpha3/core/roleplay-status.ts; edit the TypeScript source.
...
export function createRoleplayStatus(deps) {
    ...
    const escapeStatusHtml = (value) => String(value ?? '')
        ...
        .replace(/\"/g, '&quot;');
    const fallbackStatusHtml = (title, fields, rawText) => {
    ...
    const statusPanelForSpec = (value, spec, { strict = false } = {}) => {
        const raw = value;
        const reject = (path, rule, expected, actual) => {
            if (strict)
                throw new TaskValidationError(`状态结果没有保留作者模板：${path} (${rule})`, [{ path, rule, expected, actual }], 'STATUS_TEMPLATE_MISMATCH');
            return null;
        };
        ...
        if (template) {
            ...
            if (!authoredStatusTemplate({ templateHtml: html }))
    ...
    const statusSourceLive = (session, event, source) => !statusDisposed && recordSha256(statusSource(session, event)) === recordSha256(source);
    ...
    const selectedStatusGeneration = (session) => {
        ...
        if (record && !statusSourceLive(session, event, record.source))
            return { ...record, state: 'stale', result: null };
        return record ?? null;
    };
    ...
    const statusAuthorRecord = (record) => record && Object.fromEntries(Object.entries(record).filter(([key]) => !['sources', 'source', 'editedFrom', 'schemaVersion', 'importId', 'sourceRecordSessionId', 'sessionId', 'branchId', 'atSeq', 'updatedAt', 'createdAt', 'verified'].includes(key)));
    ...
    const statusStoryContext = (session, event, specHash, fixedContextHash, inputBasis) => {
        ...
        for (const [key, stored] of T.status.entries()) {
            ...
            if (!source || source.sourceHash !== candidate.provenance.sourceHash ||
                ...
                candidate.provenance.specHash !== specHash || candidate.provenance.fixedContextHash !== fixedContextHash)
                continue;
    ...
    function runStatusObligation(session, event, via = 'turn-end', { force = false, agent, signal } = {}) {
                ...
                if (record?.state !== 'completed' || force || record.source?.sourceHash !== source.sourceHash || record.specHash !== specHash || record.fixedContextHash !== fixedContextHash || record.storyContextHash !== storyContextHash) {
                    ...
                    if (!panel) {
                        ...
                        const raw = await llmJson(ctx, resolveRoute(session, agent), {
                            ...
                            validate: value => { if (!statusPanelForSpec(value, spec, { strict: true }))
                                throw new TaskValidationError('状态结果没有可用内容', [{ path: 'result', rule: 'nonempty-status', expected: 'status-content', actual: 'empty' }]); return value; },
                    ...
                    resultCommitted = true;
                await withStatusLock(session.id, async () => {
                    if (!statusSourceLive(session, event, source) || recordSha256(statusFixedContext(session)) !== fixedContextHash || recordSha256(statusStoryContext(session, event, specHash, fixedContextHash, record.inputBasis)) !== storyContextHash)
                        throw new Error('status source changed');
        ...
        }).finally(() => { cancellation.dispose(); admitted(null); if (statusJobs.get(taskKey) === job)
    ...
    const normalizeDecisionRecord = (value) => {
        const record = value;
        if (!record || typeof record !== 'object')
            return null;
        return {
