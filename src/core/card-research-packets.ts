...
export type ResearchPacketRow = SearchHit & {
    sourceHits: {
        segment: number;
        start: number;
        end: number;
...
export function expandSourceHit(source: AdaptationSource, row: SearchHit, chars: number): SearchHit {
    ...
    for (const receipt of receipts) {
        ...
        for (const [start, end] of ranges) {
            ...
            const left: [
                number,
                number
            ] = [start, Math.min(end, receipt.start)];
            const right: [
                number,
                number
            ] = [Math.max(start, receipt.end), end];
            if (left[1] > left[0])
                next.push(left);
            if (right[1] > right[0])
                next.push(right);
...
export function packetizeResearchHits(source: AdaptationSource, rows: (SearchHit & {
    queryIndex: number;
})[], receipts: readonly ReceiptRange[]) {
...
export function splitDenseResearchPackets(source: AdaptationSource, packets: readonly ResearchPacketRow[], maxChars = 4000) {
...
export function safeDeferredEvidence(packets: readonly ResearchPacketRow[]) {
    const json = JSON.stringify({
        schemaVersion: 1, packets
    });
    return json.length <= 19000 && Buffer.byteLength(json, 'utf8') <= 45000;
}
export function splitDeferredEvidence(packets: readonly ResearchPacketRow[]) {
