...
import {
  ...
  type Table,
  type Fetcher,
} from './tavern-pricing-data.js';
const KEY = 'tavern_price_catalog',
  URL = 'https://models.dev/api.json',
  TTL = 6 * 60 * 60 * 1000,
  MAX_BYTES = 16 * 1024 * 1024;
type CatalogTier = RateValues & {
  threshold: number;
};
...
const rates = (value: unknown): RateValues => {
  ...
  for (const [provider, rawProvider] of Object.entries(data as RecordLike)) {
    ...
    for (const [model, rawModel] of Object.entries(models ?? {})) {
      const modelRecord = asRecord(rawModel),
        modalities = asRecord(modelRecord?.modalities),
      ...
      if (modelRecord?.status === 'deprecated' || !Array.isArray(output) || !output.includes('text')
        || output.some(value => ['audio', 'image', 'video'].includes(value as string))
      ...
      const tiers: CatalogTier[] = Array.isArray(rawTiers) ? rawTiers.filter(rawTier => {
...
export function createPriceCatalog({ table, fetcher = fetch, now = Date.now }: {
  ...
  const state = (): CatalogState => ({
    ...((table.get(KEY) as CatalogState | undefined) ?? {
      schemaVersion: 1, source: URL, entries: [], fetchedAt: null
    }),
    syncing: Boolean(pending)
  });
  async function sync(force = false): Promise<CatalogState> {
    ...
    pending = (async () => {
      try {
        ...
        const entries = normalizeCatalog(JSON.parse(bytes.toString('utf8'))),
          record: CatalogState = {
            schemaVersion: 1,
            source: URL,
            currency: 'USD',
