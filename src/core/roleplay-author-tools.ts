...
export function registerAuthorTools({ctx, T, simpleTool, sessionOf, storyBranchIsActive}: AuthorToolsDependencies) {
  ctx.effect(() =>
    ctx.tools.register(
      simpleTool(
        'rp_card_set',
    ...
    ctx.tools.register(
      simpleTool(
        ...
        async (args, exec) => {
          ...
          const next = { ...prev }
          for (const f of ['name', 'content'] as const) if (args[f] !== undefined) next[f] = String(args[f])
          for (const f of ['aliases', 'keywords', 'triggers'] as const) if (Array.isArray(args[f])) next[f] = args[f].map(String)
