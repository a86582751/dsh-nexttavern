// Generated from runtime/alpha3/core/tavern-card.ts; edit the TypeScript source.
    ...
    while (offset < bytes.length) {
        ...
        if (type === 'tEXt') {
            ...
            if (key === 'chara' || key === 'ccv3') {
                ...
                if (encoded.length > Math.ceil(CARD_LIMITS.jsonBytes / 3) * 4 || !/^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$/.test(encoded))
