...
import type {CharacterRoute, CharacterSettings, CharacterInputSource} from './character-cluster-projection.js'
import type {TaskFailure} from './tavern-tasks.js'

export interface ClusterSession { id: string }
export interface ClusterAgent { options?: Record<string, unknown> }
export interface NativeRoute { provider: string; model: string; reasoningEffort?: unknown }
interface SettingsRecord extends Omit<CharacterSettings, 'defaultRoute'> { schemaVersion: 1; revision: number; defaultRoute?: CharacterRoute | null; updatedAt?: number }
interface Table {
  get(key: string): unknown
  put(key: string, value: unknown): void | PromiseLike<unknown>
}
interface ChildResult { stopReason: string; output?: { type: string; text?: string }[] }
interface Child {
  id: string
...
interface SpawnRequest<A> {
  ...
  signal: AbortSignal
  maxDepth: 1
  agentOptions: NativeRoute & { tavernTaskId: string; maxTokens: number }
  toolFilter: { allow: string[] }
  label: string
  persona: string
  prompt: { type: 'text'; text: string }[]
}
interface CurrentWork { source: { turn: number }; input?: { character?: CharacterInputSource['character'] } }
interface Options<S, A> {
  table: Table
  subagents: { start(kind: 'spawn', request: SpawnRequest<A>): Child | PromiseLike<Child> }
...
interface CharacterJob {
  ...
  kind: 'character'
  execution: 'spawn'
  source: { turn: number; events: { seq: number; hash: string }[]; hashKind: 'taskHash' }
  sourceHash: string
  input: ReturnType<typeof characterInput> & { taskStage: 'character' }
  allowedTools: string[]
  main: NativeRoute
  ...
  error?: string | null
  failure?: TaskFailure
  attempts?: { route: NativeRoute; failure: TaskFailure; startedAt?: number; endedAt: number }[]
  fallback?: { from: NativeRoute; to: NativeRoute; reason: string; failure: TaskFailure; at: number }
}

const record = (value: unknown): Record<string, unknown> | undefined =>
  value !== null && typeof value === 'object' && !Array.isArray(value) ? value as Record<string, unknown> : undefined
const usesMainRoute = (route: CharacterRoute | null | undefined): route is Extract<CharacterRoute, {main: true}> =>
  Boolean(route && 'main' in route && route.main)

function assertSettings(value: unknown): asserts value is SettingsRecord {
  const data = record(value)
  if (data?.schemaVersion !== 1) throw new Error('角色集群设置版本不受支持')
  if (typeof data.revision !== 'number' || !Number.isSafeInteger(data.revision) || data.revision < 0 ||
      typeof data.enabled !== 'boolean' || !record(data.characters)) throw new Error('角色集群设置损坏')
...
export function createCharacterCluster<S extends ClusterSession, A extends ClusterAgent>({table,subagents,rootOf=s=>s.id,main,isCurrent=()=>true,timeoutMs=45000}: Options<S, A>) {
  ...
  const readGlobal=()=>{
    ...
    if(value===undefined||value===null)return {schemaVersion:1 as const,revision:0,enabled:false,defaultRoute:null,characters:{}}
  ...
  const read=(s: S)=>{
    ...
    return {...local,defaultRoute:global.revision>0?global.defaultRoute:local.defaultRoute}
  }
  async function saveGlobal(value: unknown,expectedRevision: number) {
    const route=clusterRoute(record(value)?.defaultRoute)
    return withTavernLock(table,globalKey,async()=>{
      const previous=readGlobal()
      if(previous.revision!==expectedRevision)throw new Error('全局角色模型已更新，请刷新后重试')
      const next={schemaVersion:1 as const,revision:previous.revision+1,enabled:false,defaultRoute:route,characters:{},updatedAt:Date.now()}
  ...
  async function run({session,agent,turn,characters,context,signal}: RunInput<S, A>) {
    ...
    const result=await Promise.all(characters.map(async (character): Promise<CharacterResult>=>{
      ...
      const route=usesMainRoute(override)?{...primary,...(override.reasoningEffort?{reasoningEffort:override.reasoningEffort}:{})}:override??primary
      const id=taskHash({sessionId:session.id,turn,character:character.id,input,route}),key=`tavern_job__${id}`
      const existing=inflight.get(id)
      if(existing)return existing
      const old=record(table.get(key))
      ...
      const work=(async(): Promise<CharacterResult>=>{
        ...
        let job: CharacterJob={schemaVersion:1,id,generation,sessionId:session.id,branchId:session.id,kind:'character',execution:'spawn',
        ...
        const current=()=>record(table.get(key))?.generation===generation&&isCurrent(session,job)&&read(session).enabled
        ...
        for(let attempt=0;attempt<routes.length;attempt++) {
          ...
          try {
            job={...job,status:'running',childSessionId:null,actualRoute:attemptRoute,startedAt:job.startedAt??Date.now(),attemptStartedAt:Date.now(),updatedAt:Date.now()};await table.put(key,job)
            const starting=Promise.resolve(subagents.start('spawn',{parent:agent,signal:ac.signal,maxDepth:1,
              agentOptions:{...attemptRoute,tavernTaskId:id,maxTokens:nativeTaskOutputBudget(attemptRoute,4096)},
            ...
            if(response.stopReason!=='completed') {
              ...
              throw Object.assign(new Error('角色推演未完成'),{failure:record(end?.data?.reason)?.failure??end?.data?.reason??{code:response.stopReason}})
            }
            ...
            if(record(table.get(key))?.generation!==generation)break
            ...
            job={...job,failure,error:failure.label,updatedAt:Date.now(),
              attempts:[...(job.attempts??[]),{route:attemptRoute,failure,startedAt:job.attemptStartedAt,endedAt:Date.now()}]}
            if(attempt+1<routes.length&&!signal?.aborted&&current())job.fallback={from:attemptRoute,to:primary,reason:failure.category,failure,at:Date.now()}
            await table.put(key,job)
          }finally{
            clearTimeout(timer);signal?.removeEventListener('abort',abort)
            const settledChild=child
            if(settledChild)Promise.resolve().then(()=>settledChild.dispose()).catch(()=>{})
          }
        }
        if(record(table.get(key))?.generation===generation)await table.put(key,{...job,status:signal?.aborted?'cancelled':current()?'failed':'stale',updatedAt:Date.now()})
  ...
  return {read,readLocal,readGlobal,save,saveGlobal,run}
}
