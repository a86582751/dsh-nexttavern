// Generated from runtime/alpha3/core/tavern-telemetry.ts; edit the TypeScript source.
...
function isTelemetryJob(value) {
    ...
    if (['sessionId', 'branchId', 'childSessionId', 'generation'].some(key => value[key] !== undefined && typeof value[key] !== 'string') ||
        ...
        return false;
    if (value.childSessionIds !== undefined && (!Array.isArray(value.childSessionIds) || !value.childSessionIds.every(id => typeof id === 'string')))
        return false;
    if (value.input !== undefined) {
        if (!objectRecord(value.input))
            return false;
        const character = value.input.character;
        if (character !== undefined && (!objectRecord(character) || ['name', 'id'].some(key => character[key] !== undefined && typeof character[key] !== 'string')))
            return false;
    }
    ...
    if (value.fallback !== undefined) {
        ...
        if (!objectRecord(fallback) || ['from', 'to', 'failure'].some(key => fallback[key] !== undefined && !objectRecord(fallback[key])) ||
            ...
            return false;
        if (objectRecord(fallback.failure) && fallback.failure.category !== undefined && typeof fallback.failure.category !== 'string')
            return false;
    }
    return true;
}
...
function presetOf(s) { let p = s?.header?.agentPreset; for (const e of events(s))
    if (e.type === 'agent-preset/selected')
        p = e.data?.agentPreset ?? p; return p; }
...
export function createTelemetry({ table, sessions, query, jobs: readJobs = () => [] }) {
    ...
    let syncPromise = null, persistenceFailures = 0, lastSyncAt = 0, coverage = { scanning: false, scannedSessions: 0, totalSessions: 0, failedSessions: 0 };
    ...
    const owner = (s) => { let h = s.header, seen = new Set(); while (h?.origin === 'subagent' && h.parentSession && !seen.has(h.id)) {
        seen.add(h.id);
        h = known.get(h.parentSession) ?? sessions.get(h.parentSession)?.header ?? { id: h.parentSession };
    } return h?.id ?? s.id; };
    function eligible(s) { if (presetOf(s) === 'roleplay')
        return true; const id = owner(s); return (known.get(id) ?? (id ? sessions.get(id)?.header : undefined))?.agentPreset === 'roleplay'; }
    async function writeChanged(key, value) { const snapshot = JSON.stringify(value); if (persisted.get(key) === snapshot)
        return; await table.put(key, value); persisted.set(key, snapshot); }
    ...
    async function ingestUnlocked(s) {
        ...
        if (relatedJobIds.length)
            for (const stream of records().filter(r => r.sessionId === s.id && r.source.kind === 'stream')) {
                ...
                if (jobIds.length !== stream.source.jobIds?.length || jobIds.some((id, i) => id !== stream.source.jobIds?.[i]))
                    await put({ ...stream, source: { ...stream.source, jobIds } });
            }
        ...
        for (const e of events(s)) {
            ...
            if (e.type === 'assistant/message' && (attempt?.kind !== 'narrative' || !d.message?.content?.some(b => b.type === 'text') || d.message?.content?.some(b => b.type === 'tool-call')))
                continue;
    ...
    async function sync() {
        if (syncPromise)
            return syncPromise;
        coverage = { ...coverage, scanning: true, scannedSessions: 0, failedSessions: 0 };
        syncPromise = (async () => {
            ...
            for (const id of ids) {
                try {
                    ...
                    if (!s) {
                        ...
                        s = { id, header: snapshot.session, events: snapshot.events };
                        await ingest(s);
                        coldRead.add(id);
                    }
                    else
    ...
    async function recordEmbedding(call) {
        ...
        const usage = { inputTokens: n(call.inputTokens), outputTokens: null, cacheReadTokens: null, cacheWriteTokens: null, totalTokens: n(call.totalTokens) };
        ...
        if (existing) {
            ...
            const mergedUsage = { inputTokens: null, outputTokens: null, cacheReadTokens: null, cacheWriteTokens: null, totalTokens: null, ...(existing.usage ?? {}) };
            ...
            if (changed) {
                const row = { ...existing, usage: mergedUsage, providerUsage: mergedProviderUsage };
                await put(row);
                return row;
            }
            return existing;
        }
        const row = { schemaVersion: 1, id, sessionId: call.ownerSessionId, ownerSessionId: call.ownerSessionId, provider, model, kind: 'embedding', status: call.status, startedAt: call.startedAt, completedAt: call.completedAt, durationMs: call.completedAt - call.startedAt, firstTokenMs: null, usage, providerUsage, source: { kind: 'embedding', sessionId: call.ownerSessionId, workspaceId, purpose: call.purpose, invocationId: safeId(call.requestId) ?? id, routeEvidence: 'embedding-worker' } };
        await put(row);
        return row;
    }
    async function* observe(options, next) {
        const liveSession = options.sessionId ? sessions.get(options.sessionId) : undefined, s = liveSession ?? { id: options.sessionId ?? null, header: { id: options.sessionId ?? null }, events: [] };
        if (s.id)
            meta(s);
    ...
    const sharedJobIndex = (allJobs) => { const index = new Map(); for (const j of allJobs) {
    ...
    async function savePrices(rawValue, revision) {
        ...
        return withTavernLock(table, PRICE, async () => {
            ...
            if (!['USD', 'CNY', 'EUR', 'JPY', 'HKD'].includes(value?.currency) || !Array.isArray(value.rates) || value.rates.length > 500)
            ...
            if (!['auto', 'manual'].includes(defaultMode) || n(defaultMultiplier) === null || defaultMultiplier > 1e6 || typeof autoSync !== 'boolean')
            ...
            const seen = new Set(), rates = value.rates.map(r => {
                ...
                if (!['auto', 'manual'].includes(mode) || r.multiplier != null && (n(r.multiplier) === null || r.multiplier > 1e6) || r.catalogKey != null && (typeof r.catalogKey !== 'string' || r.catalogKey.length > 800 || /[\x00-\x1f]/.test(r.catalogKey)))
                    throw new Error('模型定价模式或倍率无效');
                const out = { provider, model, mode, multiplier: r.multiplier ?? null, catalogKey: r.catalogKey ?? null };
            ...
            const result = { schemaVersion: 2, revision: revision + 1, currency: value.currency, defaultMode, defaultMultiplier, autoSync, rates, source: { kind: 'player-prices' }, updatedAt: Date.now() };
            await table.put(PRICE, result);
            return result;
        });
    }
    function logs() {
        const allJobs = jobs(), jobIndex = sharedJobIndex(allJobs), actualCalls = calls(allJobs), list = actualCalls.map(c => ({ ...c, label: c.kind === 'narrative' ? '正文生成' : c.kind === 'session-title' ? '会话标题生成' : '模型调用' }));
        const jobUsage = (j) => {
            ...
            const known = linked.filter((c) => Boolean(c.usage)), sharedCalls = linked.filter(c => (jobIndex.get(jobKey(c.ownerSessionId, c.sessionId))?.length ?? 0) > 1 || (c.source.taskIds?.length ?? 0) > 1).length;
        ...
        for (const j of allJobs) {
            const usage = jobUsage(j), baseLabel = { character: '角色推演 · ' + (j.input?.character?.name ?? j.input?.character?.id ?? ''), memory: '记忆与场景整理', status: '状态栏生成', decision: '决策建议', 'card-import': '读取角色卡', 'card-export': '导出角色卡', 'novel-export': '小说整理' }[j.kind] ?? '辅助任务';
            list.push({ schemaVersion: 1, id: `job-${j.id}`, sessionId: j.sessionId ?? j.branchId, ownerSessionId: j.sessionId ?? j.branchId, kind: j.kind, label: usage.usageSource.sharedCalls ? `共享步骤 · ${baseLabel}` : baseLabel, status: j.status, provider: j.actualRoute?.provider, model: j.actualRoute?.model, startedAt: j.startedAt ?? j.createdAt, completedAt: j.completedAt ?? null, durationMs: j.completedAt ? j.completedAt - (j.startedAt ?? j.createdAt) : null, error: j.error ? '任务失败，请重试或查看关联模型调用' : null, progress: j.progress, fallback: publicFallback(j), source: { kind: 'task', jobId: j.id, generation: j.generation }, ...usage });
        }
