...
import type { SessionActionDependencies, SessionCommandDependencies } from './roleplay-session-actions-types.js'

export function registerSessionTools({ctx, T, simpleTool, sessionOf}: SessionActionDependencies) {
  ctx.effect(() =>
    ctx.tools.register(
      simpleTool(
        'rp_roll',
    ...
    ctx.tools.register(
      simpleTool(
        ...
        async (args, exec) => {
          ...
          for (const f of ['positions', 'outfits', 'items', 'moods', 'injuries'] as const) if (args[f] && typeof args[f] === 'object') next[f] = { ...(prev[f] ?? {}), ...args[f] }
...
export function registerSessionCommands({ctx, T, isRoleplaySession, startExportJob, svc}: SessionCommandDependencies) {
  ctx.effect(() =>
  ...
  ctx.effect(()=>ctx.commands.register({
    ...
    input:{hint:'直接执行即可'},handler:async invocation=>{
      ...
      }catch(error){return {kind:'error',text:String((error as Error).message)}}
    },
