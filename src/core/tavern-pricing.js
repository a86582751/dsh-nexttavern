// Generated from runtime/alpha3/core/tavern-pricing.ts; edit the TypeScript source.
...
const rates = (value) => { const record = asRecord(value); return { input: number(record?.input), output: number(record?.output), cacheRead: number(record?.cache_read), cacheWrite: number(record?.cache_write) }; };
    ...
    for (const [provider, rawProvider] of Object.entries(data)) {
        ...
        for (const [model, rawModel] of Object.entries(models ?? {})) {
            ...
            if (!provider || !model || provider.length > 200 || model.length > 400 || /[\x00-\x1f]/.test(provider + model))
                continue;
            if (modelRecord?.status === 'deprecated' || !Array.isArray(output) || !output.includes('text') || output.some(value => ['audio', 'image', 'video'].includes(value)) || !asRecord(modelRecord.cost))
                continue;
            const base = rates(modelRecord.cost);
            if (base.input === null && base.output === null)
                continue;
            const rawTiers = asRecord(modelRecord.cost)?.tiers;
            const tiers = Array.isArray(rawTiers) ? rawTiers.filter(rawTier => { const tier = asRecord(rawTier)?.tier; return asRecord(tier)?.type === 'context' && number(asRecord(tier)?.size) !== null; }).map(rawTier => { const tier = asRecord(asRecord(rawTier)?.tier); return { threshold: number(tier.size), ...rates(rawTier) }; }).sort((a, b) => a.threshold - b.threshold) : [];
...
export function createPriceCatalog({ table, fetcher = fetch, now = Date.now }) {
    ...
    async function sync(force = false) {
        ...
        pending = (async () => {
            try {
                ...
                const bytes = Buffer.concat(parts), entries = normalizeCatalog(JSON.parse(bytes.toString('utf8'))), record = { schemaVersion: 1, source: URL, currency: 'USD', unit: 'per-million-tokens', fetchedAt: now(), attemptedAt: now(), sha256: createHash('sha256').update(bytes).digest('hex'), entries, error: null };
                await table.put(KEY, record);
            }
            catch {
                const { syncing, ...prior } = previous;
                await table.put(KEY, { ...prior, schemaVersion: 1, attemptedAt: now(), error: '同步失败（网络或目录数据异常）；已保留上次成功的价格。' });
            }
            return state();
        })();
        try {
    ...
    const refresh = () => { const s = state(); if (!pending && (!s.attemptedAt || now() - s.attemptedAt >= TTL))
        void sync().catch(() => { }); return status(); };
    return { state, status, sync, refresh };
}
const indexes = new WeakMap();
function index(entries) { if (!indexes.has(entries))
    indexes.set(entries, new Map(entries.map(entry => [entry.key, entry]))); return indexes.get(entries); }
export function automaticKey(provider, model) { return `${provider === 'deepseek-official' ? 'deepseek' : provider}/${model}`; }
function buckets(usage) { const b = { input: number(usage.inputTokens), output: number(usage.outputTokens), cacheRead: number(usage.cacheReadTokens), cacheWrite: number(usage.cacheWriteTokens) }, total = number(usage.totalTokens), missing = rateFields.filter(key => b[key] === null); if (total !== null && missing.length) {
...
export function resolvePricing(call, settings = {}, entries = []) {
    const row = settings.rates?.find(rate => rate.provider === call.provider && rate.model === call.model), mode = row ? (row.mode ?? 'manual') : (settings.defaultMode ?? 'manual'), absent = (status) => ({ cost: null, baseCost: null, mode, status, currency: settings.currency ?? 'USD' });
    ...
    if (mode === 'auto') {
        ...
        chosen = { ...entry.rates };
        const configuredMultiplier = number(row?.multiplier ?? settings.defaultMultiplier ?? 1);
    ...
    if (mode === 'auto' && multiplier === 0)
        return { cost: 0, baseCost: null, mode, status: 'zero-multiplier', multiplier, catalogKey: entry.key, currency: 'USD' };
    if (!call.usage)
        return absent('missing-usage');
    const b = buckets(call.usage);
    if (entry?.tiers.length) {
        const exact = number(call.usage.totalTokens), output = number(call.usage.outputTokens), context = exact !== null && output !== null && exact >= output ? exact - output : null;
        if (context === null)
            return absent('unknown-context-tier');
        const tier = entry.tiers.findLast(item => context >= item.threshold);
        if (tier)
            chosen = { ...chosen, ...Object.fromEntries(rateFields.map(key => [key, tier[key] ?? chosen[key]])) };
    }
    let baseCost = 0;
    const billableFields = call.kind === 'embedding' ? ['input'] : rateFields;
    for (const key of billableFields) {
        const amount = b[key], rate = number(chosen[key]);
        if (rate === 0 || amount === 0)
            continue;
        if (amount === null)
            return absent('missing-usage');
...
export function createExchangeRates({ table, fetcher = fetch, now = Date.now }) {
    ...
    async function sync(force = false) {
        ...
        if (!force && (previous.error && time - (previous.attemptedAt ?? 0) < 3600000 || previous.fetchedAt && time < Math.min(previous.nextUpdateAt ?? 0, previous.fetchedAt + DAY)))
            return state();
        pending = (async () => { try {
            ...
            ;
            const bytes = Buffer.concat(parts), data = asRecord(JSON.parse(bytes.toString('utf8')));
            ...
            if (data.result !== 'success' || data.base_code !== 'USD' || ratesData?.USD !== 1 || !(number(ratesData?.CNY) > 0) || !Number.isFinite(rateAt) || rateAt <= 0 || rateAt > now() + 300000 || !Number.isFinite(nextUpdateAt) || nextUpdateAt <= rateAt)
                throw new Error('invalid quote');
            const exchangeRates = Object.fromEntries(['USD', 'CNY', 'EUR', 'JPY', 'HKD'].filter(key => number(ratesData[key]) > 0).map(key => [key, ratesData[key]]));
            await table.put(FX_KEY, { schemaVersion: 1, source: FX_URL, sourceUrl: FX_SOURCE, base: 'USD', rates: exchangeRates, rateAt, nextUpdateAt, fetchedAt: now(), attemptedAt: now(), sha256: createHash('sha256').update(bytes).digest('hex'), error: null });
        }
        catch {
            const { syncing, ...prior } = previous;
            await table.put(FX_KEY, { ...prior, schemaVersion: 1, attemptedAt: now(), error: '汇率更新失败，继续使用上次成功的汇率；没有缓存时成本显示 N/A。' });
        } })();
        try {
            await pending;
        }
...
export function convertUsageCurrency(stats, target, quote, now = Date.now()) {
    ...
    const convert = (row) => ({ ...row, cost: row.cost === null || rate === null ? null : row.cost * rate, knownCost: rate === null || row.knownCost === null ? null : row.knownCost * rate });
