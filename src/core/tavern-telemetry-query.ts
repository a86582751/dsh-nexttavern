import { convertUsageCurrency, resolvePricing, type PricingSettings, type CatalogEntry, type ExchangeQuote } from './tavern-pricing.js'
import { timeRange } from './tavern-telemetry-time-range.js'
import { nonNegativeFinite as n, normalizeUsage as usageOf, publicTelemetryError as publicError, type TelemetryProjectionCall, type TelemetryProjectionQuery } from './tavern-telemetry-normalize.js'
type CurrencyDisplay = Parameters<typeof convertUsageCurrency>[0] & { fx: ReturnType<typeof convertUsageCurrency>['fx'] }

/** A paged projection of real calls, not workflow/task completion records. */
export function queryUsageRequests(calls: readonly TelemetryProjectionCall[],query: TelemetryProjectionQuery={},prices: PricingSettings={rates:[],currency:'USD'},catalog: readonly CatalogEntry[]=[],quote: ExchangeQuote={},sessionList: readonly {id:string;label?:string|null}[]=[]) {
  ...
  let display: CurrencyDisplay ={currency:prices.currency??'USD',totals:{cost:0,knownCost:0},models:[],timeline:[],fx:null}
  if(query.currency)display=convertUsageCurrency(display,query.currency,quote)
  const rate=display.fx?display.fx.rate:1,labels=new Map<string|null,string|null|undefined>(sessionList.map(s=>[s.id,s.label]))
