...
import type {createRoleplayActions} from './session-actions.js'
import type {StateReply} from './state-store.js'

interface BranchGroup {currentOrdinal?: number; members?: {sessionId: string}[]}
export interface MessageActionState extends StateReply {
  userActionsBySeq?: Record<string, {assistantMessageId?: string; group?: BranchGroup}>
  deletedBranchMessageIds?: string[]
  assistantActionAnchorsByTurn?: Record<string, {messageId: string; seq: number}>
  surfaceNodes?: {seq: number; text?: string}[]
  internalAssistantMessageIds?: string[]
  internalMaintenanceTurns?: number[]
  failedTurnRecoveryByTurn?: Record<string, unknown>
  failedTurnBranchGroupByTurn?: Record<string, BranchGroup>
  branchGroupsByMessageId?: Record<string, BranchGroup>
  inheritedAssistantMessageIds?: string[]
}
...
interface ComponentDependencies extends Pick<Actions,'replaceMessage'|'forkAndPrompt'|'forkWithoutUserTurn'|'openNativeBranch'|'retryBranchMutation'> {
  ...
  runCommand(id: string, command: string): unknown | PromiseLike<unknown>
}
type EditorProps = {
  title: string; initialText: string; busy?: boolean
  onClose(): void
  onSave(text: string): unknown
} & ({allowSend: true; onSend(text: string): unknown} | {allowSend?: false; onSend?: never})
interface UserActionProps {sessionId: string; seq?: number | string; text?: string}
interface AssistantActionProps {sessionId: string; turn?: number; messageId?: string; text?: string}

export function createMessageActionComponents({React,isRoleplaySession,fetchState,subscribeState,invalidateState,applyImmersive,readImmersive,toast,confirmWithDialog,runCommand,replaceMessage,forkAndPrompt,forkWithoutUserTurn,openNativeBranch,retryBranchMutation}: ComponentDependencies) {
  const ICONS = {
    regenerate:
  ...
  const actionIconButton = (title: string, icon: ReactAPI.ReactNode, onClick: ReactAPI.MouseEventHandler<HTMLButtonElement>, extra: ReactAPI.ButtonHTMLAttributes<HTMLButtonElement> & {key?: string} = {}) =>
    React.createElement(
      'button',
  ...
  function MessageEditor({ title, initialText, allowSend = false, busy = false, onClose, onSave, onSend }: EditorProps) {
    ...
    return React.createElement('dialog', {
      ...
      'aria-modal': 'true',
      'aria-label': title,
      onCancel: (event: ReactAPI.SyntheticEvent<HTMLDialogElement>) => { event.preventDefault(); if (!busy) onClose() },
  ...
  function useRoleplayState(sessionId: string) {
    const [state, setState] = React.useState<MessageActionState | null>(() => isRoleplaySession(sessionId) ? null : { preset: 'other' })
  ...
  function UserActions(props: UserActionProps) {
    ...
    const perform = async (operation: () => unknown | PromiseLike<unknown>) => {
      if (busy) return
      setBusy(true)
      try { await operation() } catch (error) { toast(String(error && typeof error === 'object' && 'message' in error ? error.message : error)) } finally { setBusy(false) }
    }
  ...
  function AssistantActions(props: AssistantActionProps) {
    const sessionId = props.sessionId
    const [state, reload] = useRoleplayState(sessionId)
    ...
    const validRecoverySeq = (value: unknown): value is number => typeof value === 'number' && Number.isSafeInteger(value) && value >= 0
...
export function createToastController(documentRef: Document) {
  ...
  const managerHost = () => {
    const dialogs = [...documentRef.querySelectorAll?.<HTMLDialogElement>('dialog.dsh-rp-dialog[open]') ?? []]
    return dialogs.findLast?.(dialog => {
      try { return dialog.matches?.(':modal') } catch { return false }
    }) ?? dialogs.at(-1) ?? documentRef.body
  }
  const toast = (text: unknown) => {
...
export function confirmWithDialog(documentRef: Document, message: unknown, { title = '请确认', confirmLabel = '确认', cancelLabel = '取消' } = {}): Promise<boolean> {
