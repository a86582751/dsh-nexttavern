// Generated from runtime/alpha3/ui/reader-view.ts; edit the TypeScript source.
...
export function createAuthorRuntime(root, fill) {
    ...
    const add = (type, handler, options) => {
        if (typeof type !== 'string' || !handler)
            return;
        const registration = { type, handler: handler, options: options };
...
export function createReaderView({ React, resolveActiveSessionId, isRoleplaySession, useTavernActivity, fetchState, sessionsService, toast, UserActions, AssistantActions, actionIconButton, ICONS, pendingPlayerBubble, ActivityBanner, ACTIVITY_CSS }) {
    ...
    function ReaderView(props) {
        ...
        React.useEffect(() => {
            if (!sessionId || !activity)
                return;
            let alive = true;
            fetchState(sessionId, true).then(data => { if (alive && data?.sessionId === sessionId) {
                setState(data);
                setLoadError(null);
            } }).catch(() => { });
            return () => { alive = false; };
        ...
        const sessionSnapshot = React.useSyncExternalStore(React.useCallback((notify) => sessionFace?.subscribe?.(notify) ?? (() => { }), [sessionFace]), React.useCallback(() => sessionFace?.getSnapshot?.() ?? null, [sessionFace]), () => null);
        ...
        if (streamLeaseRef.current && (streamLeaseRef.current.sessionId !== sessionId
            || (activity?.sessionId === sessionId && Number.isSafeInteger(activity.turn) && activity.turn !== streamLeaseRef.current.turn)))
            streamLeaseRef.current = null;
        ...
        return React.createElement('div', {
        ...
        }, React.createElement('style', null, scopedCss + ACTIVITY_CSS), beautyFailed ? React.createElement('div', { className: 'rp-reader-empty', role: 'status' }, '部分美化规则未能安全完成，已保留完整正文。修改规则后会重新匹配。') : null, canLoadOlder || historyBusy || historyError
            ? React.createElement('div', { className: 'rp-reader-history', role: 'status', 'aria-live': 'polite' }, React.createElement('button', { type: 'button', className: 'rp-reader-load-older', onClick: requestOlder, disabled: historyBusy }, historyBusy ? React.createElement('span', { className: 'rp-reader-spinner', 'aria-hidden': 'true' }) : null, historyBusy ? '正在加载更早剧情…' : historyError ? '加载失败，点击重试' : '加载更早剧情'))
            : null, loadError ? React.createElement('div', { className: 'rp-reader-empty' }, '阅读器加载失败：' + loadError) : null, !loadError && state !== null && selectedNodes.length === 0
            ...
            : React.createElement('div', { className: 'rp-reader-narrative', dangerouslySetInnerHTML: { __html: part.html } }), renderReaderActions(part)))), pendingPlayerBubble(activity, nodes), React.createElement(ActivityBanner, { activity }));
    }
    return ReaderView;
}
