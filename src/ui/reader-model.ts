...
export interface NormalizedField extends UnknownRecord {
  emoji: string
  label: string
  value: string
}

export interface NormalizedOption extends UnknownRecord {
  label: string
  description: string
  heart: boolean
}

export interface NormalizedPanel extends UnknownRecord {
  atSeq: number
  turnId: unknown
  time: number
  title: string
...
export interface NormalizedDecision extends UnknownRecord {
  options: NormalizedOption[]
}
...
export interface ConversationNode extends UnknownRecord {
  kind?: unknown
  data?: unknown
}
...
const textAlias = (value: unknown, keys: readonly string[]): string => {
...
export function normalizeField(field: unknown): NormalizedField {
...
export function normalizeOption(option: unknown): NormalizedOption {
...
export function normalizePanel(value: unknown): NormalizedPanel | null {
...
export function normalizeDecision(value: unknown): NormalizedDecision | null {
...
export function userValuesFromState(state: unknown): UserValues {
...
export function renderUserVars(text: unknown, state: unknown): string {
...
const readCollectionValues = (value: unknown): unknown[] => {
  if (!isRecord(value) || typeof value.values !== 'function') return []
  const result = value.values()
  return Array.from(result as Iterable<unknown> | ArrayLike<unknown>)
}

export function normalizeConversationNodes(snapshot: unknown): unknown[] {
  const sourceValue = isRecord(snapshot) ? (snapshot.snapshot ?? snapshot) : snapshot
  if (!isRecord(sourceValue) && !Array.isArray(sourceValue)) return []
  const source = sourceValue
  let list: unknown[] = []
  let chat: unknown = null
  if (isRecord(source) && isRecord(source.views)) {
    try { chat = readCollectionValue(source.views, 'chat') } catch {}
  }
  ...
  } else if (isRecord(chat) && Array.isArray(chat.nodes)) list = chat.nodes
  else if (isRecord(chat) && isRecord(chat.nodes) && typeof chat.nodes.values === 'function') list = readCollectionValues(chat.nodes)
  else if (isRecord(chat) && isRecord(chat.legacy) && Array.isArray(chat.legacy.nodes)) list = chat.legacy.nodes
  else if (Array.isArray(source)) list = source
  else {
  ...
  return list.filter(Boolean).map(item => {
    ...
    const kind = type.includes('user') || role === 'user'
      ...
      : type.includes('assistant') || type.includes('narrator') || role === 'assistant'
        ? 'assistant'
        : item.kind
    if (isRecord(item.data)) return { ...item, kind }
    const content = item.content ?? item.text ?? item.message
    return { ...item, kind, data: { text: typeof content === 'string' ? content : '' } }
  })
}

export function textFromNodeValue(value: unknown, depth = 0): string {
...
export function readerText(node: unknown): string {
...
export function readerNodeSeq(node: unknown): number | null {
...
export function readerMessageId(node: unknown): string {
  const record = isRecord(node) ? node : {}
  const data = isRecord(record.data) ? record.data : {}
  const finalNode = isRecord(data.finalNode) ? data.finalNode : {}
...
export function readerTime(node: unknown): number {
  const record = isRecord(node) ? node : {}
  const data = isRecord(record.data) ? record.data : {}
...
export function formatReaderTime(value: unknown): string {
