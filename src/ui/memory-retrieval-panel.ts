...
type Provider = {
    ...
    name: string;
    kind: 'online' | 'local';
    protocol: 'openai' | 'dashscope-text' | 'dashscope-multimodal';
    baseUrl?: string;
    model?: string;
...
export type MemoryRetrievalReply = {
    ok?: boolean;
    revision: number;
    mode: Mode;
    scope: Scope;
...
type RebuildPending = {
    ...
    target: string;
    reason: string;
    action?: 'configure-adaptation-model' | 'rebuild-index';
};
...
interface Props {
    sessionId: string;
    LegacyPanel: ReactAPI.ComponentType<{
        scope?: {
            sessionId: string;
        };
        visible?: boolean;
...
const hostname = (baseUrl: string | undefined) => {
    if (!baseUrl)
        return '未填写地址';
    try {
...
export function createMemoryRetrievalPanel({ React, jsonFetch, toast }: Dependencies) {
    ...
    const field = (label: string, node: ReactAPI.ReactNode) => h('label', {
        className: 'dsh-rp-retrieval-field'
    }, label, node);
    const note = (value: ReactAPI.ReactNode) => h('p', {
        className: 'dsh-rp-muted'
    }, value);
    const badge = (value: string) => h('span', {
    ...
    function RetrievalPanel({ sessionId, view }: {
        ...
        const [progressReply, setProgressReply] = React.useState<{
            id: string;
            data: MemoryRetrievalReply;
        } | null>(null);
        ...
        const [staleConfirmation, setStaleConfirmation] = React.useState<{
            sessionId: string;
            providerId: string;
            revision: number;
            fingerprint: string;
        ...
        const requestRef = React.useRef(0),
            sessionRef = React.useRef(sessionId),
            dirtyRef = React.useRef(false),
            busyRef = React.useRef(false),
            editRef = React.useRef(0);
        ...
        React.useLayoutEffect(() => {
            ...
            const cancel = dialog.querySelector?.('[data-dsh-rp-stale-cancel]') as HTMLButtonElement | null;
            cancel?.focus();
            return () => {
                if (dialog.open)
                    dialog.close?.();
        ...
        React.useEffect(() => {
            ...
            }>(`/api/roleplay/usage-requests?sessionId=${encodeURIComponent(sessionId)}&scope=all&limit=1`)
                .then(reply => {
                if (alive)
        ...
        const action = async (name: string, extra: Record<string, unknown> = {}) => {
            ...
            setError(null);
            try {
                const next = await jsonFetch<MemoryRetrievalReply>('/api/roleplay/memory-retrieval', {
                    method: 'POST',
                    headers: {
        ...
        const button = (label: string, onClick: () => void, disabled = false, primary = false) => h('button', {
            type: 'button',
        ...
        const closeStaleDialog = (event: ReactAPI.SyntheticEvent<HTMLDialogElement>) => {
        ...
        const staleConfirmCard = staleConfirmation ? h('dialog', {
            ...
            id: 'dsh-rp-stale-confirm-title'
             ...
             '语义索引配置已变化'),
             note(`目标模型：${activeProvider?.name
            ?? staleConfirmation.providerId}`),
             note('范围：仅当前对话及其全部世界线。旧向量会清空并按当前模型重建；剧情正文、来源和账本保留不变。'),
             note('在线模型可能产生 API 消耗；确认后会开启当前对话自动更新。'),
             note(`目标版本指纹：${staleConfirmation.fingerprint.length > 20 ? `${staleConfirmation.fingerprint.slice(0,
             20)}…` : staleConfirmation.fingerprint
            || '未提供'}`),
             h('div',
             {
        ...
        const modeCard = h('div', {
            ...
            className: 'dsh-rp-row'
        }, (['keyword', 'semantic', 'hybrid'] as Mode[]).map((mode, i) => h('button', {
            key: mode,
            type: 'button',
        ...
        const chunkValue = chunkDraft || String(data?.chunkChars ?? 480),
            chunkNumber = Number(chunkValue),
            chunkValid = Number.isInteger(chunkNumber)
                && chunkNumber >= 128
                && chunkNumber <= 2048;
        const saveChunkSize = () => {
        ...
        const providerCard = h('div', {
            ...
            onClick: () => {
             ...
             h('div',
             null,
             badge(p.kind === 'local' ? '本地' : '在线'),
             p.kind === 'online' ? badge(protocols[p.protocol]) : null,
             p.kind === 'online'
            && geminiEmbeddingModels.has(p.model
            ?? '') ? badge('Gemini 检索格式') : null,
             draft.id === p.id ? badge('正在编辑') : null,
             p.id === data.activeProviderId ? badge('剧情记忆使用中') : null,
             p.id === data.adaptationProviderId ? badge('小说研究使用中') : null),
             h('small',
             null,
             p.kind === 'online' ? `${p.model
            ?? '未选择模型'} · ${hostname(p.baseUrl)} · ${p.ready ? '测试通过' : '待测试'} · ${p.keySet ? '已保存 Key' : '尚未配置 Key'}` : `${p.model
            ?? '未选择模型'} · ${p.ready ? '测试通过' : '待测试'}`),
             p.error ? h('small',
             {
            ...
            type: 'password', autoComplete: 'off', value: draft.apiKey ?? '', onChange: e => update({
             ...
             local ? note(`默认 512 tokens，适合 4 核 8G 的保守预算；所选模型输入上限 ${data?.catalog.find(m => m.id === draft.model)?.maxInputTokens
            ?? '—'} tokens。包含 Query／Document 前缀及特殊 token，由模型 tokenizer 实际检查。强硬件可调高；内存保护仍有效，较大预算可能失败，超长查询会回退关键词。保存并测试后，旧索引需按新指纹重建。`) : null,
             !local
            && draft.protocol === 'openai'
            && !geminiEmbeddingModels.has(draft.model
            ?? '') ? note('OpenAI 官方 Base URL：https://api.openai.com/v1；推荐模型：text-embedding-3-small 或 text-embedding-3-large。也支持中转站，请按供应商提供的地址填写，程序会追加 /embeddings；“OpenAI 官方 / 兼容接口”表示协议，不代表当前配置一定连接 OpenAI 官方。') : null,
             !local
            && draft.protocol === 'openai'
            && geminiEmbeddingModels.has(draft.model
            ?? '') ? h('div',
            ...
            className: 'dsh-rp-retrieval-card'
             ...
             null,
             'Gemini 2 检索配置'),
             note('通过 OpenAI 兼容接口接入；程序会自动匹配查询与正文角色，无需手写 task_type。向量统一进行 L2 归一化；维度默认 3072，也可填写 768、1536 或 3072。旧原文索引需重建后才会使用新的检索格式。')) : null,
        ...
        const indexActionButton = (label: string, actionName: 'rebuild-index' | 'clear-index') => h('button', {
            type: 'button',
            className: 'dsh-rp-btn',
        ...
        const confirmation = indexConfirmation && data ? h('div', {
            className: 'dsh-rp-index-confirm', 'role': 'alertdialog', 'aria-label': '确认向量数据库操作'
             ...
             h('strong',
             null,
             indexConfirmation === 'rebuild-index' ? '确认重建向量数据库？' : '确认清空向量数据库？'),
             note(`目标供应商：${activeProvider?.name
            ?? '当前未启用供应商'}；范围：当前可见对话的全部模型向量索引（包含旧版遗留索引，覆盖其所有世界线）。`),
        ...
        const progressCard = h('div', {
            ...
            type: 'button', className: 'dsh-rp-btn', disabled: busy !== null || !chunkDirty || !chunkValid, onClick: saveChunkSize
             ...
             '保存索引块大小')),
             note(`默认 480；建议 192（细线索）、480（均衡）、960（较长段落）。当前字符目标 ${data?.chunkChars
            ?? 480}，范围 128–2048；小说研究与剧情共用。字符数不等于 tokens：本地会按该模型 tokenizer 与编码预算继续分段，保证含前缀的完整输入不被截断。较大块增加开销并降低定位粒度；更改预算或分段配方需重建索引。`),
            ...
            className: 'dsh-rp-row'
             ...
             button('补齐历史 / 重试失败',
             () => void action('reindex')),
             indexActionButton('重建向量数据库',
             'rebuild-index'),
             indexActionButton('清空向量数据库',
             'clear-index')),
             confirmation,
            ...
            ?? 0} 条不会参与查询。`) : null,
             progress.fingerprint ? note(`索引指纹：${progress.fingerprint.length > 20 ? `${progress.fingerprint.slice(0,
             20)}…` : progress.fingerprint}`) : null,
             progress.embedding ? note(`嵌入元数据：${progress.embedding.embedding_model} · 修订 ${progress.embedding.embedding_revision} · 维度 ${progress.embedding.dimensions
            ?? '默认'} · task=${progress.embedding.task} · pooling=${progress.embedding.pooling}`) : null,
             note(`所选对话向量：${progress.vectors} · 待处理 ${progress.pending} · 运行中 ${progress.running} · 失败 ${progress.failed} · 结果未知 ${progress.unknown}`),
             note(`共享数据库磁盘占用：${bytes(progress.bytes)}`),
             progress.lastError ? h('div',
             {
            className: 'dsh-rp-error'
        ...
        const catalogCard = h('div', {
            className: 'dsh-rp-retrieval-card'
             ...
             null,
             '本地模型下载'),
             note('本组服务器语义 MRR 实测推荐：Jina Nano > Nomic v2 MoE > Qwen3 0.6B > BGE small > Jina Small INT8。排名仅基于 26 条正文／29 道有答案题的小样本召回结果；BGE small 适合中文、速度与内存优先；与 Small INT8 的 MRR 差距很小，在线 API 优先节省本地内存。未测 E5、BGE-M3 不列入排名。'),
             note('运行环境会按需安装或更新；已校验的模型文件会复用，不会重复下载。'),
            ...
            return h('div', {
                className: 'dsh-rp-item', key: model.id
                 ...
                 null,
                 model.name),
                 note(`${model.languages} · ${model.status
                ?? 'not-installed'}`),
                 model.license ? note(`许可：${model.license}`) : null,
                 model.description ? note(model.description) : null,
                 note(`${bytes(model.downloadedBytes)} / ${bytes(model.totalBytes)}`),
                 h('div',
                 {
    ...
    function MemoryPanel({ sessionId, LegacyPanel }: Props) {
    ...
    function RebuildPrompt({ sessionId }: {
        ...
        const requestRef = React.useRef(0),
            sessionRef = React.useRef(sessionId),
            inFlightRef = React.useRef(false),
            busyRef = React.useRef(false),
            pendingRef = React.useRef<RebuildPending | null>(null);
        ...
        const load = React.useCallback(async () => {
            ...
            try {
                const next = await jsonFetch<{
                    ...
                    error?: string;
                }>(`/api/roleplay/retrieval-confirmations?sessionId=${encodeURIComponent(sessionId)}`);
                if (request !== requestRef.current || sessionRef.current !== sessionId)
                    return;
                if (next.ok === false)
        ...
        const submit = async (action: 'confirm' | 'cancel') => {
            ...
            try {
                const next = await jsonFetch<{
                    ok?: boolean;
                    error?: string;
                }>('/api/roleplay/retrieval-confirmations', {
                    method: 'POST',
                    headers: {
                        'content-type': 'application/json'
                    },
        ...
        const close = (event: ReactAPI.SyntheticEvent<HTMLDialogElement>) => {
            event.preventDefault();
            event.stopPropagation();
            void submit('cancel');
        };
        const trap = (event: ReactAPI.KeyboardEvent<HTMLDialogElement>) => {
            if (event.key === 'Escape') {
                event.preventDefault();
                event.stopPropagation();
                void submit('cancel');
            ...
            if (!focusable.length)
                return;
            const current = focusable.indexOf(document.activeElement as HTMLElement),
                 next = (current + (event.shiftKey ? -1 : 1) + focusable.length) % focusable.length;
            event.preventDefault();
            focusable[next]?.focus();
        };
        if (!pending)
        ...
        return h('dialog', {
            ...
            id: 'dsh-rp-rebuild-title'
             ...
             configure ? '粗颗粒度需要小说检索模型' : '需要确认重建语义索引'),
             note(configure ? '粗颗粒度研究需要可用的小说专用 Embedding 模型；当前生成轮次已停止。请先配置并测试小说模型。' : `用途：${purpose}`),
             configure ? null : note(`模型：${pending.model}`),
             configure ? null : note(`目标：${pending.target}`),
             note(`原因：${pending.reason}`),
             configure ? note('确认后只关闭提示，不会启动索引重建。') : note('在线模型重建可能产生 API 费用；确认后将在后台启动。'),
             h('div',
             {
            className: 'dsh-rp-row'
        }, h('button', {
