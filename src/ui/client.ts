...
import { createConversationPresentation, type SessionSelector } from './conversation-presentation.js'
import type * as ReactAPI from 'react'
import type { SessionList, SessionSummary } from './conversation-projection.js'
...
declare function require(name: '@deepseek-ai/dsh-client-ui-primitives'): {
  Tooltip: ReactAPI.ComponentType<{
    label: string;
    side: string;
    delayMs: number;
...
type RemoteSession = NonNullable<ActionDependencies['remoteSession']> & {
  subscribeState?(listener: () => void): () => void;
  subscribe?(listener: () => void): () => void
}
interface RemoteCommands {
  execute(id: string, line: string, images: unknown[]): Promise<{
    ok?: boolean;
    error?: {
      code?: unknown;
      message?: unknown
...
interface Slots {
  ...
  register<P>(
    config: {
      ...
      order?: number;
      label?: string;
      inject?: (sessionId: string) => Partial<P>
    },
    component: ReactAPI.ComponentType<P>): unknown
}
interface Services {
...
export function apply(ctx: ClientContext) {
  ...
  if (slots !== undefined) slots.inject(
  ...
  const {
    ...
    wakeSessionForState,
    peekState,
    stateVersion } = createRoleplayStateStore(
      {
        sessionsService,
        isRoleplaySession
      })
  const {
    ...
    pendingPlayerBubble,
    DeferredPlayerInput } = createActivityComponents(
      {
        React,
        isRoleplaySession,
        peekState
  ...
  if (slots !== undefined) slots.inject(
    'conversation.chat.roleplay-progress',
    () => slots.register(
...

  {
    ...
    const {
      ...
      SettingsPanel } = createAuthorPanels(
        {
          React,
          sessionDrafts,
          fetchState,
    ...
    function RoleplayManager({ sessionId, onClose }: { sessionId: string | null | undefined; onClose: () => void }) {
      ...
      const [tab, setTabRaw] = React.useState(
        typeof managerState.requestedTab === 'string' ? managerState.requestedTab : (typeof savedDraft.tab === 'string' ? savedDraft.tab : 'cards'))
      ...
      React.useEffect(
        () => {
          const onNeedsConfig = (event: Event) => {
            const detail = (event as CustomEvent).detail;
            if (detail?.sessionId && detail.sessionId !== sessionId) return;
            if (detail?.action !== 'configure-adaptation-model') return;
            setTab('card-adaptation')
          }
          window.addEventListener(
            'dsh-roleplay-needs-config',
            onNeedsConfig);
      ...
      const dialogProps = {
        ...
        onCancel: (event: ReactAPI.SyntheticEvent<HTMLDialogElement>) => {
          event.preventDefault();
          onClose()
        }
      }
      if (!roleplay || !sessionId) return React.createElement(
        'dialog',
        dialogProps,
        React.createElement('div', { className: 'dsh-rp-muted' }, '当前会话不是角色扮演会话'),
        React.createElement('button', { className: 'dsh-rp-btn', onClick: onClose }, '关闭'))
      const selected = panelTabs.find(([id]) => id === tab)
      const Body = selected?.[2] ?? CardsPanel
      const primaryTabs: [string, string][] = [
        ['core-rules', '核心设定'],
        ['cards', '人物设定'],
        ['worldbook', '世界书'],
        ['plot-guidance', '剧情指引'],
      ...
      const secondaryTabs: [string, string][] = [
        ...
        ['userinfo', '用户信息'],
        ['logs', '日志']]
      const tabButton = ([id, label]: [string, string]) => React.createElement(
        'button',
        {
      ...
      const tabs = React.createElement(
        'nav',
        { className: 'dsh-rp-dialog-tabs' },
        React.createElement('div', { className: 'dsh-rp-dialog-tab-row' }, primaryTabs.map(tabButton)),
        React.createElement('div', { className: 'dsh-rp-dialog-tab-divider', 'aria-hidden': 'true' }),
        React.createElement('div', { className: 'dsh-rp-dialog-tab-row' }, secondaryTabs.map(tabButton)))
      ...
      return React.createElement(
        ...
        React.createElement(
          'div',
          { className: 'dsh-rp-dialog-head' },
          React.createElement('strong', null, '酒馆管理'),
          React.createElement('button', { className: 'dsh-rp-btn', onClick: onClose }, '关闭')),
        tabs,
        body)
    }
    ...
    const renderManager = () => managerRoot.render(
      managerState.open
        ...
        ? React.createElement(
          React.Fragment,
          null,
          React.createElement(ManagerEntry, { useSessions: managerState.useSessions }),
          React.createElement(
            RoleplayManager,
            {
        ...
        : null)
    const openManager = (sessionId: string | null | undefined, useSessions?: SessionSelector, requestedTab?: string, requestedFocusNonce = 0) => {
      managerState = { sessionId, useSessions, open: true, requestedTab, requestedFocusNonce };
      renderManager()
    }
    const onNeedsConfig = (event: Event) => {
      const detail = (event as CustomEvent).detail;
      if (detail?.forwarded || detail?.action !== 'configure-adaptation-model' || !detail?.sessionId) return;
      ...
      } else openManager(detail.sessionId, undefined, 'card-adaptation', 1)
    }
    ...
    const sidebarIcon = (kind: string) => React.createElement(
      'svg',
      {
        viewBox: '0 0 16 16',
        fill: 'none',
    ...
    const sidebarTooltip = (props: SidebarProps, label: string, button: ReactAPI.ReactNode) => React.createElement(
      require('@deepseek-ai/dsh-client-ui-primitives').Tooltip,
    ...
    const managerButton = (props: SidebarProps) => {
      ...
      return sidebarTooltip(
        ...
        React.createElement(
          ...
          React.createElement('span', { className: 'dsh-rp-nav-icon' }, sidebarIcon('tavern')),
    ...
    if (slots !== undefined) {
      slots.inject(
        'sidebar.workspaces.before',
        () => slots.register({ name: 'sidebar.workspaces.before', id: 'roleplay-panel-entry', order: 20, inject: () => ({}) }, managerButton))
      slots.inject(
        'sidebar.workspaces.header.action',
        () => slots.register({ name: 'sidebar.workspaces.header.action', id: 'roleplay-status-mode', order: 20, inject: () => ({}) }, statusModeButton))
      slots.inject(
        'sidebar.workspaces.header.action',
        () => slots.register({ name: 'sidebar.workspaces.header.action', id: 'roleplay-decision-toggle', order: 21, inject: () => ({}) }, decisionButton))
    }
      ...
      overlayRender = () => {
        root.render(
          React.createElement(
            ...
            overlaySessionId && isRoleplaySession(overlaySessionId)
