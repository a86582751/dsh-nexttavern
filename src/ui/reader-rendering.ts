export { decodeStatusTemplate, statusFactsHtml } from './status-rendering.js';
...
function cssBlockEnd(css: string, openIndex: number) {
    let depth = 1;
    let quote = '';
    let comment = false;
...
function scopeCssText(css: unknown, roots: readonly string[]): string {
    ...
    while (cursor < input.length) {
        ...
        if (/^@(media|supports|container|layer|document)\b/i.test(trimmed)) {
            ...
            const selectors = header.slice(leading.length).split(',').map((selector) => {
                ...
                if (!value) return '';
                if (roots.some((root) => value === root
                    || value.startsWith(root + ' ')
                    || value.startsWith(root + ':')
                    || value.startsWith(root + '['))) return value;
...
export function scopeHtmlStyles(html: unknown, roots: readonly string[]) {
    return String(html ?? '').replace(
    /<style(\s[^>]*)?>([\s\S]*?)<\/style>/gi,
        (_match, attributes = '', css) => '<style' + attributes + '>' + scopeCssText(css, roots) + '</style>'
    );
}

export function scopeReaderCss(css: unknown) {
...
function sanitizeReaderStyle(styleText: unknown) {
...
function sanitizeReaderTree(root: DocumentFragment | Element) {
    for (const element of Array.from(root.querySelectorAll('*'))) {
        ...
        for (const attribute of Array.from(element.attributes)) {
            ...
            if (!allowed || name.startsWith('on') || name === 'srcdoc' || name === 'href' || name === 'src'
                || name === 'xlink:href') {
...
function paragraphizeReaderContainer(container: DocumentFragment | Element) {
    ...
    for (const child of Array.from(container.childNodes)) {
        if (child.nodeType === 3) {
            for (const chunk of String(child.nodeValue ?? '').split(/(\r?\n+)/)) {
                if (/^\r?\n+$/.test(chunk)) {
                    ...
                    (paragraph as HTMLParagraphElement).appendChild(document.createTextNode(chunk));
                }
            }
            continue;
        }
        ...
        if (READER_BLOCK_TAGS.has(element.tagName)) {
            flushParagraph();
            if (READER_PARAGRAPH_CONTAINERS.has(element.tagName)) paragraphizeReaderContainer(element);
...
function decorateReaderSemantics(root: DocumentFragment | Element) {
...
function applyReaderInlineMarkdown(root: DocumentFragment | Element) {
    const nodes = [];
    const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT);
...
function applyReaderBlockMarkdown(root: DocumentFragment | Element) {
...
export async function renderReaderNarrativeAsync(raw: unknown, rules: ReaderRule[], run = runReaderRegex) {
    ...
    for (const i of source.deferred) {
        const rule = rules[i]!, nodes: Text[] = [], walker = document.createTreeWalker(template.content, NodeFilter.SHOW_TEXT);
        while (walker.nextNode()) if (walker.currentNode.parentElement
            && !walker.currentNode.parentElement.closest('code,pre,script,style')) nodes.push(walker.currentNode as Text);
...
function applyReaderQuoteFallback(root: DocumentFragment | Element, rule: ReaderRule) {
...
function applyReaderHtmlRule(template: HTMLTemplateElement, rule: ReaderRule) {
