...
interface ModelCatalogEntry extends ModelRoute {
    label?: string;
    reasoning?: {
        efforts?: {
            id: string;
...
interface ModelSettings {
    ...
    inherit?: boolean;
    allMain?: boolean;
    routes?: Record<string, ModelRoute | null>;
    main?: ModelRoute;
}
...
interface ClusterSettings {
    enabled?: boolean;
    revision: unknown;
    defaultRoute?: ModelRoute | null;
    characters?: Record<string, ModelRoute | null>;
}
interface ClusterReply {
    settings: ClusterSettings;
    session: ClusterSettings;
...
interface ModelDraft extends PanelDraft {
...
export function createManagementPanels({ React, sessionDrafts, jsonFetch, toast, confirmWithDialog, btn }: ManagementDependencies) {
    function CharacterClusterPanel({ sessionId }: {
        ...
        const [dataValue, setData] = React.useState<ClusterReply | null>(null),
            [modelsValue, setModels] = React.useState<ModelsReply | null>(null),
            [draftValue, setDraft] = React.useState<ClusterSettings | null>(null),
        ...
        React.useEffect(() => {
            ...
            setError('');
            Promise.all([
                jsonFetch<ClusterReply>('/api/roleplay/character-cluster?sessionId=' + encodeURIComponent(sessionId)),
                jsonFetch<ModelsReply>('/api/roleplay/models?sessionId=' + encodeURIComponent(sessionId))
            ])
                .then(([d, m]) => {
                if (!live)
                    return;
        ...
        const save = async (scope: 'global' | 'session') => {
            if (saving)
                return;
            setSaving(true);
            try {
                const d = await jsonFetch<ClusterReply>('/api/roleplay/character-cluster', {
                    method: 'POST',
                    headers: {
        ...
        return h('div', {
            ...
            onChange: (e: ReactAPI.ChangeEvent<HTMLInputElement>) => setDraft({
            ...
            className: 'dsh-rp-muted'
             ...
             '仅当前对话生效。选择“使用集群默认模型”即可跟随上方已保存的全局默认设置。'),
             data.characters.map(character => selector(character.name,
    ...
    function ModelPanel({ sessionId }: {
        ...
        const [data, setData] = React.useState<ModelsReply | null>(null),
            [scope, setScope] = React.useState<'session' | 'global'>('session'),
            [saving, setSaving] = React.useState(false),
            [allMain, setAllMain] = React.useState(false),
            [routes, setRoutes] = React.useState<Record<string, ModelRoute | null>>({});
        React.useEffect(() => {
            let live = true;
            jsonFetch<ModelsReply>(`/api/roleplay/models?sessionId=${encodeURIComponent(sessionId)}`).then(value => live
        ...
        const save = async () => {
            ...
            try {
                ...
                const updated = await jsonFetch<ModelsReply>('/api/roleplay/models', {
                    method: 'POST',
                    headers: {
                        'content-type': 'application/json'
                    },
        ...
        return React.createElement('fieldset', {
            ...
            className: 'dsh-rp-input',
            value: scope,
            onChange: (e: ReactAPI.ChangeEvent<HTMLSelectElement>) => setScope(e.target.value as 'session' | 'global')
            ...
            type: 'checkbox',
            checked: allMain,
            onChange: (e: ReactAPI.ChangeEvent<HTMLInputElement>) => rememberModel(e.target.checked, routes)
            ...
            const updateEffort = (effort: string) => rememberModel(allMain, {
                ...routes, [id]: {
            ...
            return React.createElement('div', {
                ...
                onChange: (e: ReactAPI.ChangeEvent<HTMLSelectElement>) => rememberModel(allMain, {
                ...
                onChange: (e: ReactAPI.ChangeEvent<HTMLSelectElement>) => updateEffort(e.target.value)
            }, effortOptions.map(option => React.createElement('option', {
                key: option.id || 'default', value: option.id
            }, option.name as ReactAPI.ReactNode)))), followsMain ? React.createElement('span', {
                className: 'dsh-rp-muted'
    ...
    function ResourcesPanel({ sessionId }: {
        ...
        const [data, setData] = React.useState<ResourcesReply | null>(null),
        ...
        const load = React.useCallback(async () => {
            ...
            try {
                const {
                    response,
                    raw,
                } = await fetchRoleplayText(`/api/roleplay/resources?sessionId=${encodeURIComponent(sessionId)}`);
        ...
        const view = async (resource: Resource) => {
            const ticket = ++previewSequence.current;
            setPreview(null);
            try {
                const value = await jsonFetch<ResourcePreview>(`/api/roleplay/resource?sessionId=${encodeURIComponent(sessionId)}&resourceId=${encodeURIComponent(resource.id)}`);
        ...
        return React.createElement('div', {
            ...
            'aria-label': '资源类型',
            value: type,
            onChange: (e: ReactAPI.ChangeEvent<HTMLSelectElement>) => setType(e.target.value)
        }, React.createElement('option', {
            value: 'all'
            ...
            'aria-label': '资源排序',
            value: order,
            onChange: (e: ReactAPI.ChangeEvent<HTMLSelectElement>) => setOrder(e.target.value)
        }, [
    ...
    function ExportPanel({ sessionId }: {
        ...
        const refresh = React.useCallback(() => {
            ...
            return jsonFetch<{
                jobs?: ExportJob[];
            }>(`/api/roleplay/jobs?sessionId=${encodeURIComponent(sessionId)}`).then(d => {
        ...
        const action = async (action: string, jobId: string) => {
            if (mutationBusy.current)
                return;
            mutationBusy.current = true;
            setBusy(true);
        ...
        const renderJob = (job: ExportJob) => React.createElement('div', {
