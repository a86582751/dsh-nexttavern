...
export interface BackgroundJob extends Record<string, unknown> {
  kind?: unknown
  status?: unknown
  createdAt?: unknown
}

export interface PendingPlayer extends Record<string, unknown> {
  messageId?: unknown
  text?: string
}

export interface ActivityState extends Record<string, unknown> {
  running?: unknown
  stage?: unknown
  elapsedMs?: unknown
  observedAt?: unknown
...
export interface BackgroundNotesPresentation {
  label: string
  detail: string
  seconds: number | null
  stale: boolean
...
const readCollectionValue = (value: unknown, key: unknown): unknown => {
  if (!isRecord(value) || typeof value.get !== 'function') return undefined
  return value.get(key)
}

const activityOf = (value: unknown): ActivityState | null => isRecord(value) ? value : null

export function tavernActivityPresentation(activity: ActivityState | null | undefined, now = Date.now()): ActivityPresentation | null {
...
const backgroundJobsOf = (activity: ActivityState): BackgroundJob[] =>
  Array.isArray(activity.backgroundJobs)
    ? activity.backgroundJobs.filter(isRecord).map(job => job as BackgroundJob)
    : []

export function backgroundNotesPresentation(activity: ActivityState | null | undefined, sessionId: string | null | undefined, now = Date.now()): BackgroundNotesPresentation | null {
...
interface ActivitySnapshot extends Record<string, unknown> {
  activity?: unknown
}

interface ChatNode extends Record<string, unknown> {
  kind?: unknown
}

interface ChatSnapshot extends Record<string, unknown> {
  order?: unknown
  nodes?: unknown
}
...
interface DeferredPlayerProps {
  sessionId: string
  hasNativePending?: boolean
  useChat?: (selector: (state: ChatSnapshot | null | undefined) => ChatSnapshot | null | undefined) => ChatSnapshot | null | undefined
}

export interface ActivityComponentDependencies {
  React: typeof ReactAPI
  isRoleplaySession: (sessionId: string) => boolean
  peekState: (sessionId: string) => ActivitySnapshot | null | undefined
}

export interface ActivityComponents {
  useTavernActivity: (sessionId: string | null | undefined, seed?: ActivityState | null) => ActivityState | null
  ActivityBanner: (props: { activity?: ActivityState | null }) => ReactAPI.ReactNode
  BackgroundNotesBanner: (props: { activity?: ActivityState | null; sessionId?: string | null }) => ReactAPI.ReactNode
  pendingPlayerBubble: (activity?: ActivityState | null, nodes?: readonly ChatNode[], hasNativePending?: boolean) => ReactAPI.ReactNode
  DeferredPlayerInput: (props: DeferredPlayerProps) => ReactAPI.ReactNode
}

export function createActivityComponents({ React, isRoleplaySession, peekState }: ActivityComponentDependencies): ActivityComponents {
  const useTavernActivity = (sessionId: string | null | undefined, seed: ActivityState | null = null): ActivityState | null => {
    ...
    React.useEffect(() => {
      if (!sessionId) return
      return startActivityPolling<ActivityState | null>({
        ...
        receive: data => { setRunning(!!(data && data.running)); if (data) setActivity(data) },
  ...
  const ActivityBanner = ({ activity }: { activity?: ActivityState | null }): ReactAPI.ReactNode => {
  ...
  const BackgroundNotesBanner = ({ activity, sessionId }: { activity?: ActivityState | null; sessionId?: string | null }): ReactAPI.ReactNode => {
    const [now, setNow] = React.useState(Date.now())
  ...
  const pendingPlayerBubble = (activity?: ActivityState | null, nodes: readonly ChatNode[] = [], hasNativePending = false): ReactAPI.ReactNode => {
    const pending = activityOf(activity)?.pendingPlayer
    const pendingRecord = isRecord(pending) ? pending as PendingPlayer : null
    if (!pendingRecord || hasNativePending || nodes.some(node => node.kind === 'user' && readerMessageId(node) === pendingRecord.messageId)) return null
    return React.createElement('div', { className: 'rp-pending-player', 'data-roleplay-pending-player': pendingRecord.messageId },
      React.createElement('div', null, pendingRecord.text || '已发送附件'),
      React.createElement('small', null, activity?.stage === 'paused' ? '消息已保留 · 等待继续' : '已发送 · 正在准备场景'))
  }

  function DeferredPlayerInput(props: DeferredPlayerProps): ReactAPI.ReactNode {
    const activity = useTavernActivity(props.sessionId, peekState(props.sessionId)?.activity as ActivityState | null | undefined)
    const useChat = typeof props.useChat === 'function' ? props.useChat : (() => null)
    const chat = useChat(state => state)
    const nodes: ChatNode[] = chat?.nodes && Array.isArray(chat.order) && isRecord(chat.nodes)
