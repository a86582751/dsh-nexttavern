...
export interface SurfaceRoot {render(node: ReactAPI.ReactNode): void;unmount(): void;}
...
interface DecisionSeatDependencies {
...
export function createDecisionSeat({ React, createRoot, resolveActiveSessionId, toast }: DecisionSeatDependencies) {

    function DecisionCard({ decision, decisionMinimized, onMinimize, onDismiss, fillInput, renderText }: DecisionProps) {
        const cardRef = React.useRef<HTMLDivElement>(null),
             dragRef = React.useRef<{pointerId: number;x: number;y: number;offset: Offset;} | null>(null),
             offsetRef = React.useRef({ x: 0,
             y: 0 });
        ...
        const reclamp = () => { const next = keepVisible(offsetRef.current); if (next.x !== offsetRef.current.x
            || next.y !== offsetRef.current.y) moveTo(next); };
        ...
        () => {
            ...
            return () => { window.removeEventListener('resize',
                 resize); view?.removeEventListener('resize',
                 resize); view?.removeEventListener('scroll',
                 resize); };
        },
            []
        );
        ...
        const chooseOptions = (indices: number[]) => chooseCustom('我选择：' + indices.map(index => renderText(decision.options[index]?.label
            ?? '')).filter(Boolean).join('、'));
                        ...
                        'input',
                            {
                                ...
                                onKeyDown: (e) => {
                                    if (e.key === 'Enter') {
                                        e.preventDefault();
                                        e.stopPropagation();
                                        if (!(e as ReactAPI.KeyboardEvent<HTMLInputElement> & {isComposing?: boolean;}).isComposing
    ...
    const decisionSeat = (() => {
        let host: HTMLDivElement | null = null,
             root: SurfaceRoot | null = null,
             props: DecisionSeatProps | null = null,
        ...
        const render = () => {
            const current = props;
            if (root) root.render(current ? React.createElement(
            DecisionCard,
                {
                    ...current,
