...
import type { HarnessPatch } from './patch-harness.mjs';
interface AccessOptions { apply?: boolean; stopped?: boolean; root?: string; group?: string; patches?: string; backup?: string; rollback?: string }
const options: AccessOptions = {}, args = process.argv.slice(2);
...
for (let i = 0; i < args.length; i++) {
  ...
  if (['apply','stopped'].includes(key)) options[key as 'apply' | 'stopped'] = true;
  else if (['root','group','patches','backup','rollback'].includes(key)) options[key as 'root' | 'group' | 'patches' | 'backup' | 'rollback'] = args[++i];
...
if (options.rollback) {
  ...
  const data = JSON.parse(fs.readFileSync(file, 'utf8')) as {schemaVersion: number; kind: string; patches: HarnessPatch[]};
