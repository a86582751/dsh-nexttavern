...
export function transform(bytes: Buffer, patch: HarnessPatch) {
  ...
  if (inputHash === patch.after) return {state: 'already-patched', bytes};
  if (inputHash !== patch.before) throw Error('Unknown or partially modified fingerprint: ' + patch.package + '/' + patch.file);
  let position = 0;
  const chunks: Buffer[] = [];
  for (const edit of patch.edits) {
    if (!Number.isSafeInteger(edit.offset) || !Number.isSafeInteger(edit.delete) || edit.offset < position || edit.delete < 0 || edit.offset + edit.delete > bytes.length || typeof edit.insert !== 'string') throw Error('Invalid patch delta');
  ...
  if (digest(output) !== patch.after) throw Error('Patch output fingerprint mismatch');
  return {state: 'upstream', bytes: output};
}
export function auditPatches({harness, patches}: {harness: string; patches: HarnessPatch[]}) {
  harness = fs.realpathSync(harness);
  const require = createRequire(path.join(harness, 'package.json'));
  const results: {unit?: string; package: string; version: string; file: string; before: string; after: string; state: string}[] = [], files: TransactionFile[] = [];
  for (const patch of patches) {
    ...
    let manifest;
    try {manifest = resolver.resolve(patch.package + '/package.json');}
    catch (error) {
      ...
      for (const directory of resolver.resolve.paths(patch.package) ?? []) {
        ...
        if (fs.existsSync(candidate) && (JSON.parse(fs.readFileSync(candidate, 'utf8')) as {name?: string}).name === patch.package) {manifest = candidate; break;}
      }
      if (!manifest) throw Error('Cannot find package metadata: ' + patch.package);
    }
    manifest = fs.realpathSync(manifest);
    const pkg = JSON.parse(fs.readFileSync(manifest, 'utf8')) as {name?: string; version: string};
    if (pkg.name !== patch.package || pkg.version !== patch.version) throw Error('Unsupported package version: ' + patch.package);
    ...
    if (result.state !== 'already-patched') files.push({path: relative, before: digest(original), bytes: result.bytes});
  }
  if (new Set(files.map(file => file.path)).size !== files.length) throw Error('Duplicate resolved patch target');
...
export function patchCli(args = process.argv.slice(2)) {
  ...
  for (let i = 0; i < args.length; i++) {
    const key = args[i]!;
    if (['--apply','--stopped'].includes(key)) options[key.slice(2) as 'apply' | 'stopped'] = true;
    else if (['--harness','--patches','--backup','--rollback'].includes(key)) options[key.slice(2) as 'harness' | 'patches' | 'backup' | 'rollback'] = args[++i];
    else throw Error('Unknown option: ' + key);
  }
  if (options.apply || options.rollback) {
    if (process.env.DSH_ALLOW_HARNESS_PATCH !== '1' || !options.stopped) throw Error('Explicit stopped-instance acknowledgement and DSH_ALLOW_HARNESS_PATCH=1 required');
  }
  if (options.rollback) {console.log(JSON.stringify(rollbackTransaction(options.rollback), null, 2)); return;}
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) patchCli();
