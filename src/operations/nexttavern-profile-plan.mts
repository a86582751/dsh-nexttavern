...
type ProfileReader = Pick<typeof import('@deepseek-ai/dsh-app-boot'),
  'loadProfileDirectory' | 'readProfilePatches' | 'composeEntries'>
export interface ProviderIdentity {id: string; module: string}
export interface ProviderIntent {
  /** Exact nesting is retained: an inactive parent must not be bypassed. */
  path: readonly string[]
  row: EntryOptions
  ancestors: readonly EntryOptions[]
}
...
function find(rows: readonly EntryOptions[], id: string): ProviderIntent | undefined {
  const found: ProviderIntent[] = []
  function walk(entries: readonly EntryOptions[], ancestors: readonly EntryOptions[]) {
    for (const row of entries) {
      if (row.id === id) found.push({path: [...ancestors.map(parent => parent.id!), id], row, ancestors})
      if (row.group && Array.isArray(row.config)) walk(row.config, [...ancestors, row])
    }
  }
  walk(rows, [])
  if (found.length > 1) throw Error(`Ambiguous profile row: ${id}`)
  return found[0]
}
 ...
 */
export function captureNextTavernProfilePlan(context: ProfileContext, api: ProfileReader) {
  ...
  if (productLayers.length > 1) throw Error('NextTavern appears more than once in the profile bundle list')
  const complete = api.readProfilePatches(NEXTTAVERN_BUNDLE, launch, profile)
  const withoutProduct: typeof complete = []
  let offset = 0
  for (const layer of profile.layers) {
    const end = offset + layer.patches.length
    if (layer.packageName !== NEXTTAVERN_BUNDLE) withoutProduct.push(...complete.slice(offset, end))
    offset = end
  }
  ...
  return Object.freeze({
    ...
    provider(identity: ProviderIdentity): ProviderIntent | undefined {
     ...
     */
    assertVersion(expected: string) {
      if (expected !== version) throw Error('Profile composition changed; prepare a new product activation')
    },
  })
}
