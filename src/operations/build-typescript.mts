...
export interface CompilePlan {
  ...
  typeScript: {
    ...
    contexts?: Record<string, {
      ambientDeclarations?: string[]; rootDirs?: string[]
...
const readJson = <T,>(file: string): T => JSON.parse(fs.readFileSync(file, 'utf8').replace(/^\uFEFF/, ''))
const normalize = (text: string) => text.replace(/\r\n/g, '\n')
const inside = (repo: string, relative: string) => {
  if (typeof relative !== 'string' || !relative || relative.includes('\\') || relative.includes(':') || path.posix.isAbsolute(relative) || relative.split('/').some(part => !part || part === '.' || part === '..')) throw Error('Unsafe TypeScript path')
...
export function compileTypeScript(repo = root, suppliedPlan?: CompilePlan): Compilation {
  ...
  const declarationRoots = plan.typeScript.declarationPackages.map(directory => {
    ...
    for (const file of ['package.json','package-lock.json']) {
      if (!plan.artifacts.some(artifact => artifact.source === directory + '/' + file)) throw Error('Unregistered declaration package: ' + directory)
    }
  ...
  for (const context of contexts) {
    ...
    for (const fallback of fallbacks) {
      ...
      if (!fallback.to || fallback.to.startsWith('.') || path.isAbsolute(fallback.to)) throw Error('Invalid declaration fallback target')
    }
    ...
    const verifiedTypePackages = new Set<string>()
    const externalTypeInput = (source: CompilerAPI.SourceFile) => {
      ...
      if (!source.isDeclarationFile && !source.fileName.endsWith('.json')) return false
      ...
      if (!packagePath || !root.lock.packages[packagePath]?.version) throw Error('Unpinned TypeScript declaration: ' + relative)
      if (!verifiedTypePackages.has(packageKey)) {
        const installed = readJson<{version?: string}>(path.join(repo, prefix, packagePath, 'package.json'))
        if (installed.version !== root.lock.packages[packagePath]!.version) throw Error('Type declaration differs from lockfile: ' + packagePath)
    ...
    for (const source of sources) {
      ...
      inside(repo, source)
      if (!plan.artifacts.some(artifact => artifact.source === source) || !recipes.some(recipe => recipe.entry === source || recipe.inputs.includes(source))) throw Error('Unregistered TypeScript dependency: ' + source)
      if (/\.(tsx?|mts)$/.test(source) && !/\.d\.(ts|mts)$/.test(source) && !recipes.some(recipe => recipe.entry === source)) throw Error('TypeScript dependency has no output mapping: ' + source)
    }
...
export function checkTypeScriptOwnership(repo = root, plan?: CompilePlan) {
  ...
  let files = 0
  const scan = (directory: string, sourceTree: boolean) => {
    const absolute = inside(repo, directory)
    if (!fs.existsSync(absolute) || !fs.statSync(absolute).isDirectory()) throw Error('Missing TypeScript directory: ' + directory)
    for (const item of fs.readdirSync(absolute, { withFileTypes: true }).sort((left, right) => left.name.localeCompare(right.name))) {
      const file = directory + '/' + item.name
      if (item.isDirectory()) {
        if (!['node_modules', '.git', '__pycache__'].includes(item.name)) scan(file, sourceTree)
        continue
      }
      ...
      files += 1
      if (sourceTree) { findings.push(file + ': JavaScript inside src/'); continue }
      const entry = outputs.get(file)
      if (entry) {
        // A mapping alone is not an explanation; the mapped source must exist.
        if (!fs.existsSync(inside(repo, entry)) || !fs.statSync(inside(repo, entry)).isFile()) findings.push(file + ': registered as the output of ' + entry + ', which does not exist; restore that source or delete this output')
        continue
      }
      if (allBuildOutputs.has(file)) continue
      findings.push(file + ': no registered build output; lib/ only contains generated artifacts')
    }
  }
  for (const directory of sourceRoots) scan(directory, true)
  for (const directory of outputRoots) scan(directory, false)
  if (findings.length) throw Error('Unaccounted JavaScript in TypeScript-owned directories (' + findings.length + '/' + files + '):\n  - ' + findings.join('\n  - '))
...
export function writeTypeScriptBuild(compilation: Compilation, entry: string | undefined, output: string | undefined, repo = root) {
