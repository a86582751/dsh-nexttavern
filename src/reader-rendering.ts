...
export async function renderReaderNarrativeAsync(raw: unknown,rules: ReaderRule[],run=runReaderRegex) {
  ...
  for(const i of source.deferred) {
    ...
    while(walker.nextNode())if(walker.currentNode.parentElement&&!walker.currentNode.parentElement.closest('code,pre,script,style'))nodes.push(walker.currentNode as Text)
