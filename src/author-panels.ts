...
interface RecordVersions extends Record<string, unknown> {cards?: Record<string,unknown>;worldbook?: Record<string,unknown>}
interface WorldbookEntry {id: string;name?: string;kind?: string;content?: string;keywords?: string[];priority?: number;tokenBudget?: number;alwaysOn?: boolean;locked?: boolean;enabled?: boolean}
interface CardEntry {id: string;name?: string;kind?: string;content?: string;locked?: boolean;version?: number}
interface WorldbookForm {id: string;name?: string;kind?: string;content?: string;keywords: string;priority: number | string;token_budget: number | string;always_on: boolean;locked: boolean}
interface CardForm {card_id: string;name?: string;kind?: string;content?: string;locked: boolean}
interface AuthorDraft extends PanelDraft {
  summary?: string | null; form?: WorldbookForm | CardForm;values?: Record<string,number | null>
  statusText?: string | null; core?: string | null; plot?: string | null; narrative?: string | null; reply?: string | null; styleKw?: string | null; opening?: string | null; beautyCss?: string | null; beautyJs?: string | null; beautyRules?: string | null
}
interface MemoryPolicy {defaults: Record<string,number>;global:{settings:Record<string,number | null>;revision:unknown};session:{settings:Record<string,number | null>;revision:unknown}}
export interface AuthorState extends StateReply {
  recordVersions?: RecordVersions
  directorNotes?: {text?: string}
  memory?: {summary?: string;lockedFacts?: (string|{text:string})[];deltas?: {evidenceSeq?: number;status?: string;summary?: string}[]}
  worldbook?: WorldbookEntry[]
  cards?: CardEntry[]
  rules?: {core?:string;plot?:string;style?:string;narrative?:string;reply?:string;beauty?:{regexRules?:unknown[];css?:string;js?:string}}
  statusSpec?: {text?:string};opening?: {text?:string}
}
interface PanelProps {scope?: {sessionId: string};visible?: boolean;retrievalControls?:ReactAPI.ReactNode}
interface ShellProps {title:string;children?:ReactAPI.ReactNode;visible?:boolean;sessionId?:string;error?:unknown;loading?:boolean;refresh():void;extra?:ReactAPI.ReactNode}
interface AuthorDependencies {
  ...
  saveState(body: Record<string,unknown>): Promise<{recordVersions:RecordVersions}>
...
export function createAuthorPanels({React,sessionDrafts,fetchState,invalidateState,saveState,runMaintenance,jsonFetch,toast}: AuthorDependencies) {
    const btn = (label: ReactAPI.ReactNode, onClick: ReactAPI.MouseEventHandler<HTMLButtonElement>, extra: ReactAPI.ButtonHTMLAttributes<HTMLButtonElement> = {}) =>
    ...
    const useRoleplayState = (sessionId: string | undefined, visible: boolean | undefined) => {
      ...
      React.useEffect(() => {
        ...
        const load = () => {
          ...
          }).catch((e) => { if (alive) setError(String(errorMessage(e))) }).finally(() => { if (alive) setLoading(false) })
        }
        load()
    ...
    const PanelShell = ({ title, children, visible, sessionId, error, loading, refresh, extra }: ShellProps) =>
      React.createElement(
        'div',
    ...
    // ── 记忆面板 ──
    function MemorySettingsControls({sessionId,visible,onSaved,retrievalControls}: {sessionId?: string;visible?: boolean;onSaved?(): void;retrievalControls?:ReactAPI.ReactNode}) {
      const [scope,setScope]=React.useState<'session'|'global'>('session'),[policyValue,setPolicy]=React.useState<MemoryPolicy | null>(null),[values,setValues]=React.useState<Record<string,number | null>>({}),[error,setError]=React.useState<string | null>(null),[saving,setSaving]=React.useState(false),[revision,setRevision]=React.useState(0)
      ...
      const fields: Array<[string,string,number]>=[['contextWindowTokens','窗口大小',1000],['continuityTailTokens','窗口尾部连续正文保留量',1000],['autoNotesEveryTurns','后台笔记更新频率',1]]
      const legacy: Array<[string,string,number]>=[['targetContextTokens','超出模型上下文前的兼容整理阈值',1],['archiveTokens','每次兼容归档量',1]]
      const save=async(reset=false)=>{
        ...
        try{
          const p=await jsonFetch<MemoryPolicy>('/api/roleplay/memory-settings',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({sessionId,scope,settings,expectedRevision:sessionDrafts.get(draftKey)?.expectedRevision??policy[scope].revision})})
          sessionDrafts.delete(draftKey);setPolicy(p);setValues(p[scope].settings);onSaved?.();toast(scope==='global'?'全局记忆设置已保存':'会话记忆设置已保存')
        }catch(e){setError(String(errorMessage(e)));toast('记忆设置保存失败：'+String(errorMessage(e)))}finally{setSaving(false)}
      }
      const input=([field,label,min]: [string,string,number])=>{
        ...
        return React.createElement('label',{key:field,className:'dsh-rp-field',style:{flex:'1 1 200px',minWidth:0}},label,
          React.createElement('input',{className:'dsh-rp-input',type:'number',min,step:1,disabled:saving,value:values[field]??'',placeholder:`继承：${inherited}`,onChange:(e: ReactAPI.ChangeEvent<HTMLInputElement>) => change(field,e.target.value)}),
      ...
      return React.createElement('section',{'data-roleplay-memory-settings':scope},
        ...
        React.createElement('div',{className:'dsh-rp-row',role:'group','aria-label':'记忆设置范围'},(['session','global'] as const).map(s=>React.createElement('button',{key:s,type:'button',className:'dsh-rp-btn'+(scope===s?' dsh-rp-primary':''),'aria-pressed':scope===s,disabled:saving,onClick:()=>setScope(s)},s==='session'?'本会话':'全局默认'))),
    ...
    function MemoryPanel(props: PanelProps) {
      ...
      const rememberMemory = (patch: PanelDraft) => { const prior=sessionDrafts.get(`${sessionId}:memory`);markPanelDraftFields(sessionDrafts, sessionId, 'memory', patch); if (!prior?.dirty) updatePanelDraft(sessionDrafts, sessionId, 'memory', { baseVersions: data?.recordVersions }); setDirty(true) }
      ...
      const organize = async () => {
        ...
        catch (error) { toast('记忆整理失败：' + String(errorMessage(error))) }
      ...
      const save = async () => {
        ...
        try {
          ...
          toast('记忆已保存')
          refresh()
        } catch (e) { toast('保存失败：' + String(errorMessage(e))) }
      ...
      return React.createElement(PanelShell, {
        ...
        React.createElement('textarea', { className: 'dsh-rp-textarea', rows: 14, style: { minHeight: '260px', height: 'clamp(260px, 45vh, 460px)', resize: 'vertical' }, value: summary ?? '', onChange: (e: ReactAPI.ChangeEvent<HTMLTextAreaElement>) => { setSummary(e.target.value); rememberMemory({ summary: e.target.value }) }, placeholder: '尚无导演笔记' }),
        ...
        React.createElement(MemorySettingsControls,{sessionId,visible,onSaved:refresh,retrievalControls:props.retrievalControls}),
    ...
    function WorldbookPanel(props: PanelProps) {
      ...
      const [saving, setSaving] = React.useState(false)
      const setForm = (next: WorldbookForm) => { setFormState(next); const prior = sessionDrafts.get(sessionId + ':worldbook'); updatePanelDraft(sessionDrafts, sessionId, 'worldbook', { form: next, dirty: true, baseVersion: prior?.dirty ? prior.baseVersion : data?.recordVersions?.worldbook?.[next?.id] ?? 'missing' }) }; const rememberForm = setForm
      React.useEffect(() => { const draft = sessionDrafts.get(`${sessionId}:worldbook`); if (draft?.dirty && draft.form) { setFormState(draft.form as WorldbookForm); setEditing(draft.form as WorldbookForm) } }, [sessionId])
      ...
      const save = async () => {
        ...
        try {
          ...
          toast('世界书条目已保存')
          if (sessionDrafts.get(sessionId + ':worldbook') === draftAtSubmit) sessionDrafts.delete(sessionId + ':worldbook')
          setEditing(null)
          refresh()
        } catch (e) { toast('保存失败：' + String(errorMessage(e))) }
        finally { setSaving(false) }
      ...
      const remove = async (id: string) => {
        try {
          ...
          toast('已删除')
          refresh()
        } catch (e) { toast('删除失败：' + String(errorMessage(e))) }
      }
      const field = (label: string, value: string | number | undefined, onChange: (value: string) => void, type = 'text') =>
        React.createElement('label', { className: 'dsh-rp-row' },
          React.createElement('span', { className: 'dsh-rp-muted', style: { width: '64px' } }, label),
          React.createElement('input', { className: 'dsh-rp-input', type, value: value ?? '', onChange: (e: ReactAPI.ChangeEvent<HTMLInputElement>) => onChange(e.target.value) })
        )
      return React.createElement(PanelShell, {
        ...
        editing
          ? React.createElement('fieldset', { disabled: saving, style: { border: 0, padding: 0, margin: 0 } },
            React.createElement('div', { className: 'dsh-rp-item' },
              ...
              React.createElement('textarea', { className: 'dsh-rp-textarea', value: form.content ?? '', onChange: (e: ReactAPI.ChangeEvent<HTMLTextAreaElement>) => rememberForm({ ...form, content: e.target.value }), placeholder: '条目内容（触发时以隐藏上下文注入）' }),
              React.createElement('div', { className: 'dsh-rp-row' },
                ...
                React.createElement('label', null, React.createElement('input', { type: 'checkbox', checked: form.always_on === true, onChange: (e: ReactAPI.ChangeEvent<HTMLInputElement>) => rememberForm({ ...form, always_on: e.target.checked }) }), '旧卡常驻兼容（建议整理到核心设定）'),
                React.createElement('span', { className: 'dsh-rp-muted' }, '新背景资料放核心设定；普通世界书按查询读取。'),
                React.createElement('label', null, React.createElement('input', { type: 'checkbox', checked: form.locked === true, onChange: (e: ReactAPI.ChangeEvent<HTMLInputElement>) => rememberForm({ ...form, locked: e.target.checked }) }), '锁定')
              ),
              React.createElement('div', { className: 'dsh-rp-row' },
    ...
    function CardsPanel(props: PanelProps) {
      ...
      const form = formValue!
      const [saving, setSaving] = React.useState(false)
      const setForm = (next: CardForm) => { setFormState(next); const prior = sessionDrafts.get(sessionId + ':cards'); updatePanelDraft(sessionDrafts, sessionId, 'cards', { form: next, dirty: true, baseVersion: prior?.dirty ? prior.baseVersion : data?.recordVersions?.cards?.[next?.card_id] ?? 'missing' }) }; const rememberForm = setForm
      React.useEffect(() => { const draft = sessionDrafts.get(`${sessionId}:cards`); if (draft?.dirty && draft.form) { setFormState(draft.form as CardForm); setEditing(draft.form as CardForm) } }, [sessionId])
      ...
      const save = async () => {
        ...
        try {
          ...
          toast('人物设定已保存')
          if (sessionDrafts.get(sessionId + ':cards') === draftAtSubmit) sessionDrafts.delete(sessionId + ':cards')
          setEditing(null)
          refresh()
        } catch (e) { toast('保存失败：' + String(errorMessage(e))) }
        finally { setSaving(false) }
      }
      const field = (label: string, value: string | undefined, onChange: (value: string) => void) =>
        React.createElement('label', { className: 'dsh-rp-row' },
          React.createElement('span', { className: 'dsh-rp-muted', style: { width: '64px' } }, label),
          React.createElement('input', { className: 'dsh-rp-input', value: value ?? '', onChange: (e: ReactAPI.ChangeEvent<HTMLInputElement>) => onChange(e.target.value) })
        )
      return React.createElement(PanelShell, {
          ...
          ? React.createElement('fieldset', { disabled: saving, style: { border: 0, padding: 0, margin: 0 } },
            React.createElement('div', { className: 'dsh-rp-item' },
              ...
              React.createElement('label', { className: 'dsh-rp-row' },
                ...
                React.createElement('select', { className: 'dsh-rp-input', value: form.kind, onChange: (e: ReactAPI.ChangeEvent<HTMLSelectElement>) => rememberForm({ ...form, kind: e.target.value }) },
                  React.createElement('option', { value: 'user' }, 'user（玩家角色）'),
                  React.createElement('option', { value: 'npc' }, 'npc'),
                  React.createElement('option', { value: 'other' }, 'other')
                )
              ),
              React.createElement('textarea', { className: 'dsh-rp-textarea', value: form.content ?? '', onChange: (e: ReactAPI.ChangeEvent<HTMLTextAreaElement>) => rememberForm({ ...form, content: e.target.value }), placeholder: '人设正文（性格/口吻/动机/秘密/外貌/能力/价值底线）' }),
              React.createElement('label', null, React.createElement('input', { type: 'checkbox', checked: form.locked === true, onChange: (e: ReactAPI.ChangeEvent<HTMLInputElement>) => rememberForm({ ...form, locked: e.target.checked }) }), ' 锁定事实（压缩时永不被改写）'),
              React.createElement('div', { className: 'dsh-rp-row' },
                btn(saving ? '保存中…' : '保存', save),
    ...
    function RuleSlicePanel({ scope, visible, field, title, guidance, rows }: PanelProps & {field: 'core'|'plot'|'styleKw'; title: string; guidance: string; rows: number}) {
      const sessionId = scope?.sessionId
      const { data, error, loading, refresh } = useRoleplayState(sessionId, visible)
      const [value, setValue] = React.useState<string | null>(null)
      const [saving, setSaving] = React.useState(false)
      const remember = (next: string) => { const prior = sessionDrafts.get(`${sessionId}:settings`); markPanelDraftFields(sessionDrafts, sessionId, 'settings', { [field]: next }); if (!prior?.dirty) updatePanelDraft(sessionDrafts, sessionId, 'settings', { baseVersions: data?.recordVersions }) }
      ...
      return React.createElement(PanelShell, { title, visible, sessionId, error, loading, refresh },
         ...
         React.createElement('textarea', { className: 'dsh-rp-textarea', style: { minHeight: `${rows * 20}px` }, value: value ?? '', disabled: loading || !data, onChange: (e: ReactAPI.ChangeEvent<HTMLTextAreaElement>) => { setValue(e.target.value); remember(e.target.value) } }),
        legacyEntries.length ? React.createElement('details', null,
          React.createElement('summary', null, `核心设定·旧卡常驻标记兼容投影（${legacyEntries.length}）`),
          React.createElement('p', { className: 'dsh-rp-muted' }, '原件未改写、可在世界书管理旧记录。以下为只读全文；不会自动迁移、删除或修改数据。'),
          legacyEntries.map(entry => React.createElement('div', { key: entry.id, className: 'dsh-rp-item' }, React.createElement('div', { className: 'dsh-rp-item-title' }, entry.name || entry.id || '未命名条目'), React.createElement('pre', { style: { whiteSpace: 'pre-wrap', overflowWrap: 'anywhere', margin: '8px 0 0' } }, String(entry.content ?? ''))))
        ) : null,
         React.createElement('div', { className: 'dsh-rp-row' }, btn(`保存${title}`, save, { disabled: saving || loading || !data }))
      )
    }
    function CoreRulesPanel(props: PanelProps) { return React.createElement(RuleSlicePanel, { ...props, field: 'core', title: '核心设定', rows: 10, guidance: '世界基础、核心威胁、长期矛盾；这些内容常驻且不参与剧情压缩。' }) }
    function PlotGuidancePanel(props: PanelProps) { return React.createElement(RuleSlicePanel, { ...props, field: 'plot', title: '剧情指引', rows: 10, guidance: '路线/条件/可能结局是未发生的作者指引，不是当前剧情事实。' }) }
    function StyleSpecializationPanel(props: PanelProps) { return React.createElement(RuleSlicePanel, { ...props, field: 'styleKw', title: '文风特化', rows: 10, guidance: '完整保留风格要求和写作示例，示例不视作当前剧情。' }) }
    ...
    function SettingsPanel(props: PanelProps) {
      ...
      const [saving, setSaving] = React.useState(false)
      const rememberSettings = (patch: PanelDraft) => {const prior=sessionDrafts.get(`${sessionId}:settings`);const next=markPanelDraftFields(sessionDrafts, sessionId, 'settings', patch);if(!prior?.dirty)updatePanelDraft(sessionDrafts,sessionId,'settings',{baseVersions:data?.recordVersions});return next}
      React.useEffect(() => { const draft = sessionDrafts.get(`${sessionId}:settings`); if (!draft?.dirty) return; for (const [key, setter] of [['statusText', setStatusText], ['core', setCore], ['plot', setPlot], ['narrative', setNarrative], ['reply', setReply], ['styleKw', setStyleKw], ['opening', setOpening], ['beautyCss', setBeautyCss], ['beautyJs', setBeautyJs], ['beautyRules', setBeautyRules]] as const) if (draft[key] !== undefined) setter(draft[key]) }, [sessionId])
      React.useEffect(() => {
        if (!data) return
      ...
      const save = async (body: {kind: string; expectedRevision?: unknown} & Record<string,unknown>, msg: string) => {
        setSaving(true)
        try {
          const draft=sessionDrafts.get(`${sessionId}:settings`)
      ...
      const ta = (value: string | null, onChange: (value: string) => void, rows = 6) =>
        React.createElement('textarea', { className: 'dsh-rp-textarea', style: { minHeight: rows * 20 + 'px' }, value: value ?? '', onChange: (e: ReactAPI.ChangeEvent<HTMLTextAreaElement>) => onChange(e.target.value) })
