// Generated from runtime/alpha3/ui/session-actions.ts; edit the TypeScript source.
const errorDetails = (value) => value && typeof value === 'object' ? value : {};
export function createRoleplayActions({ sessionsService, workspacesService, remoteSession, resolveActiveSessionId, isRoleplaySession, wakeSessionForState, invalidateState, acceptConversations, loadConversations, toast, fetch = globalThis.fetch, storage, now = Date.now, wait = ms => new Promise(resolve => setTimeout(resolve, ms)) }) {
    ...
    const forkAndPrompt = async ({ sourceSessionId, messageId, userSeq, kind, editedText }) => {
        ...
        let result;
        try {
            result = await childBinding.session.prompt([{ type: 'text', text: promptText }], 'queue', undefined, submission.requestId);
        }
        catch (error) {
