...
import type {NormalizedDecision} from './reader-model.js'
...
interface SurfaceRoot {render(node: ReactAPI.ReactNode): void; unmount(): void}
interface DecisionProps {
  decision: NormalizedDecision
  decisionMinimized: boolean
  onMinimize(value: boolean): void
  onDismiss(): void
  fillInput(value: string): boolean
  renderText(value: unknown): string
}
type DecisionSeatProps = Omit<DecisionProps,'fillInput'> & {sessionId: string}
...
export function createStatusSurface({React,createRoot,resolveActiveSessionId,isRoleplaySession,useTavernActivity,BackgroundNotesBanner,fetchState,stateVersion,subscribeState,invalidateState,applyImmersive,readImmersive,runMaintenance,toast}: SurfaceDependencies) {
    function DecisionCard({ decision, decisionMinimized, onMinimize, onDismiss, fillInput, renderText }: DecisionProps) {
      const cardRef = React.useRef<HTMLDivElement>(null), dragRef = React.useRef<{pointerId: number; x: number; y: number; offset: Offset} | null>(null), offsetRef = React.useRef({ x: 0, y: 0 })
      const [offset, setOffset] = React.useState({ x: 0, y: 0 })
      const [viewportHeight, setViewportHeight] = React.useState(window.visualViewport?.height ?? window.innerHeight)
      const moveTo = (next: Offset) => { offsetRef.current = next; setOffset(next) }
      const keepVisible = (next: Offset) => {
      ...
      const startDrag = (e: ReactAPI.PointerEvent<HTMLDivElement>) => {
        if (e.button !== 0 || (e.target as Element)?.closest?.('button,input') || e.isPrimary === false) return
        e.preventDefault()
        dragRef.current = { pointerId: e.pointerId, x: e.clientX, y: e.clientY, offset: offsetRef.current }
        e.currentTarget.setPointerCapture?.(e.pointerId)
      }
      const moveDrag = (e: ReactAPI.PointerEvent<HTMLDivElement>) => {
        const drag = dragRef.current
        if (drag?.pointerId !== e.pointerId) return
        moveTo(keepVisible({ x: drag.offset.x + e.clientX - drag.x, y: drag.offset.y + e.clientY - drag.y }))
      }
      const endDrag = (e: ReactAPI.PointerEvent<HTMLDivElement>) => {
        if (dragRef.current?.pointerId !== e.pointerId) return
        dragRef.current = null
        if (e.currentTarget.hasPointerCapture?.(e.pointerId)) e.currentTarget.releasePointerCapture(e.pointerId)
      }
      const chooseCustom = (value: string) => { if (fillInput(String(value).trim())) onMinimize(true) }
      const chooseOption = (index: number) => chooseCustom('我选择：' + renderText(decision.options[index]?.label ?? ''))
      const chooseOptions = (indices: number[]) => chooseCustom('我选择：' + indices.map(index => renderText(decision.options[index]?.label ?? '')).filter(Boolean).join('、'))
        ...
        React.createElement('div', { className: cardClass, ref: cardRef, style: { transform: `translate(${offset.x}px, ${offset.y}px)`, '--rp-decision-visible-height': Math.max(84, viewportHeight - 16) + 'px' } },
          React.createElement('div', { className: 'dsh-rp-decision-head', tabIndex: 0, title: '拖动决策卡；双击或按 Home 归位',
            ...
            onKeyDown: e => {
              ...
              const deltas: Record<string, readonly [number, number]> = { ArrowLeft: [-20, 0], ArrowRight: [20, 0], ArrowUp: [0, -20], ArrowDown: [0, 20] }
          ...
          decisionMinimized ? null : React.createElement('div', { className: 'dsh-rp-decision-scroll' },
            ...
            React.createElement('div', { className: 'dsh-rp-decision-custom' },
              React.createElement('input', {
                ...
                onChange: (e: ReactAPI.ChangeEvent<HTMLInputElement>) => setCustom(e.target.value),
                onKeyDown: (e) => { if (e.key === 'Enter') { e.preventDefault(); e.stopPropagation(); if (!(e as ReactAPI.KeyboardEvent<HTMLInputElement> & {isComposing?: boolean}).isComposing && !e.nativeEvent?.isComposing && e.keyCode !== 229 && custom.trim()) chooseCustom(custom) } },
              }),
              React.createElement('button', {
                type: 'button',
    ...
    const decisionSeat = (() => {
      let host: HTMLDivElement | null = null, root: SurfaceRoot | null = null, props: DecisionSeatProps | null = null, observer: MutationObserver | null = null, disposed = false
      const editorForSeat = () => [...document.querySelectorAll<HTMLElement>('[data-composer-input]')]
      ...
      const fill = (raw: unknown, sessionId: string) => {
        ...
        try {
          ...
          if (editor.tagName === 'TEXTAREA' || editor.tagName === 'INPUT') {
            ...
            setter.call(editor, (editor as HTMLInputElement | HTMLTextAreaElement).value + value)
      ...
      return {
        update(next: DecisionSeatProps | null) {
          if (disposed) return
          props = next
    ...
    function StatusOverlay({ sessionId }: {sessionId?: string | null}) {
      ...
      const visiblePoint = (point: Position | null | undefined, margin = 40) => point && Number.isFinite(Number(point.left)) && Number.isFinite(Number(point.top)) &&
        ...
        html
          ? React.createElement('div', {
              ...
              ref: (element:HTMLDivElement|null) => {
                ...
                if(roots.length===1&&['DIV','SECTION','ARTICLE','ASIDE','MAIN'].includes(roots[0]!.tagName))roots[0]!.setAttribute('data-dsh-status-fill-root','')
              },
