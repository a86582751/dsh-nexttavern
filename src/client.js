// Generated from runtime/alpha3/ui/client.ts; edit the TypeScript source.
...
export function apply(ctx) {
    ...
    const notifyConversations = () => { for (const listener of conversationListeners)
        listener(); };
    const acceptConversations = (value, knownIds = conversationCatalog.knownIds) => {
        if (value?.schemaVersion !== 1 || !value.worldlines || !value.conversations)
            return;
        if (conversationCatalog.value && value.revision !== undefined && conversationCatalog.value.revision !== undefined && value.revision < conversationCatalog.value.revision)
            return;
    ...
    function TavernWorkspacePresentation({ browserProps, renderDefault }) {
        ...
        const catalog = React.useSyncExternalStore(React.useCallback(listener => { conversationListeners.add(listener); return () => conversationListeners.delete(listener); }, []), React.useCallback(() => conversationCatalog, []));
        ...
        React.useEffect(() => {
            ...
            if (wait <= 0)
                ...
                catalogTrailing.current = setTimeout(() => { catalogTrailing.current = null; readNow(); }, wait);
            const refresh = () => { if (typeof document === 'undefined' || !document.hidden)
                readNow(); };
        ...
        if (projections.current?.catalog !== catalog)
            projections.current = { catalog, lists: new WeakMap(), workspaces: new WeakMap() };
        if (!catalog.value)
            return React.createElement('div', { role: 'status', style: { padding: 12 } }, catalog.error ?? '正在读取酒馆会话…', catalog.error && React.createElement('button', { onClick: () => void loadConversations().catch(() => { }) }, '重试'));
        const cached = (cache, value, project) => { let result = cache.get(value); if (!result) {
    ...
    // ── 原生 sidebar slot 驱动的单一角色扮演管理面板 ──────────────────────────
    {
        ...
        function RoleplayManager({ sessionId, onClose }) {
            ...
            React.useEffect(() => {
                const onNeedsConfig = (event) => { const detail = event.detail; if (detail?.sessionId && detail.sessionId !== sessionId)
                    return; if (detail?.action !== 'configure-adaptation-model')
                    return; setTab('card-adaptation'); };
            ...
            React.useLayoutEffect(() => { const dialog = dialogRef.current; if (!dialog)
                return; try {
        ...
        const openManager = (sessionId, useSessions, requestedTab, requestedFocusNonce = 0) => { managerState = { sessionId, useSessions, open: true, requestedTab, requestedFocusNonce }; renderManager(); };
        const onNeedsConfig = (event) => { const detail = event.detail; if (detail?.forwarded || detail?.action !== 'configure-adaptation-model' || !detail?.sessionId)
            return; if (managerState.open) {
        ...
        const decisionButton = (props) => { const sessionId = props?.useSessions?.(state => state.current); return sidebarTooltip(props, '显示或隐藏决策卡', React.createElement('button', { type: 'button', className: 'dsh-rp-status-toggle', disabled: !isRoleplaySession(sessionId), 'aria-label': '显示或隐藏决策卡', onClick: () => window.dispatchEvent(new Event('dsh-roleplay-decision-toggle')) }, sidebarIcon('decision'))); };
        if (slots !== undefined) {
