...
type BranchReply<A extends string> = BranchResult & (A extends 'prepare' ? {operationId: string; promptText: string} : A extends 'create-worldline' ? {childSessionId: string} : {})
...
export function createRoleplayActions({sessionsService,workspacesService,remoteSession,resolveActiveSessionId,isRoleplaySession,wakeSessionForState,invalidateState,acceptConversations,loadConversations,toast,fetch=globalThis.fetch,storage,now=Date.now,wait=ms=>new Promise(resolve=>setTimeout(resolve,ms))}: ActionDependencies) {
  ...
  const waitForBranchOperation = async (operationId: string, timeoutMs = 15 * 60 * 1000, options: {requireRequestAdmission?: boolean; admissionGraceMs?: number} = {}) => {
  ...
  const replaceMessage = async ({ sessionId, role, seq, messageId, text }: {sessionId: string; text: string} & ({role: 'user'; seq: number; messageId?: string} | {role: 'assistant'; seq?: number; messageId: string})) => {
  ...
  const forkAndPrompt = async ({ sourceSessionId, messageId, userSeq, kind, editedText }: {sourceSessionId: string; messageId?: string; userSeq?: number; kind: string; editedText?: string | null}) => {
