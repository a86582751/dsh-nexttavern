// Generated from runtime/alpha3/ui/management-panels.ts; edit the TypeScript source.
...
export function createManagementPanels({ React, sessionDrafts, jsonFetch, toast, confirmWithDialog, btn }) {
    function CharacterClusterPanel({ sessionId }) {
        ...
        React.useEffect(() => {
            ...
            Promise.all([jsonFetch('/api/roleplay/character-cluster?sessionId=' + encodeURIComponent(sessionId)), jsonFetch('/api/roleplay/models?sessionId=' + encodeURIComponent(sessionId))])
                .then(([d, m]) => { if (!live)
                return; setData(d); setModels(m); setDraft(d.session); setGlobalRoute(d.global.defaultRoute ?? null); })
                .catch(e => { if (live)
        ...
        const selector = (label, route, onChange, isDefault = false) => {
            ...
            return h('div', { className: 'dsh-rp-item', key: label }, h('div', { className: 'dsh-rp-item-head' }, label), h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(100%,240px),1fr))', gap: '12px', marginTop: '10px' } }, h('label', null, '模型', h('select', { className: 'dsh-rp-input', style: { width: '100%', minWidth: 0 }, 'aria-label': label + '模型', value, onChange: (e) => { const v = e.target.value; if (!v)
                    onChange(null);
                else if (v === 'main')
                    onChange({ main: true });
                else {
    ...
    function ModelPanel({ sessionId }) {
        ...
        React.useEffect(() => { let live = true; jsonFetch(`/api/roleplay/models?sessionId=${encodeURIComponent(sessionId)}`).then(value => live && setData(value)).catch(e => live && setData({ error: String(errorMessage(e)) })); return () => { live = false; }; }, [sessionId]);
        React.useEffect(() => {
            if (!data)
                return;
    ...
    function ResourcesPanel({ sessionId }) {
        ...
        const load = React.useCallback(async () => { const ticket = ++listSequence.current; setLoading(true); try {
            ...
            if (ticket === listSequence.current)
                setData(previous => ({ ...previous, error: String(errorMessage(e)) }));
        }
        finally {
            if (ticket === listSequence.current)
                setLoading(false);
        ...
        const view = async (resource) => { const ticket = ++previewSequence.current; setPreview(null); try {
        ...
        const importResource = async (resource) => { if (!isCardReadableResource(resource)) {
            toast('该资源不是可读角色卡，不能载入当前人设');
            return;
        } if (!await confirmWithDialog(document, '载入会替换当前人设，确定继续？', { title: '载入当前人设', confirmLabel: '确认载入' }))
            return; try {
    ...
    function ExportPanel({ sessionId }) {
        ...
        const refresh = React.useCallback(() => { const ticket = ++refreshSequence.current; refreshInFlight.current++; return jsonFetch(`/api/roleplay/jobs?sessionId=${encodeURIComponent(sessionId)}`).then(d => { if (ticket === refreshSequence.current)
            setJobs(d.jobs ?? []); }).catch(() => { }).finally(() => { refreshInFlight.current--; }); }, [sessionId]);
        React.useEffect(() => { setJobs([]); void refresh(); const timer = setInterval(() => { if (!refreshInFlight.current)
            void refresh(); }, 3000); return () => { clearInterval(timer); refreshSequence.current++; }; }, [refresh]);
        const start = async (kind) => { if (mutationBusy.current)
            return; mutationBusy.current = true; setBusy(true); try {
        ...
        const action = async (action, jobId) => { if (mutationBusy.current)
            return; mutationBusy.current = true; setBusy(true); try {
            await jsonFetch('/api/roleplay/jobs', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ sessionId, action, jobId }) });
            await refresh();
