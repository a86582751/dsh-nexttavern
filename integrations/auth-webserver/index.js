...
export function installConnectionBridge(connection) {
  ...
  connection.requestRejection = (request) => {
    ...
    return rejection === 401 && request[ACCESS_AUTHENTICATED] === true
      ? undefined
      : rejection
  }
  connection.authorizeIndex = (request, response) => request[ACCESS_AUTHENTICATED] === true
    ? true
    : authorizeIndex(request, response)
}

export function markAccessAuthenticated(request) {
  request[ACCESS_AUTHENTICATED] = true
}
