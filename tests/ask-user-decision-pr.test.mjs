...
const result = await registered.execute({ questions: [{ question: '选择？', header: '抉择', options: [{ label: '留下', description: '继续', heart: true }], multi_select: true }] }, { agent: { session: { id: 'session-a', seq: 41 } } })
assert.equal(result.ok, true)
assert.equal(result.presented, true)
assert.deepEqual(decisions, [['session-a', { source: 'tool', seq: 41, question: '选择？', header: '抉择', options: [{ label: '留下', description: '继续', heart: true }], multiSelect: true }]], 'decision provenance uses the public Session seq, not a private log length')
