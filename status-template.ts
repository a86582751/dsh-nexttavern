...
export function cardCodeBlocks(value: unknown) {
  ...
  for (const line of String(value ?? '').replace(/\r\n/g, '\n').split('\n')) {
    if (!fence) {
      ...
      if (open) fence = {marker: open[1]![0]!, length: open[1]!.length, language: open[2]!.trim().toLowerCase(), lines: []}
...
export function statusAuthorRules(value:unknown) {
...
export function statusTemplateHtml(value: unknown) {
  ...
  } else html = parsed.blocks.length || parsed.unclosed !== null ? parsed.outside : source
  return restoreStatusTemplateCss(html, source)
}

export function statusTemplateDiagnostics(value: unknown) {
  // Simple field/text status definitions intentionally use the native fallback.
  if (!/<\/?[a-z][^>]*>|^ {0,3}(?:`{3,}|~{3,})(?:html|css)\b/im.test(String(value ?? ''))) {
    return {renderable: true, mode: 'fields' as const, cssBlocks: 0, errors: [] as string[]}
  }
  const parsed = cardCodeBlocks(value), html = statusTemplateHtml(value), errors: string[] = []
  if (parsed.unclosed === 'html' || parsed.unclosed === 'css') errors.push(`状态栏 ${parsed.unclosed.toUpperCase()} 代码块未闭合`)
  for (const tag of ['style', 'script']) {
    if ((html.match(new RegExp(`<${tag}\\b`, 'gi')) ?? []).length !== (html.match(new RegExp(`</${tag}\\s*>`, 'gi')) ?? []).length) errors.push(`状态栏 ${tag} 标签未闭合`)
  }
  ...
  if (!hasHtml) errors.push('状态栏章节缺少完整 HTML 模板；HTML 与配套 CSS 必须一起提供')
  return {renderable: errors.length === 0, mode: 'author' as const, cssBlocks: (html.match(/<style\b/gi) ?? []).length, errors}
}
