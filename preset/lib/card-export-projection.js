// Generated from runtime/alpha3/core/card-export-projection.ts; edit the TypeScript source.
...
export const stableJson = (value) => JSON.stringify(value, (_key, nested) => nested && typeof nested === 'object' && !Array.isArray(nested) ? Object.fromEntries(Object.entries(nested).sort(([a], [b]) => a.localeCompare(b, 'en'))) : nested);
...
export function renderOrganizedExport(snapshot, title, sections) {
    ...
    const sources = new Map(snapshot.units.map(unit => [unit.id, unit])), used = new Map(), lineCache = new Map(), output = [`# ${safeHeading(title) || '角色扮演设定'}\n`];
    let references = 0;
    for (const section of sections) {
        if (!safeHeading(section.heading) || (section.source_ids !== undefined && !Array.isArray(section.source_ids)) || (section.source_parts !== undefined && !Array.isArray(section.source_parts)))
            throw new Error('章节缺少标题或来源');
        const requested = [...(Array.isArray(section.source_ids) ? section.source_ids : []).map(source_id => ({ source_id })), ...(Array.isArray(section.source_parts) ? section.source_parts : [])];
