// Generated from runtime/alpha3/core/roleplay-core.ts; edit the TypeScript source.
...
export async function apply(ctx, config = {}) {
    ...
    ctx.effect(() => { const tick = () => { if (telemetry.prices().autoSync)
        priceCatalog.refresh(); }; tick(); const timer = setInterval(tick, 6 * 60 * 60 * 1000); timer.unref?.(); return () => clearInterval(timer); }, 'roleplay: automatic price catalog refresh');
    ctx.effect(() => { const tick = () => { if (exchangeRates.state().fetchedAt)
        exchangeRates.refresh(); }; tick(); const timer = setInterval(tick, 60 * 60 * 1000); timer.unref?.(); return () => clearInterval(timer); }, 'roleplay: daily exchange rate refresh');
    ...
    const { sameModelRoute, canonicalModelRoute, executedMainRoute, modelPolicy, taskStory, characterCluster, characterRoster, clusterLoreVisible, clusterPhase, clusterJob, tavernTasks, nativeTask, resumeMemoryWork, resumeStatusMaintenance } = createRoleplayTaskHost({
        T, ctx, config, taskAgents, STATUS_SYSTEM, DECISION_SYSTEM, ORGANIZE_WORKER_SYSTEM, taskDependenciesCurrent,
    ...
    const novelExports = createNovelExports({ table: T.branch, history: session => {
        ...
        }, request: spec => nativeTask({ ...spec, selection: decodeTaskSelection(spec.selection) }),
    ...
    async function resumeNovelExports(session, agent, signal) {
        for (const job of novelExports.list(session).filter(j => ['queued', 'running', 'waiting-main'].includes(j.status)))
    ...
    const { memorySettingsPolicy, contextWindowKey, cloneContextWindow, buildPhaseA, storyWindowSettings, memoryForContext, contextWindowFor, memorySettingFields } = createRoleplayPreparation({
        ...
        cfg,
        svc: { awaitCommitted: (...args) => svc.awaitCommitted(...args), settings: (...args) => svc.settings(...args) },
        ensureBranch,
    ...
    const retrieval = createMemoryRetrieval({ table: T.branch, record: call => telemetry.recordEmbedding(call), session: id => ctx.sessions.get(id),
        scopeOf: session => ctx.get('tavernConversations')?.rootOf(session.id) ?? session.id,
        list: async () => await ctx.sessionQuery?.listSessions?.() ?? [], read: async (id) => { const value = await ctx.sessionQuery.readSession(id); return { session: value.session, events: value.events }; },
    ...
    ctx.on('session/event', (session, event) => { if (['turn/end', 'user/message', 'compaction/end'].includes(event.type))
        retrieval.changed(session); }, { global: true });
    ...
    registerAuthorPrompts({ ctx, T, isRoleplaySession, userValues, characterCluster, characterRoster, CARD_CLASSIFICATION_GUIDE, narrativePresets });
    ...
    const { preparationRecordKey, taskInstruction } = registerRoleplayLoop({
        ...
        importPromptCheckpoint: session => { const p = T.branch.get(importActiveKey(session.id)); return typeof p?.importId === 'string' && typeof p.normalizedSha256 === 'string' ? { importId: p.importId, normalizedSha256: p.normalizedSha256, opening: String(T.opening.get(keyOf(session.id, 'scene'))?.text ?? '没有作者开场，等待玩家行动。') } : null; },
        ctx, T, tavernTasks, clusterJob, isRoleplaySession, characterCluster, activeCardWorkflow, taskAgents, ensureState, clusterPhase, withDecisionMutationLock, normalizeDecisionRecord, resumeStatusMaintenance, resumeMemoryWork,
        resumeCardWorkflows,
    ...
    const { sessionOf } = registerTaskTools({
        ctx, T, clusterJob, resolveRoleplaySession, storyBranchIsActive, characterCluster, cardWorkflowKey, isRoleplaySession, ensureBranch, tavernTasks, activeCardWorkflow, characterRoster, contextWindowKey, clusterLoreVisible,
        startExportJob: (...args) => startExportJob(...args),
    });
    registerAuthorTools({ ctx, T, simpleTool, sessionOf, storyBranchIsActive });
    registerNarrativePresetTool({ ctx, simpleTool, sessionOf, storyBranchIsActive, presets: narrativePresets });
    const workspaceSessionOf = async (exec) => {
        const session = await sessionOf(exec);
        assertWorkspaceSession(session);
        return session;
    };
    const adaptation = registerAdaptationTools({ ctx, table: T.branch, sessionOf: workspaceSessionOf, retrieval, active: storyBranchIsActive,
        selectionStamp: session => { const catalog = ctx.get('tavernConversations'); if (!catalog)
            return session.id; const root = catalog.rootOf(session.id), book = catalog.snapshot().conversations[root]; return !book || book.activeSessionId === session.id ? `${root}:${book?.selectionRevision ?? 0}:${session.id}` : null; },
        askReadingMode: async (exec, target) => {
            if (!exec.agent)
                throw Error('当前上下文无法询问阅读方式');
            const result = await ctx.userQuestions.ask({ agent: exec.agent, signal: exec.signal, questions: [
                    { id: 'reading-mode', header: '阅读方式', question: '这本小说使用哪种改编模式？', options: [
                            { label: '精读', description: '完整阅读全书、逐段笔记并检索查证；精度优先，耗时和用量较高。' },
                            { label: '粗颗粒度', description: '建立全书索引后研究世界观与主角专题，按需回读；适合超长小说，不保证细节无遗漏。' }
                        ] },
                ] });
            const choice = result.answers.find(a => a.id === 'reading-mode');
            if (choice?.custom || choice?.selected.length !== 1 || !['精读', '粗颗粒度'].includes(choice.selected[0]))
            ...
            if (mode === 'coarse' && !protagonist.trim()) {
                const focus = await ctx.userQuestions.ask({ agent: exec.agent, signal: exec.signal, questions: [{ id: 'reading-target', header: '改编主角', question: '粗颗粒度研究重点改编哪位人物？' }] });
                const answer = focus.answers.find(a => a.id === 'reading-target');
                protagonist = answer?.custom ?? answer?.selected.join('、') ?? '';
            }
            return { mode, protagonist, openingPoint: target.openingPoint };
        },
        scopeOf: async (session) => { await ctx.get('tavernConversations')?.ready; return ctx.get('tavernConversations')?.rootOf(session.id) ?? session.id; } });
    ...
    const beforeAdaptedCardWrite = async (exec) => { const session = await sessionOf(exec); if (adaptationIsActive(eventsOf(session)))
        await adaptation.beforeWrite(exec); };
    const beforeCardImport = async (exec) => { const session = await sessionOf(exec); if (adaptationIsActive(eventsOf(session)))
        await adaptation.beforeWrite(exec); };
    // ── 可审计的来源跨度式读卡导入 ────────────────────────────────────────────
    ...
    registerCardAuthoring({ ctx, T, svc, RULE_TEXT_FIELDS, sessionOf, beforeWrite: beforeAdaptedCardWrite });
    registerSessionTools({ ctx, T, simpleTool, sessionOf });
    registerDraftCheck({ ctx, sessionOf: workspaceSessionOf, beforeWrite: beforeAdaptedCardWrite });
    registerRoleplayDiagnosis({ ctx, T, simpleTool, sessionOf, modelPolicy, tavernTasks, telemetry, preparationRecordKey, characterCluster, characterRoster, memorySettingsPolicy, enhancements: async (session) => {
            const info = {};
            try {
                info.retrieval = await retrieval.diagnose(session);
            }
            catch {
                info.retrieval = { available: false, reason: 'RETRIEVAL_STATE_UNAVAILABLE' };
            }
            try {
                const p = narrativePresets.policy(session.id);
                info.presets = { revision: p.revision, conversationId: p.conversationId, scope: p.local ? 'conversation' : 'global', effective: p.effective, name: p.preset.name, readonly: p.preset.readonly, summary: p.summary, total: p.presets.length, tool: 'rp_preset' };
            }
            catch {
                info.presets = { available: false, reason: 'PRESET_STATE_UNAVAILABLE' };
            }
            try {
                const owner = ctx.get('tavernConversations')?.rootOf(session.id) ?? session.id;
                info.research = { conversationId: owner, sources: adaptation.list(owner).map(s => ({ sourceId: s.sourceId, name: s.name, segments: s.segments, read: s.read, reviewed: s.reviewed, status: s.status, indexPolicy: s.indexPolicy })), indexProgressIncluded: false };
            }
            catch {
                info.research = { available: false, reason: 'RESEARCH_STATE_UNAVAILABLE' };
            }
            return info;
        } });
    ...
    registerSessionCommands({ ctx, T, isRoleplaySession, startExportJob: (...args) => startExportJob(...args), svc });
    ...
    ctx.effect(() => ctx.connection.fetch.register({ path: '/api/roleplay/card-adaptation', methods: ['GET', 'POST'], fetch: async (request) => {
            try {
                ...
                assertWorkspaceSession(session);
                const result = body ? await adaptation.mutate(session, body) : await adaptation.view(session, url.searchParams.get('sourceId') ?? undefined, Number(url.searchParams.get('cursor') ?? 0), url.searchParams.get('query') ?? '', url.searchParams.get('noteMode') ?? 'close-reading');
                if (body?.action === 'research-mode' && body.readingMode === 'coarse' && result && typeof result === 'object' && 'code' in result && result.code === 'ADAPTATION_COARSE_MODEL_REQUIRED')
                    taskAgents.get(session.id)?.cancel?.({ kind: 'hook', reason: '粗颗粒度模式尚未配置可用的小说检索模型，本轮已停止；请先在长文本转角色卡中配置模型。' }, { keepInbox: true });
                return new Response(JSON.stringify(result), { headers: { 'content-type': 'application/json' } });
            }
            catch (error) {
                const message = String(error.message);
                return new Response(JSON.stringify({ ok: false, error: message }), { status: message.includes('已更新') ? 409 : 400, headers: { 'content-type': 'application/json' } });
            }
        } }), 'roleplay: novel adaptation management');
    registerTelemetryRoutes({ ctx, resolveRoleplaySession, exchangeRates, priceCatalog, telemetry });
    ctx.effect(() => ctx.connection.fetch.register({ path: '/api/roleplay/retrieval-confirmations', methods: ['GET', 'POST'], fetch: async (request) => {
            try {
                const url = new URL(request.url), body = request.method === 'POST' ? await request.json() : null;
                const session = await resolveRoleplaySession(body?.sessionId ?? url.searchParams.get('sessionId'));
                if (!session)
                    throw Error('角色扮演会话不存在');
                const result = body ? await retrieval.resolveRebuild(session, String(body.id), String(body.action)) : { ok: true, pending: retrieval.pendingRebuilds(session) };
    ...
    ctx.effect(() => ctx.connection.fetch.register({ path: '/api/roleplay/memory-retrieval', methods: ['GET', 'POST'], fetch: async (request) => {
            try {
                const url = new URL(request.url), body = request.method === 'POST' ? await request.json() : null;
                const session = await resolveRoleplaySession(body?.sessionId ?? url.searchParams.get('sessionId'));
                if (!session)
                    return new Response(JSON.stringify({ ok: false, error: '角色扮演会话不存在' }), { status: 404, headers: { 'content-type': 'application/json' } });
                const result = body ? await retrieval.mutate(session, body) : url.searchParams.get('action') === 'search' ? await retrieval.search(session, Object.fromEntries(url.searchParams)) : await retrieval.view(session);
                return new Response(JSON.stringify({ ok: true, ...result }), { headers: { 'content-type': 'application/json' } });
            }
            catch (error) {
                return new Response(JSON.stringify({ ok: false, error: String(error.message) }), { status: String(error.message).includes('已更新') ? 409 : 400, headers: { 'content-type': 'application/json' } });
            }
        } }), 'roleplay: retrieval management');
    const { startExportJob } = registerJobRoutes({ ctx, T, resolveRoleplaySession, novelExports, modelPolicy, beginCardWorkflow, cardWorkflows, cardWorkflowKey, tavernTasks, taskAgents, libraryFor, migrateResources });
    registerSettingsRoutes({ ctx, T, resolveRoleplaySession, modelPolicy, ensureBranch, taskAgents, characterCluster, characterRoster, memorySettingFields, memorySettingsPolicy, svc, narrativePresets });
    registerAvatarRoute({ ctx, T, resolveRoleplaySession, ensureBranch, awaitImportBarrier, importRecordKey, assertImportRecordIntegrity });
    registerMaintenanceRoute({ ctx, resolveRoleplaySession, maintenanceJobs, latestStatusEvent, runStatusObligation, selectedStatusRecord });
    ...
    registerBranchRoutes({ ctx, T, resolveRoleplaySession, cloneBranchRecord, assertStoryBranchActive, withForkMutationLock, forkOperationKey, reconcileCanonicalPlayerVariants, buildForkLookupIndex, userForkContext, locatePlayerRecoveryTarget, assistantMessageId, forkPointerFor, hydrateForkGroup, forkGroupKey, groupMemberForSession, locateForkTarget, bootstrapChildBranch, registerRecoveryFork, registerNativeFork, forkAnchorLockKey, requestUserEvent, forkPendingKey, reconcileNativeFork, replaceAssistantText, replaceUserText });
    registerPanelRoutes({ ctx, T, resolveRoleplaySession, ensureBranch, withDecisionMutationLock, normalizeDecisionRecord, recordVersionsFor, readRoleplayState, svc, RULE_TEXT_FIELDS, withImportLock });
    registerSettingTool({ ctx, T, recordVersionsFor, svc, RULE_TEXT_FIELDS, withImportLock, sessionOf, storyBranchIsActive, tavernTasks, modelPolicy, resolveRoleplaySession,
        selectionStamp: session => { const catalog = ctx.get('tavernConversations'); if (!catalog)
            return null; const root = catalog.rootOf(session.id), book = catalog.snapshot().conversations[root]; return !book || book.activeSessionId === session.id ? `${root}:${book?.selectionRevision ?? 0}:${session.id}` : null; },
