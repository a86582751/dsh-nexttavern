// Generated from runtime/alpha3/core/card-research-planner.ts; edit the TypeScript source.
import { createHash, randomUUID } from 'node:crypto';
const hash = (v) => createHash('sha256').update(v).digest('hex');
const planKey = (owner, id) => `adaptation-research-${hash(owner)}-${id}`;
...
const baseKey = (s) => `adaptation-research-base-${hash(s.rawSha256 + ':' + s.textSha256 + ':v1')}`;
const locks = new Map();
async function lock(key, work) {
    const p = (locks.get(key) ?? Promise.resolve()).catch(() => { }).then(work);
    locks.set(key, p);
    try {
        return await p;
    }
...
const categories = ['world', 'rule', 'faction', 'relation', 'event'];
const allCategories = [...categories, 'character'];
const topics = ['identity', 'motive', 'relations', 'affiliation', 'limits', 'turning-point', 'opening'];
const modes = ['close-reading', 'coarse'];
const queryModes = ['keyword', 'semantic', 'hybrid'];
const statuses = ['choice-required', 'active', 'paused', 'finished'];
const frontierSources = ['target', 'opening', 'coverage', 'relation', 'unknown', 'manual'];
const frontierStates = ['pending', 'queried', 'deferred'];
const hex64 = (v) => typeof v === 'string' && /^[a-f0-9]{64}$/.test(v);
const object = (v) => v !== null && typeof v === 'object' && !Array.isArray(v) ? v : {};
function text(v, name, max = 1600, optional = false) {
...
function normalizedQuery(value) { return value.trim().replace(/\s+/g, ' ').toLocaleLowerCase(); }
function citation(source, raw, label = '引用') {
...
function citationsFromSource(source, raw, label = '事实') {
...
function resolveInputCitations(source, receipts, raw, label = 'citations') {
    if (!Array.isArray(raw) || raw.length < 1 || raw.length > 6)
        throw Error(`${label} 需要 1–6 条原文引用`);
...
function resolveInputFact(source, receipts, input) {
    const result = { ...input, citations: resolveInputCitations(source, receipts, input.citations) };
...
function validateChain(source, raw) {
...
function validateEdges(source, raw) {
...
function factFromStored(source, raw, scope) {
    const value = object(raw), category = String(value.category), certainty = value.certainty;
    ...
    const fact = { id: value.id, category, statement: text(value.statement, '研究条目', 1600), certainty: certainty, citations: [], storyOccursAt: text(value.storyOccursAt, '事件发生时间', 300), readerRevealedAt: text(value.readerRevealedAt, '读者获知位置', 300), characterKnowledge: text(value.characterKnowledge, '角色知情条件', 600) };
    if (category === 'character') {
        fact.entity = text(value.entity, '原著人物', 200);
        fact.topic = text(value.topic, '人物专题', 50);
        if (!topics.includes(fact.topic))
...
export function createResearchPlanner(table, loadSource, assessLongitudinal) {
    function receiptFromStored(source, raw, label = '阅读回执') {
        ...
        return { segment, start, end, sha256: sha256 };
    }
    const packetId = (owner, source, receipt) => hash(`${owner}\u0000${source.id}\u0000${source.textSha256}\u0000${receipt.segment}\u0000${receipt.start}\u0000${receipt.end}\u0000${receipt.sha256}`);
    function packetFromStored(source, owner, raw) {
        ...
        return { ...receipt, id: id, generation, ...(value.acknowledged === true ? { acknowledged: true } : {}) };
    }
    function candidateFromStored(source, raw, label = '检索候选') {
    ...
    function frontierFromStored(source, raw) {
        ...
        if (normalized !== normalizedQuery(query) || !frontierSources.includes(sourceKind) || !frontierStates.includes(state))
        ...
        return { id: value.id, query, normalizedQuery: normalized, source: sourceKind, state: state, priority: Number(value.priority), parentFactIds: [...parentFactIds], candidateRanges: candidates, attemptedFingerprints };
    }
    function validatePlan(source, raw) {
        const value = object(raw);
        if (![1, 2].includes(Number(value.schemaVersion)) || value.kind !== 'adaptation-research-plan' || typeof value.owner !== 'string' || typeof value.sourceId !== 'string' || value.textSha256 !== source.textSha256 || !Number.isSafeInteger(value.revision) || Number(value.revision) < 0 || !(value.mode === null || modes.includes(value.mode)) || !statuses.includes(value.status) || typeof value.generation !== 'string' || !value.generation || value.generation.length > 200)
            throw Error('改编研究计划损坏，不能沿用完成标记');
        const target = object(value.target), budget = object(value.budget);
        const plan = { schemaVersion: 2, kind: 'adaptation-research-plan', owner: value.owner, sourceId: value.sourceId, textSha256: source.textSha256, revision: Number(value.revision), mode: value.mode, status: value.status, generation: value.generation, target: { protagonist: text(target.protagonist, '重点主角', 200, true), openingPoint: text(target.openingPoint, '开局时间点', 500, true) }, budget: { queryBatches: integer(budget.queryBatches, '已用检索批次', 0, 200), maxQueryBatches: integer(budget.maxQueryBatches, '最大检索批次', 1, 200), readPackets: integer(budget.readPackets, '已用阅读包', 0, 500), maxReadPackets: integer(budget.maxReadPackets, '最大阅读包', 1, 500) }, receipts: [], packets: [], queries: [], entries: [], frontier: [], web: [], createdAt: integer(value.createdAt, '计划创建时间', 0, Number.MAX_SAFE_INTEGER), startedSeq: integer(value.startedSeq, '研究开始位置', -1, Number.MAX_SAFE_INTEGER) };
        ...
        plan.queries = value.queries.map(item => {
            ...
            return { query: rawQuery, fingerprint, mode: mode, seq, ranges, candidates };
        });
        ...
        plan.web = value.web.map(item => {
            const web = object(item), url = text(web.url, '网页来源', 2048), callId = text(web.callId, '网页调用 ID', 200), resultSeq = integer(web.resultSeq, '网页结果位置', 0, Number.MAX_SAFE_INTEGER), excerptHash = web.excerptHash;
            if (!/^https?:\/\//i.test(url) || !hex64(excerptHash))
                throw Error('网页低信任账本损坏');
            return { url, callId, resultSeq, excerptHash: excerptHash };
        });
    ...
    function baseRecord(source) {
        ...
        const base = { schemaVersion: 2, kind: 'original-research-base', textSha256: source.textSha256, rawSha256: source.rawSha256, revision: Number(value.revision), entries: value.entries.map(item => factFromStored(source, item, 'base')) };
        unique(base.entries, f => f.id, '共享原著事实 ID');
        return { exists: true, base: structuredClone(base) };
    }
    const base = (source) => baseRecord(source).base;
    const frontierId = (source, normalized) => hash(`${source.textSha256}\u0000${normalized}\u0000frontier-v1`);
    function queueFrontier(plan, source, query, sourceKind, priority, parentFactIds = []) {
        ...
        const item = { id: frontierId(source, normalized), query: text(query, '探索查询', 1000), normalizedQuery: normalized, source: sourceKind, state: 'pending', priority, parentFactIds: [...new Set(parentFactIds)].slice(0, 8), candidateRanges: [], attemptedFingerprints: [] };
        plan.frontier.push(item);
        return item;
    }
    function sampleSegments(source) {
    ...
    function seedFrontier(plan, source) {
    ...
    function queueFactEdges(plan, source, fact) {
        for (const edge of fact.edges ?? [])
            queueFrontier(plan, source, `${edge.from} ${edge.relation} ${edge.to}`, 'relation', 85, [fact.id]);
    }
    function delivery(owner, source, plan, packets) {
        return { schemaVersion: 1, owner, sourceId: source.id, textSha256: source.textSha256, generation: plan.generation, packetIds: packets.map(packet => packet.id) };
    }
    function currentPackets(plan, raw) {
        if (raw === undefined)
            return [];
        if (!Array.isArray(raw) || raw.length > 32 || raw.some(id => !hex64(id)))
            throw Error('source_packet_ids 需要 0–32 个交付包 ID');
        unique(raw, id => id, 'source_packet_ids');
        return raw.map(id => {
    ...
    function checkpoint(owner, source, plan, packets) {
        return { ...delivery(owner, source, plan, packets), revision: plan.revision };
    }
    function acknowledgePackets(plan, packets) {
        for (const packet of packets) {
    ...
    function entries(owner, id) {
        const source = loadSource(owner, id), shared = base(source).entries, privateEntries = load(owner, id)?.entries ?? [], ids = new Set(shared.map(f => f.id));
    ...
    function assess(owner, id) {
        ...
        const bins = Array.from({ length: count }, (_, i) => { const first = Math.floor(i * source.segments.length / count), last = Math.floor((i + 1) * source.segments.length / count) - 1; return { id: i, startSegment: first, endSegment: last, covered: verified.some(f => f.citations.some(c => c.segment >= first && c.segment <= last)) }; }), categoryCoverage = categories.map(id => ({ id, covered: verified.some(f => f.category === id) })), target = plan?.target.protagonist ?? '', missing = [];
        if (!target)
            missing.push('指定重点主角');
    ...
    async function select(owner, id, revision, mode, protagonist = '', openingPoint = '') {
        if (!modes.includes(mode))
            throw Error('请选择精读或粗颗粒度');
        return change(owner, id, revision, plan => {
            plan.mode = mode;
            plan.target = mode === 'coarse' ? { protagonist: text(protagonist, '重点主角', 200), openingPoint: text(openingPoint, '开局时间点', 500, true) } : { protagonist: '', openingPoint: '' };
            if (mode === 'coarse')
    ...
    async function refundRead(owner, id, generation) {
        return lock(planKey(owner, id), async () => {
    ...
    async function control(owner, id, revision, command, input = {}) {
        if (!['pause', 'resume', 'budget'].includes(command))
            throw Error('未知研究控制操作');
        return change(owner, id, revision, plan => {
            if (command === 'budget')
                for (const [field, max] of [['maxQueryBatches', 200], ['maxReadPackets', 500]]) {
    ...
    async function prepareQueries(owner, id, fingerprint, requested = [], mode = 'hybrid', visiblePacketIds) {
        return lock(planKey(owner, id), async () => {
    ...
    function addPackets(plan, source, input) {
    ...
    async function recordBatch(owner, id, generation, requests, valid = () => true) {
        return lock(planKey(owner, id), async () => {
            ...
            try {
                ...
                try {
                    ...
                    throw Error('研究交付保存失败且回滚未完成，请停止研究并检查存储', { cause: rollback });
                }
                throw error;
            }
            return delivered;
    ...
    async function recordPacket(owner, id, generation, rows, query, fingerprint = 'keyword', seq = -1, frontierId, candidates = [], mode = fingerprint === 'keyword' ? 'keyword' : 'semantic', completeQuery = true, valid = () => true) {
        return (await recordBatch(owner, id, generation, [{ rows, query, fingerprint, seq, frontierId, candidates, mode, completeQuery }], valid))[0];
    }
    async function read(owner, id, segment, offset = 0, maxChars = 2400, valid = () => true) {
    ...
    function citations(owner, id, raw) {
    ...
    async function append(owner, id, revision, raws, sourcePacketIds = []) {
        const source = loadSource(owner, id);
        return lock(baseKey(source), async () => lock(planKey(owner, id), async () => {
            ...
            const stored = baseRecord(source), additions = [];
            ...
            const nextPlan = structuredClone(plan), nextBase = { ...stored.base, revision: stored.base.revision + 1, entries: [...stored.base.entries, ...additions] };
    ...
    async function save(owner, id, revision, raw, sourcePacketIds = []) {
    ...
    async function finish(owner, id, fingerprint) {
        return lock(planKey(owner, id), async () => {
            const source = loadSource(owner, id), plan = load(owner, id);
            if (!plan || plan.mode !== 'coarse')
                throw Error('当前不是粗颗粒度改编');
