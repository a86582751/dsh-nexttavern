// Generated from runtime/alpha3/src/memory/memory-provenance.ts; edit the TypeScript source.
...
export function lastSeqOf(session) {
    if (record(session) && Number.isSafeInteger(session.seq))
        return Number(session.seq) - 1;
    const events = eventsOf(session);
    return events.length > 0 ? Number(events.at(-1)?.seq ?? events.length - 1) : -1;
}
export function estimateTokens(text) { return Math.ceil(String(text ?? '').length / 2.5); }
export function textOf(content) {
    if (!Array.isArray(content))
        return '';
    return content.filter(record).filter(block => block.type === 'text' && typeof block.text === 'string').map(block => block.text).join('\n');
}
export function branchScope(session) {
    ...
    if (!parent)
        return { isFork: false, seedLength: null };
    const raw = header?.seedLength;
    return { isFork: true, seedLength: Number.isSafeInteger(raw) && Number(raw) >= 0 ? Number(raw) : null };
}
export function durableSeq(value) { return typeof value === 'number' && Number.isSafeInteger(value) && value >= 0 ? value : null; }
export function provenanceSeqOf(item) {
    if (!record(item))
        return null;
...
export function belongsToBranch(item, session, scope = branchScope(session)) {
    if (!scope.isFork || !record(item))
        return !scope.isFork;
...
export function scopedLedgerItems(items, session, scope = branchScope(session)) {
    return (Array.isArray(items) ? items : []).filter(item => belongsToBranch(item, session, scope)).filter(record);
}
const summarySeqOf = (value) => {
    if (!record(value))
        return null;
    for (const key of ['summaryAtSeq', 'summarySeq', 'surfaceCheckpointSeq', 'lastCompactedSeq']) {
        const seq = durableSeq(value[key]);
        if (seq !== null)
            return seq;
    }
    return null;
};
const inheritedSummaryIsSafe = (value, session, scope, seq) => {
    if (!scope.isFork || scope.seedLength === null || !record(value) || !record(session))
        return false;
...
export function safeSummaryForBranch(value, session, checkpoint = null, scope = branchScope(session)) {
    const point = record(checkpoint) ? checkpoint : null, visibleText = String(point?.text ?? '').trim();
    if (visibleText)
        return { text: visibleText, seq: durableSeq(point?.seq), source: 'surface' };
    const summary = record(value) ? String(value.summary ?? '').trim() : '';
    if (!summary)
        return { text: '', seq: null, source: 'none' };
    const seq = summarySeqOf(value);
    if (!scope.isFork)
        return { text: summary, seq, source: 'ledger' };
    const branchId = record(session) ? String(session.id ?? '') : '';
    const owners = record(value) ? [value.summarySessionId, value.summaryBranchId].map(item => String(item ?? '').trim()).filter(Boolean) : [];
    if (branchId && owners.length > 0 && owners.every(owner => owner === branchId))
        return { text: summary, seq, source: 'child-ledger' };
    if (scope.seedLength !== null && seq !== null && seq < scope.seedLength)
        return { text: summary, seq, source: 'seed-ledger' };
    if (inheritedSummaryIsSafe(value, session, scope, seq))
        return { text: summary, seq, source: 'inherited-ledger' };
    return { text: '', seq: null, source: 'unproven' };
}
export function filterMemoryRecordForBranch(value, session, checkpoint = null) {
    const source = record(value) ? structuredClone(value) : {};
    if (!record(value) && !(record(checkpoint) && checkpoint.text))
        return null;
    const scope = branchScope(session), summary = safeSummaryForBranch(source, session, checkpoint, scope);
    const result = { ...source, summary: summary.text,
    ...
    if (summary.seq !== null)
        result.surfaceCheckpointSeq = summary.seq;
    else if (scope.isFork)
        result.surfaceCheckpointSeq = null;
    if (scope.isFork)
        result.lastCompactedSeq = summary.seq ?? -1;
    return result;
}
...
export function adaptationToolResult(value) {
    ...
    if (!Array.isArray(result.content))
        return null;
    return { text: result.content.flatMap(b => record(b) && b.type === 'text' && typeof b.text === 'string' ? [b.text] : []).join('\n'), failed: value.isError === true || result.isError === true };
}
/** Only a successful, explicit adaptation start opens research mode. Never infer it from novel/user text. */
function adaptationActivity(events) {
    const turns = new Set(), pending = new Map(), closing = new Map();
    let active = false, turn = -1;
    for (const e of events) {
        ...
        if (e.type === 'tool/call' && ['rp_source_begin', 'rp_source_library'].includes(name) && typeof callId === 'string')
            pending.set(callId, { name, turn: Number(e.data?.turn ?? turn) });
        ...
        if (e.type === 'tool/call' && /^rp_source_/.test(name))
            turns.add(Number(e.data?.turn ?? turn));
        if (e.type === 'tool/call' && ['rp_commit_card', 'rp_card_import_begin', 'rp_source_close'].includes(String(e.data?.name)) && typeof callId === 'string')
            closing.set(callId, { name: String(e.data?.name), turn: Number(e.data?.turn ?? turn) });
        if (e.type === 'tool/result') {
            ...
            if (start) {
                ...
                try {
                    const proof = adaptationToolResult(message), result = proof && !proof.failed ? JSON.parse(proof.text) : null;
                    // begin freezes/chooses a source; library only starts after an explicit attach.
                    if (result?.sourceId && (start.name === 'rp_source_begin' || result?.attached === true)) {
                        active = true;
                        turns.add(start.turn);
                    }
                }
                catch { }
            }
            ...
            if (close) {
                ...
                try {
                    const proof = adaptationToolResult(message), result = proof && !proof.failed ? JSON.parse(proof.text) : null;
                    if (result?.ok === true && (close.name !== 'rp_source_close' || ['roleplay', 'authoring'].includes(result.mode))) {
                        active = false;
                        turns.delete(close.turn);
                    }
                }
                catch { }
            }
...
export const adaptationTurns = (events) => adaptationActivity(events).turns;
export const adaptationIsActive = (events) => adaptationActivity(events).active;
...
export function importedStoryProjection(events, nodes) {
    ...
    const prose = new Set(), boundaries = [];
    const calls = new Map();
    let turn = -1, begin = null, final = null, story = false;
    for (const event of events) {
        ...
        if (event.type === 'assistant/message' && selected.has(event.seq) && Array.isArray(message?.content))
            for (const block of message.content) {
                if (!record(block) || block.type !== 'tool-call' || typeof block.id !== 'string' || typeof block.name !== 'string')
                    continue;
                if (/^(?:rp_card_|rp_source_|rp_preset$|rp_diagnose$)/.test(block.name))
                    story = false;
                if (block.name === 'rp_card_import_begin') {
                    begin = null;
                    final = null;
                }
                if (['rp_card_import_begin', 'rp_card_import_finalize'].includes(block.name))
                    calls.set(block.id, { name: block.name, turn });
            }
        if (event.type === 'tool/result' && selected.has(event.seq) && record(message?.source)) {
            ...
            if (call && call.turn === turn) {
                ...
                const decoded = adaptationToolResult(message);
                let proof;
                try {
                    proof = decoded && !decoded.failed ? JSON.parse(decoded.text) : null;
                }
                catch {
                    proof = null;
                }
                if (record(proof) && proof.ok === true && typeof proof.importId === 'string' && /^[a-f0-9]{64}$/.test(String(proof.normalizedSha256))) {
                    if (call.name === 'rp_card_import_begin')
                        begin = { id: proof.importId, hash: String(proof.normalizedSha256) };
                    else if (begin?.id === proof.importId && begin.hash === proof.normalizedSha256 && proof.coverage === 1 && typeof proof.activatedAt === 'number' && Number.isFinite(proof.activatedAt) && proof.activatedAt > 0)
                        final = { ...begin, seq: event.seq };
                }
            }
        }
        ...
        if (story && visible.has(event.seq) && event.type === 'assistant/message' && Number(event.data?.turn) === turn && Array.isArray(message?.content) && !message.content.some(b => record(b) && b.type === 'tool-call') && textOf(message.content).trim())
            prose.add(event.seq);
