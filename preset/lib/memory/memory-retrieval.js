// Generated from runtime/alpha3/src/memory/memory-retrieval.ts; edit the TypeScript source.
...
export function retrievalColdSession(id, header, events) {
    ...
    return {
        id, header, events: events, surface: {
            nodes
        }
    };
}
export function createMemoryRetrieval(deps) {
    ...
    const migration = previous && (previous.conversationScopeSince === undefined || previous.adaptationProviderId === undefined) ? (async () => {
        const since = previous.conversationScopeSince ?? Date.now(), conversationIndex = {
    ...
    function pendingRebuilds(session) {
    ...
    async function setConfiguration(ws, message, generation) {
    ...
    function settings() {
        const value = deps.table.get(SETTINGS);
        if (!value)
            return initial();
    ...
    function pauseConversation(cfg, s) {
        const root = conversation(s);
    ...
    const vectorSources = (session, rows) => eligible(session) ? rows.filter(r => !r.unresolved).map(row => ({
    ...
    async function configure(session) {
        await ensureSessionHistory(session);
        await migration;
        const cfg = settings(), ws = workspace(session), provider = cfg.providers.find(p => p.id === cfg.activeProviderId), enabled = sessionEnabled(cfg, session), generation = hash(JSON.stringify([
            provider?.revision,
            provider?.ready,
            provider?.dimensions,
            enabled,
    ...
    async function adaptationModels(_session) {
        await migration;
        const cfg = settings();
        return {
    ...
    function adaptationCurrent(revision) {
            ...
            if (action === 'mode') {
                ...
                if (!refreshes.has(id)) {
                    const run = (async () => {
                        ...
                        try {
                            const p = secret(provider, cfg), root = p.protocol === 'openai'
                                ? p.baseUrl
                            ...
                            const ids = [
                                ...new Set((Array.isArray(data.data) ? data.data : []).map(r => typeof r === 'object' && r ? String(r.id ?? '') : '').filter(id => id.length <= 200 && id.toLowerCase().includes('embedding')))
                            ].slice(0, 1000);
                            modelLists.set(id, {
                            ...
                            modelLists.set(id, {
                                ids: old?.ids ?? [], fetchedAt: old?.fetchedAt ?? 0, error: String(error.message)
                            });
                        }
                    })();
                ...
                if (action === 'delete-model'
                    && cfg.providers.some(p => (p.id === cfg.activeProviderId || p.id === cfg.adaptationProviderId)
                        && p.kind === 'local'
    ...
    async function search(session, options = {}) {
        ...
        try {
            ...
            const rebuilding = repairable && thisIsCurrent()
                && scheduleLocalRebuild(session, cfg, p, error.code === 'EMBEDDING_INDEX_INVALID');
            ...
            return {
                ...
                requestedMode: mode,
                ...confirmation,
                fallback: rebuilding ? '本地模型配置已变化，已安排当前对话后台重建；本轮使用关键词检索' : confirmation ? `${String(error.message)}；等待玩家在确认框中决定重建，当前只提供关键词结果，不代表语义查询成功` : String(error.message),
                elapsedMs: Date.now() - started
            };
        }
