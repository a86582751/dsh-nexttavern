// Generated from runtime/alpha3/src/memory/memory-history.ts; edit the TypeScript source.
...
export function surfaceSeqsOf(session) {
...
export function contentOfEvent(event) {
...
export function importManagementInputs(session, evidence = eventsOf(session)) {
    const inputs = new Map(), management = new Set();
    const exportTurns = new Set();
    for (const turn of adaptationTurns(evidence))
        exportTurns.add(turn);
    let authoring = false;
    let turn = null;
    for (const event of evidence) {
        if (event.type === 'turn/start')
            turn = Number(event.data?.turn);
        if (event.type === 'user/message' && event.data?.source?.kind === 'user' && turn !== null)
            inputs.set(turn, event.seq);
        if (event.type === 'tool/call' && event.data?.name === 'rp_card_import_begin') {
            const seq = inputs.get(Number(event.data?.turn));
            if (Number.isSafeInteger(seq))
                management.add(seq);
        }
        if (event.type === 'tool/call' && event.data?.name === 'rp_card_draft_check')
            authoring = true;
        if (event.type === 'tool/call' && ['rp_card_import_begin', 'rp_commit_card'].includes(event.data?.name ?? ''))
            authoring = false;
        if (event.type === 'user/message' && event.data?.source?.plugin === 'roleplay-tasks' && event.data.source.stage === 'after-story')
            authoring = false;
        if (authoring && event.type === 'tool/call' && event.data?.name === 'ask_user_question')
            exportTurns.add(Number(event.data?.turn ?? turn));
        if (event.type === 'tool/call' && /^(?:rp_card_export_(begin|chunk|finalize)|rp_novel_export|rp_diagnose|rp_preset|rp_card_draft_check)$/.test(event.data?.name ?? ''))
            exportTurns.add(Number(event.data?.turn ?? turn));
        if (event.type === 'user/message' && event.data?.source?.plugin === 'roleplay-tasks' && ['card-export', 'novel-export'].includes(event.data?.source?.jobKind ?? ''))
            exportTurns.add(turn);
    ...
    for (const event of evidence) {
        if (event.type === 'assistant/message' && !resumed.has(event.seq) && exportTurns.has(Number(event.data?.turn)))
            management.add(event.seq);
    }
    for (const turnId of exportTurns) {
        const seq = inputs.get(turnId);
        if (Number.isSafeInteger(seq))
            management.add(seq);
    }
    return management;
}
export function isStoryEvent(event, canonicalAssistantSeqs = null, management = new Set()) {
    if (!event)
        return false;
    if (management.has(event.seq))
        return false;
    if (event.type === 'assistant/message') {
 ...
 * are ignored.
 */
function afterStoryProofs(session, visible = new Set(surfaceSeqsOf(session)), evidence = eventsOf(session)) {
    ...
    for (const marker of evidence) {
        ...
        if (source?.kind !== 'plugin' || source?.plugin !== 'roleplay-tasks' || source?.form !== 'phase' || source?.stage !== 'after-story')
            continue;
        ...
        if (!visible.has(Number(marker.seq)) || !visible.has(seq) || Number(marker.seq) <= seq ||
            ...
            !Number.isFinite(turn) || Number(story.data?.turn) !== turn)
            continue;
        committed.add(seq);
        turns.add(turn);
    }
...
export function canonicalAssistantSeqsOf(session, seqs = surfaceSeqsOf(session), evidence = eventsOf(session)) {
    ...
    for (const event of evidence) {
        if (event?.type === 'turn/start' || event?.type === 'turn/end')
            inTask = false;
        if (event?.type === 'user/message' && event.data?.source?.kind === 'plugin' && event.data?.source?.plugin === 'roleplay-tasks' && event.data?.source?.form === 'phase')
            inTask = event.data.source.stage !== 'story';
        if (inTask && event?.type === 'assistant/message')
            internal.add(event.seq);
    }
    ...
    for (const event of evidence) {
        if (event?.type !== 'assistant/message' ||
            ...
            !textOf(contentOfEvent(event)).trim())
            continue;
        const turn = Number(event.data?.turn);
        if (afterStory.committed.has(Number(event.seq)))
            lastByTurn.set(turn, Number(event.seq));
        ...
        else if (afterStory.turns.has(turn))
            continue;
        else if (Number.isFinite(turn) && completedTurns.has(turn))
            lastByTurn.set(turn, Number(event.seq));
    }
...
function immutableStoryValue(value) {
    if (value === null || typeof value !== 'object')
        return true;
    if (immutableStoryValues.has(value))
        return true;
    if (!Object.isFrozen(value) || !Object.values(value).every(immutableStoryValue))
        return false;
    immutableStoryValues.add(value);
    return true;
}
function expandedHistorySeqs(session, evidence = eventsOf(session)) {
    ...
    const expand = (seq, depth = 0) => {
        if (typeof seq !== 'number' || durableSeq(seq) === null || visited.has(seq) || depth > 256)
            return;
        visited.add(seq);
        const event = log[seq];
        if (!event || Number(event.seq) !== seq)
            return;
        if (isCompactedStoryEvent(event)) {
            ...
            if (!id || !ended.has(id) || !summary || summary.seq >= seq)
                return;
            const archived = summary.data?.shadowedSeqs;
            if (!Array.isArray(archived) || archived.some((source) => durableSeq(source) === null || source >= seq))
                return;
            for (const source of archived)
                expand(source, depth + 1);
        }
...
export function selectedStoryHistory(session) {
    ...
    if (cached?.log === log && cached.id === session.id && cached.surfaceKey === surfaceKey)
    ...
    const researchClosures = new Set(), researchStarts = new Set();
    let turn = null;
    ...
    for (const event of log) {
        ...
        if (type === 'tool/call' && ['rp_source_begin', 'rp_source_library'].includes(event.data?.name ?? '') && typeof nativeCallId === 'string')
            researchStarts.add(nativeCallId);
        if (type === 'tool/call' && ['rp_source_close', 'rp_commit_card', 'rp_card_import_begin'].includes(event.data?.name ?? '') && typeof nativeCallId === 'string')
            researchClosures.add(nativeCallId);
        ...
        if (relevant)
            evidence.push(event);
        if (cacheable && (!Object.isFrozen(event) || (relevant && !immutableStoryValue(event))))
            cacheable = false;
        if (type === 'turn/start')
            turn = Number(event.data?.turn);
        if (event)
    ...
    const originalTurn = (event, seen = new Set()) => {
        ...
        if (typeof event.surfaceOp === 'object' && event.surfaceOp.op === 'replace')
            return originalTurn(log[event.surfaceOp.start], seen);
        return null;
    };
    ...
    if (cacheable)
        selectedStoryCache.set(session, { log, id: session.id, surfaceKey, rows: rows.map(row => ({ ...row })) });
    return rows;
}
function queryHistoryRows(session, scope) {
    if (scope === 'story' || scope === undefined)
        return selectedStoryHistory(session);
    if (scope !== 'tools')
        throw new Error('历史查询范围无效');
    const log = eventsOf(session);
    return expandedHistorySeqs(session).map(seq => log[seq]).flatMap((event) => {
        ...
        if (event.type === 'tool/result')
            ...
            content = event.data?.message?.content?.filter(block => block.type === 'tool-call');
        else
            return [];
        if (!content?.length)
            return [];
...
export function queryStoryHistory(session, { query = '', beforeSeq, limit = 12, maxChars = 12000, scope = 'story' } = {}) {
    ...
    for (let index = candidates.length - 1; index >= 0; index -= 1) {
        if (results.length >= cap || used >= budget)
            break;
...
export function historySourceKeys(entries) {
...
export function storyCadenceSlots(entries) {
    ...
    for (const entry of entries) {
        if (entry.role !== 'user' || !Number.isFinite(entry.turn) || anchors.has(entry.turn))
            continue;
        anchors.set(entry.turn, entry.seq);
    }
    const seen = new Set(), slots = [];
    for (const entry of entries) {
        if (entry.role !== 'assistant' || !Number.isFinite(entry.turn) || seen.has(entry.turn))
            continue;
        seen.add(entry.turn);
...
export function legacyCadenceSlots(entries, record) {
    ...
    for (const note of Array.isArray(record?.directorCheckpoints) ? record.directorCheckpoints : []) {
        if (Array.isArray(note?.sourceKeys))
            coveredEntries = Math.max(coveredEntries, note.sourceKeys.length);
    }
...
export function memoryNotesCadence(session, record, everyTurns = 3) {
    ...
    while (coveredTurns < slots.length && coveredTurns < storedSlots.length
        && Number(slots[coveredTurns].turn) === Number(storedSlots[coveredTurns]?.turn))
        coveredTurns += 1;
...
export function directorNotesForBranch(value, session, entries = selectedStoryHistory(session)) {
    const object = (input) => input !== null && typeof input === 'object';
    const record = object(value) ? value : {};
    const actual = historySourceKeys(entries);
    const matches = (notes) => Boolean(object(notes) && typeof notes.text === 'string' && notes.text.trim() && Array.isArray(notes.sourceKeys)
    ...
    if (!notes) {
        for (const candidate of Array.isArray(record?.directorCheckpoints) ? record.directorCheckpoints : []) {
            if (candidate?.validated === true && matches(candidate) && (!notes || candidate.sourceKeys.length > notes.sourceKeys.length))
                notes = candidate;
        }
    }
    return notes ? { ...notes, pendingEntries: entries.length - notes.sourceKeys.length } : null;
}
export function isCompactedStoryEvent(event) {
