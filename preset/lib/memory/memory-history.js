...
import { projectStoryEvent, messageViewGeneration } from '../core/roleplay-message-view.js';
...
export function selectedStoryHistory(session) {
    ...
    const originalTurn = (event, seen = new Set()) => {
        ...
        if (typeof event.surfaceOp === 'object' && event.surfaceOp.op === 'replace')
            return originalTurn(log[event.surfaceOp.startSeq], seen);
        return null;
    };
    const rows = expanded.map((seq) => {
        const event = log[seq];
