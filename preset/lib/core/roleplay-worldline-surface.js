...
const isReplacementEvent = (event) => typeof event?.surfaceOp === 'object' && event.surfaceOp?.op === 'replace';
...
export function createWorldlineSurface(deps, forks) {
    const { ctx, safeId, keyOf, sha256, T, eventsOf, surfaceEvents, textOf, cloneBranchRecord, internalTaskSeqs, isRoleplaySession, durableSeq, canonicalAssistantForTurn, surfaceEntries, withDecisionMutationLock, normalizeDecisionRecord, cloneRecord, provenanceSeq, } = deps;
    ...
    const editInvalidationKey = (sessionId, role, replacementSeq) => keyOf(sessionId, `edit-invalidated-${role}-${safeId(replacementSeq)}`);
    ...
    async function repairLegacyRootForkPointer(session, lookup = null) {
        ...
        for (const initial of candidates) {
            if (!initial || initial.rootSessionId !== session.id || !initial.anchor)
                continue;
            const rootMember = initial.members.find((member) => !member.deleted && !member.pending && ['original', 'root'].includes(member.kind) && member.sessionId === session.id);
            if (!rootMember?.assistantMessageId || visibleIds.has(rootMember.assistantMessageId))
                continue;
            const oldSeq = durableSeq(rootMember.assistantSeq);
            if (oldSeq === null)
                continue;
            const placeholders = visible.map((event, position) => ({ event, position })).filter(({ event }) => event?.type === 'user/message' && event.data?.source?.kind === 'plugin' &&
            ...
            await withForkMutationLock(forkAnchorLockKey(initial.anchor), async () => {
                ...
                const member = group?.members?.find((item) => !item.deleted && !item.pending && ['original', 'root'].includes(item.kind) && item.sessionId === session.id);
                if (!group || !member || visibleIds.has(member.assistantMessageId))
                    return;
    ...
    async function nativePlayerGroupsFor(session, assistantGroups) {
        ...
        const publish = (user, value) => {
            ...
            if (!aliases.includes(Number(user.seq)))
                aliases.push(Number(user.seq));
            for (const seq of aliases) {
                if (Number.isSafeInteger(seq))
                    result[String(seq)] = value;
            }
        };
    ...
    function replacementLineageContains(session, event, requestedSeq) {
        ...
        if (target === null || !event || typeof event !== 'object')
            return false;
    ...
    async function repairLegacyUserReplacementIdentities(session) {
        ...
        for (const event of [...surfaceEvents(session)]) {
            if (event?.type !== 'user/message' || event.data?.source?.kind !== 'user' || !isReplacementEvent(event))
                continue;
            const stableId = stableUserMessageId(session, event);
            if (!stableId)
                continue;
            const tagged = Number(event.data?.source?.roleplayRevision) === 1;
            if (tagged && String(event.data?.id ?? '') === stableId)
                continue;
    ...
    async function reconcileCanonicalPlayerVariants(session, lookup = null) {
        ...
        for (const assistant of visibleAssistants) {
            ...
            const initialGroup = pointer?.groupId
                ...
                : null;
            const initialExactMember = initialGroup?.members?.find((member) => member.sessionId === session.id && member.assistantMessageId === messageId);
            if (initialExactMember?.deleted)
                continue;
            const initialMember = initialExactMember ?? groupMemberForSession(initialGroup, session, messageId);
            if (!initialGroup || !initialMember || initialMember.pending)
                continue;
            const initialVariant = initialGroup.playerVariants?.[initialMember.playerVariantId];
            if (!initialVariant || typeof initialVariant.text !== 'string')
                continue;
            ...
            const initialUser = visibleSurfaceEvent(session, (event) => event?.type === 'user/message' && event.data?.source?.kind === 'user' && (identityCandidates.includes(String(event.data?.id ?? '')) ||
                seqCandidates.includes(Number(event.seq)) ||
                seqCandidates.some((seq) => replacementLineageContains(session, event, seq)))) ?? currentSurfaceUserBefore(session, assistant)?.event;
            if (!initialUser)
                continue;
            ...
            if (textOf(initialUser.data?.content).trim() === initialVariant.text.trim() &&
                appliedRevision >= initialRevision && initialIdentityStable)
                continue;
            await withForkMutationLock(forkAnchorLockKey(initialGroup.anchor), async () => {
                ...
                const exactMember = group?.members?.find((item) => item.sessionId === session.id && item.assistantMessageId === messageId);
                if (exactMember?.deleted)
                    return;
                ...
                const liveSeqs = [projection?.userSeq, exactMember?.userSeq, member.userSeq]
                    .map(durableSeq).filter((value) => value !== null);
                const userEvent = visibleSurfaceEvent(session, (event) => event?.type === 'user/message' && event.data?.source?.kind === 'user' && (liveIds.includes(String(event.data?.id ?? '')) ||
                    liveSeqs.includes(Number(event.seq)) ||
                    liveSeqs.some((seq) => replacementLineageContains(session, event, seq)))) ?? currentSurfaceUserBefore(session, assistant)?.event;
                if (!userEvent)
                    return;
    ...
    function userForkContext(session, seq) {
        ...
        const event = visibleSurfaceEvent(session, (candidate) => candidate?.type === 'user/message' && Number(candidate.seq) === requested && candidate.data?.source?.kind === 'user')
            ?? visibleSurfaceEvent(session, (candidate) => candidate?.type === 'user/message' && candidate.data?.source?.kind === 'user' &&
        ...
        const result = await withForkMutationLock(lockKey, async () => {
            ...
            for (const member of targets) {
                ...
                if (!targetSession || !isRoleplaySession(targetSession))
                    continue;
                const targetEvent = visibleSurfaceEvent(targetSession, (candidate) => candidate?.type === 'user/message' && candidate.data?.source?.kind === 'user' && ((member.userMessageId && String(candidate.data?.id ?? '') === String(member.userMessageId)) ||
            ...
            if (matched === 0)
                throw new Error('玩家消息替换未能写入当前 surface');
