// Generated from runtime/alpha3/src/core/roleplay-author-context.ts; edit the TypeScript source.
...
export function renderWorldbookEntry(e) {
    ...
    if (Array.isArray(e.aliases) && e.aliases.length)
        lines.push(`别名：${e.aliases.join('、')}`);
    if (Array.isArray(e.keywords) && e.keywords.length)
        lines.push(`关键词：${e.keywords.join('、')}`);
    if (e.content)
...
export async function retrieveWorldbook(T, branchId, userText, scene, budgetTokens) {
    ...
    for (const [k, v] of T.worldbook.entries()) {
        if (!k.startsWith(prefix) || !v || v.enabled === false)
            continue;
        entries.push(v);
    }
    ...
    for (const e of entries) {
        if (!e.tavern?.useRegex)
            continue;
        const indices = [];
        for (const key of [...(e.keywords ?? []), ...(e.tavern.secondaryKeys ?? [])]) {
            indices.push(requests.length);
            requests.push(worldbookRegex(key, e.tavern.caseSensitive));
        }
    ...
    scored.sort((a, b) => (Number(b.e.priority) ?? 0) - (Number(a.e.priority) ?? 0) ||
        b.score - a.score);
    const picked = [];
    let used = 0;
    for (const s of scored) {
        if (used + s.budgetTokens > budgetTokens)
            continue;
        used += s.budgetTokens;
        picked.push(s);
    }
...
export async function buildDecisionContext(T, branchId, { narrative, userText, scene, directorNotes, memory, budgetTokens = 6000 }) {
    const frozen = {};
    const prefix = `${branchId}__`;
    for (const table of ['cards', 'rules', 'worldbook']) {
        const records = new Map();
        for (const [key, value] of T[table].entries())
    ...
    for (const entry of lore.entries)
        read('worldbook', keyOf(branchId, entry.id));
...
// ── 锁定事实 / 角色卡渲染 ────────────────────────────────────────────────────
export function lockedFactsOf(T, branchId) {
    const prefix = `${branchId}__`;
    const facts = [];
    for (const [k, v] of T.cards.entries()) {
        if (!k.startsWith(prefix) || !v)
            continue;
        if (v.locked === true && v.content)
            facts.push({ source: `card:${v.id}`, text: v.content });
    }
    for (const [k, v] of T.worldbook.entries()) {
        if (!k.startsWith(prefix) || !v)
            continue;
        if (v.enabled !== false && v.locked === true && v.content)
            facts.push({ source: `worldbook:${v.id}`, text: v.content });
    }
    const mem = T.memory.get(keyOf(branchId, 'head'));
    if (mem && Array.isArray(mem.lockedFacts)) {
        for (const f of mem.lockedFacts)
            facts.push({ source: 'memory-locked', text: typeof f === 'string' ? f : f?.text ?? '' });
    }
    return facts.filter((f) => f.text);
}
export function renderCards(T, branchId) {
    const prefix = `${branchId}__`;
    const parts = [];
...
export function residentAuthorContext(T, branchId) {
...
export function authorUserValues(T, branchId, textAlias) {
...
export function registerAuthorPrompts({ ctx, T, isRoleplaySession, userValues, characterCluster, characterRoster, CARD_CLASSIFICATION_GUIDE, narrativePresets }) {
    ctx.effect(() => ctx.systemPrompt.section({ name: 'roleplay:style-policy', order: 140, text: context => {
            const session = context.agent?.session;
            if (!session || Number(context.agent?.options?.subagentDepth) > 0 || !isRoleplaySession(session))
                return '';
    ...
    ctx.effect(() => ctx.systemPrompt.section({
        ...
        text: context => {
            if (Number(context.agent?.options?.subagentDepth) > 0)
                return '';
...
恢复：用工具实际返回的 ID、游标和哈希；漏页/缺项按错误补齐，临时失败以相同参数有限重试，导出期间设定已改变就重新 begin。无效文件、权限错误或 recoveryRequired 时用普通语言说明未完成及可行下一步；不声称成功、不检查服务器代码、不改原卡或绕开校验。详细规范可读取 roleplay-card-authoring 技能，用户不需要承担这些约束。`;
    ...
    ctx.effect(() => ctx.systemPrompt.variable('user', (context) => {
        const session = context.agent?.session;
        return userValues(session?.id).name;
    }), 'roleplay: user variable');
    ctx.effect(() => ctx.systemPrompt.variable('user_gender', (context) => {
    ...
    ctx.effect(() => ctx.systemPrompt.section({ name: 'roleplay:character-cluster', order: 170, text: context => {
            const session = context.agent?.session;
            if (!session || Number(context.agent?.options?.subagentDepth) > 0 || !isRoleplaySession(session) || !characterCluster.read(session).enabled)
                return '';
