// Generated from runtime/alpha3/src/core/roleplay-card-workflow.ts; edit the TypeScript source.
...
export function createCardWorkflows(deps) {
    ...
    function assertCardWorkflow(session, record) {
        ...
        if (!job || job.sessionId !== session.id || ['cancelled', 'stale', 'failed'].includes(job.status) || record.workflowGeneration !== job.generation || !storyBranchIsActive(session))
            throw new Error('角色卡任务授权已取消或来源已失效');
        ...
        try {
            ...
            const result = await nativeTask({ session, agent, kind: job.kind, format: 'workflow', selection: job.selection, source, signal, timeoutMs: 600000,
                ...
                validate: async () => {
                    ...
                    if (!completed)
                        throw new Error('角色卡任务还没有可核验的完整结果，请继续原生工具流程');
            ...
            if (live?.generation === job.generation && live.status !== 'cancelled')
                await T.branch.put(cardWorkflowKey(job.id), { ...live, status: 'completed', result, resourceId: result.resourceId, progress: { done: 1, total: 1 }, completedAt: Date.now() });
        }
        catch (error) {
            const live = T.branch.get(cardWorkflowKey(job.id));
            if (live?.generation === job.generation && live.status !== 'cancelled')
    ...
    async function completeCardWorkflow(session, record, result) {
        if (!record.workflowId)
            return;
        const job = T.branch.get(cardWorkflowKey(record.workflowId));
        if (!job || job.sessionId !== session.id || ['cancelled', 'stale', 'failed'].includes(job.status))
            throw new Error('角色卡任务已失效');
        if (job.kind === 'card-import' ? (!record.importId || record.status !== 'active' || record.rawSha256 !== job.source.sha256) : !record.exportId)
            throw new Error('角色卡任务与完成结果不匹配');
        if (!result.resourceId) {
            await T.branch.put(cardWorkflowKey(job.id), { ...job, status: 'failed', error: '设定已保存，资源入库待重试' });
            return;
        }
        for (const task of tavernTasks.pending(session).filter(t => t.source?.workflowId === job.id)) {
            try {
                await tavernTasks.submit({ session, id: task.id, generation: task.generation, value: { finished: true } });
            }
            catch { }
        }
