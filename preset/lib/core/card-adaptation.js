// Generated from runtime/alpha3/src/core/card-adaptation.ts; edit the TypeScript source.
...
export function createAdaptationStore(table) {
    async function select(owner, id) {
        load(owner, id);
        const current = table.get(selectedKey(owner));
        if (current?.schemaVersion === 1
            && current.sourceId === id)
            return;
        await table.put(selectedKey(owner), { schemaVersion: 1,
            owner,
            sourceId: id });
    }
    ...
    function libraryRecord() {
        ...
        if (value.schemaVersion !== 1
            ...
            || value.assets.some(a => !/^[a-f0-9]{64}$/.test(a.assetId)
                ...
                || a.owners.some(o => typeof o !== 'string')))
    ...
    function original(assetId) {
        ...
        if (!metadata
            ...
            || asset.source.textSha256 !== metadata.textSha256)
    ...
    function library(owner) {
        ...
        return { revision: data.revision,
            assets: data.assets.map(({ owners, ...a }) => ({ ...a,
    ...
    function notebook(source) {
        ...
        if (value.schemaVersion !== 1
            ...
            || value.notes.some(n => !source.segments[n.segment]
                ...
                || !source.text.slice(source.segments[n.segment].start, source.segments[n.segment].end).includes(n.evidence)))
    ...
    async function shareNotebook(owner, source, state) {
        ...
        await locked(notebookKey(source.assetId), async () => {
            const before = notebook(source), next = before ? structuredClone(before) : { schemaVersion: 1,
    ...
    function shared(owner, id) {
        ...
        return { assetId: asset.assetId,
            ...
            indexEnabled: asset.owners.some(o => table.get(stateKey(o, asset.sourceId))?.indexPolicy === 'running') };
    }
    async function bind(owner, metadata, data) {
        ...
        if (!table.get(stateKey(owner, id)))
            await table.put(stateKey(owner, id), { schemaVersion: 1,
    ...
    async function detach(owner, id, expectedRevision, expectedLibraryRevision, reconcile) {
        return locked(libraryKey, async () => {
            ...
            const remaining = { assetId: binding.assetId,
                ...
                indexEnabled: metadata.owners.some(o => table.get(stateKey(o, id))?.indexPolicy === 'running') };
    ...
    function load(owner, id) {
        ...
        const source = reference ? { ...original(reference.assetId).source,
            owner,
            assetId: reference.assetId } : table.get(sourceKey(owner, id));
        ...
        if (!Array.isArray(source.segments)
            ...
            || source.segments.some((p, i) => p.id !== i
                || p.start !== (i ? source.segments[i - 1].end : 0)
                || !Number.isSafeInteger(p.end)
                || p.end <= p.start
                || p.end - p.start > 24000)
            || source.segments.at(-1).end !== source.text.length)
    ...
    async function begin(owner, cwd, path, encoding) {
        ...
        return locked(libraryKey, async () => {
            ...
            if (!ids.includes(id) && ids.length >= 12)
                throw Error('当前会话最多保留 12 份改编资料');
            const source = ids.includes(id) ? load(owner, id).source : { schemaVersion: 1,
    ...
    async function read(owner, id, segment) {
        return locked(stateKey(owner, id), async () => {
            ...
            const result = { sourceId: id,
                ...part,
                segment: part.id,
                sha256: hash(source.text.slice(part.start, part.end)),
                text: source.text.slice(part.start, part.end),
...
export function keywordAdaptation(source, query, limit = 8) {
    ...
    return source.segments.map(segment => {
        ...
        return { segment: segment.id,
            ...
            start,
            end: Math.min(segment.end, start + 1200),
            score,
            text: source.text.slice(start, Math.min(segment.end, start + 1200)) };
