// Generated from runtime/alpha3/src/core/roleplay-task-tools.ts; edit the TypeScript source.
...
export function assertSteeringAgent(agent) {
    if (!agent || typeof agent.steer !== 'function')
        throw new Error('当前会话代理尚未就绪');
}
export const simpleTool = (name, description, parameters, execute) => ({
    name,
    description,
    parameters,
    output: {
...
export function registerTaskTools({ ctx, T, clusterJob, resolveRoleplaySession, storyBranchIsActive, characterCluster, cardWorkflowKey, isRoleplaySession, ensureBranch, tavernTasks, startExportJob, activeCardWorkflow, characterRoster, contextWindowKey, clusterLoreVisible }) {
    const sessionOf = async (exec) => {
        ...
        if (roleJob) {
            if (roleJob.status !== 'running' || roleJob.childSessionId !== session?.id)
                throw new Error('角色推演授权已失效');
            const owner = await resolveRoleplaySession(roleJob.sessionId);
            if (!owner || !storyBranchIsActive(owner) || !characterCluster.read(owner).enabled)
                throw new Error('角色所属世界线已失效');
            return owner;
        }
        ...
        if (descriptor) {
            ...
            if (!task || task.generation !== match[2] || task.source?.workflowType !== 'card' || ['cancelled', 'stale', 'failed', 'completed'].includes(task.status) || task.sessionId !== session.header?.parentSession)
                throw new Error('辅助任务授权已失效');
            const workflow = T.branch.get(cardWorkflowKey(task.source.workflowId));
            if (!workflow || workflow.generation !== task.source.generation || ['cancelled', 'stale', 'failed', 'completed'].includes(workflow.status))
                throw new Error('角色卡任务已取消或替换');
    ...
    ctx.effect(() => ctx.tools.register(simpleTool('rp_novel_export', '将当前分支完整剧情整理为一章一章的 Markdown 小说，包含压缩前历史，排除工具和管理请求。玩家说导出小说或整理完整剧情时使用；任务可恢复并在酒馆管理下载。', { type: 'object', properties: {}, additionalProperties: false }, async (_args, exec) => ({ ok: true, jobId: (await startExportJob(await sessionOf(exec), 'novel-export', exec.agent)).id }))), 'roleplay: novel export tool');
    ctx.effect(() => ctx.tools.register(simpleTool('rp_task_submit', '提交当前循环完成的维护任务结果。id和generation来自系统任务元数据（完整内联或rp_task_read）；result为任务指定JSON对象或完整文本。各任务独立校验。', { type: 'object', properties: { id: { type: 'string' }, generation: { type: 'string' }, result: {} }, required: ['id', 'generation', 'result'], additionalProperties: false }, async (args, exec) => ({ ok: true, result: await tavernTasks.submit({ session: await sessionOf(exec), id: args.id, generation: args.generation, value: args.result }) }))), 'roleplay: task submit tool');
    ctx.effect(() => ctx.tools.register(simpleTool('rp_history', '查询当前世界线的玩家输入和有效剧情正文，包含已归档旧正文。排除状态、样式、工具和其他世界线。search 按玩家设置使用关键词、语义或混合；可用自然语言描述伏笔或事件，关键词模式用空格分词。read 按 seq 与 nextOffset 连续读正文。语义相近不等于事实成立。', { type: 'object', properties: {
        ...
        }, required: ['action'], additionalProperties: false }, async (args, exec) => {
        const session = await sessionOf(exec);
        const engine = ctx.get('compaction');
        if (!engine?.history)
            throw new Error('剧情历史服务尚未就绪');
        if (clusterJob(exec.agent))
            args = { ...args, scope: 'story', maxChars: Math.min(12000, Math.max(1000, Number(args.maxChars) || 6000)), limit: Math.min(8, Math.max(1, Number(args.limit) || 5)) };
        ...
        if (current.id !== session.id || !storyBranchIsActive(current))
            throw new Error('历史查询授权已失效');
        return result;
    ...
    ctx.effect(() => ctx.tools.register(simpleTool('rp_character_cast', '正式剧情动笔前选定本回合主要人物 ID，程序并行推演人物意向。无需传剧情、人设或笔记；新重要人物先保存独立人设。每轮只调用一次，结果仅供主笔参考。', { type: 'object', properties: { character_ids: { type: 'array', items: { type: 'string' }, uniqueItems: true, maxItems: 24 } }, required: ['character_ids'], additionalProperties: false }, async (args, exec) => {
        ...
        if (!snapshot || activeCardWorkflow(session))
            throw new Error('集群只用于正式剧情回合');
        ...
        if (imported || adaptationTurns(eventsOf(session)).has(turn))
            throw new Error('读卡、创作或管理回合不执行角色推演');
        const roster = characterRoster(session), ids = [...new Set(args.character_ids ?? [])];
        if (ids.length > 24 || ids.some(id => !roster.some(c => c.id === id)))
            throw new Error('选角含未知人物，请刷新人物列表或先登记新重要人物');
        ...
        return withTavernLock(T.branch, planKey, async () => {
            ...
            if (previous?.result)
                return deliver(previous.result);
            ...
            const result = await characterCluster.run({ session, agent, turn, characters: ids.map(id => roster.find(c => c.id === id)), signal: exec.signal,
                context: { stories, notes, lore: lore, core: T.rules.get(keyOf(session.id, 'spec'))?.core ?? '', userText: snapshot.userText } });
